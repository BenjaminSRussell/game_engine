...
static int send_packet(int socket_fd, const struct sockaddr_in* addr, PacketType type, const void* data, size_t data_size);
static int receive_packet(int socket_fd, struct sockaddr_in* addr, PacketHeader* header, void* data, size_t max_size);
static void handle_server_packet(PacketType type, const void* data, size_t size);
static void handle_client_packet(int client_index, PacketType type, const void* data, size_t size);
...
static bool authenticate_client(ClientConnection* client, const char* username, const char* password);
...
int network_server_stop(void) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (g_server.clients[i].connected) {
            send_packet(g_server.clients[i].socket_fd, &g_server.clients[i].address, PACKET_TYPE_DISCONNECT, NULL, 0);
...
int network_server_broadcast(PacketType type, const void* data, size_t data_size) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (g_server.clients[i].connected && g_server.clients[i].authenticated) {
            if (send_packet(g_server.clients[i].socket_fd, &g_server.clients[i].address, type, data, data_size) == 0) {
                sent_count++;
            }
        }
    }
...
int network_server_send_to_client(uint32_t client_id, PacketType type, const void* data, size_t data_size) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (g_server.clients[i].connected && g_server.clients[i].client_id == client_id) {
            return send_packet(g_server.clients[i].socket_fd, &g_server.clients[i].address, type, data, data_size);
        }
    }
...
int network_client_connect(const char* server_address, uint16_t port, const char* username, const char* password) {
    ...
    strncpy(g_client.server_address, server_address, sizeof(g_client.server_address) - 1);
    ...
    if (send_packet(g_client.client_socket, &g_client.server_addr, PACKET_TYPE_CONNECT, &client_info, sizeof(client_info)) != 0) {
        LOG_ERROR("Failed to send connection request");
        close(g_client.client_socket);
        return -1;
    }
    
    g_client.connection_retry_time = (float)clock() / CLOCKS_PER_SEC + CONNECTION_RETRY_DELAY_MS / 1000.0f;
    g_network_initialized = true;
...
int network_client_disconnect(void) {
    ...
    send_packet(g_client.client_socket, &g_client.server_addr, PACKET_TYPE_DISCONNECT, NULL, 0);
...
int network_client_send(PacketType type, const void* data, size_t data_size) {
    if (!g_network_initialized || !g_client.connected || !g_client.authenticated) {
        return -1;
    }
    
    return send_packet(g_client.client_socket, &g_client.server_addr, type, data, data_size);
}
...
static int send_packet(int socket_fd, const struct sockaddr_in* addr, PacketType type, const void* data, size_t data_size) {
    ...
    ssize_t sent = sendto(socket_fd, packet_buffer, total_size, 0, (struct sockaddr*)addr, sizeof(struct sockaddr_in));
...
static int receive_packet(int socket_fd, struct sockaddr_in* addr, PacketHeader* header, void* data, size_t max_size) {
    ...
    ssize_t received = recvfrom(socket_fd, packet_buffer, sizeof(packet_buffer), 0, (struct sockaddr*)addr, &addr_len);
...
static void update_server(float delta_time) {
    ...
    if (current_time - g_server.last_tick >= 1.0f / SERVER_TICK_RATE) {
        ...
        int client_socket = accept(g_server.server_socket, (struct sockaddr*)&client_addr, &addr_len);
        ...
        if (client_socket >= 0) {
            ...
            for (int i = 0; i < MAX_CLIENTS; i++) {
                if (!g_server.clients[i].connected) {
                    ...
                    LOG_INFO("New client connected: ID=%d", g_server.clients[i].client_id);
                    break;
                }
            }
        }
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (g_server.clients[i].connected) {
            ...
            int received = receive_packet(g_server.clients[i].socket_fd, &g_server.clients[i].address, &header, data, sizeof(data));
            
            if (received >= 0) {
                g_server.clients[i].last_heartbeat = current_time;
                g_server.clients[i].packets_received++;
                handle_client_packet(i, header.packet_type, data, received);
            }
        }
    }
    ...
    if (current_time - g_server.last_tick >= 1.0f) {
        for (int i = 0; i < MAX_CLIENTS; i++) {
            if (g_server.clients[i].connected && g_server.clients[i].authenticated) {
                send_packet(g_server.clients[i].socket_fd, &g_server.clients[i].address, PACKET_TYPE_HEARTBEAT, NULL, 0);
            }
        }
    }
}
...
static void update_client(float delta_time) {
    ...
    int received = receive_packet(g_client.client_socket, &g_client.server_addr, &header, data, sizeof(data));
    
    if (received >= 0) {
        g_client.last_heartbeat = current_time;
        g_client.retry_count = 0;
        handle_server_packet(header.packet_type, data, received);
    }
    
    // Handle connection timeout
    if (g_client.connected && current_time - g_client.last_heartbeat > CLIENT_TIMEOUT_MS / 1000.0f) {
    ...
    if (!g_client.connected && g_client.retry_count < MAX_RETRIES) {
        if (current_time >= g_client.connection_retry_time) {
            LOG_INFO("Retrying connection to server (attempt %d/%d)", g_client.retry_count + 1, MAX_RETRIES);
            
            ClientInfo client_info = {0};
            strncpy(client_info.username, g_client.username, sizeof(client_info.username) - 1);
            
            if (send_packet(g_client.client_socket, &g_client.server_addr, PACKET_TYPE_CONNECT, &client_info, sizeof(client_info)) == 0) {
                g_client.retry_count++;
                g_client.connection_retry_time = current_time + CONNECTION_RETRY_DELAY_MS / 1000.0f;
            }
        }
    }
}
...
static void handle_server_packet(PacketType type, const void* data, size_t size) {
    switch (type) {
        ...
        case PACKET_TYPE_AUTH_RESPONSE:
            if (size >= sizeof(AuthResponse)) {
                ...
                if (response->success) {
                    ...
                    LOG_INFO("Authentication successful, client ID: %d", g_client.client_id);
                } else {
        ...
        case PACKET_TYPE_HEARTBEAT:
            ...
            send_packet(g_client.client_socket, &g_client.server_addr, PACKET_TYPE_HEARTBEAT, NULL, 0);
            break;
...
static void handle_client_packet(int client_index, PacketType type, const void* data, size_t size) {
    ...
    switch (type) {
        case PACKET_TYPE_CONNECT:
            if (size >= sizeof(ClientInfo)) {
                ...
                if (authenticate_client(client, info->username, "")) {
                    ...
                    send_packet(client->socket_fd, &client->address, PACKET_TYPE_AUTH_RESPONSE, &response, sizeof(response));
                    
                    LOG_INFO("Client authenticated: %s (ID: %d)", client->username, client->client_id);
                } else {
                    AuthResponse response = {
                        .success = false,
                        .client_id = 0,
        ...
        case PACKET_TYPE_DISCONNECT:
            LOG_INFO("Client disconnected: %s (ID: %d)", client->username, client->client_id);
        ...
        default:
            LOG_DEBUG("Received unknown packet type: %d from client %d", type, client->client_id);
            break;
    }
}
...
static void cleanup_disconnected_clients(void) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (g_server.clients[i].connected) {
            // Check timeout
            if (current_time - g_server.clients[i].last_heartbeat > CLIENT_TIMEOUT_MS / 1000.0f) {
                LOG_INFO("Client timed out: %s (ID: %d)", g_server.clients[i].username, g_server.clients[i].client_id);
                close(g_server.clients[i].socket_fd);
                g_server.clients[i].connected = false;
                g_server.connected_clients--;
            }
...
static bool authenticate_client(ClientConnection* client, const char* username, const char* password) {
    // Simple authentication - in a real implementation, this would check against a database
