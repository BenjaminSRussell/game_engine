...
typedef enum {
    ...
    PACKET_FLAG_SEQUENCED = 1 << 1,
    PACKET_FLAG_COMPRESSED = 1 << 2,
    PACKET_FLAG_ENCRYPTED = 1 << 3,
...
bool protocol_send_fragmented(NetworkProtocol* protocol, uint32_t connection_id, MessageType type, const void* data, uint32_t data_size);
bool protocol_reassemble_fragments(NetworkProtocol* protocol, const NetworkPacket* packet, void** out_data, uint32_t* out_size);
