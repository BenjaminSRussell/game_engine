...
#ifndef LUMEN_RADIANCE_INJECT_H
#define LUMEN_RADIANCE_INJECT_H
...
typedef struct lumen_radiance_inject_handle {
    uint32_t id;
} lumen_radiance_inject_handle_t;

typedef struct lumen_radiance_inject_desc {
    uint32_t flags;
    void* user_data;
} lumen_radiance_inject_desc_t;

typedef struct lumen_radiance_inject_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} lumen_radiance_inject_info_t;
