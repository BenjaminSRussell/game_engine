...
static TerrainConfigValidation terrain_config_validate(const TerrainChunkConfig* config);
...
static bool lod_manager_initialize(LODManager* manager, const TerrainChunkConfig* config);
static void lod_manager_shutdown(LODManager* manager);
static uint32_t lod_manager_calculate_lod(const LODManager* manager, float distance);
static float lod_manager_get_distance_threshold(const LODManager* manager, uint32_t lod);
static void lod_manager_update_chunk_lod(LODManager* manager, TerrainChunk* chunk, const float* camera_pos);

static WorldCoord64 float_to_world_coord_64bit(float x, float y, float z);
static void world_coord_64bit_to_float(WorldCoord64 coord, float* out_x, float* out_y, float* out_z);
...
static bool generate_chunk_heightmap_64bit(TerrainChunk* chunk, int64_t chunk_x, int64_t chunk_z);
...
bool terrain_system_init_advanced(TerrainSystem* system, const char* config_file_path) {
    ...
    if (config_file_path && strlen(config_file_path) > 0) {
        strncpy(g_terrain_state.config_file_path, config_file_path, sizeof(g_terrain_state.config_file_path) - 1);
        if (!terrain_config_load_from_file(config_file_path)) {
            fprintf(stderr, "Warning: Failed to load terrain config from %s, using defaults\n", config_file_path);
        }
    }
    
    // Validate configuration
    TerrainConfigValidation validation = terrain_config_validate(&g_terrain_state.config);
    ...
    system->config.world_size_x = g_terrain_state.config.chunk_size * g_terrain_state.config.max_world_size;
    system->config.world_size_z = g_terrain_state.config.chunk_size * g_terrain_state.config.max_world_size;
    ...
    if (!chunk_pool_initialize(&g_terrain_state.chunk_pool, g_terrain_state.config.chunk_pool_size)) {
        fprintf(stderr, "Failed to initialize chunk pool\n");
        return false;
    }
    
    // Initialize LOD manager
    if (!lod_manager_initialize(&g_terrain_state.lod_manager, &g_terrain_state.config)) {
    ...
    system->chunks = calloc(g_terrain_state.config.max_chunks, sizeof(TerrainChunk));
    if (!system->chunks) {
    ...
    system->streaming.load_radius = 1000.0f * g_terrain_state.config.load_radius_multiplier;
    system->streaming.unload_radius = 1200.0f * g_terrain_state.config.load_radius_multiplier;
    system->streaming.max_chunks_per_frame = g_terrain_state.config.max_chunks_per_frame;
    ...
    printf("  - Max world size: %u chunks (%u km)\n", 
           ...
           (g_terrain_state.config.max_world_size * g_terrain_state.config.chunk_size) / 1000);
...
bool terrain_set_chunk_size(uint32_t new_chunk_size) {
    ...
    TerrainConfigValidation validation = terrain_config_validate(&g_terrain_state.config);
    if (!validation.is_valid) {
        fprintf(stderr, "Configuration validation failed after chunk size change: %s\n", 
                validation.error_message);
        return false;
    }
...
bool terrain_enable_large_worlds(uint32_t max_world_size_chunks) {
    ...
    g_terrain_state.config.max_chunks = max_world_size_chunks * max_world_size_chunks;
 ...
 */
TerrainConfigValidation terrain_config_validate(const TerrainChunkConfig* config) {
    ...
    if (config->max_chunks < 256 || config->max_chunks > 1000000) {
        snprintf(result.error_message, sizeof(result.error_message),
                "Invalid max chunks: %u (must be 256-1000000)", config->max_chunks);
        result.is_valid = false;
        return result;
    }
    ...
    for (uint32_t i = 1; i < config->lod_levels; i++) {
        if (config->lod_distances[i] <= config->lod_distances[i-1]) {
            snprintf(result.error_message, sizeof(result.error_message),
                    "LOD distances must be increasing: level %u distance %.1f <= level %u distance %.1f",
                    i, config->lod_distances[i], i-1, config->lod_distances[i-1]);
            result.is_valid = false;
            return result;
        }
    }
    ...
    if (config->chunk_pool_size < 64 || config->chunk_pool_size > config->max_chunks) {
        snprintf(result.error_message, sizeof(result.error_message),
    ...
    if (world_size.x > max_world_size || world_size.y > max_world_size || world_size.z > max_world_size) {
        snprintf(result.error_message, sizeof(result.error_message),
                "World boundaries exceed maximum size: (%lld, %lld, %lld) > %lld",
                (long long)world_size.x, (long long)world_size.y, (long long)world_size.z,
                (long long)max_world_size);
        result.is_valid = false;
        return result;
    }
    ...
    if (config->chunk_size < 64) {
        snprintf(result.warnings[result.warning_count++], 128,
                "Small chunk size (%u) may impact performance", config->chunk_size);
    }
    
    if (config->chunk_pool_size < config->max_chunks / 4) {
        snprintf(result.warnings[result.warning_count++], 128,
                "Chunk pool size (%u) is less than 25%% of max chunks", config->chunk_pool_size);
    }
    
    if (config->streaming_budget_mb < 512) {
        snprintf(result.warnings[result.warning_count++], 128,
                "Low streaming budget (%.1f MB) may cause frequent loading", config->streaming_budget_mb);
    }
 ...
 */
bool terrain_config_serialize(const TerrainChunkConfig* config, char* buffer, size_t buffer_size) {
    ...
    for (uint32_t i = 0; i < config->lod_levels; i++) {
        offset += snprintf(buffer + offset, buffer_size - offset, "%.1f%s", 
            config->lod_distances[i], (i < config->lod_levels - 1) ? ", " : "");
    }
    ...
    offset += snprintf(buffer + offset, buffer_size - offset,
        ...
        config->enable_compression ? "true" : "false",
        config->enable_gpu_streaming ? "true" : "false",
        (long long)config->world_min.x, (long long)config->world_min.y, (long long)config->world_min.z,
        (long long)config->world_max.x, (long long)config->world_max.y, (long long)config->world_max.z,
...
static bool chunk_pool_initialize(ChunkPool* pool, uint32_t pool_size) {
    ...
    printf("Initialized chunk pool: %u chunks, %.1f MB\n", 
           pool_size, (pool_size * sizeof(TerrainChunk)) / (1024.0f * 1024.0f));
...
static void chunk_pool_shutdown(ChunkPool* pool) {
    ...
    printf("Chunk pool shutdown: allocations=%u, deallocations=%u, peak_memory=%.1f MB\n",
...
static size_t chunk_pool_get_memory_usage(const ChunkPool* pool) {
    ...
    for (uint32_t i = 0; i < pool->pool_size; i++) {
        if (pool->chunks[i].is_loaded && pool->chunks[i].heightmap) {
            total_usage += pool->chunks[i].heightmap_size * pool->chunks[i].heightmap_size * sizeof(float);
        }
    }
...
static bool lod_manager_initialize(LODManager* manager, const TerrainChunkConfig* config) {
    ...
    for (uint32_t i = 0; i < config->lod_levels; i++) {
        manager->lod_multipliers[i] = 1.0f / (1 << i); // Each LOD level halves the detail
        manager->vertex_reductions[i] = 1 << i; // Each LOD level reduces vertices by power of 2
    }
    ...
    for (uint32_t i = 0; i < config->lod_levels; i++) {
        printf("  LOD %u: distance=%.1f, multiplier=%.2f, vertex_reduction=%u\n",
               i, manager->distance_thresholds[i], manager->lod_multipliers[i], 
...
static void lod_manager_shutdown(LODManager* manager) {
    ...
    if (manager->distance_thresholds) free(manager->distance_thresholds);
    if (manager->chunk_counts) free(manager->chunk_counts);
    if (manager->lod_multipliers) free(manager->lod_multipliers);
    if (manager->vertex_reductions) free(manager->vertex_reductions);
...
static void lod_manager_update_chunk_lod(LODManager* manager, TerrainChunk* chunk, const float* camera_pos) {
    ...
    float chunk_center_x = (chunk->chunk_x + 0.5f) * g_terrain_state.config.chunk_size;
    float chunk_center_z = (chunk->chunk_z + 0.5f) * g_terrain_state.config.chunk_size;
...
static WorldCoord64 float_to_world_coord_64bit(float x, float y, float z) {
    WorldCoord64 coord;
    coord.x = (int64_t)(x * g_terrain_state.coordinate_scale) + g_terrain_state.world_origin.x;
    coord.y = (int64_t)(y * g_terrain_state.coordinate_scale) + g_terrain_state.world_origin.y;
    coord.z = (int64_t)(z * g_terrain_state.coordinate_scale) + g_terrain_state.world_origin.z;
    return coord;
}

static void world_coord_64bit_to_float(WorldCoord64 coord, float* out_x, float* out_y, float* out_z) {
    if (out_x) *out_x = (float)((coord.x - g_terrain_state.world_origin.x) / g_terrain_state.coordinate_scale);
    if (out_y) *out_y = (float)((coord.y - g_terrain_state.world_origin.y) / g_terrain_state.coordinate_scale);
    if (out_z) *out_z = (float)((coord.z - g_terrain_state.world_origin.z) / g_terrain_state.coordinate_scale);
}
...
void terrain_system_update_advanced(TerrainSystem* system, float dt) {
    ...
    g_terrain_state.last_update_time_ms = (end_time - start_time) * 1000.0 / get_performance_frequency();
    
    // Log performance if update takes too long
    if (g_terrain_state.last_update_time_ms > 16.67f) { // > 16.67ms = longer than 60fps frame
        printf("Warning: Terrain update took %.2f ms (%.1f fps)\n", 
               g_terrain_state.last_update_time_ms, 1000.0f / g_terrain_state.last_update_time_ms);
    }
}
 ...
 */
void terrain_get_performance_stats(uint32_t* chunks_loaded, uint32_t* chunks_unloaded, 
                                 float* update_time_ms, uint64_t* total_triangles) {
    if (chunks_loaded) *chunks_loaded = g_terrain_state.chunks_loaded_this_frame;
    if (chunks_unloaded) *chunks_unloaded = g_terrain_state.chunks_unloaded_this_frame;
    if (update_time_ms) *update_time_ms = g_terrain_state.last_update_time_ms;
    if (total_triangles) *total_triangles = g_terrain_state.total_triangles_rendered;
}
...
void terrain_get_chunk_pool_stats(uint32_t* pool_size, uint32_t* used_chunks, 
    ...
    if (pool_size) *pool_size = g_terrain_state.chunk_pool.pool_size;
    if (used_chunks) *used_chunks = g_terrain_state.chunk_pool.used_count;
    if (free_chunks) *free_chunks = g_terrain_state.chunk_pool.free_count;
    if (memory_usage) *memory_usage = chunk_pool_get_memory_usage(&g_terrain_state.chunk_pool);
}
...
static bool terrain_config_save_to_file(const char* config_path) {
    ...
    if (!terrain_config_serialize(&g_terrain_state.config, config_json, sizeof(config_json))) {
        return false;
    }
