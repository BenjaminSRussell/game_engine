...
// Game state updates
void audio_manager_set_player_transform(AudioManager *manager, Vec3 position, Vec3 forward, Vec3 up);
...
void audio_manager_set_combat_state(AudioManager *manager, bool in_combat, f32 intensity);
void audio_manager_set_environment_state(AudioManager *manager, bool underground, f32 level, 
                                        bool underwater, f32 depth, bool in_nether, bool in_end);
...
u32 audio_manager_play_sound(AudioManager *manager, SoundType sound, Vec3 position, f32 volume);
u32 audio_manager_play_sound_2d(AudioManager *manager, SoundType sound, f32 volume);
...
void audio_manager_set_environment(AudioManager *manager, const char *environment);
void audio_manager_set_biome(AudioManager *manager, const char *biome, f32 forest_density);

// Performance and debugging
void audio_manager_set_performance_mode(AudioManager *manager, bool enabled);
void audio_manager_get_statistics(AudioManager *manager, u32 *total_sounds, u32 *active_sounds, f32 *cpu_usage);
