// include/math/mat4.h
//
// Purpose: Defines the `Mat4` structure for 4x4 matrices and provides a comprehensive
// API for common 3D linear algebra operations. This header is crucial for handling
// transformations (translation, rotation, scaling), camera projections (perspective,
// orthographic), and view matrices (look-at) within the game engine.
// Matrices are stored in column-major order, consistent with OpenGL conventions.
//
// Public APIs:
// - `Mat4`: A union representing a 4x4 floating-point matrix, allowing access
//   via a 1D array (`m`), 2D array (`data`), or named members (`m00`, `m10`, etc.).
...
// - `mat4_translate`, `mat4_scale`: Creates transformation matrices for translation and scaling.
// - `mat4_rotate_x`, `mat4_rotate_y`, `mat4_rotate_z`, `mat4_rotate`: Creates rotation matrices
//   around an axis or arbitrary axis-angle.
// - `mat4_look_at`: Creates a view matrix for a camera, looking from an eye position at a target.
// - `mat4_perspective`, `mat4_ortho`: Creates projection matrices for perspective and orthographic views.
...
//
// Ownership: `Mat4` instances are value types. Functions return new `Mat4` instances
// or modify existing ones passed by pointer, but they do not manage dynamic memory.
//
// Invariants:
// - Matrices are assumed to be 4x4 floating-point matrices.
// - Column-major storage order must be respected when interacting with graphics APIs.
// - Input vectors for transformations are assumed to be `Vec3` types (from `vec3.h`).
