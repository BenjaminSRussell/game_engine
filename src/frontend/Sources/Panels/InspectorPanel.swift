            ...
            if let firstSelected = selectionManager.selectedEntities.first {
                ScrollView {
                    VStack(spacing: DesignSystem.Spacing.md) {
                        ...
                        EditorButton("Add Component", icon: "plus", style: .primary) {
            ...
            HStack {
                Toggle("", isOn: $isActive)
                    ...
                    .onChange(of: isActive) { newValue in
                        EngineBridge.shared.setEntityActive(entityID, isActive: newValue)
                    }
            ...
            HStack(spacing: DesignSystem.Spacing.md) {
                ...
                HStack(spacing: 4) {
                    ...
                    .onChange(of: tag) { newValue in
                        EngineBridge.shared.setEntityTag(entityID, tag: newValue)
                    }
                }
                ...
                HStack(spacing: 4) {
                    ...
                    .pickerStyle(.menu)
                    .font(DesignSystem.Typography.small)
                    .onChange(of: layer) { newValue in
                        EngineBridge.shared.setEntityLayer(entityID, layer: newValue)
                    }
                }
                ...
                Toggle("Static", isOn: $isStatic)
                    ...
                    .onChange(of: isStatic) { newValue in
                        EngineBridge.shared.setEntityStatic(entityID, isStatic: newValue)
                    }
            }
        }
        .padding(DesignSystem.Spacing.sm)
    ...
    func removeComponent(_ componentID: UUID) {
        if let index = components.firstIndex(where: { $0.id == componentID }),
           let entityID = currentEntityID {
            ...
            EngineBridge.shared.removeComponent(entityID, componentType: component.componentType.rawValue)
            NotificationManager.shared.notify("Removed \(component.componentType.rawValue) component", type: .warning)
        }
    }
    
    // Support for undo/redo property modification
    func setProperty(_ entityID: UUID, path: String, value: Any) {
        // Find which component this property belongs to
        // Path format: "ComponentType.propertyName" e.g., "Transform.position"
        let parts = path.split(separator: ".")
        guard parts.count >= 2 else { return }
        
        let componentType = String(parts[0])
        guard components.contains(where: { String(describing: $0.componentType.rawValue) == componentType }) else { return }
        
        // This is a simplified implementation. Real reflection would be needed for arbitrary depth.
        // For now, trigger a reload to catch external changes or update if specific logic exists.
        // In a real implementation, we'd map "value" back to the specific field.
        print("Setting property \(path) to \(value) on entity \(entityID)")
        
        // Force reload to sync state
        loadComponents(for: entityID)
    }
}
