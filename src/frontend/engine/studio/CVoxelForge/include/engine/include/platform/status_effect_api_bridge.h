...
typedef enum {
  STATUS_EFFECT_BURNING = 0,
  STATUS_EFFECT_POISON = 1,
  STATUS_EFFECT_FREEZING = 2,
  STATUS_EFFECT_HEALING = 3,
  STATUS_EFFECT_SHIELD = 4,
  STATUS_EFFECT_SPEED = 5
} StatusEffectType;
...
void status_effect_apply(uint64_t entity_id, StatusEffectType effect_type,
                         float intensity);

/// Remove a status effect from an entity
void status_effect_remove(uint64_t entity_id, StatusEffectType effect_type);
