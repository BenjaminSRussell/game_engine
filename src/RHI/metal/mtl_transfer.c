...
static void *allocate_staging(metal_transfer_manager_t *mgr, size_t size) {
  if (!mgr->staging_buffer) {
    id<MTLDevice> mtl_device =
        (__bridge id<MTLDevice>)metal_get_device(mgr->device);
    mgr->staging_buffer =
        [mtl_device newBufferWithLength:STAGING_BUFFER_SIZE
  ...
  void *ptr =
      (uint8_t *)[mgr->staging_buffer contents] + mgr->staging_buffer_offset;
...
metal_transfer_manager_t *
metal_transfer_manager_create(metal_device_t *device) {
  if (!device) {
    return NULL;
  }

  metal_transfer_manager_t *mgr =
      (metal_transfer_manager_t *)calloc(1, sizeof(metal_transfer_manager_t));
  if (!mgr) {
    return NULL;
  }
  ...
  mgr->transfer_queue =
      [(__bridge id<MTLDevice>)metal_get_device(device) newCommandQueue];
...
metal_transfer_manager_get_stats(metal_transfer_manager_t *mgr) {
  if (mgr) {
    return mgr->stats;
  }
...
void metal_transfer_upload_buffer_staged(metal_transfer_manager_t *mgr,
                                         metal_buffer_t *dst, const void *data,
  ...
  if (callback && mgr->current_cmd_buffer) {
    id<MTLCommandBuffer> cmd_buf = mgr->current_cmd_buffer;
    [cmd_buf addCompletedHandler:^(id<MTLCommandBuffer> _Nonnull buf) {
      callback(user_data);
      mgr->stats.active_transfers--;
    }];
  }
...
void metal_transfer_copy_buffer(metal_transfer_manager_t *mgr,
                                metal_buffer_t *src, metal_buffer_t *dst,
...
void metal_transfer_upload_texture_staged(metal_transfer_manager_t *mgr,
  ...
  uint32_t depth = region
                       ? region->depth
                       : (dst->type == METAL_TEXTURE_TYPE_3D ? dst->depth : 1);
  ...
  if (region) {
    mtl_region = MTLRegionMake3D(region->x, region->y, region->z, region->width,
...
void metal_transfer_blit_texture(metal_transfer_manager_t *mgr,
                                 metal_texture_t *src, metal_texture_t *dst,
  ...
  if (src_region) {
    mtl_src_region = MTLRegionMake3D(src_region->x, src_region->y,
                                     src_region->z, src_region->width,
                                     src_region->height, src_region->depth);
  } else {
...
void metal_transfer_buffer_to_texture(metal_transfer_manager_t *mgr,
                                      metal_buffer_t *src, metal_texture_t *dst,
                                      size_t src_offset,
                                      const metal_texture_region_t *dst_region,
  ...
  if (dst_region) {
    mtl_region = MTLRegionMake3D(0, 0, 0, dst_region->width, dst_region->height,
                                 dst_region->depth);
    dst_origin = MTLOriginMake(dst_region->x, dst_region->y, dst_region->z);
...
void metal_transfer_texture_to_buffer(metal_transfer_manager_t *mgr,
                                      metal_texture_t *src, metal_buffer_t *dst,
                                      const metal_texture_region_t *src_region,
                                      size_t dst_offset, size_t bytes_per_row,
                                      metal_transfer_completion_t callback,
                                      void *user_data) {
  if (!mgr || !src || !dst) {
    if (callback)
  ...
  if (src_region) {
    mtl_region = MTLRegionMake3D(0, 0, 0, src_region->width, src_region->height,
                                 src_region->depth);
