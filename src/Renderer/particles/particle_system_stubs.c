...
static void update_particle_physics(Particle *particle, float dt, const ParticleEmitter *emitter) {
    ...
    if (emitter->turbulence > 0.0f) {
        float noise_x = sinf(particle->position[0] * 0.1f + particle->age * 2.0f) * emitter->turbulence;
        float noise_y = cosf(particle->position[1] * 0.1f + particle->age * 1.5f) * emitter->turbulence;
        float noise_z = sinf(particle->position[2] * 0.1f + particle->age * 1.8f) * emitter->turbulence;
        force[0] += noise_x;
        force[1] += noise_y;
        force[2] += noise_z;
    }
...
static void emit_particle(ParticleEmitter *emitter) {
    if (emitter->free_list_size == 0) return;
    ...
    switch (emitter->emission_type) {
        ...
        case PARTICLE_EMISSION_TYPE_SPHERE: {
            ...
            particle->position[0] = emitter->position[0] + radius * sinf(phi) * cosf(theta);
            particle->position[1] = emitter->position[1] + radius * sinf(phi) * sinf(theta);
    ...
    particle->velocity[0] = emitter->initial_velocity_min[0] + 
                          (float)rand() / RAND_MAX * (emitter->initial_velocity_max[0] - emitter->initial_velocity_min[0]);
    particle->velocity[1] = emitter->initial_velocity_min[1] + 
                          (float)rand() / RAND_MAX * (emitter->initial_velocity_max[1] - emitter->initial_velocity_min[1]);
    particle->velocity[2] = emitter->initial_velocity_min[2] + 
                          (float)rand() / RAND_MAX * (emitter->initial_velocity_max[2] - emitter->initial_velocity_min[2]);
    
    // Random initial color
    particle->color[0] = emitter->initial_color[0] + 
                       (float)rand() / RAND_MAX * emitter->initial_color_variation[0];
    particle->color[1] = emitter->initial_color[1] + 
                       (float)rand() / RAND_MAX * emitter->initial_color_variation[1];
    particle->color[2] = emitter->initial_color[2] + 
                       (float)rand() / RAND_MAX * emitter->initial_color_variation[2];
    particle->color[3] = emitter->initial_color[3] + 
    ...
    particle->size = emitter->initial_size_min + 
                   (float)rand() / RAND_MAX * (emitter->initial_size_max - emitter->initial_size_min);
    
    // Random lifetime
    particle->lifetime = emitter->initial_lifetime_min + 
                       (float)rand() / RAND_MAX * (emitter->initial_lifetime_max - emitter->initial_lifetime_min);
...
bool particle_system_init(uint32_t max_emitters, uint64_t max_total_particles) {
    ...
    LOG_INFO("Particle system initialized (max emitters: %u, max particles: %llu)", 
...
ParticleEmitter *particle_system_create_emitter(const char *name, uint32_t max_particles) {
    ...
    if (g_particle_system.total_particles + max_particles > g_particle_system.max_total_particles) {
    ...
    emitter->simulation_type = g_particle_system.enable_gpu_simulation ? 
                              PARTICLE_SIMULATION_GPU_COMPUTE : PARTICLE_SIMULATION_CPU;
    ...
    LOG_INFO("Created particle emitter: %s (max particles: %u)", name, max_particles);
    return emitter;
}
...
void particle_system_destroy_emitter(ParticleEmitter *emitter) {
    ...
    for (uint32_t i = 0; i < g_particle_system.emitter_count; i++) {
        if (g_particle_system.emitters[i] == emitter) {
            g_particle_system.emitters[i] = g_particle_system.emitters[g_particle_system.emitter_count - 1];
...
void particle_system_update(float dt) {
    ...
    // Clamp delta time
    dt = fminf(dt, g_particle_system.max_simulation_time) * g_particle_system.global_time_scale;
    ...
    for (uint32_t i = 0; i < g_particle_system.emitter_count; i++) {
        ...
        while (emitter->emission_timer >= emission_interval && emitter->active_count < emitter->max_particles) {
        ...
        if (emitter->emission_burst > 0.0f) {
            ...
            for (uint32_t j = 0; j < burst_count && emitter->active_count < emitter->max_particles; j++) {
                emit_particle(emitter);
            }
...
void particle_system_render(const float *view_matrix, const float *proj_matrix) {
    if (!g_particle_system.initialized) return;
    ...
    for (uint32_t i = 0; i < g_particle_system.emitter_count; i++) {
        ParticleEmitter *emitter = g_particle_system.emitters[i];
        if (!emitter->enabled || emitter->active_count == 0) continue;
        
        // TODO: Render this emitter's particles
        LOG_DEBUG("Rendering emitter: %s (%u active particles)", 
                 emitter->name, emitter->active_count);
    }
...
void particle_emitter_set_position(ParticleEmitter *emitter, float x, float y, float z) {
    if (!emitter) return;
...
void particle_emitter_set_color(ParticleEmitter *emitter, float r, float g, float b, float a) {
    if (!emitter) return;
...
void particle_system_get_stats(uint64_t *total_particles, uint32_t *active_emitters,
    ...
    if (total_particles) *total_particles = active_particles;
    if (active_emitters) *active_emitters = g_particle_system.emitter_count;
    if (simulation_time) *simulation_time = g_particle_system.total_simulation_time;
    if (render_time) *render_time = g_particle_system.total_render_time;
}
