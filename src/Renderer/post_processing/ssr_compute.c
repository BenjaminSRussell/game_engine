...
typedef struct SSRComputeContext {
  ...
  Texture *depth_hierarchy_texture; // Mip chain for hierarchical ray marching

  // Compute shader
  u32 compute_shader; // Module ID
...
SSRComputeContext *ssr_compute_create(u32 width, u32 height) {
  ...
  desc.format = TEXFMT_RGBA16F; // Fallback for R16F
...
void ssr_compute_process(SSRComputeContext *ctx, TextureID scene_color,
  ...
  // texture_generate_mipmaps(depth_buffer); // Need Texture* not ID? Or API
  // takes ID? texture_copy_to_texture(depth_buffer,
  // ctx->depth_hierarchy_texture);
  ...
  texture_bind_compute(texture_get_id(ctx->depth_hierarchy_texture), 3);
