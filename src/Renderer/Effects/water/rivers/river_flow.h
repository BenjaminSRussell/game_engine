...
#ifndef WATER_RIVER_FLOW_H
#define WATER_RIVER_FLOW_H
...
typedef struct water_river_flow_handle {
    uint32_t id;
} water_river_flow_handle_t;

typedef struct water_river_flow_desc {
    uint32_t flags;
    void* user_data;
} water_river_flow_desc_t;

typedef struct water_river_flow_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} water_river_flow_info_t;
