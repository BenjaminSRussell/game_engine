...
static void interpolate_keyframes(const AnimationKeyframe *keyframe1, const AnimationKeyframe *keyframe2, 
    ...
    // Implement proper spherical linear interpolation
    float dot = q1[0] * q2[0] + q1[1] * q2[1] + q1[2] * q2[2] + q1[3] * q2[3];
    
    // If quaternions are very close, use linear interpolation
    const float DOT_THRESHOLD = 0.9995f;
    if (fabsf(dot) > DOT_THRESHOLD) {
        q_result[0] = q1[0] + t * (q2[0] - q1[0]);
        q_result[1] = q1[1] + t * (q2[1] - q1[1]);
        q_result[2] = q1[2] + t * (q2[2] - q1[2]);
        q_result[3] = q1[3] + t * (q2[3] - q1[3]);
    } else {
        // Calculate SLERP
        ...
        q_result[0] = a * q1[0] + b * q2[0];
        q_result[1] = a * q1[1] + b * q2[1];
        q_result[2] = a * q1[2] + b * q2[2];
        q_result[3] = a * q1[3] + b * q2[3];
    }
...
bool animation_system_init(uint32_t max_animations, uint32_t max_bones, uint32_t max_morph_targets, 
    ...
    if (!g_animation_system.bone_buffer) {
        ...
        // Continue without GPU buffer for now
    }
