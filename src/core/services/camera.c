...
void camera_update(Camera *camera, f32 delta_time) {
  ...
  camera->front = vec3(cosf(yaw_rad) * cosf(pitch_rad), sinf(pitch_rad),
                       sinf(yaw_rad) * cosf(pitch_rad));
  camera->front = vec3_normalize(camera->front);

  camera->right =
      vec3_normalize(vec3_cross(camera->front, vec3(0.0f, 1.0f, 0.0f)));
...
Mat4 camera_get_projection_matrix(Camera *camera, f32 aspect) {
  return mat4_perspective(camera->fov * DEG_TO_RAD, aspect, camera->near_plane,
                          camera->far_plane);
}

Vec3 camera_get_forward(Camera *camera) { return camera->front; }
...
void camera_update_shake(Camera *camera, f32 delta_time) {
  ...
  if (camera->shake_duration > 0.0f) {
    ...
    if (camera->shake_time >= camera->shake_duration) {
      ...
      f32 random_x = (sinf(camera->shake_time * 15.0f) +
                      sinf(camera->shake_time * 23.7f)) *
                     0.5f;
      f32 random_y = (cosf(camera->shake_time * 17.3f) +
                      cosf(camera->shake_time * 29.1f)) *
                     0.5f;
      f32 random_z = (sinf(camera->shake_time * 19.7f) +
                      cosf(camera->shake_time * 31.3f)) *
                     0.5f;

      camera->shake_offset =
          vec3(random_x * current_intensity, random_y * current_intensity,
