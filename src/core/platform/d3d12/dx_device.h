...
#ifndef PLATFORM_DX_DEVICE_H
#define PLATFORM_DX_DEVICE_H
...
typedef struct platform_dx_device_handle {
    uint32_t id;
} platform_dx_device_handle_t;

typedef struct platform_dx_device_desc {
    uint32_t flags;
    void* user_data;
} platform_dx_device_desc_t;

typedef struct platform_dx_device_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} platform_dx_device_info_t;
