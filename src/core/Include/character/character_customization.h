...
uint32_t customization_add_option(customization_part_t part,
                                  customization_type_t type, const char *name);

// Set color values for a color option
// option_id: Option to modify
// r, g, b, a: RGBA color values (0.0 to 1.0)
bool customization_set_color_option(uint32_t option_id, float r, float g,
                                    float b, float a);
...
bool customization_set_model_option(uint32_t option_id, uint32_t model_id,
                                    uint32_t variant_id);
...
bool customization_set_size_option(uint32_t option_id, float width,
...
uint32_t customization_create_outfit_preset(const char *name,
...
bool customization_apply_option_to_character(uint32_t character_id,
                                             uint32_t option_id);

// Apply an entire outfit preset to a character
// character_id: Character to modify
// preset_id: Preset to apply
bool customization_apply_preset_to_character(uint32_t character_id,
                                             uint32_t preset_id);

// Save current character customization as an outfit
// character_id: Character to save
// outfit_name: Name for the saved outfit
bool customization_save_character_outfit(uint32_t character_id,
                                         const char *outfit_name);

// Load a saved character outfit
// character_id: Character to modify
// outfit_id: Saved outfit to load
bool customization_load_character_outfit(uint32_t character_id,
                                         uint32_t outfit_id);
...
void customization_set_body_parameters(uint32_t character_id, float height,
                                       float body_scale, float muscle_tone);
...
void customization_get_body_parameters(uint32_t character_id, float *height,
                                       float *body_scale, float *muscle_tone);
...
void customization_get_current_options(uint32_t character_id,
                                       const customization_option_t **options,
                                       uint32_t *option_count);

// Get all available outfit presets
// presets: Output array of presets
// preset_count: Output number of presets
void customization_get_available_presets(const outfit_preset_t **presets,
