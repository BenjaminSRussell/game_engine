...
typedef enum AdvancedFontHinting {
  ADVANCED_FONT_HINTING_NONE,
  ADVANCED_FONT_HINTING_LIGHT,
  ADVANCED_FONT_HINTING_NORMAL,
  ADVANCED_FONT_HINTING_AUTO
} AdvancedFontHinting;

typedef enum AdvancedFontRenderMode {
  ADVANCED_FONT_RENDER_BITMAP,
  ADVANCED_FONT_RENDER_SDF,
  ADVANCED_FONT_RENDER_MSDF,
  ADVANCED_FONT_RENDER_VECTOR
} AdvancedFontRenderMode;
...
typedef struct AdvancedKerningPair {
  uint32_t left_glyph;
  uint32_t right_glyph;
  float kerning;
} AdvancedKerningPair;

typedef struct AdvancedFontMetrics {
  float units_per_em;
  float ascender;
  float descender;
  float line_gap;
...
typedef struct AdvancedFont {
  ...
  AdvancedFontMetrics metrics;
  AdvancedFontRenderMode render_mode;
  AdvancedFontHinting hinting;
  ...
  AdvancedKerningPair *kerning_pairs;
...
typedef struct AdvancedFontLibrary {
  AdvancedFont *fonts;
  uint32_t font_count;
...
uint32_t advanced_font_library_load_font(AdvancedFontLibrary *library,
                                         ...
                                         AdvancedFontRenderMode render_mode);
uint32_t advanced_font_library_load_font_from_memory(
    AdvancedFontLibrary *library, const uint8_t *data, size_t data_size,
    float size, AdvancedFontRenderMode render_mode);
AdvancedFont *advanced_font_library_get_font(AdvancedFontLibrary *library,
                                             uint32_t font_id);
bool advanced_font_library_unload_font(AdvancedFontLibrary *library,
                                       uint32_t font_id);
bool advanced_font_library_set_fallback_chain(AdvancedFontLibrary *library,
                                              uint32_t font_id,
...
typedef struct TextRenderer {
  AdvancedFontLibrary *font_library;
