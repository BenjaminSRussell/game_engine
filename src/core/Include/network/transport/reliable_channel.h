...
typedef struct {
  ...
  u32 remote_sequence;
  u32 ack_bits;
  u32 last_ack; // Highest ack received from remote
  u32 last_ack_bits;
...
void reliable_channel_process_ack(ReliableChannel *channel, u32 ack_sequence,
                                  u32 ack_bits);
...
// Statistics
void reliable_channel_get_stats(const ReliableChannel *channel, u32 *sent,
                                u32 *received, u32 *resent, u32 *dropped,
                                f32 *rtt_ms, f32 *loss_rate);
...
void reliable_channel_set_max_retries(ReliableChannel *channel,
                                      u32 max_retries);
void reliable_channel_set_resend_interval(ReliableChannel *channel,
                                          u32 interval_ms);

// Internal Functions
static u32 reliable_channel_generate_sequence(void);
static bool reliable_channel_should_resend(const ReliablePacket *packet,
                                           u64 current_time,
                                           u64 resend_interval);
static void reliable_channel_update_rtt(ReliableChannel *channel, u64 ack_time,
                                        u64 send_time);
