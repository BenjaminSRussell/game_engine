...
Allocator *allocator_create_linear(size_t size, const char *name,
                                   MemoryTag tag);
Allocator *allocator_create_stack(size_t size, const char *name, MemoryTag tag);
Allocator *allocator_create_pool(size_t block_size, size_t block_count,
                                 const char *name, MemoryTag tag);
Allocator *allocator_create_buddy(size_t min_block_size, size_t max_block_size,
                                  const char *name, MemoryTag tag);
Allocator *allocator_create_tracked(Allocator *backing, const char *name,
                                    MemoryTag tag);
...
void *allocator_alloc_aligned(Allocator *allocator, size_t size,
                              size_t alignment);
...
#define ALLOC_ALIGNED(allocator, size, alignment)                              \
  allocator_alloc_aligned(allocator, size, alignment)
#define REALLOC_A(allocator, ptr, size) allocator_realloc(allocator, ptr, size)
#define FREE_A(allocator, ptr) allocator_free(allocator, ptr)
...
#define ALLOC_TEMP(allocator, size)                                            \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_TEMP)
#define ALLOC_PERSISTENT(allocator, size)                                      \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_PERSISTENT)
#define ALLOC_ASSET(allocator, size)                                           \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_ASSET)
#define ALLOC_RENDERER(allocator, size)                                        \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_RENDERER)
#define ALLOC_AUDIO(allocator, size)                                           \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_AUDIO)
#define ALLOC_PHYSICS(allocator, size)                                         \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_PHYSICS)
#define ALLOC_AI(allocator, size) ALLOC_TAGGED(allocator, size, MEMORY_TAG_AI)
#define ALLOC_NETWORK(allocator, size)                                         \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_NETWORK)
#define ALLOC_UI(allocator, size) ALLOC_TAGGED(allocator, size, MEMORY_TAG_UI)
#define ALLOC_GAMEPLAY(allocator, size)                                        \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_GAMEPLAY)
#define ALLOC_GEOMETRY(allocator, size)                                        \
  ALLOC_TAGGED(allocator, size, MEMORY_TAG_GEOMETRY)
...
void allocator_capture_stack_trace(void **buffer, u32 max_depth,
                                   u32 *out_depth);
...
void allocator_enable_advanced_leak_detection(Allocator *allocator,
                                              bool enable);
