...
bool bvh_tree_init(BVHTree* tree, u32 max_triangles, u32 max_nodes) {
    ...
    LOG_INFO("BVH tree initialized (triangles: %u, nodes: %u)", max_triangles, max_nodes);
    return true;
}
...
bool bvh_tree_build(BVHTree* tree, Triangle* triangles, u32 triangle_count) {
    ...
    tree->max_depth = 0;
    bool success = bvh_tree_build_recursive(tree, tree->root, tree->triangles, triangle_count, 0);
    
    tree->total_build_time_ns = get_current_time_ns() - start_time;
    
    if (success) {
        bvh_tree_calculate_statistics(tree);
        LOG_INFO("BVH tree built successfully (nodes: %u, depth: %u, time: %.2f ms)", 
                 tree->node_count, tree->max_depth, tree->total_build_time_ns / 1000000.0f);
    } else {
...
bool bvh_tree_build_recursive(BVHTree* tree, BVHNode* node, Triangle* triangles, u32 triangle_count, u32 depth) {
    ...
    if (triangle_count <= tree->max_triangles_per_leaf || depth >= tree->max_depth_limit) {
    ...
    if (extent.z > extent.y && extent.z > extent.x) split_axis = 2;
    ...
    switch (tree->split_strategy) {
        ...
        case BVH_SPLIT_OBJECT_MEDIAN:
            split_index = bvh_split_object_median(tree, triangles, triangle_count, split_axis);
            break;
        case BVH_SPLIT_SPATIAL_MEDIAN:
            split_index = bvh_split_spatial_median(tree, triangles, triangle_count, split_axis);
            break;
    }
    ...
    // Build sub-trees
    bool left_success = bvh_tree_build_recursive(tree, node->data.internal.left, 
                                                 triangles, split_index, depth + 1);
    bool right_success = bvh_tree_build_recursive(tree, node->data.internal.right, 
                                                  triangles + split_index, triangle_count - split_index, depth + 1);
...
u32 bvh_split_middle(BVHTree* tree, Triangle* triangles, u32 triangle_count, u32 axis) {
    ...
    float middle = 0.0f;
    if (axis == 0) middle = (tree->nodes[0].bounds.min.x + tree->nodes[0].bounds.max.x) * 0.5f;
    else if (axis == 1) middle = (tree->nodes[0].bounds.min.y + tree->nodes[0].bounds.max.y) * 0.5f;
    else middle = (tree->nodes[0].bounds.min.z + tree->nodes[0].bounds.max.z) * 0.5f;
    ...
    while (left <= right) {
        float center = 0.0f;
        if (axis == 0) {
            center = (triangles[left].vertices[0].x + triangles[left].vertices[1].x + triangles[left].vertices[2].x) / 3.0f;
        } else if (axis == 1) {
            center = (triangles[left].vertices[0].y + triangles[left].vertices[1].y + triangles[left].vertices[2].y) / 3.0f;
        } else {
            center = (triangles[left].vertices[0].z + triangles[left].vertices[1].z + triangles[left].vertices[2].z) / 3.0f;
        }
...
u32 bvh_split_median(BVHTree* tree, Triangle* triangles, u32 triangle_count, u32 axis) {
    ...
    // Sort triangles by centroid along axis (simplified bubble sort for demonstration)
    for (u32 i = 0; i < triangle_count - 1; i++) {
        for (u32 j = 0; j < triangle_count - i - 1; j++) {
            float center1 = 0.0f, center2 = 0.0f;
            
            if (axis == 0) {
                center1 = (triangles[j].vertices[0].x + triangles[j].vertices[1].x + triangles[j].vertices[2].x) / 3.0f;
                center2 = (triangles[j + 1].vertices[0].x + triangles[j + 1].vertices[1].x + triangles[j + 1].vertices[2].x) / 3.0f;
            } else if (axis == 1) {
                center1 = (triangles[j].vertices[0].y + triangles[j].vertices[1].y + triangles[j].vertices[2].y) / 3.0f;
                center2 = (triangles[j + 1].vertices[0].y + triangles[j + 1].vertices[1].y + triangles[j + 1].vertices[2].y) / 3.0f;
            } else {
                center1 = (triangles[j].vertices[0].z + triangles[j].vertices[1].z + triangles[j].vertices[2].z) / 3.0f;
                center2 = (triangles[j + 1].vertices[0].z + triangles[j + 1].vertices[1].z + triangles[j + 1].vertices[2].z) / 3.0f;
            }
...
bool bvh_node_intersect_ray(BVHNode* node, const Ray* ray, RayTriangleIntersection* result) {
    ...
    if (node->type == BVH_NODE_LEAF) {
        ...
        for (u32 i = 0; i < node->data.leaf.triangle_count; i++) {
            ...
            if (bvh_intersect_ray_triangle(ray, &node->data.leaf.triangles[i], &triangle_result)) {
        ...
        // Traverse children
        bool left_hit = bvh_node_intersect_ray(node->data.internal.left, ray, result);
        bool right_hit = bvh_node_intersect_ray(node->data.internal.right, ray, result);
...
void bvh_tree_calculate_statistics(BVHTree* tree) {
    ...
    tree->average_depth = tree->node_count > 0 ? (float)total_depth / tree->node_count : 0.0f;
    tree->leaf_utilization = leaf_count > 0 ? (float)total_tris_in_leaves / (leaf_count * tree->max_triangles_per_leaf) : 0.0f;
}
...
void debug_bvh_render(DebugBVHVisualization* viz, const Mat4* view_matrix, const Mat4* projection_matrix) {
    ...
    // Set uniforms
    GLint view_loc = glGetUniformLocation(viz->bounding_box_shader, "uViewMatrix");
    GLint proj_loc = glGetUniformLocation(viz->bounding_box_shader, "uProjectionMatrix");
    ...
    for (u32 i = 0; i < viz->tree_count; i++) {
        if (viz->bvh_trees[i].root) {
            debug_bvh_render_tree(viz, &viz->bvh_trees[i], view_matrix, projection_matrix);
        }
    }
...
void debug_bvh_render_node(DebugBVHVisualization* viz, BVHNode* node, const Mat4* view_matrix, const Mat4* projection_matrix) {
    ...
    // Render bounding box
    if (viz->show_bounds) {
        Vec4 color = node->is_highlighted ? tree->highlight_color : node->debug_color;
    ...
    if (node->type == BVH_NODE_INTERNAL) {
        debug_bvh_render_node(viz, node->data.internal.left, view_matrix, projection_matrix);
        debug_bvh_render_node(viz, node->data.internal.right, view_matrix, projection_matrix);
    } else {
...
void debug_bvh_render_bounding_box(DebugBVHVisualization* viz, const BoundingBox* box, const Vec4* color, const Mat4* model_matrix) {
    ...
    // Set model matrix
    GLint model_loc = glGetUniformLocation(viz->bounding_box_shader, "uModelMatrix");
...
void debug_bvh_get_statistics(DebugBVHVisualization* viz, u32* total_nodes, u32* leaf_nodes, 
    ...
    if (leaf_nodes) *leaf_nodes = leaf;
    if (internal_nodes) *internal_nodes = internal;
    if (total_triangles) *total_triangles = tris;
}
...
void debug_bvh_print_statistics(DebugBVHVisualization* viz) {
    ...
    debug_bvh_get_statistics(viz, &total_nodes, &leaf_nodes, &internal_nodes, &total_triangles);
