// Skeleton System with Bone Hierarchy Data implementation
...
bool skeleton_update_bone_hierarchy(Skeleton* skeleton) {
    ...
    // Update hierarchy starting from root bone
    return skeleton_update_bone_hierarchy_recursive(skeleton, skeleton->root_bone_id, mat4_identity());
}

bool skeleton_update_bone_hierarchy_recursive(Skeleton* skeleton, u32 bone_id, const Mat4 parent_matrix) {
    Bone* bone = skeleton_get_bone_by_id(skeleton, bone_id);
    if (!bone) {
        return false;
    }
    ...
    for (u32 i = 0; i < bone->child_count; i++) {
        skeleton_update_bone_hierarchy_recursive(skeleton, bone->child_ids[i], bone->current_pose_matrix);
    }
