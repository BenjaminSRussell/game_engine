...
typedef struct {
    ...
    struct unified_memory_allocation_t* next;
    struct unified_memory_allocation_t* prev;
...
#define UNIFIED_ALLOC(size) \
    unified_memory_alloc(size, UNIFIED_MEMORY_STRATEGY_DEFAULT, \
                       UNIFIED_MEMORY_FLAG_NONE, __FILE__, __LINE__, __func__, "unknown")

#define UNIFIED_ALLOC_ZERO(size) \
    unified_memory_alloc(size, UNIFIED_MEMORY_STRATEGY_DEFAULT, \
                       UNIFIED_MEMORY_FLAG_ZERO, __FILE__, __LINE__, __func__, "zeroed")

#define UNIFIED_ALLOC_TRACKED(size, type) \
    unified_memory_alloc(size, UNIFIED_MEMORY_STRATEGY_TRACKED, \
                       UNIFIED_MEMORY_FLAG_TRACK, __FILE__, __LINE__, __func__, type)

#define UNIFIED_ALLOC_ALIGNED(size, alignment) \
    unified_memory_alloc(size, UNIFIED_MEMORY_STRATEGY_DEFAULT, \
                       UNIFIED_MEMORY_FLAG_ALIGN, __FILE__, __LINE__, __func__, "aligned")
...
#define UNIFIED_NEW_ARRAY(type, count) \
    ((type*)unified_memory_alloc(sizeof(type) * (count), UNIFIED_MEMORY_STRATEGY_TRACKED, \
                                UNIFIED_MEMORY_FLAG_ZERO, __FILE__, __LINE__, __func__, #type "[]"))

#define UNIFIED_NEW_PERSISTENT(type) \
    ((type*)unified_memory_alloc(sizeof(type), UNIFIED_MEMORY_STRATEGY_TRACKED, \
                                UNIFIED_MEMORY_FLAG_TRACK | UNIFIED_MEMORY_FLAG_PERSISTENT, \
                                __FILE__, __LINE__, __func__, #type))

// Reallocation macros
#define UNIFIED_REALLOC(ptr, new_size) \
    unified_memory_realloc(ptr, new_size, UNIFIED_MEMORY_STRATEGY_DEFAULT, \
                         UNIFIED_MEMORY_FLAG_NONE, __FILE__, __LINE__, __func__, "realloc")

#define UNIFIED_REALLOC_TRACKED(ptr, new_size, type) \
    unified_memory_realloc(ptr, new_size, UNIFIED_MEMORY_STRATEGY_TRACKED, \
                         UNIFIED_MEMORY_FLAG_TRACK, __FILE__, __LINE__, __func__, type)

// Free macros
#define UNIFIED_FREE(ptr) unified_memory_free(ptr, __FILE__, __LINE__, __func__)

// String duplication macros
#define UNIFIED_STRDUP(str) \
    unified_memory_strdup(str, UNIFIED_MEMORY_STRATEGY_TRACKED, \
                       UNIFIED_MEMORY_FLAG_TRACK, __FILE__, __LINE__, __func__)

#define UNIFIED_STRDUP_TEMP(str) \
    unified_memory_strdup(str, UNIFIED_MEMORY_STRATEGY_STACK, \
                       UNIFIED_MEMORY_FLAG_TEMP, __FILE__, __LINE__, __func__)

// Array allocation macros
#define UNIFIED_CALLOC(count, size) \
    unified_memory_calloc(count, size, UNIFIED_MEMORY_STRATEGY_DEFAULT, \
                        UNIFIED_MEMORY_FLAG_ZERO, __FILE__, __LINE__, __func__, "calloc")

#define UNIFIED_CALLOC_TRACKED(count, size, type) \
    unified_memory_calloc(count, size, UNIFIED_MEMORY_STRATEGY_TRACKED, \
                        UNIFIED_MEMORY_FLAG_TRACK | UNIFIED_MEMORY_FLAG_ZERO, \
                        __FILE__, __LINE__, __func__, type)

/* ============================================================================
 * MIGRATION MACROS - For existing code consolidation
 * ============================================================================ */

// Direct replacements for standard library functions
#define malloc(size) UNIFIED_ALLOC(size)
#define calloc(count, size) UNIFIED_CALLOC(count, size)
#define realloc(ptr, size) UNIFIED_REALLOC(ptr, size)
#define free(ptr) UNIFIED_FREE(ptr)
#define strdup(str) UNIFIED_STRDUP(str)

// Replacements for memory_utils.h functions
#define memory_utils_malloc_zeroed(size, type) UNIFIED_ALLOC_TRACKED(size, type)
#define memory_utils_calloc(count, size, type) UNIFIED_CALLOC_TRACKED(count, size, type)
#define memory_utils_realloc(ptr, size, type) UNIFIED_REALLOC_TRACKED(ptr, size, type)
#define memory_utils_free(ptr, type) UNIFIED_FREE(ptr)
#define memory_utils_strdup(str, context) UNIFIED_STRDUP(str)

// Replacements for unified memory allocator functions
#define unified_memory_alloc(size, strategy, flags, file, line, function) \
    unified_memory_alloc(size, strategy, flags, file, line, function, "unified")

#define UNIFIED_ALLOC(size) UNIFIED_ALLOC(size)
#define UNIFIED_FREE(ptr) UNIFIED_FREE(ptr)
#define UNIFIED_REALLOC(ptr, size) UNIFIED_REALLOC(ptr, size)

/* ============================================================================
 * VALIDATION MACROS - Enhanced error checking
 * ============================================================================ */
...
#define VALIDATE_POINTER_RETURN_NULL(ptr) \
    do { \
        if (!unified_memory_validate_pointer(ptr)) { \
            REPORT_ERROR(ERROR_INVALID_PARAMETER, "Invalid pointer detected"); \
            return NULL; \
...
#define CHECK_MEMORY_CORRUPTION(ptr) \
    do { \
        if (unified_memory_check_corruption(ptr)) { \
            REPORT_ERROR(ERROR_BUFFER_OVERFLOW, "Memory corruption detected"); \
        } \
    } while(0)
...
#define MEMORY_POOL_ALLOC(type, pool_size) \
    unified_memory_pool_alloc(sizeof(type), pool_size, \
                            UNIFIED_MEMORY_FLAG_ZERO, __FILE__, __LINE__, __func__)

#define MEMORY_ARENA_ALLOC(size) \
    unified_memory_arena_alloc(size, UNIFIED_MEMORY_FLAG_ZERO, \
                              __FILE__, __LINE__, __func__)

#define MEMORY_STACK_ALLOC(size) \
    unified_memory_stack_alloc(size, UNIFIED_MEMORY_FLAG_TEMP, \
                              __FILE__, __LINE__, __func__)
...
#define DEBUG_FREE(ptr) do { VALIDATE_POINTER(ptr); UNIFIED_FREE(ptr); } while(0)
...
#define THREAD_SAFE_REALLOC(ptr, size) UNIFIED_REALLOC_TRACKED(ptr, size, "thread_safe")
#else
...
void memory_boundary_get_stats(memory_boundary_t* boundary, unified_memory_stats_t* stats);
...
void unified_memory_print_allocations_by_strategy(unified_memory_strategy_t strategy);
