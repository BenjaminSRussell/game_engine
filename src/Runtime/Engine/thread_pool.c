...
ThreadPool *thread_pool_create(u32 thread_count) {
  ...
  ThreadPool *pool = ALLOC_TYPE_ZERO(ThreadPool);
  if (!pool) {
  ...
  // Create threads
  pool->threads = ALLOC_ARRAY_ZERO(ThreadData, thread_count);
  if (!pool->threads) {
  ...
  for (u32 i = 0; i < thread_count; i++) {
    ...
    thread->handle = CreateThread(
        NULL, 0, (LPTHREAD_START_ROUTINE)worker_thread, thread, 0, NULL);
  ...
  // Create job memory pool
  MemoryPoolConfig job_pool_config = {.block_size = sizeof(Job),
                                      .block_count = 2048, // 2K jobs
                                      .auto_expand = true};
  pool->job_pool = unified_memory_pool_create(&job_pool_config);
  if (!pool->job_pool) {
    LOG_ERROR("Failed to create job memory pool");
    // Continue without pool, will fall back to regular allocation
  } else {
    LOG_INFO(" Job memory pool created: %zu jobs", job_pool_config.block_count);
  }
...
void thread_pool_destroy(ThreadPool *pool) {
  ...
  // Destroy job pool
  if (pool->job_pool) {
    unified_memory_pool_destroy(pool->job_pool);
  }
  ...
  LOG_INFO("Thread pool stats: Submitted: %llu, Completed: %llu, Failed: %llu",
           pool->total_jobs_submitted, pool->total_jobs_completed,
...
u32 thread_pool_submit(ThreadPool *pool, JobFunction function, void *user_data,
                       const char *name) {
  ...
  // Allocate from job pool if available
  Job *job = NULL;
  if (pool->job_pool) {
    job = (Job *)POOL_ALLOC(pool->job_pool, sizeof(Job));
  }
  if (!job) {
...
u32 thread_pool_submit_with_dependencies(ThreadPool *pool, JobFunction function,
                                         void *user_data, u32 *dependency_ids,
                                         u32 dependency_count,
                                         const char *name) {
  if (!pool || !pool->initialized || !function) {
  ...
  if (dependency_count > 0 && dependency_ids) {
    job->dependencies = ALLOC_ARRAY(Job *, dependency_count);
    if (job->dependencies) {
...
void thread_pool_get_stats(ThreadPool *pool, u64 *submitted, u64 *completed,
                           u64 *failed) {
  if (!pool || !pool->initialized) {
    if (submitted)
      *submitted = 0;
  ...
  if (submitted)
    *submitted = pool->total_jobs_submitted;
  if (completed)
    *completed = pool->total_jobs_completed;
  if (failed)
    *failed = pool->total_jobs_failed;
...
static void *worker_thread(void *arg) {
  ...
  while (!thread->should_stop) {
    ...
    if (job) {
      ...
      SleepConditionVariableCS(&pool->queue_condition, &pool->queue_mutex,
                               INFINITE);
...
static void complete_job(ThreadPool *pool, Job *job, bool success) {
  ...
  LOG_DEBUG("Job completed: %s (ID: %u) - %s", job->name, job->job_id,
...
static void cleanup_job(Job *job) {
  ThreadPool *pool = g_global_thread_pool;

  if (job->dependencies) {
    FREE(job->dependencies);
  }

  // Free to pool if available
  if (pool && pool->job_pool) {
    POOL_FREE(pool->job_pool, job);
  } else {
    FREE(job);
  }
}
...
ThreadPool *thread_pool_get_global(void) { return g_global_thread_pool; }
