...
void entity_set_parent(uint64_t entity_id, uint64_t parent_id) {
  if (entity_id == parent_id) {
    LOG_WARN("Cannot set entity %llu as its own parent",
  ...
  if (parent_node->child_count < MAX_CHILDREN_PER_ENTITY) {
    ...
    LOG_INFO("Entity %llu now child of %llu", (unsigned long long)entity_id,
             (unsigned long long)parent_id);
  } else {
    LOG_WARN("Parent entity %llu has max children",
             (unsigned long long)parent_id);
  }
}
...
uint32_t entity_get_children(uint64_t entity_id, uint64_t *out_children,
                             uint32_t max_count) {
  EntityNode *node = get_node(entity_id);
  if (!node)
    return 0;
  ...
  uint32_t count =
      node->child_count < max_count ? node->child_count : max_count;
  memcpy(out_children, node->children, count * sizeof(uint64_t));
  return count;
}

void entity_traverse_hierarchy(uint64_t root_id,
                               void (*callback)(uint64_t entity_id,
                                                void *user_data),
                               void *user_data) {
  if (!callback)
    return;
