 ...
 * =================================================================================================
 *
 * Implementation of dynamic weather with smooth transitions, time of day, and
 * atmospheric effects.
 *
 * =================================================================================================
 */
...
WeatherParameters *weather_params_create(WeatherType type) {
  ...
  case WEATHER_CLEAR:
    ...
    params->sky_color[0] = 0.53f;
    params->sky_color[1] = 0.81f;
    params->sky_color[2] = 0.92f;
  ...
  case WEATHER_PARTLY_CLOUDY:
    ...
    params->sky_color[0] = 0.63f;
    params->sky_color[1] = 0.78f;
    params->sky_color[2] = 0.85f;
  ...
  case WEATHER_CLOUDY:
    ...
    params->sky_color[0] = 0.73f;
    params->sky_color[1] = 0.76f;
    params->sky_color[2] = 0.78f;
  ...
  case WEATHER_OVERCAST:
    ...
    params->sky_color[0] = 0.5f;
    params->sky_color[1] = 0.52f;
    params->sky_color[2] = 0.54f;
  ...
  case WEATHER_LIGHT_RAIN:
    ...
    params->sky_color[0] = 0.45f;
    params->sky_color[1] = 0.48f;
    params->sky_color[2] = 0.5f;
  ...
  case WEATHER_RAIN:
    ...
    params->sky_color[0] = 0.35f;
    params->sky_color[1] = 0.38f;
    params->sky_color[2] = 0.4f;
    params->cloud_coverage = 1.0f;
    params->has_precipitation = true;
  ...
  case WEATHER_HEAVY_RAIN:
    ...
    params->sky_color[0] = 0.25f;
    params->sky_color[1] = 0.28f;
    params->sky_color[2] = 0.3f;
    params->cloud_coverage = 1.0f;
    params->has_precipitation = true;
  ...
  case WEATHER_THUNDERSTORM:
    ...
    params->sky_color[0] = 0.15f;
    params->sky_color[1] = 0.18f;
    params->sky_color[2] = 0.2f;
    params->cloud_coverage = 1.0f;
    params->has_precipitation = true;
  ...
  case WEATHER_LIGHT_SNOW:
    ...
    params->sky_color[0] = 0.75f;
    params->sky_color[1] = 0.78f;
    params->sky_color[2] = 0.8f;
    params->cloud_coverage = 0.8f;
    params->has_precipitation = true;
  ...
  case WEATHER_SNOW:
    ...
    params->sky_color[0] = 0.7f;
    params->sky_color[1] = 0.73f;
    params->sky_color[2] = 0.75f;
    params->cloud_coverage = 0.9f;
    params->has_precipitation = true;
  ...
  case WEATHER_BLIZZARD:
    ...
    params->sky_color[0] = 0.6f;
    params->sky_color[1] = 0.63f;
    params->sky_color[2] = 0.65f;
    params->cloud_coverage = 1.0f;
    params->has_precipitation = true;
  ...
  case WEATHER_FOG:
    ...
    params->sky_color[0] = 0.65f;
    params->sky_color[1] = 0.68f;
    params->sky_color[2] = 0.7f;
  ...
  case WEATHER_SANDSTORM:
    ...
    params->sky_color[0] = 0.8f;
    params->sky_color[1] = 0.7f;
    params->sky_color[2] = 0.5f;
  ...
  case WEATHER_HAIL:
    ...
    params->sky_color[0] = 0.4f;
    params->sky_color[1] = 0.43f;
    params->sky_color[2] = 0.45f;
    params->cloud_coverage = 1.0f;
    params->has_precipitation = true;
  ...
  case WEATHER_SLEET:
    ...
    params->sky_color[0] = 0.5f;
    params->sky_color[1] = 0.53f;
    params->sky_color[2] = 0.55f;
    params->cloud_coverage = 0.9f;
    params->has_precipitation = true;
    params->precipitation_intensity = 0.5f;
    params->sun_intensity_multiplier = 0.4f;
...
bool weather_params_lerp(const WeatherParameters *from,
                         const WeatherParameters *to, float t,
                         WeatherParameters *result) {
  if (!from || !to || !result || t < 0.0f || t > 1.0f)
    return false;
  ...
  for (int i = 0; i < 3; i++) {
    result->sky_color[i] =
        from->sky_color[i] * (1.0f - t) + to->sky_color[i] * t;
    result->horizon_color[i] =
        from->horizon_color[i] * (1.0f - t) + to->horizon_color[i] * t;
    result->fog_color[i] =
        from->fog_color[i] * (1.0f - t) + to->fog_color[i] * t;
    result->wind_direction[i] =
        from->wind_direction[i] * (1.0f - t) + to->wind_direction[i] * t;
  }

  // Float parameters
  result->cloud_coverage =
      from->cloud_coverage * (1.0f - t) + to->cloud_coverage * t;
  ...
  result->precipitation_intensity = from->precipitation_intensity * (1.0f - t) +
                                    to->precipitation_intensity * t;
  result->precipitation_size =
      from->precipitation_size * (1.0f - t) + to->precipitation_size * t;
  result->precipitation_speed =
      from->precipitation_speed * (1.0f - t) + to->precipitation_speed * t;
  result->fog_density = from->fog_density * (1.0f - t) + to->fog_density * t;
  result->fog_height_falloff =
      from->fog_height_falloff * (1.0f - t) + to->fog_height_falloff * t;
  result->fog_start_distance =
      from->fog_start_distance * (1.0f - t) + to->fog_start_distance * t;
  result->fog_end_distance =
      from->fog_end_distance * (1.0f - t) + to->fog_end_distance * t;
  result->sun_intensity_multiplier =
      from->sun_intensity_multiplier * (1.0f - t) +
      to->sun_intensity_multiplier * t;
  result->ambient_intensity_multiplier =
      from->ambient_intensity_multiplier * (1.0f - t) +
      to->ambient_intensity_multiplier * t;
  result->shadow_intensity_multiplier =
      from->shadow_intensity_multiplier * (1.0f - t) +
      to->shadow_intensity_multiplier * t;
  result->wind_speed = from->wind_speed * (1.0f - t) + to->wind_speed * t;
  result->wind_gustiness =
      from->wind_gustiness * (1.0f - t) + to->wind_gustiness * t;
  result->ambient_volume =
      from->ambient_volume * (1.0f - t) + to->ambient_volume * t;
  result->visibility_range =
      from->visibility_range * (1.0f - t) + to->visibility_range * t;
  result->movement_speed_modifier = from->movement_speed_modifier * (1.0f - t) +
                                    to->movement_speed_modifier * t;
  result->fire_extinguish_rate =
      from->fire_extinguish_rate * (1.0f - t) + to->fire_extinguish_rate * t;

  // Boolean parameters (threshold-based)
  result->has_precipitation =
      t < 0.5f ? from->has_precipitation : to->has_precipitation;
  result->precipitation_affects_water = t < 0.5f
                                            ? from->precipitation_affects_water
                                            : to->precipitation_affects_water;

  // String parameters
  strcpy(result->name, t < 0.5f ? from->name : to->name);
  strcpy(result->ambient_sound,
         t < 0.5f ? from->ambient_sound : to->ambient_sound);

  // Texture parameters
  result->cloud_texture = t < 0.5f ? from->cloud_texture : to->cloud_texture;
  result->precipitation_texture =
      t < 0.5f ? from->precipitation_texture : to->precipitation_texture;
...
bool weather_params_save(const WeatherParameters *params,
                         const char *filename) {
  if (!params || !filename)
    return false;
...
WeatherZone *weather_zone_create(const char *name, const float bounds_min[3],
  ...
  if (!name || !bounds_min || !bounds_max)
    return NULL;

  // Expand zones array if needed
  if (weather_zone_count >= weather_zone_capacity) {
    uint32_t new_capacity =
        weather_zone_capacity == 0 ? 16 : weather_zone_capacity * 2;
    WeatherZone *new_zones =
        realloc(weather_zones, new_capacity * sizeof(WeatherZone));
    if (!new_zones)
      return NULL;
...
bool weather_zone_check_position(const WeatherZone *zone,
                                 const float position[3]) {
  if (!zone || !position)
    return false;

  return (position[0] >= zone->bounds_min[0] &&
...
bool transition_update(WeatherTransition *transition, float delta_time,
  ...
  if (!transition || !to_params || !transition->is_active)
    return false;
  ...
  // Get "from" parameters
  WeatherParameters *from_params =
      weather_params_create(transition->from_weather);
  if (!from_params)
    return false;

  // Interpolate between from and to parameters
  bool success =
      weather_params_lerp(from_params, to_params, transition->progress,
...
bool transition_interrupt(WeatherTransition *transition,
                          WeatherType new_weather, float new_duration) {
  if (!transition)
    return false;
  ...
  return transition_start(transition, current_weather, new_weather,
                          new_duration);
}
