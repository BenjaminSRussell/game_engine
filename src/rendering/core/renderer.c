...
static void renderer_log_validation_error(Renderer *renderer, const char *error);
...
Renderer *renderer_create(uint32_t width, uint32_t height) {
    ...
    if (!window_init(renderer->window, width, height, "VoxelForge Engine", false)) {
    ...
    if (!renderer_validate_framebuffer(renderer->main_framebuffer)) {
        renderer_log_validation_error(renderer, "Main framebuffer validation failed");
    ...
    LOG_INFO("Renderer created successfully (%ux%u) with validation enabled", width, height);
    return renderer;
}
...
void renderer_destroy(Renderer *renderer) {
    ...
    if (renderer->validation_enabled) {
        ...
        LOG_INFO("  Uptime: %lu seconds", 
                (renderer_get_timestamp() - renderer->initialization_time) / 1000000000ULL);
...
static void renderer_window_resize_callback(Window *window, u32 width, u32 height) {
    if (g_renderer) {
...
static bool renderer_validate_dimensions(uint32_t width, uint32_t height) {
    ...
    if (width < 64 || height < 64) {
        LOG_ERROR("Renderer dimensions too small: %ux%u (minimum 64x64)", width, height);
        return false;
    }
    
    // Check maximum dimensions (reasonable limits)
    if (width > 16384 || height > 16384) {
        LOG_ERROR("Renderer dimensions too large: %ux%u (maximum 16384x16384)", width, height);
        return false;
    }
...
static bool renderer_validate_window(Window *window) {
    ...
    if (window->width == 0 || window->height == 0) {
        LOG_ERROR("Window has invalid dimensions: %ux%u", window->width, window->height);
        return false;
    }
...
static bool renderer_validate_framebuffer(Framebuffer *framebuffer) {
    ...
    if (framebuffer->width == 0 || framebuffer->height == 0) {
        LOG_ERROR("Framebuffer has invalid dimensions: %ux%u", 
...
static void renderer_log_validation_error(Renderer *renderer, const char *error) {
    ...
    LOG_ERROR("Renderer Validation Error [%lu]: %s", 
             renderer->validation_errors, error);
}
...
bool renderer_validate_state(Renderer *renderer) {
    ...
    // Validate window
    if (!renderer_validate_window(renderer->window)) {
        renderer_log_validation_error(renderer, "Window validation failed during state check");
        valid = false;
    }
    
    // Validate device
    if (!renderer_validate_device(renderer->device)) {
        renderer_log_validation_error(renderer, "Device validation failed during state check");
        valid = false;
    }
    
    // Validate framebuffer
    if (!renderer_validate_framebuffer(renderer->main_framebuffer)) {
        renderer_log_validation_error(renderer, "Framebuffer validation failed during state check");
        valid = false;
    }
    
    // Check for excessive validation errors
    if (renderer->validation_errors > 100) {
        renderer_log_validation_error(renderer, "Excessive validation errors detected");
        valid = false;
    }
    ...
    if (time_since_init > 60000000000ULL && renderer->stats.frames_rendered == 0) { // 1 minute
        renderer_log_validation_error(renderer, "No frames rendered in first minute");
        valid = false;
    }
...
void renderer_get_validation_stats(Renderer *renderer, uint64_t *validation_errors,
                                  uint64_t *initialization_time, uint64_t *last_validation_time) {
    if (!renderer) return;
    
    if (validation_errors) *validation_errors = renderer->validation_errors;
    if (initialization_time) *initialization_time = renderer->initialization_time;
    if (last_validation_time) *last_validation_time = renderer->last_validation_time;
}
