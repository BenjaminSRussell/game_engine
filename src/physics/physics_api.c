...
static void quaternion_from_euler(float* quat, float yaw, float pitch, float roll) {
...
static void quaternion_rotate_vector(float* result, const float* quat, const float* vec) {
...
PhysicsWorld* physics_world_create(PhysicsConfig config) {
    ...
    if (config.gravity[0] == 0.0f && config.gravity[1] == 0.0f && config.gravity[2] == 0.0f) {
        vector3_set(world->config.gravity, 0.0f, -9.81f, 0.0f);
    } else {
        vector3_copy(world->config.gravity, config.gravity);
    }
    
    world->config.timestep = (config.timestep > 0.0f) ? config.timestep : TIMESTEP_DEFAULT;
    world->config.max_substeps = (config.max_substeps > 0) ? config.max_substeps : MAX_SUBSTEPS_DEFAULT;
    world->config.enable_debug_draw = config.enable_debug_draw;
    
    log_info("Physics world created with gravity: (%.2f, %.2f, %.2f)", 
             world->config.gravity[0], world->config.gravity[1], world->config.gravity[2]);
...
void physics_world_step(PhysicsWorld* world, float dt) {
    if (!world || world->is_stepping) return;
    ...
    int substeps = 0;
    while (world->accumulated_time >= world->config.timestep && substeps < world->config.max_substeps) {
        // Integrate forces
        for (uint32_t i = 0; i < world->body_count; i++) {
            RigidBodyInternal* body = &world->bodies[i];
            if (!body->is_active || body->is_static) continue;
            
            // Apply gravity
            if (body->mass > 0.0f) {
                float gravity_force[3];
                vector3_multiply(gravity_force, world->config.gravity, body->mass);
                vector3_add(body->force_accumulator, body->force_accumulator, gravity_force);
            }
            
            // Update velocity (F = ma, a = F/m, v = v0 + a*dt)
            float acceleration[3];
            vector3_multiply(acceleration, body->force_accumulator, body->inverse_mass);
        ...
        for (uint32_t i = 0; i < world->constraint_count; i++) {
            ...
            if (!constraint->enabled || !constraint->is_active) continue;
        ...
        for (uint32_t i = 0; i < world->controller_count; i++) {
            ...
            physics_body_set_velocity((RigidBody*)controller->body, controller->velocity);
        }
...
RigidBody* physics_body_create(uint32_t id, RigidBodyType type, CollisionShape* shape) {
    // Find the world that owns this shape
    PhysicsWorld* world = NULL;
    // This is a simplified approach - in a real implementation, we'd track world->shape relationships
    // For now, we'll assume the shape is valid and create a body
    
    if (!shape) return NULL;
    
    // Find a free world (simplified - should be passed as parameter)
    // TODO: Pass world as parameter to body_create
    
    return NULL; // TODO: Implement body creation
}
...
void physics_body_set_position(RigidBody* body, const float* position) { /* TODO */ }
void physics_body_set_rotation(RigidBody* body, const float* rotation) { /* TODO */ }
void physics_body_set_velocity(RigidBody* body, const float* velocity) { /* TODO */ }
void physics_body_set_angular_velocity(RigidBody* body, const float* angular_velocity) { /* TODO */ }
void physics_body_get_position(RigidBody* body, float* out_position) { /* TODO */ }
void physics_body_get_rotation(RigidBody* body, float* out_rotation) { /* TODO */ }
void physics_body_get_velocity(RigidBody* body, float* out_velocity) { /* TODO */ }
void physics_body_get_angular_velocity(RigidBody* body, float* out_angular_velocity) { /* TODO */ }
void physics_body_apply_force(RigidBody* body, const float* force) { /* TODO */ }
void physics_body_apply_impulse(RigidBody* body, const float* impulse) { /* TODO */ }
void physics_body_apply_torque(RigidBody* body, const float* torque) { /* TODO */ }
...
void physics_body_set_restitution(RigidBody* body, float restitution) { /* TODO */ }
...
void physics_body_get_aabb(RigidBody* body, float* min_out, float* max_out) { /* TODO */ }
bool physics_body_get_velocity_at_point(RigidBody* body, const float* point, float* out_vel) { return false; /* TODO */ }

CollisionShape* physics_shape_capsule_create(float radius, float height) { return NULL; /* TODO */ }
CollisionShape* physics_shape_cylinder_create(float radius, float height) { return NULL; /* TODO */ }
CollisionShape* physics_shape_cone_create(float radius, float height) { return NULL; /* TODO */ }
CollisionShape* physics_shape_convex_hull_create(float* vertices, uint32_t vertex_count) { return NULL; /* TODO */ }
CollisionShape* physics_shape_mesh_create(float* vertices, uint32_t vertex_count, uint32_t* indices, uint32_t index_count) { return NULL; /* TODO */ }
CollisionShape* physics_shape_heightfield_create(float* heights, uint32_t width, uint32_t depth, float scale[3]) { return NULL; /* TODO */ }
CollisionShape* physics_shape_compound_create(void) { return NULL; /* TODO */ }
void physics_shape_destroy(CollisionShape* shape) { /* TODO */ }
void physics_shape_compound_add_shape(CollisionShape* compound, CollisionShape* shape, float* transform) { /* TODO */ }
void physics_shape_calculate_local_inertia(CollisionShape* shape, float mass, float* inertia) { /* TODO */ }
ShapeType physics_shape_get_type(CollisionShape* shape) { return SHAPE_SPHERE; /* TODO */ }

RigidBody* physics_world_add_body(PhysicsWorld* world, RigidBody* body) { return NULL; /* TODO */ }
void physics_world_remove_body(PhysicsWorld* world, RigidBody* body) { /* TODO */ }
bool physics_world_raycast(PhysicsWorld* world, const float* origin, const float* direction, float max_distance, RayHit* out_hit) { return false; /* TODO */ }
void physics_world_query_aabb(PhysicsWorld* world, const float* min_aabb, const float* max_aabb, RigidBody** out_bodies, uint32_t* out_count, uint32_t max_count) { /* TODO */ }

Constraint* physics_constraint_point_to_point_create(RigidBody* body_a, RigidBody* body_b, const float* pivot_a, const float* pivot_b) { return NULL; /* TODO */ }
Constraint* physics_constraint_hinge_create(RigidBody* body_a, RigidBody* body_b, const float* pivot_a, const float* pivot_b, const float* axis_a, const float* axis_b) { return NULL; /* TODO */ }
Constraint* physics_constraint_slider_create(RigidBody* body_a, RigidBody* body_b, const float* axis_a, const float* axis_b) { return NULL; /* TODO */ }
Constraint* physics_constraint_fixed_create(RigidBody* body_a, RigidBody* body_b, const float* transform_a, const float* transform_b) { return NULL; /* TODO */ }
void physics_constraint_destroy(Constraint* constraint) { /* TODO */ }
void physics_constraint_set_enabled(Constraint* constraint, bool enabled) { /* TODO */ }
void physics_constraint_set_breaking_impulse(Constraint* constraint, float threshold) { /* TODO */ }
bool physics_constraint_is_enabled(Constraint* constraint) { return false; /* TODO */ }

CharacterController* physics_character_controller_create(float radius, float height, float step_height) { return NULL; /* TODO */ }
void physics_character_controller_destroy(CharacterController* controller) { /* TODO */ }
void physics_character_controller_set_world(CharacterController* controller, PhysicsWorld* world) { /* TODO */ }
void physics_character_controller_set_position(CharacterController* controller, const float* position) { /* TODO */ }
void physics_character_controller_get_position(CharacterController* controller, float* out_position) { /* TODO */ }
void physics_character_controller_set_velocity(CharacterController* controller, const float* velocity) { /* TODO */ }
void physics_character_controller_get_velocity(CharacterController* controller, float* out_velocity) { /* TODO */ }
void physics_character_controller_set_gravity(CharacterController* controller, float gravity) { /* TODO */ }
void physics_character_controller_set_fall_speed(CharacterController* controller, float fall_speed) { /* TODO */ }
void physics_character_controller_set_jump_speed(CharacterController* controller, float jump_speed) { /* TODO */ }
void physics_character_controller_set_slope_angle(CharacterController* controller, float slope_angle) { /* TODO */ }
bool physics_character_controller_on_ground(CharacterController* controller) { return false; /* TODO */ }
void physics_character_controller_jump(CharacterController* controller) { /* TODO */ }
void physics_character_controller_move(CharacterController* controller, const float* direction, float dt) { /* TODO */ }

void physics_world_debug_draw(PhysicsWorld* world, DebugLine** out_lines, uint32_t* out_count) { /* TODO */ }
void physics_world_set_debug_drawer(PhysicsWorld* world, void (*draw_line)(const float* from, const float* to, const float* color)) { /* TODO */ }
