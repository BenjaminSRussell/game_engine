...
static void
animation_synthesis_generate_keyframes(AnimationSynthesisContext *context,
                                       f32 duration);
static Vec3
animation_synthesis_calc_foot_trajectory(f32 phase,
                                         const WalkParameters *params);
static Quat animation_synthesis_calc_spine_twist(f32 phase, f32 arm_swing);
static void
animation_synthesis_apply_personality(AnimationSynthesisContext *context,
                                      const AIAnimationParams *ai);
static AnimationClip *animation_synthesis_create_clip(const char *name,
                                                      u32 channel_count,
                                                      f32 duration);
...
void animation_synthesis_init(AnimationSynthesisSystem *system) {
  ...
  LOG_INFO("  AI synthesis: %s",
           system->enable_ai_synthesis ? "enabled" : "disabled");
}
...
void animation_synthesis_shutdown(AnimationSynthesisSystem *system) {
  ...
  LOG_INFO("  Cache hit ratio: %.2f%%",
           system->cache_hits + system->cache_misses > 0
               ? (100.0f * system->cache_hits) /
                     (system->cache_hits + system->cache_misses)
               : 0.0f);

  memset(system, 0, sizeof(AnimationSynthesisSystem));
}

void animation_synthesis_update(AnimationSynthesisSystem *system,
                                f32 delta_time) {
  if (!system)
    return;
  ...
  for (u32 i = 0; i < ANIM_SYNTH_COUNT; i++) {
    ...
    if (context->generated_clip &&
      ...
      animation_synthesis_generate_keyframes(context,
                                             context->generated_clip->duration);
    }
  }
}

AnimationSynthesisContext *
animation_synthesis_get_context(AnimationSynthesisSystem *system,
                                AnimationSynthType type) {
  if (!system || type >= ANIM_SYNTH_COUNT)
    return NULL;
  return &system->contexts[type];
}

void animation_synthesis_start(AnimationSynthesisSystem *system,
                               AnimationSynthType type) {
  if (!system || type >= ANIM_SYNTH_COUNT)
    return;
...
void animation_synthesis_stop(AnimationSynthesisSystem *system,
                              AnimationSynthType type) {
  if (!system || type >= ANIM_SYNTH_COUNT)
    return;
...
void animation_synthesis_set_speed(AnimationSynthesisSystem *system,
                                   AnimationSynthType type, f32 speed) {
  if (!system || type >= ANIM_SYNTH_COUNT)
    return;
...
AnimationClip *animation_synthesis_generate(AnimationSynthesisSystem *system,
  ...
  case ANIM_SYNTH_WALK_CYCLE:
    context->params.walk = animation_synthesis_calc_walk_params(speed);
    return animation_synthesis_walk_cycle(system, &context->params.walk,
                                          duration);

  case ANIM_SYNTH_RUN_CYCLE:
    context->params.run = animation_synthesis_calc_run_params(speed);
    return animation_synthesis_run_cycle(system, &context->params.run,
                                         duration);

  case ANIM_SYNTH_IDLE_BREATH:
    context->params.idle = animation_synthesis_calc_idle_params(0.5f);
    return animation_synthesis_idle_breathing(system, &context->params.idle,
                                              duration);
...
AnimationClip *animation_synthesis_walk_cycle(AnimationSynthesisSystem *system,
  ...
  AnimationClip *clip =
      animation_synthesis_create_clip("ProceduralWalk", 16, duration);
  if (!clip)
    return NULL;
  ...
  for (u32 joint_idx = 0; joint_idx < system->joint_count; joint_idx++) {
    ...
    channel->keyframes =
        malloc(sizeof(AnimationKeyframe) * ANIM_SYNTH_MAX_KEYFRAMES);
    ...
    for (u32 frame = 0; frame < ANIM_SYNTH_MAX_KEYFRAMES; frame++) {
      ...
      if (system->joints[joint_idx].is_foot_joint) {
        ...
        f32 foot_phase =
            (joint_idx == 14 || joint_idx == 15) ? phase : phase + 0.5f;
        if (foot_phase >= 1.0f)
          foot_phase -= 1.0f;

        Vec3 foot_pos =
            animation_synthesis_calc_foot_trajectory(foot_phase, params);
        ...
        f32 arm_phase =
            (joint_idx >= 6 && joint_idx <= 8) ? phase + 0.5f : phase;
        if (arm_phase >= 1.0f)
          arm_phase -= 1.0f;
        ...
        keyframe->rotation =
            quat_from_axis_angle(vec3_up(), sinf(arm_phase * PI) * 0.2f);
  ...
  // Cache the result
  if (system->enable_caching) {
    animation_synthesis_cache_store(system, ANIM_SYNTH_WALK_CYCLE,
                                    params->speed, clip);
  }
...
AnimationClip *animation_synthesis_run_cycle(AnimationSynthesisSystem *system,
  ...
  AnimationClip *clip =
      animation_synthesis_create_clip("ProceduralRun", 16, duration);
  if (!clip)
    return NULL;
  ...
  for (u32 joint_idx = 0; joint_idx < system->joint_count; joint_idx++) {
    ...
    for (u32 frame = 0; frame < ANIM_SYNTH_MAX_KEYFRAMES; frame++) {
      ...
      if (system->joints[joint_idx].is_foot_joint) {
        ...
        Vec3 foot_pos = animation_synthesis_calc_foot_trajectory(
            foot_phase, (WalkParameters *)params);
        ...
        keyframe->position = vec3(swing_x, swing_y, 0.0f);
        keyframe->rotation =
            quat_from_axis_angle(vec3_up(), sinf(arm_phase * PI) * 0.4f);
...
AnimationClip *
animation_synthesis_idle_breathing(AnimationSynthesisSystem *system,
                                   const IdleParameters *params, f32 duration) {
  if (!system || !params)
    return NULL;

  AnimationClip *clip =
      animation_synthesis_create_clip("ProceduralIdle", 16, duration);
  if (!clip)
    return NULL;
  ...
  for (u32 joint_idx = 0; joint_idx < system->joint_count; joint_idx++) {
    ...
    for (u32 frame = 0; frame < ANIM_SYNTH_MAX_KEYFRAMES; frame++) {
      ...
      if (system->joints[joint_idx].is_spine_joint || joint_idx == 2) { // Chest
        ...
        f32 micro =
            params->micro_movements
                ? (sinf(phase * 7.3f) * 0.01f + cosf(phase * 11.7f) * 0.01f)
                : 0.0f;
...
AnimationClip *animation_synthesis_jump(AnimationSynthesisSystem *system,
  ...
  AnimationClip *clip =
      animation_synthesis_create_clip("ProceduralJump", 16, duration);
  if (!clip)
    return NULL;
  ...
  for (u32 joint_idx = 0; joint_idx < system->joint_count; joint_idx++) {
    ...
    for (u32 frame = 0; frame < ANIM_SYNTH_MAX_KEYFRAMES; frame++) {
      ...
      if (system->joints[joint_idx].is_foot_joint) {
        ...
        keyframe->rotation =
            quat_from_axis_angle(vec3_forward(), arm_swing * 0.3f);
        ...
        if (phase < 0.2f) {
          ...
          spine_compress =
              -params->crouch_depth * 0.2f * ((phase - 0.8f) / 0.2f);
        }
...
bool animation_synthesis_cache_lookup(AnimationSynthesisSystem *system,
  ...
  if (!system || !result || type >= ANIM_SYNTH_COUNT)
    return false;
...
void animation_synthesis_cache_store(AnimationSynthesisSystem *system,
                                     AnimationSynthType type, f32 speed,
                                     AnimationClip *clip) {
  if (!system || !clip || type >= ANIM_SYNTH_COUNT)
    return;
