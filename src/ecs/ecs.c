...
// Memory pools for ECS
static MemoryPool *g_entity_pool = NULL;
static MemoryPool *g_archetype_pool = NULL;
...
void ecs_world_init(World *world, u32 max_entities, u32 max_components,
  ...
  if (!g_entity_pool) {
    MemoryPoolConfig entity_pool_config = {.block_size = sizeof(EntityRecord),
                                           .block_count = max_entities,
                                           .auto_expand = true};
    g_entity_pool = unified_memory_pool_create(&entity_pool_config);
    LOG_INFO("[ECS] Entity pool created: %u entities", max_entities);
  }

  if (!g_archetype_pool) {
    MemoryPoolConfig archetype_pool_config = {.block_size =
                                                  sizeof(SimpleArchetype),
                                              .block_count = 128,
                                              .auto_expand = true};
    g_archetype_pool = unified_memory_pool_create(&archetype_pool_config);
    LOG_INFO("[ECS] Archetype pool created: 128 archetypes");
  }

  // Initialize Entity Data
  EntityData *ed = ALLOC_TYPE_ZERO(EntityData);
  ed->capacity = max_entities;
  ed->records = ALLOC_ARRAY_ZERO(EntityRecord, max_entities);
  ed->free_entity_ids = ALLOC_ARRAY(u32, max_entities);
  ed->next_entity_id = 1; // 0 is invalid
  world->entities = ed;

  // Initialize Component Data
  ComponentDataInternal *cd = ALLOC_TYPE_ZERO(ComponentDataInternal);
  cd->capacity = max_components;
  cd->infos = ALLOC_ARRAY_ZERO(ComponentInfo, max_components);
  world->component_info = cd;

  // Initialize Archetype Data
  ArchetypeData *ad = ALLOC_TYPE_ZERO(ArchetypeData);
  world->archetypes = ad;

  // CRITICAL: Create empty archetype (archetype 0) for new entities
  // All entities start with no components in this archetype
  SimpleArchetype *empty =
      POOL_ALLOC(g_archetype_pool, sizeof(SimpleArchetype));
  if (!empty) {
    empty = ALLOC_TYPE_ZERO(SimpleArchetype);
  } else {
  ...
  empty->entities = ALLOC_ARRAY_ZERO(Entity, 16);
  empty->next = NULL;
  ad->head = empty;
...
void ecs_world_free(World *world) {
  ...
  if (world->archetypes) {
    ...
    while (curr) {
      ...
      // Free archetype to pool if possible
      if (g_archetype_pool) {
        POOL_FREE(g_archetype_pool, curr);
      } else {
        FREE(curr);
      }
      curr = next;
  ...
  // Cleanup pools on last world free
  // Note: In a real implementation, track world count
  // For now, keep pools alive for reuse
}
...
Entity ecs_create_entity(World *world) {
  ...
  if (empty_arch) {
    ...
    if (empty_arch->entity_count >= empty_arch->entity_capacity) {
      ...
      Entity *new_entities = ALLOC_ARRAY_ZERO(Entity, new_cap);
...
find_or_create_archetype(World *world, ComponentType *types, u32 count) {
  ...
  SimpleArchetype *new_arch =
      POOL_ALLOC(g_archetype_pool, sizeof(SimpleArchetype));
  if (!new_arch) {
    new_arch = ALLOC_TYPE_ZERO(SimpleArchetype);
  } else {
  ...
  new_arch->entities = ALLOC_ARRAY(Entity, new_arch->entity_capacity);
  new_arch->component_data = ALLOC_ARRAY(void *, count);
  ...
  for (u32 i = 0; i < count; i++) {
    ...
    new_arch->component_data[i] =
        ALLOC_ARRAY_ZERO(u8, new_arch->entity_capacity * size);
  }
