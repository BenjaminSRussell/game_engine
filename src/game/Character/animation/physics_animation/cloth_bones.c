...
int animation_cloth_bones_init(void) {
  if (atomic_load(&g_cloth_bones_ctx.initialized)) {
    LOG_WARN_CAT(LOG_CAT_ANIMATION, "Cloth bones system already initialized");
    return 0; // Already initialized
  }

  if (pthread_mutex_init(&g_cloth_bones_ctx.global_mutex, NULL) != 0) {
    LOG_FATAL_CAT(LOG_CAT_ANIMATION,
  ...
  LOG_INFO_CAT(LOG_CAT_ANIMATION, "Cloth bones system initialized");
  return 0;
}
...
void animation_cloth_bones_shutdown(void) {
  ...
  LOG_INFO_CAT(LOG_CAT_ANIMATION, "Cloth bones system shutdown");
}
...
int animation_cloth_bones_create(animation_cloth_bones_handle_t *out_handle,
  ...
  if (index == -1) {
    pthread_mutex_unlock(&g_cloth_bones_ctx.global_mutex);
    LOG_ERROR_CAT(LOG_CAT_ANIMATION, "Cloth bones capacity reached");
...
void animation_cloth_bones_debug_print(void) {
  LOG_INFO_CAT(LOG_CAT_ANIMATION, "Cloth Bones Debug Info:");
  LOG_INFO_CAT(LOG_CAT_ANIMATION, "  Active Sims: %u",
           atomic_load(&g_cloth_bones_ctx.stat_active_sims));
}
