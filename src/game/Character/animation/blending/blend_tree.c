...
// Types removed - using math.h types
...
// Helpers removed - using math.h functions
...
static void pose_init(pose_t *pose, uint32_t bone_count) {
  pose->translations = calloc(bone_count, sizeof(Vec3));
  pose->rotations = calloc(bone_count, sizeof(Quat));
  pose->scales = calloc(bone_count, sizeof(Vec3));
  pose->bone_count = bone_count;

  for (uint32_t i = 0; i < bone_count; i++) {
    pose->scales[i] = (Vec3){1, 1, 1};
    pose->rotations[i] = (Quat){0, 0, 0, 1};
  }
}

static void pose_cleanup(pose_t *pose) {
  if (pose->translations)
    free(pose->translations);
  if (pose->rotations)
    free(pose->rotations);
  if (pose->scales)
    free(pose->scales);
...
static void pose_blend(pose_t *out, const pose_t *a, const pose_t *b, float t) {
  uint32_t count =
      (a->bone_count < b->bone_count) ? a->bone_count : b->bone_count;
  if (out->bone_count < count)
    count = out->bone_count;
...
static void evaluate_node(animation_blend_tree_internal_t *tree,
                          uint16_t node_idx, float dt) {
  ...
  case NODE_TYPE_CLIP:
    ...
    // sample_animation(node->data.clip.clip_handle, node->time,
    // &node->cached_pose);
    break;
  ...
  case NODE_TYPE_LERP: {
    ...
    pose_blend(&node->cached_pose,
               ...
               &tree->nodes[node->data.lerp.input_b].cached_pose, alpha);
    break;
  }
  ...
  case NODE_TYPE_BLEND_1D: {
    ...
    if (b1d->point_count == 0)
      break;

    if (b1d->point_count == 1 || param <= b1d->points[0].value) {
      evaluate_node(tree, b1d->points[0].input_node, dt);
      // Copy pose
      // pose_copy(&node->cached_pose,
      // &tree->nodes[b1d->points[0].input_node].cached_pose);
...
int animation_blend_tree_init(void) {
  ...
  g_blend_tree_ctx.items = calloc(g_blend_tree_ctx.capacity,
                                  sizeof(animation_blend_tree_internal_t));
...
int animation_blend_tree_get_info(animation_blend_tree_handle_t handle,
  ...
  const animation_blend_tree_internal_t *item =
      &g_blend_tree_ctx.items[handle.id];
  out_info->id = item->id;
...
uint32_t animation_blend_tree_get_count(void) { return g_blend_tree_ctx.count; }
