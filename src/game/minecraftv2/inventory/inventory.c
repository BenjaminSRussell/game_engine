...
static void inventory_emit(Inventory *inv, InventoryEventType event, u32 item_id,
...
bool inventory_add_item(Inventory *inv, u32 item_id, u16 count) {
  ...
  if (remaining > 0) {
    LOG_WARN("Inventory full: could not add %u items of type %u", remaining, item_id);
...
bool inventory_set_slot(Inventory *inv, u32 slot_index, u32 item_id,
  ...
  if (inv->slots[slot_index].item_id != 0 && inv->slots[slot_index].item_id != item_id) {
    if (inv->slots[slot_index].enchantments) {
      free(inv->slots[slot_index].enchantments);
      inv->slots[slot_index].enchantments = NULL;
    }
...
void inventory_sort_by_type(Inventory *inv, InventorySortType sort_type) {
  ...
  // Simple bubble sort for demonstration - in production would use more efficient algorithm
  for (u32 i = 0; i < MAX_INVENTORY_SLOTS - 1; i++) {
    for (u32 j = i + 1; j < MAX_INVENTORY_SLOTS; j++) {
      ...
      switch (sort_type) {
        case SORT_BY_NAME:
          ...
          should_swap = (inv->slots[i].item_id == 0 && inv->slots[j].item_id != 0) ||
...
bool inventory_can_stack_items(u32 item_id1, u32 item_id2) {
  if (item_id1 == 0 || item_id2 == 0) return false;
  if (item_id1 != item_id2) return false;
...
bool inventory_merge_stacks(Inventory *inv, u32 source_slot, u32 target_slot) {
  if (!inv || source_slot >= MAX_INVENTORY_SLOTS || target_slot >= MAX_INVENTORY_SLOTS) {
    return false;
  }
  ...
  if (source->item_id == 0 || target->item_id == 0) return false;
  if (!inventory_can_stack_items(source->item_id, target->item_id)) return false;
...
bool inventory_split_stack(Inventory *inv, u32 source_slot, u32 target_slot, u16 amount) {
  ...
  if (source->item_id == 0 || amount == 0 || amount > source->count) return false;
  
  // Check if target slot is empty or can stack
  if (target->item_id != 0 && !inventory_can_stack_items(source->item_id, target->item_id)) {
    return false;
  }
...
u32 inventory_find_best_stack_slot(Inventory *inv, u32 item_id, u16 count) {
  if (!inv || item_id == 0) return UINT32_MAX;
...
bool inventory_move_item(Inventory *inv, u32 source_slot, u32 target_slot) {
  ...
  if (source_slot == target_slot) return true;
...
void inventory_set_favorite(Inventory *inv, u32 slot_index, bool favorite) {
  if (!inv || slot_index >= MAX_INVENTORY_SLOTS) return;
...
bool inventory_is_favorite(Inventory *inv, u32 slot_index) {
  if (!inv || slot_index >= MAX_INVENTORY_SLOTS) return false;
...
bool inventory_has_space_for(Inventory *inv, u32 item_id, u16 count) {
  if (!inv || item_id == 0 || count == 0) return false;
...
void inventory_debug_print(Inventory *inv) {
  ...
  for (u32 i = 0; i < MAX_INVENTORY_SLOTS; i++) {
    if (inv->slots[i].item_id != 0) {
      LOG_DEBUG("  Slot %u: Item %u, Count %u, Favorite: %s, Locked: %s",
               i, inv->slots[i].item_id, inv->slots[i].count,
...
u32 inventory_quick_stack(Inventory *inv, Inventory *target) {
  ...
  for (u32 i = 0; i < MAX_INVENTORY_SLOTS; i++) {
    if (inv->slots[i].item_id == 0 || inv->slots[i].count == 0) continue;
    ...
    if (available_count > 0) {
      for (u32 j = 0; j < MAX_INVENTORY_SLOTS && available_count > 0; j++) {
        if (target->slots[j].item_id == 0) {
          u16 transfer = (available_count < STACK_SIZE_DEFAULT) ? available_count : STACK_SIZE_DEFAULT;
