...
static u32 furnace_get_fuel_value(u32 item_id) {
  // Fuel items and their values
  switch (item_id) {
  case 5:
    return 15; // Wood
  case 24:
...
void furnace_update(FurnaceState *furnace, Inventory *inventory,
  ...
  if (furnace->burn_time <= 0.0f && fuel_item && fuel_item->item_id > 0) {
    u32 fuel_value = furnace_get_fuel_value(fuel_item->item_id);
    if (fuel_value > 0) {
      furnace->burn_time = (f32)fuel_value;
      furnace->fuel_remaining = (f32)fuel_value;
