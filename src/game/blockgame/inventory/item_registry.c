...
// Register a new item
bool item_registry_register(ItemRegistry *registry, u32 item_id, const ExtendedItemDefinition *def) {
    char reason[128];
    if (!registry || !def) return false;
    if (item_id >= registry->capacity) return false;
    ...
    if (item_id < registry->count && registry->items[item_id].base.name[0] != '\0') {
...
// Get item by ID
const ExtendedItemDefinition *item_registry_get(const ItemRegistry *registry, u32 item_id) {
    if (!registry || item_id >= registry->count) return NULL;
    return &registry->items[item_id];
}

// Get item by name
const ExtendedItemDefinition *item_registry_get_by_name(const ItemRegistry *registry, const char *name) {
    if (!registry || !name || !registry->name_lookup) return NULL;
...
bool item_registry_add_dependency(ItemRegistry *registry, u32 item_id, u32 requires_item_id) {
    if (!registry) return false;

    if (registry->dependency_count >= registry->dependency_capacity) {
        u32 new_capacity = registry->dependency_capacity == 0 ? 32 : registry->dependency_capacity * 2;
        ItemDependency *deps = REALLOC(registry->dependencies, new_capacity * sizeof(ItemDependency));
    ...
    registry->dependencies[registry->dependency_count++] = (ItemDependency){
        .item_id = item_id,
        .requires_item_id = requires_item_id
    };
    return true;
}
...
u32 item_registry_find_by_type(const ItemRegistry *registry, ItemType type,
    ...
    if (!registry || !out_ids || max_out == 0) return 0;

    u32 count = 0;
    for (u32 i = 0; i < registry->count && count < max_out; i++) {
        const ExtendedItemDefinition *def = &registry->items[i];
        if (def->base.name[0] == '\0') continue;
...
u32 item_registry_find_by_name_contains(const ItemRegistry *registry, const char *needle,
                                        u32 *out_ids, u32 max_out) {
    if (!registry || !needle || !out_ids || max_out == 0) return 0;
...
bool item_registry_save(const ItemRegistry *registry, const char *path) {
    ...
    ItemRegistryFileHeader header = {
        .magic = ITEM_REGISTRY_MAGIC,
        .version = registry->version,
        .count = registry->count,
        .capacity = registry->capacity,
        .dependency_count = registry->dependency_count
    };

    bool ok = fwrite(&header, sizeof(header), 1, fp) == 1;
    if (ok && registry->count > 0) {
        ok = fwrite(registry->items, sizeof(ExtendedItemDefinition), registry->count, fp) == registry->count;
    }
    if (ok && registry->dependency_count > 0) {
        ok = fwrite(registry->dependencies, sizeof(ItemDependency), registry->dependency_count, fp) ==
...
bool item_registry_load(ItemRegistry *registry, const char *path) {
    ...
    bool ok = true;
    if (registry->count > 0) {
        ok = fread(registry->items, sizeof(ExtendedItemDefinition), registry->count, fp) == registry->count;
    }

    if (ok && header.dependency_count > 0) {
        registry->dependencies = CALLOC(header.dependency_count, sizeof(ItemDependency));
        registry->dependency_capacity = header.dependency_count;
        registry->dependency_count = header.dependency_count;
        ok = fread(registry->dependencies, sizeof(ItemDependency), header.dependency_count, fp) ==
    ...
    for (u32 i = 0; i < registry->count; i++) {
        if (registry->items[i].base.name[0] == '\0') continue;
...
bool item_registry_migrate(ItemRegistry *registry, u32 from_version, u32 to_version) {
    if (!registry) return false;
...
bool item_registry_write_docs(const ItemRegistry *registry, const char *path) {
    ...
    for (u32 i = 0; i < registry->count; i++) {
        ...
        fprintf(fp, "| %u | %s | %s | %u | %.0f |\n",
                ...
                def->base.max_stack_size,
                def->base.max_durability);
    }
...
void item_registry_init_defaults(ItemRegistry *registry) {
    ...
    ExtendedItemDefinition wooden_pickaxe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Wooden Pickaxe",
                 .description = "Basic mining tool", .max_stack_size = 1, .durability = 59, .max_durability = 59},
        .properties = {.tool = {.mining_speed = 2.0f, .harvest_level = 1, .attack_damage = 2.0f, .attack_speed = 1.2f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_PLANKS, .fuel_value = 10.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_WOODEN_PICKAXE, wooden_pickaxe);

    ExtendedItemDefinition wooden_axe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Wooden Axe",
                 .description = "Chops wood faster", .max_stack_size = 1, .durability = 59, .max_durability = 59},
        .properties = {.tool = {.mining_speed = 2.0f, .harvest_level = 1, .attack_damage = 7.0f, .attack_speed = 0.8f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_PLANKS, .fuel_value = 10.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_WOODEN_AXE, wooden_axe);

    ExtendedItemDefinition wooden_shovel = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Wooden Shovel",
                 .description = "Digs dirt and sand faster", .max_stack_size = 1, .durability = 59, .max_durability = 59},
        .properties = {.tool = {.mining_speed = 2.0f, .harvest_level = 1, .attack_damage = 2.5f, .attack_speed = 1.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_PLANKS, .fuel_value = 10.0f, .rarity = 0
    };
    ...
    ExtendedItemDefinition stone_pickaxe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Stone Pickaxe",
                 .description = "Improved mining tool", .max_stack_size = 1, .durability = 131, .max_durability = 131},
        .properties = {.tool = {.mining_speed = 4.0f, .harvest_level = 2, .attack_damage = 3.0f, .attack_speed = 1.2f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_COBBLESTONE, .fuel_value = 0.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_STONE_PICKAXE, stone_pickaxe);

    ExtendedItemDefinition stone_axe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Stone Axe",
                 .description = "Efficient wood chopping", .max_stack_size = 1, .durability = 131, .max_durability = 131},
        .properties = {.tool = {.mining_speed = 4.0f, .harvest_level = 2, .attack_damage = 9.0f, .attack_speed = 0.8f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_COBBLESTONE, .fuel_value = 0.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_STONE_AXE, stone_axe);

    ExtendedItemDefinition stone_shovel = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Stone Shovel",
                 .description = "Efficient digging tool", .max_stack_size = 1, .durability = 131, .max_durability = 131},
        .properties = {.tool = {.mining_speed = 4.0f, .harvest_level = 2, .attack_damage = 3.5f, .attack_speed = 1.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_COBBLESTONE, .fuel_value = 0.0f, .rarity = 0
    };
    ...
    ExtendedItemDefinition iron_pickaxe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Iron Pickaxe",
                 .description = "Fast and durable mining tool", .max_stack_size = 1, .durability = 250, .max_durability = 250},
        .properties = {.tool = {.mining_speed = 6.0f, .harvest_level = 3, .attack_damage = 4.0f, .attack_speed = 1.2f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .fuel_value = 0.0f, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_PICKAXE, iron_pickaxe);

    ExtendedItemDefinition iron_axe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Iron Axe",
                 .description = "Rapid wood chopping", .max_stack_size = 1, .durability = 250, .max_durability = 250},
        .properties = {.tool = {.mining_speed = 6.0f, .harvest_level = 3, .attack_damage = 9.0f, .attack_speed = 0.9f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .fuel_value = 0.0f, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_AXE, iron_axe);

    ExtendedItemDefinition iron_shovel = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Iron Shovel",
                 .description = "Professional digging tool", .max_stack_size = 1, .durability = 250, .max_durability = 250},
        .properties = {.tool = {.mining_speed = 6.0f, .harvest_level = 3, .attack_damage = 4.5f, .attack_speed = 1.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .fuel_value = 0.0f, .rarity = 1
    };
    ...
    ExtendedItemDefinition diamond_pickaxe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Diamond Pickaxe",
                 .description = "The finest mining tool", .max_stack_size = 1, .durability = 1561, .max_durability = 1561},
        .properties = {.tool = {.mining_speed = 8.0f, .harvest_level = 4, .attack_damage = 5.0f, .attack_speed = 1.2f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .fuel_value = 0.0f, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_PICKAXE, diamond_pickaxe);

    ExtendedItemDefinition diamond_axe = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Diamond Axe",
                 .description = "Ultimate wood chopping tool", .max_stack_size = 1, .durability = 1561, .max_durability = 1561},
        .properties = {.tool = {.mining_speed = 8.0f, .harvest_level = 4, .attack_damage = 9.0f, .attack_speed = 1.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .fuel_value = 0.0f, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_AXE, diamond_axe);

    ExtendedItemDefinition diamond_shovel = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_TOOL, .name = "Diamond Shovel",
                 .description = "Unmatched digging speed", .max_stack_size = 1, .durability = 1561, .max_durability = 1561},
        .properties = {.tool = {.mining_speed = 8.0f, .harvest_level = 4, .attack_damage = 5.5f, .attack_speed = 1.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .fuel_value = 0.0f, .rarity = 3
    };
    ...
    ExtendedItemDefinition wooden_sword = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Wooden Sword",
                 .description = "Basic melee weapon", .max_stack_size = 1, .durability = 59, .max_durability = 59},
        .properties = {.weapon = {.attack_damage = 4.0f, .attack_speed = 1.6f, .knockback = 0.0f, .reach = 3.0f, .is_ranged = false}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_PLANKS, .fuel_value = 10.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_WOODEN_SWORD, wooden_sword);

    ExtendedItemDefinition stone_sword = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Stone Sword",
                 .description = "Sturdy combat weapon", .max_stack_size = 1, .durability = 131, .max_durability = 131},
        .properties = {.weapon = {.attack_damage = 5.0f, .attack_speed = 1.6f, .knockback = 0.0f, .reach = 3.0f, .is_ranged = false}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_COBBLESTONE, .fuel_value = 0.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_STONE_SWORD, stone_sword);

    ExtendedItemDefinition iron_sword = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Iron Sword",
                 .description = "Reliable combat weapon", .max_stack_size = 1, .durability = 250, .max_durability = 250},
        .properties = {.weapon = {.attack_damage = 6.0f, .attack_speed = 1.6f, .knockback = 0.0f, .reach = 3.0f, .is_ranged = false}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .fuel_value = 0.0f, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_SWORD, iron_sword);

    ExtendedItemDefinition diamond_sword = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Diamond Sword",
                 .description = "Elite combat weapon", .max_stack_size = 1, .durability = 1561, .max_durability = 1561},
        .properties = {.weapon = {.attack_damage = 7.0f, .attack_speed = 1.6f, .knockback = 0.0f, .reach = 3.0f, .is_ranged = false}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .fuel_value = 0.0f, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_SWORD, diamond_sword);

    ExtendedItemDefinition bow = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Bow",
                 .description = "Ranged weapon", .max_stack_size = 1, .durability = 384, .max_durability = 384},
        .properties = {.weapon = {.attack_damage = 9.0f, .attack_speed = 1.0f, .knockback = 0.0f, .reach = 50.0f, .is_ranged = true, .projectile_speed = 40.0f}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_STRING, .fuel_value = 0.0f, .rarity = 1
    };
    REGISTER_ITEM(ITEM_BOW, bow);

    ExtendedItemDefinition arrow = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Arrow",
                 .description = "Ammunition for bows", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .is_repairable = false, .rarity = 0
    };
    REGISTER_ITEM(ITEM_ARROW, arrow);

    ExtendedItemDefinition shield = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_WEAPON, .name = "Shield",
                 .description = "Blocks incoming attacks", .max_stack_size = 1, .durability = 336, .max_durability = 336},
        .properties = {.weapon = {.attack_damage = 0.0f, .attack_speed = 0.0f, .knockback = 1.0f, .reach = 0.0f, .is_ranged = false}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_PLANKS, .fuel_value = 0.0f, .rarity = 1
    };
    ...
    ExtendedItemDefinition iron_helmet = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Iron Helmet",
                 .description = "Protects your head", .max_stack_size = 1, .durability = 165, .max_durability = 165},
        .properties = {.armor = {.defense = 2.0f, .toughness = 0.0f, .knockback_resistance = 0.0f, .armor_slot = 0}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_HELMET, iron_helmet);

    ExtendedItemDefinition iron_chestplate = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Iron Chestplate",
                 .description = "Protects your torso", .max_stack_size = 1, .durability = 240, .max_durability = 240},
        .properties = {.armor = {.defense = 6.0f, .toughness = 0.0f, .knockback_resistance = 0.0f, .armor_slot = 1}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_CHESTPLATE, iron_chestplate);

    ExtendedItemDefinition iron_leggings = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Iron Leggings",
                 .description = "Protects your legs", .max_stack_size = 1, .durability = 225, .max_durability = 225},
        .properties = {.armor = {.defense = 5.0f, .toughness = 0.0f, .knockback_resistance = 0.0f, .armor_slot = 2}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_LEGGINGS, iron_leggings);

    ExtendedItemDefinition iron_boots = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Iron Boots",
                 .description = "Protects your feet", .max_stack_size = 1, .durability = 195, .max_durability = 195},
        .properties = {.armor = {.defense = 2.0f, .toughness = 0.0f, .knockback_resistance = 0.0f, .armor_slot = 3}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_IRON_INGOT, .rarity = 1
    };
    ...
    ExtendedItemDefinition diamond_helmet = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Diamond Helmet",
                 .description = "Maximum head protection", .max_stack_size = 1, .durability = 363, .max_durability = 363},
        .properties = {.armor = {.defense = 3.0f, .toughness = 2.0f, .knockback_resistance = 0.0f, .armor_slot = 0}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_HELMET, diamond_helmet);

    ExtendedItemDefinition diamond_chestplate = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Diamond Chestplate",
                 .description = "Maximum torso protection", .max_stack_size = 1, .durability = 528, .max_durability = 528},
        .properties = {.armor = {.defense = 8.0f, .toughness = 2.0f, .knockback_resistance = 0.0f, .armor_slot = 1}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_CHESTPLATE, diamond_chestplate);

    ExtendedItemDefinition diamond_leggings = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Diamond Leggings",
                 .description = "Maximum leg protection", .max_stack_size = 1, .durability = 495, .max_durability = 495},
        .properties = {.armor = {.defense = 6.0f, .toughness = 2.0f, .knockback_resistance = 0.0f, .armor_slot = 2}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND_LEGGINGS, diamond_leggings);

    ExtendedItemDefinition diamond_boots = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_ARMOR, .name = "Diamond Boots",
                 .description = "Maximum feet protection", .max_stack_size = 1, .durability = 429, .max_durability = 429},
        .properties = {.armor = {.defense = 3.0f, .toughness = 2.0f, .knockback_resistance = 0.0f, .armor_slot = 3}},
        .is_stackable = false, .is_repairable = true, .repair_material_id = ITEM_DIAMOND, .rarity = 3
    };
    ...
    ExtendedItemDefinition poisonous_potato = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_FOOD, .name = "Poisonous Potato",
                 .description = "Dangerous to eat", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .properties = {.food = {.hunger_restored = 2.0f, .saturation_modifier = 0.3f, .eat_duration = 1.6f, .has_effects = true, .effect_id = 0, .effect_chance = 0.6f}},
        .is_stackable = true, .rarity = 0
    };
    ...
    ExtendedItemDefinition stick = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Stick",
                 .description = "Basic crafting material", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .fuel_value = 5.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_STICK, stick);

    ExtendedItemDefinition coal = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Coal",
                 .description = "Fuel and crafting material", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .fuel_value = 80.0f, .rarity = 0
    };
    REGISTER_ITEM(ITEM_COAL, coal);

    ExtendedItemDefinition iron_ingot = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Iron Ingot",
                 .description = "Refined iron metal", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 1
    };
    REGISTER_ITEM(ITEM_IRON_INGOT, iron_ingot);

    ExtendedItemDefinition gold_ingot = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Gold Ingot",
                 .description = "Valuable precious metal", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 2
    };
    REGISTER_ITEM(ITEM_GOLD_INGOT, gold_ingot);

    ExtendedItemDefinition diamond = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Diamond",
                 .description = "Rare and precious gem", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 3
    };
    REGISTER_ITEM(ITEM_DIAMOND, diamond);

    ExtendedItemDefinition emerald = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Emerald",
                 .description = "Valuable trading currency", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 3
    };
    REGISTER_ITEM(ITEM_EMERALD, emerald);

    ExtendedItemDefinition string_item = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "String",
                 .description = "Spider silk thread", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 0
    };
    REGISTER_ITEM(ITEM_STRING, string_item);

    ExtendedItemDefinition leather = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Leather",
                 .description = "Animal hide material", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 0
    };
    REGISTER_ITEM(ITEM_LEATHER, leather);

    ExtendedItemDefinition feather = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Feather",
                 .description = "Light and fluffy", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 0
    };
    ...
    ExtendedItemDefinition rubber = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Rubber",
                 .description = "Flexible industrial material", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 1
    };
    REGISTER_ITEM(ITEM_RUBBER, rubber);

    ExtendedItemDefinition circuit = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Electronic Circuit",
                 .description = "Advanced tech component", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 2
    };
    REGISTER_ITEM(ITEM_CIRCUIT, circuit);

    ExtendedItemDefinition battery = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MATERIAL, .name = "Battery",
                 .description = "Stores electrical energy", .max_stack_size = 16, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 2
    };
    ...
    ExtendedItemDefinition bucket = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MISC, .name = "Bucket",
                 .description = "Holds liquids", .max_stack_size = 16, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 0
    };
    REGISTER_ITEM(ITEM_BUCKET, bucket);

    ExtendedItemDefinition book = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MISC, .name = "Book",
                 .description = "For writing and enchanting", .max_stack_size = 64, .durability = 0, .max_durability = 0},
        .is_stackable = true, .rarity = 0
    };
    REGISTER_ITEM(ITEM_BOOK, book);

    ExtendedItemDefinition minecart = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MISC, .name = "Minecart",
                 .description = "Rail transportation", .max_stack_size = 1, .durability = 0, .max_durability = 0},
        .is_stackable = false, .rarity = 1
    };
    REGISTER_ITEM(ITEM_MINECART, minecart);

    ExtendedItemDefinition boat = {
        .base = {.block_id = 0, .item_type = ITEM_TYPE_MISC, .name = "Boat",
                 .description = "Water transportation", .max_stack_size = 1, .durability = 0, .max_durability = 0},
        .is_stackable = false, .fuel_value = 60.0f, .rarity = 0
    };
