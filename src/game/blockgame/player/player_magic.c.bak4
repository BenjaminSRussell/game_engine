...
static const SpellConfig kSpellConfigs[SPELL_COUNT] = {
    [SPELL_FIREBALL] = {15.0f, 1.5f, 30.0f, 0.0f, 0.0f, true},
    [SPELL_HEAL] = {12.0f, 4.0f, 0.0f, 8.0f, 0.0f, false},
    [SPELL_TELEPORT] = {20.0f, 6.0f, 24.0f, 0.0f, 0.0f, false},
    [SPELL_SHIELD] = {18.0f, 10.0f, 0.0f, 0.0f, 3.0f, false},
    [SPELL_LIGHTNING] = {20.0f, 5.0f, 28.0f, 0.0f, 0.0f, true},
    [SPELL_FREEZE] = {16.0f, 5.0f, 18.0f, 0.0f, 0.0f, true},
    [SPELL_INVISIBILITY] = {25.0f, 12.0f, 0.0f, 0.0f, 8.0f, false},
    [SPELL_FLIGHT] = {30.0f, 15.0f, 0.0f, 0.0f, 8.0f, false},
};
...
bool player_cast_spell(PlayerSystem *system, SpellType spell, Vec3 target) {
  ...
  case SPELL_FREEZE:
    if (system->combat_system && system->ecs_world) {
      combat_ranged_attack(system->combat_system, system->ecs_world,
                           system->player->entity_id, direction, config->range);
    }
    break;

  case SPELL_HEAL:
    player_heal(system, config->magnitude);
  ...
  case SPELL_INVISIBILITY:
    status_effect_add(&system->player->status_effects,
                      STATUS_EFFECT_INVISIBILITY, config->duration, 1.0f);
    break;

  case SPELL_FLIGHT:
    player_fly(system, true);
    spell_state->is_channeling = true;
    spell_state->channel_time = config->duration;
