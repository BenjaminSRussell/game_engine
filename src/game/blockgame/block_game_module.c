// src/games/block_game/block_game_module.c
//
// Purpose: Block Game implementation as a game module using the engine API.
// This file demonstrates how to convert existing game logic to work with
// the decoupled engine architecture.
//
// TODO: Migrate all Block Game systems to use engine APIs
...
// Block Game specific includes (these will be gradually migrated)
...
static bool block_game_init(GameModule *module, Engine *engine);
static void block_game_shutdown(GameModule *module);
static void block_game_update(GameModule *module, Engine *engine,
                                f32 delta_time);
static void block_game_render(GameModule *module, Engine *engine);
static void block_game_handle_input(GameModule *module, Engine *engine);

// Module lifecycle implementation
static bool block_game_load(GameModule *module, const char *module_path) {
  if (!module || !module_path) {
    LOG_ERROR("Block Game: Invalid load parameters");
    return false;
  }

  LOG_INFO("Loading Block Game module from: %s", module_path);

  // Initialize module info
  module->info.name = "Block Game";
  ...
  module->game_data = malloc(sizeof(BlockGameState));
  if (!module->game_data) {
    LOG_ERROR("Block Game: Failed to allocate game state");
    return false;
  }

  memset(module->game_data, 0, sizeof(BlockGameState));

  LOG_INFO("Block Game module loaded successfully");
  return true;
}

static void block_game_unload(GameModule *module) {
  if (!module)
    return;

  LOG_INFO("Unloading Block Game module");
  ...
  LOG_INFO("Block Game module unloaded");
}

static bool block_game_initialize(GameModule *module, Engine *engine) {
  if (!module || !engine || !module->game_data) {
    LOG_ERROR("Block Game: Invalid initialize parameters");
    return false;
  }

  LOG_INFO("Initializing Block Game game module");
  module->info.state = GAME_MODULE_STATE_INITIALIZING;

  BlockGameState *game_state = (BlockGameState *)module->game_data;
  ...
  if (!game_state->renderer || !game_state->input) {
    LOG_ERROR("Block Game: Required engine subsystems not available");
    return false;
  }
  ...
  LOG_INFO("Block Game game module initialized successfully");
  return true;
}
...
static void block_game_shutdown(GameModule *module) {
  ...
  LOG_INFO("Shutting down Block Game game module");
  ...
  module->info.state = GAME_MODULE_STATE_LOADED;
  LOG_INFO("Block Game game module shutdown complete");
}
...
// Event handlers
static void block_game_on_window_resize(GameModule *module, Engine *engine,
  ...
  LOG_INFO("Block Game: Window resized to %ux%u", width, height);
}

static void block_game_on_focus_gained(GameModule *module, Engine *engine) {
  if (!module || !module->game_data)
    return;
  ...
  LOG_INFO("Block Game: Focus gained");
}

static void block_game_on_focus_lost(GameModule *module, Engine *engine) {
  if (!module || !module->game_data)
    return;
  ...
  LOG_INFO("Block Game: Focus lost");
}

// Save/Load system
static bool block_game_save_game(GameModule *module, Engine *engine,
                                   const char *save_path) {
  if (!module || !engine || !save_path)
    return false;

  LOG_INFO("Block Game: Saving game to %s", save_path);

  // TODO: Implement save system using engine file I/O
  // This should serialize game state, world data, player data, etc.

  LOG_INFO("Block Game: Game saved successfully");
  return true;
}

static bool block_game_load_game(GameModule *module, Engine *engine,
                                   const char *save_path) {
  if (!module || !engine || !save_path)
    return false;

  LOG_INFO("Block Game: Loading game from %s", save_path);

  // TODO: Implement load system using engine file I/O
  // This should deserialize game state, world data, player data, etc.

  LOG_INFO("Block Game: Game loaded successfully");
  return true;
}

// Module creation function
GameModule *create_block_game_module(void) {
  ...
  module->config = game_module_create_default_config("Block Game");
  ...
  module->load = block_game_load;
  module->unload = block_game_unload;
  module->initialize = block_game_initialize;
  module->shutdown = block_game_shutdown;
  module->update = block_game_update;
  module->render = block_game_render;
  module->handle_input = block_game_handle_input;
  module->on_window_resize = block_game_on_window_resize;
  module->on_focus_gained = block_game_on_focus_gained;
  module->on_focus_lost = block_game_on_focus_lost;
  module->save_game = block_game_save_game;
  module->load_game = block_game_load_game;

  return module;
}

// Module destruction function
void destroy_block_game_module(GameModule *module) {
  if (!module)
    return;
...
DECLARE_GAME_MODULE("Block Game", "1.0.0", "Game Engine Team",
                    "A voxel-based sandbox game")

IMPLEMENT_GAME_MODULE("Block Game", "1.0.0", "Game Engine Team",
                      "A voxel-based sandbox game")
...
GAME_MODULE_API GameModule *create_game_module(void) {
  return create_block_game_module();
}

GAME_MODULE_API void destroy_game_module(GameModule *module) {
  destroy_block_game_module(module);
}
