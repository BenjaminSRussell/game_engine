...
//
// Purpose: Defines the public API and data structures for a generic inventory
// system. This header provides the foundational elements for managing items
// within a player's or entity's storage, including item categorization
// (`ItemType`), detailed item properties (`ItemDefinition`), individual storage
// units (`InventorySlot`), and the overall `Inventory` structure. It also
// declares functions for comprehensive item management and inventory
// manipulation.
//
// Public APIs:
// - `MAX_INVENTORY_SLOTS`, `MAX_HOTBAR_SLOTS`, `STACK_SIZE_DEFAULT`: Constants
// defining
//   inventory dimensions and default stack limits.
// - `ItemType`: Enumeration categorizing items (e.g., Block, Tool, Weapon,
// Food).
// - `ItemDefinition`: Structure detailing global properties of an item, such as
// its
...
// - `Inventory`: The main structure encapsulating a collection of
// `InventorySlot`s,
//   a selected hotbar slot, and the total count of distinct items.
// - `inventory_init`: Initializes an `Inventory` instance, setting all slots to
// empty.
// - `inventory_free`: Frees resources associated with the `Inventory`.
// - `inventory_add_item`, `inventory_remove_item`, `inventory_has_item`:
// Functions for
//   manipulating items within the inventory.
// - `inventory_get_item_count`: Returns the total quantity of a specific item
// in the inventory.
// - `inventory_get_slot`, `inventory_set_slot`: Functions to retrieve or modify
// items in specific slots.
// - `inventory_select_hotbar`, `inventory_get_selected_item`,
// `inventory_get_selected_slot`:
//   Functions specific to hotbar interaction within the inventory.
// - `inventory_is_full`, `inventory_get_empty_slots`: Queries about inventory
// capacity.
// - `inventory_sort`, `inventory_compact`: Utility functions for organizing
// inventory contents.
// - `inventory_clear`: Empties the entire inventory.
//
// Ownership: An `Inventory` instance owns its array of `InventorySlot`s.
// `ItemDefinition`s are global data, not owned by individual `Inventory`
// instances.
//
// Invariants:
// - An `Inventory` must be initialized with `inventory_init` before use.
// - `item_id` values must correspond to valid `ItemDefinition`s.
// - `count` in `InventorySlot` should not exceed `max_stack_size` for that item
// type.
...
typedef struct {
  ...
  f32 quality_modifier; // Quality multiplier (e.g. 1.0 = normal, 1.5 = high
                        // quality)
  // Enchantments (stored as void* to avoid circular dependency with
  // enchanting.h) In enchanting.c, cast to Enchantment* when accessing
...
typedef void (*InventoryEventCallback)(Inventory *inv, InventoryEventType event,
...
// TODO: Validate item presence and decrease count; remove slot if count becomes
// 0.
...
// TODO: Simulate adding an item to see how much would fit without modifying
// inventory. Returns the amount that *could* be added.
...
// TODO: Iterate inventory, finding partial stacks of same item_id and merging
// them.
...
// TODO: Implement sorting algorithm (e.g., stable sort) based on SortType
// criteria.
...
// TODO: Iterate through 'inv', find matching stacks in 'target', and transfer
// items.
...
// TODO: Verify source has enough items, find/create target slot, transfer
// specific amount.
...
// Serialization
// TODO: Implement JSON or binary serialization for saving player data/chest
// contents.
