...
//
// Purpose: Defines the enchanting system for applying magical properties to items.
// Supports 30+ enchantments with level-based costs, lapis requirement, and
// bookshelf proximity bonuses.
//
// Public APIs:
...
void enchanting_table_update(EnchantingTable *table, ChunkManager *chunk_manager,
                            BlockRegistry *block_registry);

// Get available enchantment offers
// TODO: Use a seeded random number generator (based on player seed + item encryption seed) for consistent offers.
...
// TODO: Check ItemType vs EnchantmentType (e.g., Sharpness only on Swords/Axes).
bool enchanting_can_apply(EnchantmentType enchant_type, u32 item_id,
                         ItemRegistry *item_registry);

// Check for conflicting enchantments
// TODO: Check exclusion rules (e.g., Silk Touch vs Fortune, Protection vs Blast Protection).
bool enchanting_has_conflict(const InventorySlot *item, EnchantmentType new_enchant);
...
void enchanting_emit_apply_vfx(Enchantment *enchantment, Vec3 position, struct ParticleSystem *particle_system, struct AudioSystem *audio_system);

// Apply enchantment and emit VFX/audio at position if provided
bool enchanting_apply_with_vfx(Enchantment *enchantment, InventorySlot *item_slot,
                               ItemRegistry *item_registry, Vec3 position,
                               struct ParticleSystem *particle_system, struct AudioSystem *audio_system);
