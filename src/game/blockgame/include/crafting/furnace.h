...
//
// Purpose: Defines the API and data structures for managing a furnace, a
// processing block in the game that smelts or cooks items using fuel. This
// header outlines the structure of the `FurnaceState`, detailing its slots for
// input, fuel, and output, as well as variables for tracking smelting progress
// and fuel consumption.
//
// Public APIs:
// - `FurnaceState`: Structure representing the current state of a furnace,
// including
//   its active status, the items in its input, fuel, and output slots, current
//   recipe, smelting progress, and fuel remaining.
// - `furnace_init`: Initializes a `FurnaceState` instance.
// - `furnace_update`: Updates the furnace state each frame, handling fuel
// consumption,
//   smelting progress, and item transfer to the output slot when complete.
// - `furnace_get_progress`: Returns the current smelting progress as a value
// between 0.0 and 1.0.
// - `furnace_is_active`: Checks if the furnace is currently operating (smelting
// an item).
// - `furnace_get_burn_time`: Returns the remaining burn time for the current
// fuel.
//
// Ownership: A `FurnaceState` instance manages its internal state variables. It
// interacts with an `Inventory` instance (typically a block inventory) but does
// not own it.
//
// Invariants:
// - A `FurnaceState` must be initialized with `furnace_init` before use.
// - `furnace_update` should be called regularly (e.g., every game tick) to
// ensure proper
//   smelting progression and fuel management.
// - The `Inventory` structure (defined in `inventory.h`) is assumed to be
// correctly defined.
// - Fuel and smeltable items are expected to be recognized by the underlying
