...
//   entity, considering the current `ChunkManager` and `BlockRegistry` for
//   world information. The resulting path is stored in the provided `path`
//   array, with its length returned in `path_length`.
// - `npc_is_walkable`: A helper function that determines if a specific block
//   coordinate (`x`, `y`, `z`) is traversable by an NPC, based on block
//   properties.
//
// Ownership: These functions operate on existing data structures like
// `ECSWorld`, `ChunkManager`, and `BlockRegistry` (which are owned elsewhere).
// The `path` array passed to `npc_find_path` is owned by the caller.
//
// Invariants:
// - Valid `ECSWorld`, `ChunkManager`, and `BlockRegistry` instances must be
// provided.
// - `start` and `goal` `Vec3` positions should be valid world coordinates.
// - `path` must be a pre-allocated array capable of holding `max_path_length`
// `Vec3` points.
// - Pathfinding algorithm's success depends on the complexity of the terrain
// and
...
bool npc_find_path(struct World *ecs, EntityID entity,
                   ...
                   Vec3 start, Vec3 goal, Vec3 *path, u32 *path_length,
                   u32 max_path_length);
