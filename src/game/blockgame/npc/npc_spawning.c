...
static u32 count_npcs_in_chunk(NPCSystem *system, ChunkPos chunk_pos) {
  ...
  Entity e;
  void *c[2];
  while (ecs_query_next(query, &e, c)) {
    TransformComponent *transform = (TransformComponent *)c[1];
    if (transform) {
...
void npc_despawn_distant(NPCSystem *system) {
  ...
  // Collect player positions for distance checks
  Vec3 player_positions[16];
  u32 player_count = 0;
  Entity p_ent;
  void *p_comp_ptrs[2];
  while (ecs_query_next(player_query, &p_ent, p_comp_ptrs) &&
         player_count < 16) {
    TransformComponent *pt = (TransformComponent *)p_comp_ptrs[1];
    if (pt) {
      player_positions[player_count++] = pt->position;
    }
  }
  ...
  Entity n_ent;
  void *n_comp_ptrs[2];
  while (ecs_query_next(npc_query, &n_ent, n_comp_ptrs)) {
    TransformComponent *npc_transform = (TransformComponent *)n_comp_ptrs[1];
    if (!npc_transform)
      continue;
    ...
    for (u32 j = 0; j < player_count; j++) {
      f32 distance =
          vec3_length(vec3_sub(player_positions[j], npc_transform->position));
    ...
    if (nearest_distance > DESPAWN_DISTANCE) {
      ecs_destroy_entity((World *)system->ecs, n_ent);
    }
  }

  ecs_query_destroy((World *)system->ecs, npc_query);
}
