...
void furnace_init(FurnaceState *furnace) {
  ...
  // Set default values for burn time and progress
  furnace->burn_time = 0.0f;
  furnace->fuel_remaining = 0.0f;
...
void furnace_update(FurnaceState *furnace, Inventory *inventory,
  ...
  if (input_item && input_item->item_id > 0 && furnace->burn_time <= 0.0f) {
    if (fuel_item && fuel_item->item_id > 0) {
  ...
  if (output_item) {
    if (output_item->item_id > 0 && output_item->item_id != recipe->output_item) {
      furnace->active = false;
  ...
  if (furnace->smelt_progress >= 1.0f) {
    ...
    // Add XP to stored XP (based on smelted item)
    furnace->xp_stored += 0.1f; // Base XP per smelt
    if (recipe->output_item == ITEM_IRON_INGOT) {
      furnace->xp_stored += 0.7f; // Iron gives more XP
    } else if (recipe->output_item == ITEM_GOLD_INGOT) {
      furnace->xp_stored += 1.0f; // Gold gives even more XP
    }
