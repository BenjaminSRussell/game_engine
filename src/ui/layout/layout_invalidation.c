...
static LayoutCacheEntry* find_cache_entry(LayoutCacheLevel* cache, uint32_t element_id) {
...
LayoutInvalidationSystem* layout_invalidation_create(void) {
    ...
    system->element_generations = malloc(sizeof(uint32_t) * system->element_capacity);
    system->element_last_layout_time = malloc(sizeof(uint32_t) * system->element_capacity);
    system->element_count = 0;

    /* Initialize invalidation queue */
    system->invalidation_queue.entry_capacity = 128;
    system->invalidation_queue.entries = malloc(sizeof(InvalidationEntry) * system->invalidation_queue.entry_capacity);
    ...
    for (int level = 0; level <= CACHE_LEVEL_FULL; level++) {
        ...
        system->cache_levels[level].entries = malloc(sizeof(LayoutCacheEntry) * system->cache_levels[level].entry_capacity);
...
void layout_invalidation_set_cache_mode(LayoutInvalidationSystem* system, CacheLevel level) {
    if (system) {
...
void layout_invalidation_enable_aggressive_caching(LayoutInvalidationSystem* system, bool enable) {
    if (system) {
        system->enable_aggressive_caching = enable;
    }
}

void layout_invalidation_set_max_cache_entries(LayoutInvalidationSystem* system, uint32_t max_entries) {
    if (system) {
...
void layout_invalidation_register_element(LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);
    ...
    if (system->element_count >= system->element_capacity) {
        ...
        system->element_ids = realloc(system->element_ids, sizeof(uint32_t) * system->element_capacity);
        system->element_generations = realloc(system->element_generations, sizeof(uint32_t) * system->element_capacity);
        system->element_last_layout_time = realloc(system->element_last_layout_time, sizeof(uint32_t) * system->element_capacity);
    }

    system->element_ids[system->element_count] = element_id;
    system->element_generations[system->element_count] = 0;
    system->element_last_layout_time[system->element_count] = system->current_timestamp;
    system->element_count++;
}

void layout_invalidation_unregister_element(LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);
    ...
    for (uint32_t i = 0; i < system->element_count; i++) {
        if (system->element_ids[i] == element_id) {
            ...
            system->element_generations[i] = system->element_generations[system->element_count - 1];
            system->element_last_layout_time[i] = system->element_last_layout_time[system->element_count - 1];
            system->element_count--;
...
void layout_invalidation_increment_generation(LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);
...
void layout_invalidation_invalidate(LayoutInvalidationSystem* system,
    ...
    if (system->invalidation_queue.entry_count < system->max_queue_size) {
        InvalidationEntry* entry = &system->invalidation_queue.entries[system->invalidation_queue.entry_count++];
...
void layout_invalidation_invalidate_subtree(LayoutInvalidationSystem* system,
    ...
    /* In a full implementation, this would invalidate the element and all children */
    layout_invalidation_invalidate(system, element_id, invalidation_flags, "subtree invalidation");
}
...
void layout_invalidation_invalidate_all(LayoutInvalidationSystem* system) {
    ...
    for (uint32_t i = 0; i < system->element_count; i++) {
        layout_invalidation_invalidate(system, system->element_ids[i], INVALIDATE_ALL, "global invalidation");
    }
...
bool layout_invalidation_is_dirty(const LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);

    /* Check if element appears in invalidation queue */
    for (uint32_t i = system->invalidation_queue.read_index; i < system->invalidation_queue.entry_count; i++) {
...
bool layout_invalidation_has_flag(const LayoutInvalidationSystem* system,
    ...
    for (uint32_t i = system->invalidation_queue.read_index; i < system->invalidation_queue.entry_count; i++) {
        if (system->invalidation_queue.entries[i].element_id == element_id) {
            return (system->invalidation_queue.entries[i].invalidation_flags & flag) != 0;
        }
    }
...
bool layout_invalidation_dequeue(LayoutInvalidationSystem* system, InvalidationEntry* out_entry) {
    assert(system && out_entry);

    if (system->invalidation_queue.read_index >= system->invalidation_queue.entry_count) {
        return false;
    }

    *out_entry = system->invalidation_queue.entries[system->invalidation_queue.read_index++];
    return true;
}

bool layout_invalidation_peek_queue(const LayoutInvalidationSystem* system, InvalidationEntry* out_entry) {
    assert(system && out_entry);
...
uint32_t layout_invalidation_get_queue_size(const LayoutInvalidationSystem* system) {
    assert(system);
    return system->invalidation_queue.entry_count - system->invalidation_queue.read_index;
}
...
void layout_invalidation_cache_layout(LayoutInvalidationSystem* system,
    ...
    if (entry) {
        ...
        entry->element_generation = layout_invalidation_get_element_generation(system, element_id);
        return;
    }
    ...
    if (cache->entry_count >= cache->entry_capacity) {
        if (cache->entry_count >= system->max_cache_entries) {
            ...
            cache->entries = realloc(cache->entries, sizeof(LayoutCacheEntry) * cache->entry_capacity);
        }
    }
...
bool layout_invalidation_is_cached(const LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);
...
void layout_invalidation_cache_invalidate(LayoutInvalidationSystem* system, uint32_t element_id) {
    assert(system);
...
void layout_invalidation_cache_clear_level(LayoutInvalidationSystem* system, CacheLevel level) {
...
void layout_invalidation_invalidate_batch(LayoutInvalidationSystem* system,
    ...
    for (uint32_t i = 0; i < count; i++) {
        layout_invalidation_invalidate(system, element_ids[i], invalidation_flags, "batch invalidation");
        g_batch_invalidations++;
    }
}
...
void layout_invalidation_get_performance_stats(const LayoutInvalidationSystem* system,
    ...
    assert(system && out_total_invalidations && out_total_solves && out_avg_solve_time_ms && out_cache_evictions);

    *out_total_invalidations = system->total_invalidations;
    *out_total_solves = system->total_layout_solves;
    *out_avg_solve_time_ms = system->total_layout_solves > 0 ?
                              system->total_solve_time_ms / (float)system->total_layout_solves : 0.0f;
...
void layout_invalidation_print_cache_stats(const LayoutInvalidationSystem* system) {
    ...
    for (int level = 0; level <= CACHE_LEVEL_FULL; level++) {
        ...
        float hit_rate;
        layout_invalidation_get_cache_stats(system, level, &entries, &hits, &misses, &hit_rate);

        const char* level_name = "Unknown";
        switch (level) {
            case CACHE_LEVEL_NONE: level_name = "None"; break;
            case CACHE_LEVEL_ELEMENT: level_name = "Element"; break;
            case CACHE_LEVEL_SUBTREE: level_name = "Subtree"; break;
            case CACHE_LEVEL_FULL: level_name = "Full"; break;
        }
...
void layout_invalidation_print_history(const LayoutInvalidationSystem* system, uint32_t last_n_entries) {
    if (!system) return;

    printf("=== Recent Invalidations (last %u) ===\n", last_n_entries);

    uint32_t start_idx = system->invalidation_queue.entry_count > last_n_entries ?
                        system->invalidation_queue.entry_count - last_n_entries : 0;

    for (uint32_t i = start_idx; i < system->invalidation_queue.entry_count; i++) {
