...
Entity ecs_create_entity(World *world) {
  ...
  if (empty_arch) {
      ...
      if (empty_arch->entity_count >= empty_arch->entity_capacity) {
          ...
          memcpy(new_entities, empty_arch->entities, empty_arch->entity_capacity * sizeof(Entity));
      ...
      empty_arch->entities[empty_arch->entity_count++] = (Entity){id, rec->generation};
  }
...
static SimpleArchetype *find_or_create_archetype(World *world,
                                                 ComponentType *types,
                                                 u32 count) {
  ArchetypeData *ad = (ArchetypeData *)world->archetypes;
  SimpleArchetype *curr = ad->head;
...
static void move_entity(World *world, EntityRecord *rec, SimpleArchetype *old_arch,
  ...
  if (new_index >= new_arch->entity_capacity) {
    ...
    for (u32 i = 0; i < new_arch->component_count; i++) {
      ...
      memcpy(new_data, new_arch->component_data[i], new_arch->entity_capacity * size);
  ...
  if (old_arch->entity_count > 0) {
      ...
      if (old_index != last_index) {
        ...
        for (u32 i = 0; i < old_arch->component_count; i++) {
           ...
           memcpy((u8*)data + (old_index * size), (u8*)data + (last_index * size), size);
        }
...
bool ecs_add_component(World *world, Entity entity, ECSComponentID type,
                       const void *data) {
  if (!ecs_is_valid(world, entity)) return false;
  ...
  if (old_arch->component_count > 0) {
      memcpy(new_types, old_arch->component_types, old_arch->component_count * sizeof(ComponentType));
  }
  new_types[old_arch->component_count] = type;

  // Find/Create new archetype
  SimpleArchetype *new_arch = find_or_create_archetype(world, new_types, new_count);
  free(new_types);
...
void *ecs_get_component(World *world, Entity entity, ECSComponentID type) {
  if (!ecs_is_valid(world, entity)) return NULL;
...
Query *ecs_query_create(World *world, const QueryDesc *desc) {
  ...
  if (desc->all_count > 0) {
      ...
      memcpy(query->component_types, desc->all_components, desc->all_count * sizeof(ComponentType));
  }
  ...
  // In a real optimized system, this would be updated dynamically as archetypes are created
  // For now, we scan existing archetypes
  u32 match_cap = 16;
  ...
  while (curr) {
      ...
      if (matches && desc->any_count > 0) {
          ...
          if (!any_found) matches = false;
      }
      
      if (matches) {
          if (query->archetype_count >= match_cap) {
              match_cap *= 2;
              query->matching_archetypes = realloc(query->matching_archetypes, match_cap * sizeof(Archetype*));
          }
...
void ecs_query_destroy(World *world, Query *query) {
  if (!query) return;
  if (query->component_types) free(query->component_types);
  if (query->matching_archetypes) free(query->matching_archetypes);
...
bool ecs_query_next(Query *query, Entity *entity, void **components) {
  ...
  if (query->archetype_count == 0) return false;

  // Check if we need to advance archetype
  SimpleArchetype *arch = (SimpleArchetype*)query->matching_archetypes[query->current_archetype];
  ...
  while (query->current_entity >= arch->entity_count) {
      ...
      if (query->current_archetype >= query->archetype_count) {
          ...
          // User must manually reset query if they want to reuse it, or just create new one.
          // Resetting indices for potential reuse:
          query->current_archetype = 0;
          query->current_entity = 0; 
          return false;
      }
      arch = (SimpleArchetype*)query->matching_archetypes[query->current_archetype];
      query->current_entity = 0;
  }
  ...
  if (components) {
      ...
      // OPTIMIZATION: Cache offsets in query->component_offsets when switching archetypes
      ...
      for (u32 i = 0; i < query->component_count; i++) {
          ...
          for (u32 j = 0; j < arch->component_count; j++) {
              if (arch->component_types[j] == type) {
                  ...
                  // Wait, arch->component_data is void**. We need component size to offset.
                  // Only way is to access global world or if components are stored differently.
                  // 'SimpleArchetype' implementation in this file uses component_info size.
                  
                  // CRITICAL: We need component size to iterate.
                  // Let's use the global g_ecs_world helper for now or change API to pass world
                  // The standard API ecs_query_next doesnt take world.
                  
                  if (g_ecs_world) {
                      ComponentDataInternal *cd = (ComponentDataInternal *)g_ecs_world->component_info;
                      u32 size = cd->infos[type].size;
...
static ECSComponentID get_static_id_for_name(const char *name) {
    if (!name) return (ECSComponentID)-1;
    if (strcmp(name, "TransformComponent") == 0) return TRANSFORM_COMPONENT_ID;
    if (strcmp(name, "RigidbodyComponent") == 0) return RIGIDBODY_COMPONENT_ID;
    if (strcmp(name, "NPCComponent") == 0) return NPC_COMPONENT_ID;
    if (strcmp(name, "HealthComponent") == 0) return HEALTH_COMPONENT_ID;
    if (strcmp(name, "PlayerComponent") == 0) return PLAYER_COMPONENT_ID;
    if (strcmp(name, "HitboxComponent") == 0) return HITBOX_COMPONENT_ID;
    if (strcmp(name, "DamageComponent") == 0) return DAMAGE_COMPONENT_ID;
    if (strcmp(name, "ResistanceComponent") == 0) return RESISTANCE_COMPONENT_ID;
    if (strcmp(name, "ProjectileComponent") == 0) return PROJECTILE_COMPONENT_ID;
    if (strcmp(name, "InventoryComponent") == 0) return INVENTORY_COMPONENT_ID;
    if (strcmp(name, "ItemComponent") == 0) return ITEM_COMPONENT_ID;
...
ECSComponentID ecs_register_component(World *world, const ComponentInfo *info) {
  ...
  for (u32 i = 0; i < cd->count; i++) {
    if (cd->infos[i].name && info->name && strcmp(cd->infos[i].name, info->name) == 0) {
      return i;
    }
  }
...
ECSComponentID ecs_get_component_id(World *world, const char *name) {
    if (!world || !world->component_info || !name) return 0;
    ComponentDataInternal *cd = (ComponentDataInternal *)world->component_info;
    
    // First try the static mapping for speed
    ECSComponentID static_id = get_static_id_for_name(name);
    if (static_id != (ECSComponentID)-1 && static_id < cd->count && cd->infos[static_id].name) {
        return static_id;
    }
...
const ComponentInfo *ecs_get_component_info(World *world, ECSComponentID id) {
    if (!world || !world->component_info) return NULL;
    ComponentDataInternal *cd = (ComponentDataInternal *)world->component_info;
    if (id >= cd->count) return NULL;
    return &cd->infos[id];
}
...
System *ecs_system_create(World *world, const char *name, SystemFunc execute, const QueryDesc *query_desc) {
  if (!world || !execute) return NULL;
  ...
  if (query_desc) {
      ...
      // Should we pre-compile the query? The system structure in ecs.h doesn't hold a Query* 
      // but just QueryDesc. 
      // The SystemContext passed to execute holds a Query*.
      // We will create the query ONCE during system creation or first run usually.
      // But System struct definition in ecs.h only has QueryDesc.
      // Wait, SystemContext has Query*.
      // We need to store the Query object somewhere.
      // Easiest is to change System struct in ecs.c to hold it, 
      // but ecs.h defines System struct opaquely? No, it defines it fully (line 152).
      // Line 155 is `QueryDesc query;`.
      // So where do we store the compiled Query*?
      // We can use `user_data` or just re-create it (slow).
      // Or we assume the system function manages it.
      // ACTUALLY: Let's assume we re-create or cache it.
      // Better: Let's abuse the fact we are inside the engine and can store it 
      // if we extend the struct or use a parallel array.
      // For now, let's create it every frame (slow but correct) or use static in function?
      // No, let's look at `ecs_world_update` below.
  }
...
void ecs_world_update(World *world, f32 delta_time) {
  ...
  for (u32 i = 0; i < sd->count; i++) {
      ...
      if (sys->query.all_count > 0 || sys->query.any_count > 0 || sys->query.none_count > 0) {
...
void ecs_system_set_priority(World *world, System *system, u32 priority) {
    if (system) system->priority = priority;
