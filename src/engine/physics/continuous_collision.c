...
bool aabb_intersect(Vec3 min_a, Vec3 max_a, Vec3 min_b, Vec3 max_b);
bool point_in_aabb(Vec3 point, Vec3 min_bounds, Vec3 max_bounds);

// Forward declarations of internal helpers
void ccd_calculate_swept_bounds_for_entry(const CCDBroadphaseEntry* entry, float time_step, Vec3* min_out, Vec3* max_out);
bool ccd_detect_collision_for_entries(const CCDBroadphaseEntry* entry_a, const CCDBroadphaseEntry* entry_b, float time_step, CCDCollisionResult* result);

// Global CCD world instance
static CCDWorld g_ccd_world = {0};

bool ccd_world_init(CCDWorld* world, uint32_t max_entities) {
    if (!world || max_entities == 0) return false;
    
    memset(world, 0, sizeof(CCDWorld));
    
    // Initialize broadphase entries
    world->entry_capacity = max_entities;
    world->entries = (CCDBroadphaseEntry*)core_alloc(world->entry_capacity * sizeof(CCDBroadphaseEntry));
    ...
    world->spatial_hash.hash_table = (uint32_t*)core_alloc(world->spatial_hash.table_size * sizeof(uint32_t));
    world->spatial_hash.entry_lists = (uint32_t*)core_alloc(world->spatial_hash.table_size * sizeof(uint32_t));
...
void ccd_world_update(CCDWorld* world, float time_step) {
    ...
    for (int step = 0; step < world->max_substeps; step++) {
        ...
        for (uint32_t i = 0; i < world->entry_count; i++) {
            ...
            if (!entry_a || entry_a->entity == 0) continue;
            
            // Get swept bounds for this entry
            Vec3 bounds_min_a, bounds_max_a;
            ccd_calculate_swept_bounds_for_entry(entry_a, sub_step_time, &bounds_min_a, &bounds_max_a);
            
            // Query spatial hash for potential collisions
            uint32_t potential_collisions[32];
            uint32_t collision_count = 0;
            ccd_spatial_hash_query(world, bounds_min_a, bounds_max_a, potential_collisions, &collision_count);
            
            for (uint32_t j = 0; j < collision_count; j++) {
                uint32_t entry_index = potential_collisions[j];
                if (entry_index <= i) continue; // Avoid duplicate checks
                
                CCDBroadphaseEntry* entry_b = &world->entries[entry_index];
                if (!entry_b || entry_b->entity == 0) continue;
                
                // Check collision masks
                if ((entry_a->collision_mask & entry_b->collision_group) == 0) continue;
                if ((entry_b->collision_mask & entry_a->collision_group) == 0) continue;
                
                // Perform narrowphase CCD
                CCDCollisionResult result = {0};
                if (ccd_detect_collision_for_entries(entry_a, entry_b, sub_step_time, &result)) {
        ...
        for (uint32_t i = 0; i < world->entry_count; i++) {
            ...
            if (entry && entry->entity != 0 && !entry->is_static) {
                entry->position = vec3_add(entry->position, vec3_mul(entry->velocity, sub_step_time));
            }
        }
    }
...
bool ccd_world_add_sphere(CCDWorld* world, EntityID entity, const CCDSphere* sphere) {
    if (!world || !sphere || world->entry_count >= world->entry_capacity) return false;
    ...
    entry->shape_ptr = (const CCDShape*)sphere;
...
bool ccd_world_add_box(CCDWorld* world, EntityID entity, const CCDBox* box) {
    if (!world || !box || world->entry_count >= world->entry_capacity) return false;
    ...
    entry->broadphase_index = world->entry_count;
    entry->shape_ptr = (const CCDShape*)box;
...
bool ccd_detect_collision(const CCDShape* shape_a, const CCDShape* shape_b, float time_step, CCDCollisionResult* result) {
    if (!shape_a || !shape_b || !result) return false;
    
    // Check if shapes are moving fast enough to require CCD
    if (!ccd_should_use_ccd(shape_a, time_step) && !ccd_should_use_ccd(shape_b, time_step)) {
        return false;
    }
    ...
    switch (shape_a->type) {
        case CCD_SHAPE_SPHERE:
            switch (shape_b->type) {
                ...
                case CCD_SHAPE_BOX:
                    return ccd_sphere_vs_box((const CCDSphere*)shape_a, (const CCDBox*)shape_b, time_step, result);
                case CCD_SHAPE_CAPSULE:
                    return ccd_sphere_vs_capsule((const CCDSphere*)shape_a, (const CCDCapsule*)shape_b, time_step, result);
                default:
                    return false;
            }
        case CCD_SHAPE_BOX:
            switch (shape_b->type) {
                case CCD_SHAPE_SPHERE:
                    return ccd_sphere_vs_box((const CCDSphere*)shape_b, (const CCDBox*)shape_a, time_step, result);
                case CCD_SHAPE_BOX:
                    return ccd_box_vs_box((const CCDBox*)shape_a, (const CCDBox*)shape_b, time_step, result);
                default:
                    return false;
            }
        case CCD_SHAPE_CAPSULE:
            switch (shape_b->type) {
                case CCD_SHAPE_SPHERE:
                    return ccd_sphere_vs_capsule((const CCDSphere*)shape_b, (const CCDCapsule*)shape_a, time_step, result);
                case CCD_SHAPE_CAPSULE:
                    return ccd_capsule_vs_capsule((const CCDCapsule*)shape_a, (const CCDCapsule*)shape_b, time_step, result);
                default:
                    return false;
            }
        default:
...
bool ccd_sphere_vs_sphere(const CCDSphere* sphere_a, const CCDSphere* sphere_b, float time_step, CCDCollisionResult* result) {
    if (!sphere_a || !sphere_b || !result) return false;
    
    // Calculate relative motion
    Vec3 relative_velocity = vec3_sub(sphere_b->base.linear_velocity, sphere_a->base.linear_velocity);
    Vec3 relative_position = vec3_sub(sphere_b->base.position, sphere_a->base.position);
    ...
    if (distance_sq < combined_radius * combined_radius) {
        ...
        result->contact_normal = (distance > 0.001f) ? vec3_div(relative_position, distance) : (Vec3){0, 1, 0};
        result->contact_point = vec3_add(sphere_a->base.position, vec3_mul(result->contact_normal, sphere_a->radius));
        result->penetration_depth = vec3_length(relative_position) - combined_radius;
    ...
    float c = vec3_length_sq(relative_position) - combined_radius * combined_radius;
    ...
    Vec3 position_at_impact_a = vec3_add(sphere_a->base.position, vec3_mul(sphere_a->base.linear_velocity, t));
    Vec3 position_at_impact_b = vec3_add(sphere_b->base.position, vec3_mul(sphere_b->base.linear_velocity, t));
    ...
    result->contact_point = vec3_add(position_at_impact_a, vec3_mul(result->contact_normal, sphere_a->radius));
...
bool ccd_sphere_vs_box(const CCDSphere* sphere, const CCDBox* box, float time_step, CCDCollisionResult* result) {
    if (!sphere || !box || !result) return false;
    ...
    Vec3 local_sphere_vel = vec3_sub(sphere->base.linear_velocity, box->base.linear_velocity);
    ...
    if (distance_sq < radius_sq) {
        ...
        result->impact_velocity = vec3_length(vec3_sub(sphere->base.linear_velocity, box->base.linear_velocity));
        return true;
    }
    ...
    Vec3 swept_bounds_min = vec3_sub(box->base.position, vec3_add(box->half_extents, (Vec3){sphere->radius, sphere->radius, sphere->radius}));
    Vec3 swept_bounds_max = vec3_add(box->base.position, vec3_add(box->half_extents, (Vec3){sphere->radius, sphere->radius, sphere->radius}));
    
    Vec3 sphere_end_pos = vec3_add(sphere->base.position, vec3_mul(sphere->base.linear_velocity, time_step));
    ...
    if (point_in_aabb(sphere_end_pos, swept_bounds_min, swept_bounds_max)) {
        ...
        result->contact_normal = vec3_normalize(vec3_sub(sphere->base.position, box->base.position));
        result->contact_point = vec3_add(sphere->base.position, vec3_mul(result->contact_normal, sphere->radius));
...
bool ccd_box_vs_box(const CCDBox* box_a, const CCDBox* box_b, float time_step, CCDCollisionResult* result) {
    if (!box_a || !box_b || !result) return false;
    
    // Simplified AABB vs AABB continuous collision
    // For full implementation, would need to use separating axis theorem with time
    
    Vec3 relative_velocity = vec3_sub(box_b->base.linear_velocity, box_a->base.linear_velocity);
    ...
    if (aabb_intersect(a_min, a_max, b_min, b_max)) {
        ...
        Vec3 penetration = {
            fminf(a_max.x - b_min.x, b_max.x - a_min.x),
        ...
        if (penetration.x < penetration.y && penetration.x < penetration.z) {
            result->contact_normal = (relative_position.x > 0) ? (Vec3){1, 0, 0} : (Vec3){-1, 0, 0};
            result->penetration_depth = penetration.x;
        } else if (penetration.y < penetration.z) {
            result->contact_normal = (relative_position.y > 0) ? (Vec3){0, 1, 0} : (Vec3){0, -1, 0};
            result->penetration_depth = penetration.y;
        } else {
            result->contact_normal = (relative_position.z > 0) ? (Vec3){0, 0, 1} : (Vec3){0, 0, -1};
    ...
    Vec3 a_end_min = vec3_add(a_min, vec3_mul(box_a->base.linear_velocity, time_step));
    Vec3 a_end_max = vec3_add(a_max, vec3_mul(box_a->base.linear_velocity, time_step));
...
void ccd_spatial_hash_insert(CCDWorld* world, uint32_t entry_index) {
    if (!world || entry_index >= world->entry_count) return;
    
    CCDBroadphaseEntry* entry = &world->entries[entry_index];
    
    // Calculate hash cells for the entry's bounds
    Vec3 min_cell = {
        floorf((entry->bounds_min.x - world->spatial_hash.world_min.x) / world->spatial_hash.cell_size),
        floorf((entry->bounds_min.y - world->spatial_hash.world_min.y) / world->spatial_hash.cell_size),
        floorf((entry->bounds_min.z - world->spatial_hash.world_min.z) / world->spatial_hash.cell_size)
    };
    
    Vec3 max_cell = {
        floorf((entry->bounds_max.x - world->spatial_hash.world_min.x) / world->spatial_hash.cell_size),
        floorf((entry->bounds_max.y - world->spatial_hash.world_min.y) / world->spatial_hash.cell_size),
        floorf((entry->bounds_max.z - world->spatial_hash.world_min.z) / world->spatial_hash.cell_size)
    };
    ...
    for (int x = (int)min_cell.x; x <= (int)max_cell.x; x++) {
        for (int y = (int)min_cell.y; y <= (int)max_cell.y; y++) {
            for (int z = (int)min_cell.z; z <= (int)max_cell.z; z++) {
                uint32_t hash = spatial_hash_coordinate(x, y, z) & (world->spatial_hash.table_size - 1);
                
                // Insert at head of list
                world->spatial_hash.entry_lists[entry_index] = world->spatial_hash.hash_table[hash];
...
bool ccd_should_use_ccd(const CCDShape* shape, float time_step) {
    if (!shape || shape->is_static) return false;
...
void ccd_update_broadphase_bounds(CCDWorld* world, uint32_t entry_index) {
    ...
    Vec3 velocity_extent = vec3_mul(vec3_abs(entry->velocity), world->max_time_step);
...
void ccd_resolve_collision(CCDWorld* world, const CCDCollisionResult* collision) {
    if (!world || !collision || !collision->hit) return;
    ...
    if (world->enable_motion_clamping && collision->penetration_depth > 0.001f) {
        ...
        Vec3 correction = vec3_mul(collision->contact_normal, collision->penetration_depth * world->contact_erp);
    ...
    if (collision->impact_velocity > 0.1f) {
        ...
        float impulse_magnitude = -(1.0f + restitution) * collision->impact_velocity;
        
        ccd_apply_impulse(collision, impulse_magnitude);
    }
}

void ccd_apply_impulse(const CCDCollisionResult* collision, float impulse_magnitude) {
    if (!collision) return;
...
void ccd_calculate_swept_bounds_for_entry(const CCDBroadphaseEntry* entry, float time_step, Vec3* min_out, Vec3* max_out) {
    if (!entry || !min_out || !max_out) return;
    
    // Start with static bounds (at t=0)
    // Note: Since we don't store shape dimensions in broadphase entry, 
    // we use the cached bounds from add_entity.
    Vec3 start_min = entry->bounds_min;
    Vec3 start_max = entry->bounds_max;
    
    // Calculate end bounds (at t=time_step)
    Vec3 displacement = vec3_mul(entry->velocity, time_step);
    Vec3 end_min = vec3_add(start_min, displacement);
    Vec3 end_max = vec3_add(start_max, displacement);
    
    // Union of start and end positions
    min_out->x = fminf(start_min.x, end_min.x);
    min_out->y = fminf(start_min.y, end_min.y);
    min_out->z = fminf(start_min.z, end_min.z);
    
    max_out->x = fmaxf(start_max.x, end_max.x);
    max_out->y = fmaxf(start_max.y, end_max.y);
    max_out->z = fmaxf(start_max.z, end_max.z);
}

bool ccd_detect_collision_for_entries(const CCDBroadphaseEntry* entry_a, const CCDBroadphaseEntry* entry_b, float time_step, CCDCollisionResult* result) {
    if (!entry_a || !entry_b || !result) return false;
    
    // If we have shape pointers, use them for precise collision
    if (entry_a->shape_ptr && entry_b->shape_ptr) {
        return ccd_detect_collision(entry_a->shape_ptr, entry_b->shape_ptr, time_step, result);
    }
    
    // Fallback if shapes are missing (should not happen with new add code)
    return false;
}
...
void cleanup_ccd_system(void) {
    ccd_world_cleanup(&g_ccd_world);
}
