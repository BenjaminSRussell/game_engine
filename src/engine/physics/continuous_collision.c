...
bool ccd_world_init(CCDWorld *world, uint32_t max_entities) {
  ...
  world->entries = (CCDBroadphaseEntry *)calloc(world->entry_capacity,
  ...
  LOG_INFO_CAT(LOG_CAT_PHYSICS, "CCD World initialized with capacity %d",
               world->entry_capacity);
  return true;
}
...
void ccd_world_cleanup(CCDWorld *world) {
  ...
  if (world->spatial_hash.hash_table)
    free(world->spatial_hash.hash_table);
  if (world->spatial_hash.entry_lists)
    free(world->spatial_hash.entry_lists);
...
static CCDBroadphaseEntry *get_free_entry(CCDWorld *world) {
  if (world->entry_count >= world->entry_capacity)
    return NULL;
...
void ccd_world_update(CCDWorld *world, float time_step) {
  ...
  for (uint32_t i = 0; i < world->entry_count; i++) {
    ...
    if (shape) {
      ...
      ccd_calculate_swept_bounds(shape, time_step, &entry->bounds_min,
                                 &entry->bounds_max);
    }
  }
  ...
  for (uint32_t i = 0; i < world->entry_count; i++) {
    ...
    for (uint32_t j = i + 1; j < world->entry_count; j++) {
      ...
      // Filter
      if (entry_a->is_static && entry_b->is_static)
        continue;
      // if (!(entry_a->collision_mask & entry_b->collision_group)) continue; //
      // Basic filtering
      ...
      if (ccd_bounds_intersect(entry_a->bounds_min, entry_a->bounds_max,
        ...
        if (ccd_detect_collision(entry_a->shape_ptr, entry_b->shape_ptr,
                                 time_step, &result)) {
...
bool ccd_detect_collision(const CCDShape *shape_a, const CCDShape *shape_b,
  ...
  if (shape_a->type == CCD_SHAPE_SPHERE && shape_b->type == CCD_SHAPE_SPHERE) {
    return ccd_sphere_vs_sphere((const CCDSphere *)shape_a,
                                (const CCDSphere *)shape_b, time_step, result);
  }
...
static bool ray_vs_sphere(const Vec3 *origin, const Vec3 *dir, float max_dist,
  ...
  Vec3 closest_point =
      vec3_add(*origin, vec3_mul(*dir, projection)); // vec3_mul for scalar
...
bool ccd_sphere_vs_sphere(const CCDSphere *sphere_a, const CCDSphere *sphere_b,
  ...
  Vec3 end =
      vec3_add(start, vec3_mul(rel_vel, time_step)); // vec3_mul for scalar

  if (ray_vs_sphere(&start, &dir, dist, &sphere_b->base.position,
                    combined_radius, result)) {
    // Adjust normal (it's center to center at impact)
    // result->contact_normal is already correct from ray_vs_sphere (relative to
    // combined center)
    return true;
  }
...
bool ccd_ray_cast(const CCDWorld *world, Vec3 ray_start, Vec3 ray_direction,
  ...
  for (uint32_t i = 0; i < world->entry_count; i++) {
    ...
    if (shape->type == CCD_SHAPE_SPHERE) {
      hit = ccd_ray_cast_sphere((const CCDSphere *)shape, ray_start,
                                ray_direction, max_distance, &temp_res);
    } else if (shape->type == CCD_SHAPE_BOX) {
      hit = ccd_ray_cast_box((const CCDBox *)shape, ray_start, ray_direction,
                             max_distance, &temp_res);
    }

    if (hit) {
      float dist =
          vec3_distance(ray_start, temp_res.contact_point); // Fix: dereference
...
bool ccd_ray_cast_sphere(const CCDSphere *sphere, Vec3 ray_start,
                         ...
                         CCDCollisionResult *result) {
  return ray_vs_sphere(&ray_start, &ray_direction, max_distance,
                       &sphere->base.position, sphere->radius, result);
}

bool ccd_ray_cast_box(const CCDBox *box, Vec3 ray_start, Vec3 ray_direction,
                      float max_distance, CCDCollisionResult *result) {
  // AABB ray intersection using slab method
  Vec3 box_min = vec3_sub(box->base.position, box->half_extents);
  Vec3 box_max = vec3_add(box->base.position, box->half_extents);
  ...
  // Test each axis
  for (int i = 0; i < 3; i++) {
    float axis_val = ((float *)&ray_direction)[i];
    float origin_val = ((float *)&ray_start)[i];
    float min_val = ((float *)&box_min)[i];
    float max_val = ((float *)&box_max)[i];

    if (fabsf(axis_val) < 0.0001f) {
      // Ray parallel to slab, check if origin is within slab
      if (origin_val < min_val || origin_val > max_val) {
        return false;
      }
    } else {
      float t1 = (min_val - origin_val) / axis_val;
      float t2 = (max_val - origin_val) / axis_val;
  ...
  if (result && t_min >= 0 && t_min <= max_distance) {
    result->hit = true;
    result->time_of_impact = t_min / max_distance;
    result->contact_point = vec3_add(ray_start, vec3_mul(ray_direction, t_min));

    // Calculate normal based on which face was hit
    Vec3 local = vec3_sub(result->contact_point, box->base.position);
    Vec3 abs_local = {fabsf(local.x), fabsf(local.y), fabsf(local.z)};
    Vec3 normal = {0, 0, 0};
...
bool ccd_should_use_ccd(const CCDShape *shape, float time_step) {
  ...
  if (shape->type == CCD_SHAPE_SPHERE)
    min_dim = ((CCDSphere *)shape)->radius;
  // ... other shapes
...
float ccd_calculate_swept_bounds(const CCDShape *shape, float time_step,
  ...
  Vec3 end_pos =
      vec3_add(start_pos, vec3_mul(shape->linear_velocity,
                                   time_step)); // vec3_mul for scalar
...
void ccd_integrate_motion(CCDShape *shape, float time_step) {
  if (shape) {
    shape->position =
        vec3_add(shape->position, vec3_mul(shape->linear_velocity,
                                           time_step)); // vec3_mul for scalar
  }
}
...
void ccd_resolve_collision(CCDWorld *world,
  ...
  CCDBroadphaseEntry *entry_b = find_entry(world, collision->entity_b);

  if (!entry_a || !entry_b)
    return;

  CCDShape *shape_a = (CCDShape *)entry_a->shape_ptr;
  CCDShape *shape_b = (CCDShape *)entry_b->shape_ptr;

  if (!shape_a || !shape_b)
    return;

  // Position correction: Move object A to contact point
  if (!shape_a->is_static) {
    // Rewind to time of impact
    Vec3 displacement =
        vec3_mul(shape_a->linear_velocity, collision->time_of_impact);
    shape_a->position = vec3_add(shape_a->position, displacement);
    entry_a->position = shape_a->position;

    // Apply impulse-based velocity change
    float restitution = 0.5f; // Bounciness
    Vec3 relative_vel = shape_b->is_static ? shape_a->linear_velocity
                                           : vec3_sub(shape_a->linear_velocity,
                                                      shape_b->linear_velocity);

    float vel_along_normal = vec3_dot(relative_vel, collision->contact_normal);

    if (vel_along_normal < 0) { // Objects moving toward each other
      float impulse = -(1.0f + restitution) * vel_along_normal;
      Vec3 impulse_vec = vec3_mul(collision->contact_normal, impulse);

      shape_a->linear_velocity =
          vec3_add(shape_a->linear_velocity, impulse_vec);
      entry_a->velocity = shape_a->linear_velocity;

      if (!shape_b->is_static) {
        shape_b->linear_velocity =
            vec3_sub(shape_b->linear_velocity, impulse_vec);
        entry_b->velocity = shape_b->linear_velocity;
      }
    }
  }
}
...
bool ccd_sphere_vs_box(const CCDSphere *sphere, const CCDBox *box,
                       float time_step, CCDCollisionResult *result) {
  // Swept sphere vs AABB using ray-cast against expanded box
  Vec3 rel_vel =
      vec3_sub(sphere->base.linear_velocity, box->base.linear_velocity);
  float rel_speed = vec3_length(rel_vel);

  if (rel_speed < 0.0001f)
    return false;

  Vec3 dir = vec3_mul(rel_vel, 1.0f / rel_speed);
  float dist = rel_speed * time_step;

  // Expand box by sphere radius (Minkowski sum)
  Vec3 expanded_extents =
      vec3_add(box->half_extents,
               (Vec3){sphere->radius, sphere->radius, sphere->radius});
  CCDBox expanded_box = *box;
  expanded_box.half_extents = expanded_extents;

  // Ray cast sphere center against expanded box
  return ccd_ray_cast_box(&expanded_box, sphere->base.position, dir, dist,
                          result);
}
bool ccd_sphere_vs_capsule(const CCDSphere *sphere, const CCDCapsule *capsule,
  ...
  // Relative velocity
  Vec3 rel_vel =
      vec3_sub(sphere->base.linear_velocity, capsule->base.linear_velocity);
  float rel_speed = vec3_length(rel_vel);
  ...
  Vec3 dir = vec3_mul(rel_vel, 1.0f / rel_speed);
  float dist = rel_speed * time_step;
  float combined_radius = sphere->radius + capsule->radius;

  // Capsule line segment (along Y axis by convention)
  Vec3 capsule_start =
      vec3_add(capsule->base.position, (Vec3){0, -capsule->height / 2, 0});
  Vec3 capsule_end =
      vec3_add(capsule->base.position, (Vec3){0, capsule->height / 2, 0});

  // Find closest point on swept ray to capsule line segment
  // For simplicity, treat as two sphere tests (caps) + cylinder test
  // Use sphere-sphere test against both caps

  CCDSphere cap1;
  memset(&cap1, 0, sizeof(CCDSphere));
  cap1.base = capsule->base;
  cap1.base.type = CCD_SHAPE_SPHERE;
  cap1.base.position = capsule_start;
  cap1.radius = capsule->radius;

  CCDSphere cap2 = cap1;
  cap2.base.position = capsule_end;

  if (ccd_sphere_vs_sphere(sphere, &cap1, time_step, result))
    return true;
  if (ccd_sphere_vs_sphere(sphere, &cap2, time_step, result))
    return true;

  // TODO: Add cylinder middle section test for completeness
  return false;
}
...
void ccd_apply_impulse(const CCDCollisionResult *collision,
  ...
  // Impulse application would require access to rigid body masses
  // This is a callback function for external physics integration
  // In a full implementation, this would update velocities based on mass
  // For now, this is a stub that external code can hook into
}
void ccd_apply_position_correction(const CCDCollisionResult *collision,
                                   float correction_magnitude) {
  // Position correction along contact normal
  // External hook for physics integration
  // In full implementation: position += contact_normal * correction_magnitude
}
Vec3 ccd_extrapolate_position(const CCDShape *shape, float time) {
  return shape ? shape->position : (Vec3){0};
}
Quat ccd_extrapolate_rotation(const CCDShape *shape, float time) {
  return shape ? shape->rotation : (Quat){0, 0, 0, 1};
}
