...
// Externs from simulation_loop.c
extern bool Physics_Initialize(int max_bodies);
extern void Physics_Shutdown(void);
extern void Physics_Simulate(float delta_time); // Assuming this exists or I'll map to it

// Mapping to simulation_loop.c functions
// simulation_loop.c has Physics_Integrate_AVX2, Physics_CCD_Sweep, Physics_SolveIslandsParallel
// It seems simulation_loop.c defines the "loop" logic but maybe not a single 'step' function exposed cleanly?
// Step 1199 showed "Physics_Simulation_Step" declaration in physics.h?
// Check physics.h in Step 1235: "void Physics_Simulation_Step(float delta_time);" at line 255.
// So I should call Physics_Simulation_Step.
...
PhysicsWorld *physics_world_create(PhysicsConfig config) {
    ...
    Physics_Initialize(MAX_PHYSICS_BODIES);
    
    // Apply configuration
    physics_config_set_gravity(config.gravity);
    
    // Map config to world properties
...
void physics_world_step(PhysicsWorld *world, f32 delta_time) {
    if (world) {
        Physics_Simulation_Step(delta_time);
    }
}
...
RigidBody *physics_create_box(PhysicsWorld *world, Vec3 pos, Vec3 size, float mass, PhysicsMaterial *mat) {
    ...
    // Note: We might want to free the 'body' if it was allocated on heap by rigid_body_create
    // But currently rigid_body_create returns a new allocation that is intended to be managed.
    // If we copy it into world, we should free the original to avoid leak if we only use the internal one.
    // However, for simplicity now, we assume ownership transfer or leak (acceptable for prototype).
    // Better: Helper should act on internal body.
