...
#define DEFAULT_MEMORY_BUDET (64 * 1024 * 1024) // 64MB
...
void physics_world_update(PhysicsWorld *world, float dt) {
    ...
    while (world->accumulated_time >= world->config.time_step) {
        ...
        for (int i = 0; i < world->body_count; i++) {
            ...
            vec3_add(body->linear_velocity, body->linear_velocity, 
                    body->linear_acceleration, substep_dt);
            ...
            vec3_add(body->position, body->position, body->linear_velocity, substep_dt);
