...
void gpu_material_unregister(GPUMaterialManager* manager, u32 material_id) {
    ...
    if (gpu_mat->texture_bindings.descriptor_set != VK_NULL_HANDLE) {
        gpu_material_free_descriptor_set(manager, gpu_mat->texture_bindings.descriptor_set);
    }

    // Swap with last material
    if (material_id < manager->material_count - 1) {
        manager->materials[material_id] = manager->materials[manager->material_count - 1];
    }
...
bool gpu_material_bind_texture(GPUMaterialManager* manager, u32 material_id,
                               u32 slot, VkImageView image_view, VkSampler sampler) {
    if (!manager || !manager->initialized) {
        return false;
    }
...
bool gpu_material_create_descriptor_set(GPUMaterialManager* manager, u32 material_id) {
    if (!manager || !manager->initialized) {
        return false;
    }
    ...
    gpu_mat->texture_bindings.descriptor_set = gpu_material_allocate_descriptor_set(manager);
    if (gpu_mat->texture_bindings.descriptor_set == VK_NULL_HANDLE) {
...
bool gpu_material_update_descriptor_set(GPUMaterialManager* manager, u32 material_id) {
    if (!manager || !manager->initialized) {
        return false;
    }
...
void gpu_material_destroy_pipeline(GPUMaterialManager* manager, u32 material_id) {
    if (!manager || !manager->initialized) {
        return;
    }
...
VkPipeline gpu_material_get_pipeline(GPUMaterialManager* manager, u32 material_id) {
    if (!manager || !manager->initialized) {
...
void gpu_material_bind(GPUMaterialManager* manager, VkCommandBuffer cmd_buffer,
    ...
    // Bind pipeline
    if (gpu_mat->pipeline != VK_NULL_HANDLE) {
        vkCmdBindPipeline(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gpu_mat->pipeline);
    }
...
void gpu_material_push_constants(GPUMaterialManager* manager, VkCommandBuffer cmd_buffer,
                                 u32 material_id) {
    if (!manager || !manager->initialized || !cmd_buffer) {
        return;
    }
    ...
    vkCmdPushConstants(cmd_buffer, manager->shared_pipeline_layout,
                      ...
                      sizeof(GPUMaterialPushConstant),
                      &gpu_mat->push_constant);
}
...
bool gpu_material_create_samplers(GPUMaterialManager* manager, f32 max_anisotropy) {
    if (!manager || !manager->initialized) {
        return false;
    }
...
VkSampler gpu_material_get_sampler(GPUMaterialManager* manager, TextureFilter filter) {
    if (!manager || !manager->initialized) {
        return VK_NULL_HANDLE;
    }
...
bool gpu_material_create_descriptor_pool(GPUMaterialManager* manager, u32 pool_size) {
    if (!manager || !manager->initialized) {
        return false;
    }
...
VkDescriptorSet gpu_material_allocate_descriptor_set(GPUMaterialManager* manager) {
    if (!manager || !manager->initialized) {
        return VK_NULL_HANDLE;
    }
...
void gpu_material_free_descriptor_set(GPUMaterialManager* manager, VkDescriptorSet set) {
    if (!manager || !set) {
        return;
    }
...
void gpu_material_update_push_constants(GPUMaterialManager* manager, u32 material_id) {
    if (!manager || !manager->initialized) {
        return;
    }
...
GPUMaterialPushConstant* gpu_material_get_push_constants(GPUMaterialManager* manager,
                                                          u32 material_id) {
    if (!manager || !manager->initialized) {
        return NULL;
    }
...
void gpu_material_sort_for_batching(GPUMaterialManager* manager, u32* material_ids,
                                    u32 count) {
