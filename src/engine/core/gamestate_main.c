...
// Accessor for debug visualization (used by renderer)
// PhysicsDebugVisualization *get_debug_vis(void) { return &g_game.debug_vis; }
...
static void mesh_generation_job(void *data) {
  ...
  double t0 = now_seconds();
  ChunkMesh mesh;
  chunk_mesh_init(&mesh, 65536, 131072);

  chunk_mesh_generate_chunk(&mesh, job->chunk, job->registry, job->options);

  // Post-process: Optimize vertex cache
  // mesh_optimize_vertex_cache(&mesh); // Incompatible with ChunkMesh
...
static void furnace_update_legacy(FurnaceState *state, Inventory *inventory,
                                  f32 delta_time);
...
static void gamestate_camera_add_shake(f32 intensity, f32 duration) {
...
static void update_mining_effects(f32 delta_time) {
  ...
  // Get current tool (used for both sound and camera shake)
  ItemID current_tool =
  ...
  if (current_tool == ITEM_IRON_PICKAXE ||
    ...
    if (shake_timer >= 0.1f) { // Shake every 100ms
      shake_timer = 0.0f;
      gamestate_camera_add_shake(0.02f, 0.05f); // Small, quick shake
    }
  }
...
static InitResult init_plant_vfx(void) {
  ...
  if (!physics_debug_visualization_init(&g_game.debug_vis, 8192)) {
    LOG_ERROR("Failed to initialize debug visualization");
  } else {
    physics_debug_set_enabled(&g_game.debug_vis, true);
    physics_debug_set_flags(&g_game.debug_vis, DEBUG_VIS_COLLISION_SHAPES |
...
static InitResult init_physics(void) {
  PhysicsConfig pconfig = {.gravity = vec3(0.0f, g_game.config.gravity, 0.0f),
                           .fixed_timestep = 1.0f / 60.0f,
                           .velocity_iterations = 8,
                           .position_iterations = 3};
  g_game.physics_world = physics_world_create(pconfig);
...
static InitResult init_ecs(void) {
  ...
  ecs_register_component(
      &g_game.ecs_world,
      &(ComponentInfo){.size = sizeof(TransformComponent),
                       .alignment = __alignof__(TransformComponent),
                       .name = "TransformComponent"});

  ecs_register_component(
      &g_game.ecs_world,
      &(ComponentInfo){.size = sizeof(RigidBodyComponent),
                       .alignment = __alignof__(RigidBodyComponent),
                       .name = "RigidBodyComponent"});

  ecs_register_component(
      &g_game.ecs_world,
      &(ComponentInfo){.size = sizeof(NPCComponent),
                       .alignment = __alignof__(NPCComponent),
                       .name = "NPCComponent"});

  ecs_register_component(
      &g_game.ecs_world,
      &(ComponentInfo){.size = sizeof(HealthComponent),
                       .alignment = __alignof__(HealthComponent),
                       .name = "HealthComponent"});

  ecs_register_component(
      &g_game.ecs_world,
      &(ComponentInfo){.size = sizeof(PlayerComponent),
...
static void game_init(void) {
  ...
  init_progress_start("Core Systems", 15.0f);
  ...
  npc_visuals_init();
  // Initialize dynamic NPC mesh
  g_game.npc_batch_mesh.vertex_capacity = 65536;
  g_game.npc_batch_mesh.index_capacity = 131072;
  g_game.npc_batch_mesh.vertices =
      (Vertex *)calloc(g_game.npc_batch_mesh.vertex_capacity, sizeof(Vertex));
  g_game.npc_batch_mesh.indices =
      (u32 *)calloc(g_game.npc_batch_mesh.index_capacity, sizeof(u32));
  g_game.npc_batch_mesh.vertex_count = 0;
  g_game.npc_batch_mesh.index_count = 0;
  g_game.npc_batch_mesh.is_uploaded = false;
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (npc != 0) {
      ...
      HealthComponent *health = ecs_get_component(
          &g_game.ecs_world, ecs_get_entity_by_id(&g_game.ecs_world, npc),
          HEALTH_COMPONENT_ID);
      if (health) {
        health->health = 20.0f;
  ...
  init_progress_update_stage("Scripting System");
  if (!ScriptSystem_Init(&g_game.script_system)) {
    LOG_ERROR("Failed to initialize script system");
    result.success = false;
    result.message = "Failed to initialize script system";
    result.error = -1;
  }
  if (!result.success) {
  ...
  if (!g_game.in_game) {
    ...
    if (menu_get_state(&g_game.menu_system) == MENU_STATE_IN_GAME) {
      ...
      */
      ...
      // transition_progress not directly supported in current HUD struct
      ...
      // Update furnace
      if (g_game.player_system.player) {
        furnace_update_legacy(&g_game.furnace_state,
                              &g_game.player_system.player->inventory,
                              g_game.delta_time);
      }
      ...
      if (g_game.game_mode.mode == GAME_MODE_SURVIVAL) {
        survival_update(
            &g_game.game_mode, &g_game.ecs_world,
            ecs_get_entity_by_id(&g_game.ecs_world,
...
static void game_shutdown(void) {
  ...
  audio_system_free(&g_game.audio_system);
  ScriptSystem_Shutdown(&g_game.script_system);
  lighting_system_shutdown(&g_game.lighting_system);

  weather_particles_free(&g_game.weather_particles);
  weather_system_free(&g_game.weather_system);

  particle_renderer_free(&g_game.particle_renderer);
  physics_debug_visualization_cleanup(&g_game.debug_vis);
  plant_vfx_free(&g_game.plant_vfx);
  water_integration_free(&g_game.water_integration);
  water_physics_free(&g_game.water_physics);
  water_system_free(&g_game.water_system);
  ...
  // Cleanup NPC visuals
  if (g_game.npc_batch_mesh.vertices)
    free(g_game.npc_batch_mesh.vertices);
  if (g_game.npc_batch_mesh.indices)
    free(g_game.npc_batch_mesh.indices);
...
// =================================================================================================
// MORE GLOBALS AND STUBS
// =================================================================================================

// Global pointers referenced by other modules
ThreadPool *g_thread_pool = NULL;
struct WorldGenerator *g_world_generator = NULL;

// ecs_world_update is now provided by ecs_archetype.c
...
void furnace_update_legacy(FurnaceState *state, Inventory *inventory,
                           f32 delta_time) {
  (void)state;
  (void)inventory;
  (void)delta_time;
