...
static bool
core_indirect_commands_validate(const core_indirect_commands_internal_t *item) {
  if (!item)
    return false;
  if (!item->initialized)
    return false;
...
static void core_indirect_commands_cleanup_internal(
    core_indirect_commands_internal_t *item) {
  ...
  // Since we don't have D3D12 headers, this is a placeholder for where the
  // ComPtr Release would happen.
#endif
...
CoreIndirectCommandsResult core_indirect_commands_init(void) {
  ...
  g_indirect_commands_ctx.items =
      calloc(g_indirect_commands_ctx.capacity,
             sizeof(core_indirect_commands_internal_t));
...
core_indirect_commands_create(core_indirect_commands_handle_t *out_handle,
  ...
  if (g_indirect_commands_ctx.count >= g_indirect_commands_ctx.capacity) {
    ...
    core_indirect_commands_internal_t *new_items =
        realloc(g_indirect_commands_ctx.items,
                new_capacity * sizeof(core_indirect_commands_internal_t));
    if (!new_items) {
      mutex_unlock(g_indirect_commands_ctx.mutex);
      return CORE_INDIRECT_COMMANDS_ERROR_OUT_OF_MEMORY;
    }
    ...
    memset(new_items + g_indirect_commands_ctx.capacity, 0,
           (new_capacity - g_indirect_commands_ctx.capacity) *
               sizeof(core_indirect_commands_internal_t));
  ...
  core_indirect_commands_internal_t *item =
      &g_indirect_commands_ctx.items[index];
...
void core_indirect_commands_destroy(core_indirect_commands_handle_t handle) {
  ...
  core_indirect_commands_cleanup_internal(
      &g_indirect_commands_ctx.items[handle.id]);
...
core_indirect_commands_get_info(core_indirect_commands_handle_t handle,
  ...
  const core_indirect_commands_internal_t *item =
      &g_indirect_commands_ctx.items[handle.id];
  out_info->id = item->id;
...
size_t core_indirect_commands_get_memory_usage(void) {
  ...
  total += g_indirect_commands_ctx.capacity *
           sizeof(core_indirect_commands_internal_t);
