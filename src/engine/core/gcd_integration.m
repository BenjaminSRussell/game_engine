...
bool gcd_initialize(void) {
  ...
  g_serial_queue = dispatch_queue_create("com.minecraft.engine.serial",
  ...
  g_global_queue = dispatch_queue_create("com.minecraft.engine.concurrent",
...
void gcd_submit_task(void (*function)(void *), void *data, GCDPriority priority,
                     const char *name) {
...
void gcd_submit_task_with_counter(void (*function)(void *), void *data,
                                  atomic_int *counter, GCDPriority priority,
                                  const char *name) {
  if (!g_gcd_initialized || !function) {
    return;
  }
...
void gcd_wait_for_counter(atomic_int *counter) {
  ...
  // Use dispatch group for waiting
  ...
  // Enter group for each remaining task
  int remaining = atomic_load(counter);
  for (int i = 0; i < remaining; i++) {
    dispatch_group_enter(group);
  }

  // Create a task that leaves the group
  dispatch_async(g_serial_queue, ^{
    atomic_int *counter_ptr = counter;
    while (atomic_load(counter_ptr) > 0) {
      dispatch_group_leave(group);
      atomic_fetch_sub(counter_ptr, 1);
    }
  });
...
void gcd_parallel_for(uint32_t start, uint32_t end, uint32_t chunk_size,
  ...
  GCDParallelForData parallel_data = {.start = start,
                                      .end = end,
  ...
  dispatch_apply_f(chunk_count, g_concurrent_queue, &parallel_data,
                   gcd_parallel_for_wrapper);
}

// Parallel for with counter
void gcd_parallel_for_with_counter(uint32_t start, uint32_t end,
                                   uint32_t chunk_size,
                                   void (*function)(uint32_t, uint32_t, void *),
                                   void *data, atomic_int *counter) {
  if (!g_gcd_initialized || !function || start >= end || chunk_size == 0) {
    return;
  }
...
void gcd_submit_serial_task(void (*function)(void *), void *data,
                            const char *name) {
  if (!g_gcd_initialized || !function) {
    return;
  }
...
void gcd_submit_sync_task(void (*function)(void *), void *data,
                          const char *name) {
  if (!g_gcd_initialized || !function) {
    return;
  }
  ...
  GCDTask task = {
      .function = function, .data = data, .counter = NULL, .name = name};

  dispatch_sync_f(g_global_queue, &task, gcd_task_wrapper);
}

// Get GCD system information
void gcd_get_system_info(uint32_t *active_processors,
                         uint32_t *logical_processors) {
  if (active_processors) {
    *active_processors =
        (uint32_t)[NSProcessInfo processInfo].activeProcessorCount;
  }
...
bool gcd_is_available(void) { return g_gcd_initialized; }
