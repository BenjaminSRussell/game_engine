...
bool lock_free_mpmc_enqueue(LockFreeQueue *queue, void *data,
  ...
  while (retries < MAX_RETRIES) {
    ...
    if (tail_node && next_tail.ptr == NULL) {
      ...
      if (compare_exchange_tagged_pointer(&tail_node->next, &next_tail,
                                          new_next)) {
        break;
      }
      ...
      if (current_head.ptr == NULL) {
        TaggedPointer new_head =
            make_tagged_pointer(node, current_head.tag + 1);
        TaggedPointer new_tail = make_tagged_pointer(node, tail.tag + 1);

        if (compare_exchange_tagged_pointer(&queue->head_ptr, &current_head,
                                            new_head)) {
          compare_exchange_tagged_pointer(&queue->tail_ptr, &tail, new_tail);
          break;
        }
