...
void memory_allocator_shutdown(void) { unified_memory_shutdown(); }

// Legacy allocation - redirects to unified system
void *memory_allocate(size_t size, const char *file, int line) {
  return unified_memory_alloc(size, MEMORY_STRATEGY_DEFAULT, MEMORY_FLAG_TRACK,
                              file, line, __func__);
}

// Legacy reallocation - redirects to unified system
void *memory_reallocate(void *ptr, size_t new_size, const char *file,
                        int line) {
  return unified_memory_realloc(ptr, new_size, MEMORY_FLAG_TRACK, file, line,
                                __func__);
}
...
void memory_allocator_get_stats(size_t *total_allocated, size_t *peak_allocated,
...
void memory_allocator_check_leaks(void) { unified_memory_check_leaks(); }
