...
static AllocationRecord* find_record(void* ptr) {
    if (!g_tracker.records) return NULL;
...
static bool add_record(AllocationRecord* record) {
    if (g_tracker.count >= g_tracker.capacity) return false;
...
static void set_canaries(void* ptr, AllocationRecord* record) {
    if (!g_tracker.canary_enabled) return;
...
static bool check_canaries(AllocationRecord* record) {
    if (!g_tracker.canary_enabled) return true;
    ...
    if (before_canary != CANARY_VALUE) return false;
    
    // Check after canary
    u64 after_canary;
    memcpy(&after_canary, bytes + record->size, CANARY_SIZE);
    if (after_canary != CANARY_VALUE) return false;
...
static size_t capture_stack_trace(void** frames, size_t max_frames) {
    if (!g_tracker.stack_capture_enabled) return 0;
...
bool tracker_init(size_t capacity, bool enable_canaries, bool enable_stack_capture) {
    ...
    if (!g_tracker.records) return false;
...
void* tracked_malloc(size_t size, const char* system_tag) {
    ...
    // Capture stack trace
    record.frame_count = capture_stack_trace(record.stack_frames, MAX_STACK_FRAMES);
...
void tracker_dump_leaks(void) {
    if (!g_tracker.tracking_enabled) return;
    ...
    for (size_t i = 0; i < g_tracker.capacity; i++) {
        ...
        if (record->ptr && !record->freed) {
            printf("LEAK %llu: %zu bytes from %s (ID: %llu)\n",
                   leak_count + 1, record->size, record->system_tag, record->allocation_id);
...
bool tracker_validate_heap(void) {
    if (!g_tracker.tracking_enabled) return true;
...
void tracker_get_stats(u64* total_allocated, u64* peak_allocated, 
    ...
    if (total_allocated) *total_allocated = g_tracker.total_allocated;
    if (peak_allocated) *peak_allocated = g_tracker.peak_allocated;
    if (total_allocations) *total_allocations = g_tracker.total_allocations;
    if (total_frees) *total_frees = g_tracker.total_frees;
    if (active_count) *active_count = g_tracker.count;
}

// ✅ COMPLETED: Control functions
void tracker_enable(bool enabled) {
    g_tracker.tracking_enabled = enabled;
}

bool tracker_is_enabled(void) {
    return g_tracker.tracking_enabled;
}
