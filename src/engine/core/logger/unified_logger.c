...
bool unified_logger_init(const LoggerConfig* config) {
    ...
    if (config->use_buffering) {
        ...
        g_unified_logger.buffer = malloc(g_unified_logger.buffer_size * sizeof(LogEntry));
...
void unified_logger_set_level(LogLevel level) {
    if (!g_unified_logger.initialized) return;
...
void unified_logger_enable_category(LogCategory category, bool enabled) {
    if (!g_unified_logger.initialized || category >= LOG_CAT_COUNT) return;
...
void unified_logger_log(LogLevel level, LogCategory category, const char* file, int line, 
    ...
    if (level < g_unified_logger.config.min_level) return;
    
    // Check if category is enabled
    if (!g_unified_logger.config.enabled_categories[category]) return;
        ...
        if (g_unified_logger.config.use_buffering) {
            ...
            if (g_unified_logger.log_file) {
                ...
                fprintf(g_unified_logger.log_file, "[%s] [%s] %s:%d %s(): %s\n",
                        ...
                        entry.message);
                fflush(g_unified_logger.log_file);
                g_unified_logger.stats.bytes_written += strlen(entry.message) + 100; // Approximate
            }
        }
    }
...
void unified_logger_log_structured(LogLevel level, LogCategory category, const char* file, 
    ...
    if (context_json && strlen(context_json) > 0) {
        snprintf(entry.message, sizeof(entry.message), "%s | Context: %s", message, context_json);
...
void unified_logger_rotate(void) {
    if (!g_unified_logger.initialized || !g_unified_logger.log_file) return;
    ...
    snprintf(backup_path, sizeof(backup_path), "%s%s", g_unified_logger.config.log_file_path, time_suffix);
...
void unified_logger_send_to_remote(const LogEntry* entry) {
    if (!entry || !g_unified_logger.config.enable_remote) return;
