...
int main() {
    ...
    LOG_TRACE(LOG_CAT_GENERAL, "This is a trace message from consolidated logger");
    LOG_DEBUG(LOG_CAT_PHYSICS, "Physics system initialized - consolidated backend");
    LOG_INFO(LOG_CAT_RENDERER, "Renderer started - unified logging active");
    LOG_WARN(LOG_CAT_AUDIO, "Audio device not found - using fallback");
    LOG_ERROR(LOG_CAT_NETWORK, "Failed to connect to server");
    ...
    LOG_DEBUG(LOG_CAT_AI, "AI state machine updated");
    LOG_DEBUG(LOG_CAT_MEMORY, "Memory allocation: 1024 bytes");
    LOG_DEBUG(LOG_CAT_IO, "File loaded: assets/texture.png");
    LOG_DEBUG(LOG_CAT_SCRIPT, "Script executed: player_controller.lua");
