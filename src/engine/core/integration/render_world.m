...
// Includes are now largely in .h, but we might need more for implementation
// details #include
// "rendering/3d_rendering/rendering/render_graph/render_graph.h"
...
void scene_render_frame(render_world_t *world, const scene_t *scene,
                        const struct Camera *camera);

render_world_t *render_world_create(id<MTLDevice> device, uint32_t width,
                                    uint32_t height) {
  ...
  world->lights->deferred = rendering_deferred_lighting_create(
      device, MTLPixelFormatRGBA16Float, MTLPixelFormatDepth32Float);

  // 3. Atmosphere
  world->atmosphere =
      (atmosphere_system_t *)calloc(1, sizeof(atmosphere_system_t));
  atmosphere_init(
      world->atmosphere,
      (__bridge void *)device); // Cast to match void* signature logic if needed
  ...
  // 9. Debug
  world->debug =
      debug_renderer_create(device, 100000); // Allocate space for 100k lines
...
void render_world_destroy(render_world_t *world) {
  ...
  // Profiling cleanup
  if (world->gpu_profiler)
    gpu_profiler_destroy(world->gpu_profiler);
  if (world->cpu_gpu_timing)
...
void render_world_resize(render_world_t *world, uint32_t width,
                         uint32_t height) {
  if (!world)
    return;
  world->width = width;
...
void render_world_render(render_world_t *world, const scene_t *scene,
                         const struct Camera *camera) {
