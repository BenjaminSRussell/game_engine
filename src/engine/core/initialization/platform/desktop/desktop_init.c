...
static void glfw_error_callback(int error, const char *description) {
  LOG_ERROR_CAT(LOG_CAT_PLATFORM, "GLFW Error (%d): %s", error, description);
}

bool platform_init(const EngineConfig *config) {
  LOG_INFO_CAT(LOG_CAT_PLATFORM, "Initializing Desktop Platform...");

  glfwSetErrorCallback(glfw_error_callback);

  if (!glfwInit()) {
    LOG_FATAL_CAT(LOG_CAT_PLATFORM, "Failed to initialize GLFW");
    return false;
  }
...
void platform_shutdown(void) {
  ...
  glfwTerminate();
  LOG_INFO_CAT(LOG_CAT_PLATFORM, "Desktop Platform Shutdown");
}
