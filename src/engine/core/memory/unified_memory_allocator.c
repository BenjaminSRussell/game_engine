...
bool unified_memory_init(const MemoryPolicy *policy) {
  ...
  // Initialize mutexes
  if (pthread_mutex_init(&g_allocator.global_mutex, NULL) != 0)
    return false;
  if (pthread_mutex_init(&g_allocator.allocations_mutex, NULL) != 0)
    return false;
  if (pthread_mutex_init(&g_allocator.pools_mutex, NULL) != 0)
    return false;
  if (pthread_mutex_init(&g_allocator.stacks_mutex, NULL) != 0)
    return false;
  if (pthread_mutex_init(&g_allocator.arenas_mutex, NULL) != 0)
    return false;
  if (pthread_mutex_init(&g_allocator.stats_mutex, NULL) != 0)
    return false;
  ...
  if (policy) {
    ...
    // Default policy
    g_allocator.policy =
        (MemoryPolicy){.global_limit = SIZE_MAX,
...
void *unified_memory_alloc(size_t size, MemoryStrategy strategy,
                           MemoryFlags flags, const char *file, int line,
                           const char *function) {
  if (!g_allocator.initialized)
  ...
  if (g_allocator.policy.global_limit != SIZE_MAX &&
      g_allocator.stats.total_allocated + size >
          g_allocator.policy.global_limit) {
  ...
  if (g_allocator.policy.max_allocations > 0 &&
      g_allocator.stats.current_allocations >=
          g_allocator.policy.max_allocations) {
    g_allocator.stats.allocation_failures++;
    pthread_mutex_unlock(&g_allocator.global_mutex);
    return NULL;
  }
...
void unified_memory_free(void *ptr, const char *file, int line,
  ...
  if (g_allocator.policy.enable_canaries) {
    ...
    u64 *back_canary =
        (u64 *)((u8 *)metadata + metadata->actual_size - sizeof(u64));

    if (!check_canary(front_canary, sizeof(u64)) ||
        !check_canary(back_canary, sizeof(u64))) {
  ...
  // Free memory
  if (metadata->flags & MEMORY_FLAG_GUARD &&
      g_allocator.policy.enable_guard_pages) {
...
MemoryPool *unified_memory_pool_create(const MemoryPoolConfig *config) {
  if (!config || !g_allocator.initialized)
    return NULL;
...
void *unified_memory_pool_alloc(MemoryPool *pool, size_t size,
                                MemoryFlags flags) {
...
void *unified_memory_stack_alloc(StackAllocator *stack, size_t size,
                                 MemoryFlags flags) {
...
void *unified_memory_arena_alloc(ArenaAllocator *arena, size_t size,
                                 MemoryFlags flags) {
  ...
  if (arena->current_offset + size <= current_size) {
    void *ptr =
        (u8 *)arena->blocks[arena->current_block] + arena->current_offset;
  ...
  // Expand arrays
  // Safe realloc: update struct only on success, but handle partial success to
  // avoid dangling pointers
  void **new_blocks =
      realloc(arena->blocks, (arena->block_count + 1) * sizeof(void *));
  if (new_blocks) {
  ...
  size_t *new_sizes =
      realloc(arena->block_sizes, (arena->block_count + 1) * sizeof(size_t));
  if (new_sizes) {
    arena->block_sizes = new_sizes;
  } else {
    // Blocks expanded but sizes failed. We are consistent (count unchanged) but
    // can't proceed.
    free(new_block);
    pthread_mutex_unlock(&arena->mutex);
    return NULL;
  ...
  if (g_allocator.allocations) {
    ...
    while (alloc) {
      printf("Leak #%u: %zu bytes at %p (%s:%d in %s)\n", leak_count++,
             alloc->size, alloc->ptr, alloc->file ? alloc->file : "unknown",
             alloc->line, alloc->function ? alloc->function : "unknown");
      alloc = alloc->next;
    }
...
static void free_with_guard_pages(void *ptr, size_t size) {
  ...
  mprotect((u8 *)actual_ptr + GUARD_PAGE_SIZE + size, GUARD_PAGE_SIZE,
