...
LinearAllocator *linear_allocator_create(u32 size, u32 buffer_count) {
  ...
  LOG_DEBUG("Linear allocator created: %u bytes, %u buffers", size,
            buffer_count);
  return alloc;
}
...
void *linear_allocator_alloc(LinearAllocator *alloc, u32 size, u32 alignment) {
  ...
  // Align the offset
  u32 aligned_offset = (alloc->offset + alignment - 1) & ~(alignment - 1);
  u32 total_size = aligned_offset + size - alloc->offset;

  if (alloc->offset + total_size > alloc->size) {
    LOG_WARN("Linear allocator out of memory: requested %u bytes, available %u",
  ...
  return (char *)alloc->base + aligned_offset;
}
...
BuddyAllocator *buddy_allocator_create(u32 min_block_size, u32 max_block_size) {
  ...
  if ((min_block_size & (min_block_size - 1)) != 0 ||
    ...
    LOG_ERROR("Invalid block sizes for buddy allocator");
    return NULL;
  }
  ...
  LOG_DEBUG("Buddy allocator created: min=%u, max=%u, levels=%u",
            min_block_size, max_block_size, alloc->max_level);
  return alloc;
}
...
void *buddy_allocator_alloc(BuddyAllocator *alloc, u32 size) {
  ...
  if (level > alloc->max_level) {
    LOG_WARN("Allocation too large: %u bytes", size);
    return NULL;
  }
  ...
  if (!block) {
    LOG_WARN("No free blocks available for %u bytes", size);
    return NULL;
  }
...
void buddy_allocator_free(BuddyAllocator *alloc, void *ptr) {
  ...
  if (offset >= alloc->pool_size) {
    LOG_ERROR("Invalid pointer passed to buddy_allocator_free");
    return;
  }
  ...
  if (block && !block->is_free) {
    ...
    LOG_ERROR("Invalid or double free in buddy allocator");
  }
}
...
void buddy_allocator_print_stats(BuddyAllocator *alloc) {
  ...
  LOG_INFO("=== Buddy Allocator Stats ===");
  LOG_INFO("Pool size: %u bytes", alloc->pool_size);
  LOG_INFO("Min block: %u bytes", alloc->min_block_size);
  LOG_INFO("Max block: %u bytes", alloc->max_block_size);
  LOG_INFO("Max levels: %u", alloc->max_level);
  LOG_INFO("Total blocks: %u", alloc->total_blocks);
  LOG_INFO("Free blocks: %u", alloc->free_blocks);
  ...
  LOG_INFO("Free memory: %u bytes", free_memory);
  LOG_INFO("Fragmentation: %.2f%%",
           (1.0 - (f64)free_memory / alloc->pool_size) * 100.0);
  LOG_INFO("============================");
}
...
void *stack_allocator_alloc(StackAllocator *alloc, u32 size) {
  if (!alloc || alloc->offset + size > alloc->size) {
    LOG_WARN("Stack allocator out of memory");
    return NULL;
  }

  void *ptr = (char *)alloc->base + alloc->offset;
  alloc->offset += size;
...
void stack_allocator_end_frame(StackAllocator *alloc) {
  if (alloc) {
    alloc->frame_offset = alloc->offset;
  }
}
