...
Vec3 quat_rotate_vec3(Quat q, Vec3 v) {
  ...
  Vec3 term2 = {s2_minus_dot_uu * v.x, s2_minus_dot_uu * v.y,
                s2_minus_dot_uu * v.z};

  // 2.0f * s * cross(u, v)
  Vec3 cross_uv = {u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z,
                   u.x * v.y - u.y * v.x};
  Vec3 term3 = {2.0f * s * cross_uv.x, 2.0f * s * cross_uv.y,
                2.0f * s * cross_uv.z};

  Vec3 result = {term1.x + term2.x + term3.x, term1.y + term2.y + term3.y,
...
Quat quat_slerp(Quat a, Quat b, f32 t) {
  ...
  if (cosTheta > 0.9995f) {
    ...
    // Normalize
    f32 len = sqrtf(result.w * result.w + result.x * result.x +
  ...
  Quat result = {a.w * t1 + b.w * t2, a.x * t1 + b.x * t2, a.y * t1 + b.y * t2,
                 a.z * t1 + b.z * t2};
  return result;
}
