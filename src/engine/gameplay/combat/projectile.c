...
// Helper functions
static Vec3 calculate_homing_direction(const Vec3 *current_pos, const Vec3 *target_pos, const Vec3 *current_vel, f32 homing_strength) {
  ...
  Vec3 homing_direction = vec3_lerp(current_direction, desired_direction, homing_strength);
...
static void projectile_handle_collision(ProjectileInstance *instance, const Vec3 *hit_point, 
                                      const Vec3 *hit_normal, Entity hit_entity) {
  if (!instance || !hit_point || !hit_normal) return;
  ...
  LOG_DEBUG(LOG_CAT_GAME, "Projectile collision: entity %u hit entity %u at (%.2f, %.2f, %.2f)",
           (u32)instance->entity.id, (u32)hit_entity.id, hit_point->x, hit_point->y, hit_point->z);
  ...
  switch (proj->behavior) {
    ...
    case PROJECTILE_BEHAVIOR_BOUNCE:
      ...
      proj->velocity = vec3_scale(reflected, vec3_create(0.8f, 0.8f, 0.8f)); // Energy loss on bounce
      break;
      
    case PROJECTILE_BEHAVIOR_EXPLODE:
      // Create explosion
      projectile_apply_explosion(hit_point, proj->explosion_radius, proj->explosion_damage, proj->source);
      instance->is_active = false;
      break;
  ...
  if (hit_entity.id != 0) {
    ...
    LOG_DEBUG(LOG_CAT_GAME, "Applied %.1f damage (type: %d) to entity %u",
             proj->damage, proj->damage_type, (u32)hit_entity.id);

    // Apply type-specific effects based on projectile type
    /* Disabled status effects until system linked
    if (proj->damage_type == DAMAGE_TYPE_FIRE) {
      ...
      LOG_DEBUG(LOG_CAT_GAME, "Applied BURNING effect to entity %u from fire projectile", (u32)hit_entity.id);
      ...
      LOG_DEBUG(LOG_CAT_GAME, "Applied FREEZING effect to entity %u from ice projectile", (u32)hit_entity.id);
      ...
      LOG_DEBUG(LOG_CAT_GAME, "Applied POISON effect to entity %u from poison projectile", (u32)hit_entity.id);
    }
    */
  }
}

static void projectile_update_physics(ProjectileInstance *instance, f32 delta_time) {
  if (!instance) return;
  ...
  instance->position_index = (instance->position_index + 1) % PROJECTILE_RAYCAST_STEPS;
  
  // Apply gravity
  if (proj->gravity_scale > 0.0f) {
    Vec3 gravity_force = vec3_scale(g_projectile_system.gravity, vec3_create(proj->gravity_scale, proj->gravity_scale, proj->gravity_scale));
    proj->velocity = vec3_add(proj->velocity, vec3_scale(gravity_force, vec3_create(delta_time, delta_time, delta_time)));
  }
  ...
  if (proj->drag > 0.0f) {
    ...
    proj->velocity = vec3_scale(proj->velocity, vec3_create(drag_factor, drag_factor, drag_factor));
  }
  
  // Clamp to max speed
  f32 current_speed = vec3_length(proj->velocity);
  if (current_speed > proj->max_speed) {
    proj->velocity = vec3_scale(vec3_normalize(proj->velocity), vec3_create(proj->max_speed, proj->max_speed, proj->max_speed));
  }
  
  // Update position
  proj->last_position = proj->last_position; // Would get from transform component
  Vec3 new_position = vec3_add(proj->last_position, vec3_scale(proj->velocity, vec3_create(delta_time, delta_time, delta_time)));
  ...
  if (proj->is_homing && proj->homing_target.id != 0 && proj->age >= proj->homing_delay) {
    ...
    proj->velocity = vec3_scale(homing_dir, vec3_create(current_speed, current_speed, current_speed));
  }
  ...
  if (proj->check_terrain || proj->collision_radius > 0.0f) {
    ...
    if (projectile_raycast(&proj->last_position, &new_position, proj->source, 
      ...
      projectile_handle_collision(instance, &hit_point, &hit_normal, hit_entity);
    }
  }
  ...
  if (proj->age >= proj->lifetime) {
    instance->is_active = false;
    LOG_DEBUG(LOG_CAT_GAME, "Projectile %u expired after %.2f seconds", (u32)instance->entity.id, proj->age);
  }
}
...
static void projectile_remove_instance(u32 index) {
  if (index >= g_projectile_system.instance_count) return;
  
  // Move last instance to this position
  if (index < g_projectile_system.instance_count - 1) {
    g_projectile_system.instances[index] = g_projectile_system.instances[g_projectile_system.instance_count - 1];
  }
...
bool projectile_system_init(World *world) {
  if (g_projectile_system.is_initialized) {
    LOG_WARN(LOG_CAT_GAME, "Projectile system already initialized");
    return true;
  }
  ...
  LOG_INFO(LOG_CAT_GAME, "Projectile system initialized");
  return true;
}
...
void projectile_system_shutdown(void) {
  ...
  LOG_INFO(LOG_CAT_GAME, "Projectile system shutdown");
}
...
Entity projectile_spawn(World *world, Vec3 position, Vec3 direction, f32 speed, Entity source, f32 damage) {
  ...
  Vec3 velocity = vec3_scale(direction, vec3_create(speed, speed, speed));
  ...
  ProjectileInstance *instance = &g_projectile_system.instances[g_projectile_system.instance_count++];
  ...
  LOG_DEBUG(LOG_CAT_GAME, "Spawned projectile entity %u (speed: %.1f, damage: %.1f)", (u32)entity.id, speed, damage);
...
ProjectileComponent projectile_create_fireball(Vec3 velocity, f32 damage, f32 explosion_radius) {
  ProjectileComponent proj = projectile_create(velocity, 2.0f); // 2 second lifetime
  proj.damage = damage;
...
ProjectileComponent projectile_create_homing(Vec3 velocity, Entity target, f32 damage) {
  ProjectileComponent proj = projectile_create(velocity, 5.0f); // 5 second lifetime
  proj.damage = damage;
...
void projectile_set_homing(ProjectileComponent *proj, Entity target, f32 strength) {
  if (!proj) return;
  ...
  LOG_DEBUG(LOG_CAT_GAME, "Set projectile homing to entity %u with strength %.2f", (u32)target.id, strength);
}
...
void projectile_get_statistics(u32 *out_active_projectiles, u32 *out_total_spawned, 
                              f32 *out_average_speed) {
  if (!out_active_projectiles || !out_total_spawned || !out_average_speed) return;
...
void projectile_debug_render(World *world) {
  ...
  for (u32 i = 0; i < g_projectile_system.instance_count; i++) {
    ...
    switch (instance->projectile.behavior) {
      case PROJECTILE_BEHAVIOR_DESTROY: behavior_name = "Destroy"; break;
      case PROJECTILE_BEHAVIOR_PIERCE: behavior_name = "Pierce"; break;
      case PROJECTILE_BEHAVIOR_BOUNCE: behavior_name = "Bounce"; break;
      case PROJECTILE_BEHAVIOR_EXPLODE: behavior_name = "Explode"; break;
