...
static const char* crash_type_to_string(CrashType type) {
    switch (type) {
        case CRASH_TYPE_ACCESS_VIOLATION: return "Access Violation";
        case CRASH_TYPE_STACK_OVERFLOW: return "Stack Overflow";
        case CRASH_TYPE_ASSERTION_FAILED: return "Assertion Failed";
        case CRASH_TYPE_NULL_POINTER: return "Null Pointer";
        case CRASH_TYPE_DIVIDE_BY_ZERO: return "Divide by Zero";
        case CRASH_TYPE_MEMORY_CORRUPTION: return "Memory Corruption";
        case CRASH_TYPE_DEADLOCK: return "Deadlock";
        case CRASH_TYPE_UNKNOWN: return "Unknown";
...
static const char* crash_severity_to_string(CrashSeverity severity) {
    switch (severity) {
        case CRASH_SEVERITY_INFO: return "Info";
        case CRASH_SEVERITY_WARNING: return "Warning";
        case CRASH_SEVERITY_ERROR: return "Error";
        case CRASH_SEVERITY_CRITICAL: return "Critical";
        case CRASH_SEVERITY_FATAL: return "Fatal";
        default: return "Unknown";
    }
}
...
void crash_signal_handler(int signal, siginfo_t* info, void* context) {
    ...
    CrashReport* report = crash_report_create(g_crash_system, type, severity, 
                                            strsignal(signal), "Unhandled signal");
...
LONG WINAPI crash_exception_handler(EXCEPTION_POINTERS* exception_info) {
    ...
    CrashReport* report = crash_report_create(g_crash_system, type, severity, message, "Unhandled exception");
    
    if (report) {
        // Set exception information
        report->crash_context.exception_address = (u64)exception_info->ExceptionRecord->ExceptionAddress;
        report->crash_context.exception_code = exception_info->ExceptionRecord->ExceptionCode;
...
void crash_signal_handler(int signal) {
    ...
    if (g_crash_system && g_crash_system->on_crash_detected) {
        CrashReport* report = crash_report_create(g_crash_system, CRASH_TYPE_UNKNOWN, 
...
CrashReport* crash_report_create(CrashReportSystem* system, CrashType type, CrashSeverity severity, 
    ...
    if (message) {
        strncpy(report->crash_context.exception_message, message, sizeof(report->crash_context.exception_message) - 1);
        report->crash_context.exception_message[sizeof(report->crash_context.exception_message) - 1] = '\0';
    }
    
    if (description) {
        strncpy(report->user_description, description, sizeof(report->user_description) - 1);
    ...
    // Generate file paths
    snprintf(report->log_file_path, sizeof(report->log_file_path), "crash_logs/%s.log", report->report_id);
    snprintf(report->screenshot_path, sizeof(report->screenshot_path), "crash_screenshots/%s.png", report->report_id);
    snprintf(report->dump_file_path, sizeof(report->dump_file_path), "crash_dumps/%s.dmp", report->report_id);
    snprintf(report->config_file_path, sizeof(report->config_file_path), "crash_configs/%s.json", report->report_id);
...
bool crash_report_add_stack_trace(CrashReport* report, void** frames, u32 frame_count) {
    ...
    for (u32 i = 0; i < frame_count && i < 64; i++) {
        ...
        if (symbols && symbols[0]) {
            ...
            report->crash_context.stack_symbols[i][sizeof(report->crash_context.stack_symbols[i]) - 1] = '\0';
            free(symbols);
        }
    }
...
void crash_report_collect_system_info(SystemInfo* info) {
    ...
    if (uname(&uname_info) == 0) {
        ...
        strncpy(info->cpu_architecture, uname_info.machine, sizeof(info->cpu_architecture) - 1);
    }
    ...
    info->total_memory_mb = sysconf(_SC_PHYS_PAGES) * sysconf(_SC_PAGESIZE) / (1024 * 1024);
...
bool crash_report_upload(CrashReportSystem* system, const char* report_id) {
    ...
    snprintf(request_data, sizeof(request_data),
             ...
             report->report_id, report->crash_context.type, report->crash_context.severity,
             report->crash_context.exception_message, report->user_description,
             report->crash_time_ms, report->system_info.os_name,
             report->system_info.os_version, report->system_info.cpu_cores,
    ...
    if (!make_http_request(url, "POST", request_data, response, sizeof(response))) {
        ...
        strncpy(report->upload_error, "HTTP request failed", sizeof(report->upload_error) - 1);
