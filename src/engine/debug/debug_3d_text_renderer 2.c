...
static void calculate_text_bounds(Text3D* text, Vec3* min_bounds, Vec3* max_bounds) {
    if (!text || !text->font || !min_bounds || !max_bounds) {
        return;
    }
    
    float text_width = strlen(text->text) * text->font->character_width * text->scale.x;
    ...
    switch (text->anchor) {
        case TEXT_ANCHOR_TOP_LEFT:
            ...
            *max_bounds = (Vec3){text->position.x + text_width, text->position.y - text_height, text->position.z};
            break;
        case TEXT_ANCHOR_TOP_CENTER:
            *min_bounds = (Vec3){text->position.x - text_width * 0.5f, text->position.y, text->position.z};
            *max_bounds = (Vec3){text->position.x + text_width * 0.5f, text->position.y - text_height, text->position.z};
            break;
        case TEXT_ANCHOR_TOP_RIGHT:
            *min_bounds = (Vec3){text->position.x - text_width, text->position.y, text->position.z};
            *max_bounds = (Vec3){text->position.x, text->position.y - text_height, text->position.z};
            break;
        case TEXT_ANCHOR_MIDDLE_LEFT:
            *min_bounds = (Vec3){text->position.x, text->position.y - text_height * 0.5f, text->position.z};
            *max_bounds = (Vec3){text->position.x + text_width, text->position.y + text_height * 0.5f, text->position.z};
            break;
        case TEXT_ANCHOR_MIDDLE_CENTER:
            *min_bounds = (Vec3){text->position.x - text_width * 0.5f, text->position.y - text_height * 0.5f, text->position.z};
            *max_bounds = (Vec3){text->position.x + text_width * 0.5f, text->position.y + text_height * 0.5f, text->position.z};
            break;
        case TEXT_ANCHOR_MIDDLE_RIGHT:
            *min_bounds = (Vec3){text->position.x - text_width, text->position.y - text_height * 0.5f, text->position.z};
            *max_bounds = (Vec3){text->position.x, text->position.y + text_height * 0.5f, text->position.z};
            break;
        case TEXT_ANCHOR_BOTTOM_LEFT:
            *min_bounds = (Vec3){text->position.x, text->position.y - text_height, text->position.z};
            *max_bounds = (Vec3){text->position.x + text_width, text->position.y, text->position.z};
            break;
        case TEXT_ANCHOR_BOTTOM_CENTER:
            *min_bounds = (Vec3){text->position.x - text_width * 0.5f, text->position.y - text_height, text->position.z};
            *max_bounds = (Vec3){text->position.x + text_width * 0.5f, text->position.y, text->position.z};
            break;
        case TEXT_ANCHOR_BOTTOM_RIGHT:
            *min_bounds = (Vec3){text->position.x - text_width, text->position.y - text_height, text->position.z};
...
static u32 create_shader_program(void) {
    ...
    const char* fragment_shader_source = 
        ...
        "        finalColor.rgb += uGlowColor.rgb * uGlowIntensity * texColor.a;\n"
        "    }\n"
        "    \n"
...
bool debug_text_renderer_init(Debug3DTextRenderer* renderer, u32 max_fonts, u32 max_batches) {
    ...
    LOG_INFO("3D text renderer initialized (fonts: %u, batches: %u)", max_fonts, max_batches);
    return true;
}
...
FontMetrics* debug_text_load_system_font(Debug3DTextRenderer* renderer, const char* font_name, u32 font_size) {
    ...
    u8* atlas_data = NULL;
    if (!platform_load_system_font(font_name, font_size, &atlas_data, &font->atlas_width, &font->atlas_height)) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, font->atlas_width, font->atlas_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, atlas_data);
...
Text3D* debug_text_create(Debug3DTextRenderer* renderer, const char* text, const Vec3* position, FontMetrics* font) {
...
void debug_text_render(Debug3DTextRenderer* renderer, const Mat4* view_matrix, const Mat4* projection_matrix) {
    ...
    // Set global uniforms
    GLint view_loc = glGetUniformLocation(renderer->shader_program, "uViewMatrix");
    GLint proj_loc = glGetUniformLocation(renderer->shader_program, "uProjectionMatrix");
    GLint color_loc = glGetUniformLocation(renderer->shader_program, "uGlobalColor");
    GLint alpha_loc = glGetUniformLocation(renderer->shader_program, "uGlobalAlpha");
    ...
    glUniform4f(color_loc, renderer->global_color.x, renderer->global_color.y, renderer->global_color.z, renderer->global_color.w);
    glUniform1f(alpha_loc, renderer->global_alpha);
    
    // Bind texture atlas
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, renderer->texture_atlas);
    GLint texture_loc = glGetUniformLocation(renderer->shader_program, "uTextureAtlas");
...
void debug_text_render_batch(Debug3DTextRenderer* renderer, TextBatch* batch, const Mat4* view_matrix, const Mat4* projection_matrix) {
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 9, (void*)0); // Position
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 9, (void*)(sizeof(float) * 3)); // TexCoord
    glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(float) * 9, (void*)(sizeof(float) * 5)); // Color
    
    glEnableVertexAttribArray(0);
    glEnableVertexAttribArray(1);
    glEnableVertexAttribArray(2);
    
    // Draw all visible texts in the batch
    u32 visible_count = 0;
    for (u32 i = 0; i < batch->text_count; i++) {
        ...
        if (text->visible) {
            ...
            GLint model_loc = glGetUniformLocation(renderer->shader_program, "uModelMatrix");
    ...
    renderer->total_vertices += visible_count * 4; // Assuming 4 vertices per character
    renderer->total_indices += visible_count * 6;  // Assuming 6 indices per character (2 triangles)
}
...
void debug_text_print_statistics(Debug3DTextRenderer* renderer) {
    ...
    debug_text_get_statistics(renderer, &total_texts, &visible_texts, &total_vertices, &total_batches);
