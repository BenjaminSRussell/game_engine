...
bool stack_capture_init(StackCaptureSystem* system, u32 max_allocations, u32 max_stack_frames) {
    ...
    system->leak_detector.allocations = (MemoryAllocation*)calloc(max_allocations, sizeof(MemoryAllocation));
    ...
    system->stack_cache = (StackTrace*)calloc(max_allocations, sizeof(StackTrace));
    ...
    LOG_INFO("Stack capture system initialized (allocations: %u, stack frames: %u)", 
    ...
    for (int i = 0; i < frame_count; i++) {
        ...
        if (dladdr(buffer[i], &dl_info)) {
            if (dl_info.dli_sname) {
                strncpy(frame->function_name, dl_info.dli_sname, sizeof(frame->function_name) - 1);
                ...
                if (g_stack_system && g_stack_system->enable_demangling) {
                    char demangled[256];
                    if (stack_capture_demangle_symbol(dl_info.dli_sname, demangled, sizeof(demangled))) {
            ...
            if (dl_info.dli_fname) {
                strncpy(frame->module_name, dl_info.dli_fname, sizeof(frame->module_name) - 1);
...
bool memory_leak_detector_record_allocation(MemoryLeakDetector* detector, void* address, u64 size, 
    ...
    MemoryAllocation* allocation = &detector->allocations[detector->allocation_count++];
    ...
    if (tag) {
        strncpy(allocation->allocation_tag, tag, sizeof(allocation->allocation_tag) - 1);
        allocation->allocation_tag[sizeof(allocation->allocation_tag) - 1] = '\0';
    }
    
    // Capture stack trace if enabled
    if (detector->enable_stack_capture && g_stack_system) {
        stack_capture_current_thread(&allocation->allocation_stack, detector->max_stack_frames);
    }
...
bool memory_leak_detector_record_free(MemoryLeakDetector* detector, void* address) {
    ...
    // Find the allocation
    MemoryAllocation* allocation = memory_leak_detector_find_allocation(detector, address);
    if (!allocation) {
    ...
    if (detector->enable_stack_capture && g_stack_system) {
        stack_capture_current_thread(&allocation->free_stack, detector->max_stack_frames);
    }
...
MemoryAllocation* memory_leak_detector_find_allocation(MemoryLeakDetector* detector, void* address) {
    ...
    for (u32 i = 0; i < detector->allocation_count; i++) {
        if (detector->allocations[i].address == address && !detector->allocations[i].is_freed) {
...
void memory_leak_detector_report_leaks(MemoryLeakDetector* detector) {
    ...
    for (u32 i = 0; i < detector->allocation_count; i++) {
        ...
        if (!allocation->is_freed) {
            ...
            printf("Leak %u: %p (%llu bytes) - %s\n", 
                   leak_count, allocation->address, allocation->size, allocation->allocation_tag);
            ...
            if (allocation->allocation_stack.frame_count > 0) {
                ...
                for (u32 j = 0; j < allocation->allocation_stack.frame_count && j < 5; j++) {
                    const StackFrame* frame = &allocation->allocation_stack.frames[j];
                    printf("    %u: 0x%p %s\n", j, frame->instruction_pointer, frame->function_name);
                }
            }
            printf("\n");
        }
    ...
    if (leak_count > 0) {
        LOG_ERROR("Detected %u memory leaks (%llu bytes)", leak_count, leaked_bytes);
    } else {
...
bool stack_capture_export_leaks_to_json(StackCaptureSystem* system, const char* filename) {
    ...
    // (This is a simplified approach - in a real implementation, you'd rewrite the file)
