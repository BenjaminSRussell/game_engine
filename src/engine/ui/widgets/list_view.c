...
static void list_view_layout(Widget *widget, float available_width,
                             float available_height);
static void content_container_layout(Widget *widget, float available_width,
                                     float available_height);
...
Widget *list_view_create(const char *name) {
  ...
  if (!data) {
    LOG_ERROR("Failed to allocate ListView data");
  ...
  if (!data->content_container) {
    widget_destroy(
        data->scroll_view); // This might double free if scroll_view_destroy
                            // frees it? No, it's ok.
    memory_free(data);
...
static bool on_item_click(Widget *item, UIEvent *event, void *user_data) {
  ...
  if (data->selection_mode == SELECTION_MODE_NONE)
    return false;
...
void list_view_add_item(Widget *list_view, Widget *item) {
  ...
  widget_add_event_handler(item, UI_EVENT_MOUSE_CLICK, on_item_click,
                           list_view);
...
void list_view_remove_item(Widget *list_view, int index) {
  ...
  if (index >= 0 && index < (int)data->content_container->child_count) {
    ...
    // The API says "remove_item", generally in UI libs this might just remove
    // from view. But for memory management, if we created it, who destroys it?
    // Let's assume caller handles destruction if they want, or we destroy it?
    // "list_view_clear" usually destroys.
    // Let's destroy it to avoid leaks for now as items are usually created for
    // the list.
    widget_destroy(item);
...
void list_view_clear(Widget *list_view) {
  ...
  while (data->content_container->child_count > 0) {
    Widget *item = data->content_container
                       ->children[data->content_container->child_count - 1];
    widget_remove_child(data->content_container, item);
    widget_destroy(item);
  }
...
void list_view_set_selected_index(Widget *list_view, int index) {
  ...
  if (data->selected_index != -1 &&
      data->selected_index < (int)data->content_container->child_count) {
...
void list_view_set_on_select(Widget *list_view, ListViewSelectCallback callback,
                             void *user_data) {
  ListViewData *data = (ListViewData *)list_view->user_data;
  if (data) {
