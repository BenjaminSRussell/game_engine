...
static void insert_char(TextFieldData *data, char c) {
  ensure_capacity(data,
                  data->length + 2); // +1 for char, +1 for null terminator
...
static void insert_text(TextFieldData *data, const char *text) {
  ...
  memmove(data->text + data->cursor_pos + len, data->text + data->cursor_pos,
          data->length - data->cursor_pos + 1);
...
static void delete_char_at(TextFieldData *data) {
  if (data->cursor_pos < data->length) {
    memmove(data->text + data->cursor_pos, data->text + data->cursor_pos + 1,
            data->length -
                data->cursor_pos); // +1 included in memmove length implicitly
                                   // if we want null terminator, wait.
    // data->length - data->cursor_pos is the count of chars after cursor
    // excluding the one at cursor? Example: "abc", pos 1 ('b'). len 3. delete
    // at 1. move 'c' and '\0' to 1. chars to move: 'c' (index 2) and '\0'
    // (index 3). count = 2. formula: len - pos. 3 - 1 = 2. Correct.
    data->length--;
  }
}
...
static void text_field_destroy(Widget *widget) {
  ...
  if (data) {
    if (data->text)
      memory_free(data->text);
    if (data->placeholder)
      memory_free(data->placeholder);
    memory_free(data);
    widget->user_data = NULL;
  }
  // Base widget destruction is handled by the caller (widget_destroy in
  // widget.c)
}
...
static void text_field_render(Widget *widget) {
  ...
  // For now we just log if debug needed, but usually we don't spam logs in
  // render
}
...
void text_field_set_placeholder(Widget *widget, const char *placeholder) {
  ...
  if (placeholder) {
    ...
    data->placeholder = memory_alloc(len + 1);
    if (data->placeholder) {
      strcpy(data->placeholder, placeholder);
    }
  } else {
    data->placeholder = NULL;
  }
