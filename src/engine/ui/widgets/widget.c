...
static void widget_add_handler_internal(Widget *widget,
...
static void widget_remove_handler_internal(Widget *widget,
                                           UIEventHandler *handler) {
  if (!widget || !handler)
    return;
...
static bool widget_propagate_event(Widget *widget, UIEvent *event) {
  if (!widget || !event || event->stop_propagation)
    return false;
  ...
  if (!handled || !event->stop_propagation) {
    ...
    while (handler && !event->stop_propagation) {
      if (handler->active && handler->callback) {
        bool handler_result =
            handler->callback(widget, event, handler->user_data);
        if (handler_result) {
          handled = true;
          event->handled = true;
        }
  ...
  if (!event->stop_propagation && !handled) {
    ...
    default:
      ...
      for (uint32_t i = 0; i < widget->child_count; i++) {
        ...
        if (child->visible &&
            widget_contains_point(child, event->mouse.position)) {
...
void widget_add_child(Widget *parent, Widget *child) {
  ...
  if (parent->child_count >= parent->child_capacity) {
    uint32_t new_capacity =
        parent->child_capacity == 0 ? 8 : parent->child_capacity * 2;
    Widget **new_children =
        memory_realloc(parent->children, new_capacity * sizeof(Widget *));
    if (!new_children) {
...
void widget_add_event_handler(Widget *widget, UIEventType event_type,
  ...
  LOG_DEBUG("Added event handler for type %d to widget %s", event_type,
            widget->name);
}

void widget_remove_all_event_handlers(Widget *widget, UIEventType event_type) {
  if (!widget || !widget->event_handlers)
    return;
  ...
  while (current) {
    bool remove =
        (event_type == UI_EVENT_NONE || current->event_type == event_type);
...
bool widget_handle_event(Widget *widget, UIEvent *event) {
  if (!widget || !event || !widget->enabled || !widget->visible)
    return false;
...
bool widget_contains_point(const Widget *widget, Vec2 point) {
  ...
  return point.x >= 0.0f && point.x <= widget->size.x && point.y >= 0.0f &&
         point.y <= widget->size.y;
}

Widget *widget_hit_test(const Widget *widget, Vec2 point) {
  if (!widget || !widget_contains_point(widget, point))
    return NULL;
...
UIEvent *ui_event_create_mouse(UIEventType type, Vec2 position,
...
UIEvent *ui_event_create_keyboard(UIEventType type, uint32_t key_code,
                                  uint32_t scan_code) {
  UIEvent *event = ui_event_create(type);
  if (!event)
    return NULL;
...
bool ui_event_is_mouse_event(const UIEvent *event) {
  ...
  return event->type >= UI_EVENT_MOUSE_ENTER &&
         event->type <= UI_EVENT_MOUSE_DOUBLE_CLICK;
}
...
uint32_t widget_generate_id(void) { return g_widget_id_counter++; }
...
void widget_print_hierarchy(const Widget *widget, int depth) {
  ...
  printf("%s (%s) - Pos:(%.1f,%.1f) Size:(%.1fx%.1f) %s\n", widget->name,
         widget->focused ? "FOCUSED" : "normal", widget->position.x,
         widget->position.y, widget->size.x, widget->size.y,
