...
typedef bool (*UIEventCallback)(Widget *widget, UIEvent *event,
                                void *user_data);
...
struct Widget {
  ...
  Size (*measure)(Widget *widget, float available_width,
...
void widget_layout(Widget *widget, float available_width,
                   float available_height);
...
UIEvent *ui_event_create_keyboard(UIEventType type, uint32_t key_code,
                                  uint32_t scan_code);
UIEvent *ui_event_create_touch(UIEventType type, Vec2 position,
                               uint32_t touch_id);
