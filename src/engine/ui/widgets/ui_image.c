...
Widget *ui_image_create(const char *name, uint32_t texture_id) {
  ...
  if (!data) {
    LOG_ERROR("Failed to allocate UI image data");
  ...
  // Transparent background by default for images, usually we just want the
  // texture
...
static void ui_image_render(Widget *widget) {
  ...
  // Stub implementation - actual rendering would happen in UI renderer using
  // this data The renderer would look at widget type or user_data and draw the
  // texture
}
