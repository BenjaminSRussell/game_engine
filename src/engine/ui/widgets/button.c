...
static void button_render_impl(Widget *widget) {
  ...
  //           current_color.x, current_color.y, current_color.z,
  //           current_color.w);
}
...
static bool button_handle_event_impl(Widget *widget, UIEvent *event) {
  ...
  case UI_EVENT_MOUSE_LEAVE:
    if (widget->state != WIDGET_STATE_DISABLED) {
      // If we were pressed, we might want to stay pressed if drag-out behavior
      // is supported But for simple buttons, leaving usually resets state
    ...
    if (event->mouse.button == UI_MOUSE_BUTTON_LEFT) {
      if (widget->state == WIDGET_STATE_PRESSED) {
        ...
        if (widget_contains_point(widget, event->mouse.position)) {
          ...
          // Emit CLICK event
          UIEvent *click_event =
              ui_event_create_mouse(UI_EVENT_MOUSE_CLICK, event->mouse.position,
...
Button *button_create(const char *name, const char *label) {
  ...
  if (!button) {
    LOG_ERROR("Failed to allocate button");
    return NULL;
  }
...
void button_set_on_click(Button *button, UIEventCallback callback,
                         void *user_data) {
  if (!button)
    return;
...
void button_set_colors(Button *button, Vec4 normal, Vec4 hover, Vec4 pressed,
                       Vec4 disabled) {
  if (!button)
    return;
...
void button_set_text_colors(Button *button, Vec4 normal, Vec4 hover,
