...
static Vec4 ui_button_get_current_color(UIButton *button) {
  ...
  if (widget_get_state(&button->base) == WIDGET_STATE_PRESSED) {
    ...
    Vec4 result;
    result.x = button->normal_color.x +
               (button->hover_color.x - button->normal_color.x) * hover;
    result.y = button->normal_color.y +
               (button->hover_color.y - button->normal_color.y) * hover;
    result.z = button->normal_color.z +
               (button->hover_color.z - button->normal_color.z) * hover;
    result.w = button->normal_color.w +
               (button->hover_color.w - button->normal_color.w) * hover;
    return result;
  }
...
static void ui_button_render_impl(Widget *widget) {
  ...
  LOG_DEBUG("Button render: pos=(%.1f, %.1f) size=(%.1f, %.1f) text=%s", pos.x,
            pos.y, size.x, size.y, button->text ? button->text : "");
}
...
static void ui_button_layout_impl(Widget *widget, float available_width,
...
static Size ui_button_measure_impl(Widget *widget, float available_width,
                                   float available_height) {
  if (!widget)
    return (Size){0, 0};
...
UIButton *ui_button_create(const char *name, const char *text) {
  ...
  button->disabled_color =
      (Vec4){0.5f, 0.5f, 0.5f, 0.5f};                  // Semi-transparent gray
  ...
  LOG_INFO("Created UI button: %s with text '%s'", name ? name : "unnamed",
           text ? text : "");
  return button;
}
...
void ui_button_set_colors(UIButton *button, Vec4 normal, Vec4 hover,
                          Vec4 pressed, Vec4 disabled) {
  if (!button)
    return;
