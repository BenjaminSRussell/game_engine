...
static void ui_label_render_impl(Widget *widget) {
  ...
  LOG_DEBUG("Label render: pos=(%.1f, %.1f) size=(%.1f, %.1f) text=%s", pos.x,
            pos.y, size.x, size.y, label->text ? label->text : "");
}

static void ui_label_layout_impl(Widget *widget, float available_width,
...
static Size ui_label_measure_impl(Widget *widget, float available_width,
                                  float available_height) {
  if (!widget)
    return (Size){0, 0};
  ...
  if (label->text && label->text_length > 0) {
    ...
    if (label->word_wrap && available_width > 0) {
      uint32_t chars_per_line =
          (uint32_t)(available_width / (label->font_size * 0.6f));
      if (chars_per_line > 0) {
        uint32_t lines =
            (label->text_length + chars_per_line - 1) / chars_per_line;
        if (label->max_lines > 0 && lines > label->max_lines) {
          lines = label->max_lines;
        }
  ...
  return (Size){
      ...
      .y =
          fminf(text_height, available_height > 0 ? available_height : 100.0f)};
}
...
UILabel *ui_label_create(const char *name, const char *text) {
  ...
  LOG_INFO("Created UI label: %s with text '%s'", name ? name : "unnamed",
           text ? text : "");
  return label;
}
...
void ui_label_set_vertical_alignment(UILabel *label,
...
void ui_label_set_shadow(UILabel *label, float offset_x, float offset_y,
                         Vec4 color, bool show_shadow) {
  if (!label)
    return;
