...
typedef struct {
    ...
    // Let's use a dummy root node for consistency in API if needed, but API takes NULL for root.
    // We will store top level nodes in a list or just rely on root_container.
    // For recursion, we need to map Widgets back to TreeNodes or store TreeNodes.
    // Let's just store TreeNodes in `children` array of TreeNode.
    // For top level, we need a list of TreeNodes.
    TreeNode** root_nodes;
...
static void vertical_stack_layout(Widget* widget, float available_width, float available_height);
static void horizontal_row_layout(Widget* widget, float available_width, float available_height);
...
Widget* tree_view_create(const char* name) {
    ...
    if (!data) {
        LOG_ERROR(LOG_CAT_GENERAL, "Failed to allocate TreeView data");
...
static void tree_view_destroy(Widget* widget) {
    if (widget && widget->user_data) {
        ...
        if (data->root_nodes) memory_free(data->root_nodes);
...
static void vertical_stack_layout(Widget* widget, float available_width, float available_height) {
    ...
    for (uint32_t i = 0; i < widget->child_count; i++) {
        ...
        if (available_width > 0) {
            ...
            // widget_set_size(child, child_size); // Don't force width on children unless needed?
            // For tree nodes, we want them to stretch?
            // Yes, usually.
             widget_set_size(child, (Vec2){available_width, child_size.y > 0 ? child_size.y : 20.0f});
        }
...
static void horizontal_row_layout(Widget* widget, float available_width, float available_height) {
    ...
    for (uint32_t i = 0; i < widget->child_count; i++) {
        ...
        if (i == 1) { // Content widget, stretch it
             ...
             if (remaining > 0) size.x = remaining;
...
TreeNode* tree_view_add_node(Widget* tree_view, TreeNode* parent, const char* text) {
    ...
    widget_set_background_color(node->toggle_button, (Vec4){0.5f, 0.5f, 0.5f, 1.0f}); // Placeholder visual
    widget_add_child(node->header_widget, node->toggle_button);
    widget_add_event_handler(node->toggle_button, UI_EVENT_MOUSE_CLICK, on_toggle_click, node);
    ...
    widget_add_event_handler(node->header_widget, UI_EVENT_MOUSE_CLICK, on_node_click, node);

    // Children Container
    node->children_container = widget_create("node_children");
    node->children_container->layout = vertical_stack_layout;
    widget_set_padding(node->children_container, (BoxEdges){0.0f, 0.0f, 0.0f, 16.0f}); // Indent
    ...
    if (parent) {
        if (parent->child_count >= parent->child_capacity) {
            ...
            parent->children = memory_realloc(parent->children, sizeof(TreeNode*) * new_cap);
        ...
        if (data->root_node_count >= data->root_node_capacity) {
            uint32_t new_cap = data->root_node_capacity == 0 ? 4 : data->root_node_capacity * 2;
            data->root_nodes = memory_realloc(data->root_nodes, sizeof(TreeNode*) * new_cap);
...
static void destroy_tree_node_recursive(TreeNode* node) {
    ...
    if (node->children) memory_free(node->children);

    // Widgets are destroyed by widget_destroy hierarchy, but we allocated TreeNode struct
    memory_free(node);
}

void tree_view_remove_node(Widget* tree_view, TreeNode* node) {
    if (!tree_view || !node) return;
    // Implementation requires finding node in parent list and removing it
    // TODO: Implement removal logic (remove widget from parent, remove node from parent children list, free node)
}
...
void tree_node_set_selected(TreeNode* node, bool selected) {
    ...
    if (selected) {
        ...
        if (data->selected_node && data->selected_node != node) {
            ...
            widget_set_background_color(data->selected_node->header_widget, (Vec4){0,0,0,0}); // Transparent
        }

        node->selected = true;
        data->selected_node = node;
        widget_set_background_color(node->header_widget, (Vec4){0.2f, 0.4f, 0.8f, 1.0f}); // Selection color
        ...
        if (node->selected) {
            ...
            if (data->selected_node == node) data->selected_node = NULL;
        }
    }
}
