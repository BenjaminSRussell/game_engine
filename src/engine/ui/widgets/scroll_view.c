...
static void scroll_view_layout(Widget* widget, float available_width, float available_height);
...
Widget* scroll_view_create(const char* name, Vec2 size) {
    ...
    if (!data) {
        LOG_ERROR(LOG_CAT_GENERAL, "Failed to allocate scroll view data");
...
static void scroll_view_layout(Widget* widget, float available_width, float available_height) {
    ...
    // For a robust implementation, we might overlay them or reserve space only if needed (auto).
    // Here we reserve if enabled.
    float content_view_w = viewport_w;
    float content_view_h = viewport_h;

    if (data->show_v_scrollbar) content_view_w -= data->scrollbar_width;
    if (data->show_h_scrollbar) content_view_h -= data->scrollbar_width;
    ...
    // If content doesn't have layout (leaf), we assume its size is already set or we don't touch it?
    // Widget system usually requires calling layout.
    ...
    data->scroll_offset.x = fmaxf(0.0f, fminf(data->scroll_offset.x, max_scroll_x));
    data->scroll_offset.y = fmaxf(0.0f, fminf(data->scroll_offset.y, max_scroll_y));

    // Update content position
    // We access position directly to avoid triggering unnecessary invalidations if possible,
    // but widget_set_position handles logic.
    // Note: setting position to negative offset moves content up/left.
...
static bool scroll_view_handle_event(Widget* widget, UIEvent* event) {
    ...
    if (event->type == UI_EVENT_MOUSE_WHEEL) {
        ...
        // We can just set dirty and let layout handle it, but we also want to consume event only if we actually scrolled?
        // For now, always consume wheel if we have scrollbars.
...
void scroll_view_set_content(Widget* scroll_view, Widget* content) {
    if (!scroll_view || !scroll_view->user_data) return;
...
Widget* scroll_view_get_content(const Widget* scroll_view) {
    if (!scroll_view || !scroll_view->user_data) return NULL;
    ScrollViewData* data = (ScrollViewData*)scroll_view->user_data;
    return data->content;
}
...
Vec2 scroll_view_get_scroll_offset(const Widget* scroll_view) {
    if (!scroll_view || !scroll_view->user_data) return (Vec2){0,0};
    ScrollViewData* data = (ScrollViewData*)scroll_view->user_data;
...
void scroll_view_set_show_scrollbars(Widget* scroll_view, bool horizontal, bool vertical) {
    ...
    if (data->show_h_scrollbar != horizontal || data->show_v_scrollbar != vertical) {
...
void scroll_view_get_show_scrollbars(const Widget* scroll_view, bool* horizontal, bool* vertical) {
    ...
    if (horizontal) *horizontal = data->show_h_scrollbar;
    if (vertical) *vertical = data->show_v_scrollbar;
}
