...
static float resolve_track_size(const GridTrack *track, float container_size,
...
static TrackMeasurement measure_tracks(const GridTrack *tracks,
  ...
  for (uint32_t i = 0; i < track_count; i++) {
    ...
    if (track->type == GRID_TRACK_FRACTION) {
      ...
      float size =
          resolve_track_size(track, container_size, min_size, max_size);
...
static void distribute_fraction_space(TrackLayout *layout,
...
static void calculate_track_positions(TrackLayout *layout) {
  ...
  layout->total_size =
      layout->positions[layout->count - 1] + layout->sizes[layout->count - 1];
}

static TrackLayout create_track_layout(const GridTrack *tracks,
                                       uint32_t track_count,
                                       float container_size,
                                       const float *min_sizes,
                                       const float *max_sizes) {
  ...
  if (!layout.positions || !layout.sizes) {
    ...
    if (layout.positions)
      memory_free(layout.positions);
    if (layout.sizes)
      memory_free(layout.sizes);
    return layout;
  }
  ...
  TrackMeasurement measurement =
      measure_tracks(tracks, track_count, container_size, min_sizes, max_sizes);

  // Distribute fraction space
  if (measurement.total_fraction > 0.0f) {
    distribute_fraction_space(&layout, tracks, track_count,
                              measurement.free_space);
  }
...
static void position_grid_items(GridContainer *container,
                                const TrackLayout *columns,
  ...
  for (uint32_t i = 0; i < container->base.child_count; i++) {
    ...
    float cell_width =
        columns->positions[col_end - 1] + columns->sizes[col_end - 1] - cell_x;
    float cell_height =
        rows->positions[row_end - 1] + rows->sizes[row_end - 1] - cell_y;
    ...
    child->layout.size.width =
        fmaxf(0.0f, cell_width - get_main_margin(&child->margin, true));
    child->layout.size.height =
        fmaxf(0.0f, cell_height - get_main_margin(&child->margin, false));
  }
}
...
void grid_set_columns(GridContainer *container, const GridTrack *tracks,
                      uint32_t count) {
...
void grid_set_rows(GridContainer *container, const GridTrack *tracks,
                   uint32_t count) {
  if (!container || !tracks || count == 0)
    return;
...
void grid_add_child(GridContainer *container, UIElement *child) {
  ...
  LOGI("Added child %s to grid container %s", child->name,
       container->base.name);
}

void grid_layout(GridContainer *container, float available_width,
  ...
  if (container->config.column_count > 0) {
    column_layout = create_track_layout(container->config.columns,
                                        container->config.column_count,
  ...
  if (container->config.row_count > 0) {
    row_layout =
        create_track_layout(container->config.rows, container->config.row_count,
  ...
  LOGD("Grid layout completed for container %s: %.2fx%.2f",
       container->base.name, container->base.layout.size.width,
       container->base.layout.size.height);
}
