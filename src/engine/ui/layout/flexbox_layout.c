...
static float resolve_length(float value, float parent_size, float auto_value) {
  if (value < 0.0f)
    return auto_value; // Auto value
  return value;
}

static bool is_main_axis_horizontal(FlexDirection direction) {
  return direction == FLEX_DIRECTION_ROW ||
         direction == FLEX_DIRECTION_ROW_REVERSE;
}
...
static float get_cross_margin(const BoxEdges *margins,
...
static FlexSize measure_element(UIElement *element, float available_main,
  ...
  float preferred_main =
      resolve_length(element->preferred_size.width, available_main, -1.0f);
  float preferred_cross =
      resolve_length(element->preferred_size.height, available_cross, -1.0f);

  // Apply min/max constraints
  if (is_main_axis_horizontal(direction)) {
    preferred_main = fmaxf(element->min_size.width,
                           fminf(element->max_size.width, preferred_main));
    preferred_cross = fmaxf(element->min_size.height,
                            fminf(element->max_size.height, preferred_cross));
  } else {
    preferred_main = fmaxf(element->min_size.height,
                           fminf(element->max_size.height, preferred_main));
    preferred_cross = fmaxf(element->min_size.width,
                            fminf(element->max_size.width, preferred_cross));
  }
...
static void resolve_flexible_lengths(FlexLine *line, float remaining_free_space,
                                     bool shrinking) {
  if (line->count == 0)
    return;
  ...
  for (uint32_t i = 0; i < line->count; i++) {
    ...
    if (factor > 0.0f) {
      ...
      if (shrinking) {
        // Shrink the item
        float current_size =
            is_main_axis_horizontal(line->container->config.direction)
                ? item->layout.size.width
                : item->layout.size.height;
...
static void justify_content_line(FlexLine *line, JustifyContent justify,
  ...
  for (uint32_t i = 0; i < line->count; i++) {
    UIElement *item = line->items[i];
    float item_main_size =
        is_main_axis_horizontal(line->container->config.direction)
            ? item->layout.size.width
            : item->layout.size.height;
    total_item_size +=
        item_main_size +
        get_main_margin(&item->margin, line->container->config.direction);
  }
  ...
  switch (justify) {
  case FLEX_JUSTIFY_FLEX_START:
    // Items start at the beginning
    break;

  case FLEX_JUSTIFY_FLEX_END:
...
static void align_items_line(FlexLine *line, AlignItems align,
  ...
  for (uint32_t i = 0; i < line->count; i++) {
    ...
    AlignItems item_align =
        (align_self == FLEX_ALIGN_SELF_AUTO) ? align : (AlignItems)align_self;
    ...
    case FLEX_ALIGN_ITEMS_FLEX_END:
      cross_pos =
          container_cross_size - item_cross_size -
    ...
    case FLEX_ALIGN_ITEMS_STRETCH:
      ...
      if (is_main_axis_horizontal(line->container->config.direction)) {
        item->layout.size.height = fmaxf(
            0.0f, container_cross_size -
                      get_cross_margin(&item->margin,
                                       line->container->config.direction));
      } else {
        item->layout.size.width = fmaxf(
            0.0f, container_cross_size -
                      get_cross_margin(&item->margin,
                                       line->container->config.direction));
      }
...
FlexboxContainer *flexbox_container_create(const char *name) {
  ...
  container->config.justify_content = FLEX_JUSTIFY_FLEX_START;
  container->config.align_items = FLEX_ALIGN_ITEMS_STRETCH;
  container->config.align_content = FLEX_ALIGN_CONTENT_STRETCH;
  container->config.wrap = FLEX_WRAP_MODE_NOWRAP;
...
void flexbox_set_direction(FlexboxContainer *container,
                           FlexDirection direction) {
  if (!container)
    return;
...
void flexbox_set_justify_content(FlexboxContainer *container,
...
UIElement *ui_element_create(const char *name) {
  ...
  element->flex_item.align_self = FLEX_ALIGN_SELF_AUTO;
...
void flexbox_add_child(FlexboxContainer *container, UIElement *child) {
  ...
  if (container->base.child_count >= container->base.child_capacity) {
    uint32_t new_capacity = container->base.child_capacity == 0
                                ? 8
                                : container->base.child_capacity * 2;
    UIElement **new_children = memory_realloc(
        container->base.children, new_capacity * sizeof(UIElement *));
    if (!new_children) {
...
void flexbox_layout(FlexboxContainer *container, float available_width,
                    float available_height) {
  if (!container || !container->needs_layout)
    return;
  ...
  float inner_width = available_width - container->config.padding.left -
                      container->config.padding.right;
  float inner_height = available_height - container->config.padding.top -
                       container->config.padding.bottom;

  inner_width = fmaxf(0.0f, inner_width);
  inner_height = fmaxf(0.0f, inner_height);

  // Determine main and cross axis sizes
  float main_size = is_main_axis_horizontal(container->config.direction)
                        ? inner_width
                        : inner_height;
  float cross_size = is_main_axis_horizontal(container->config.direction)
                         ? inner_height
                         : inner_width;
  ...
  for (uint32_t i = 0; i < container->base.child_count; i++) {
    ...
    FlexSize measured = measure_element(child, main_size, cross_size,
                                        container->config.direction);
  ...
  for (uint32_t i = 0; i < container->base.child_count; i++) {
    ...
    total_main_size +=
        child_main_size +
        get_main_margin(&child->margin, container->config.direction);
    visible_children++;
  }
  ...
  if (fabsf(remaining_space) > LAYOUT_TOLERANCE) {
    ...
    if (remaining_space > 0.0f) {
      ...
      for (uint32_t i = 0; i < container->base.child_count; i++) {
        UIElement *child = container->base.children[i];
        if (!child->visible || child->flex_item.grow <= 0.0f)
          continue;

        float child_main_size =
            is_main_axis_horizontal(container->config.direction)
                ? child->layout.size.width
                : child->layout.size.height;
        float new_size =
            child_main_size +
            (remaining_space * child->flex_item.grow / visible_children);
      ...
      for (uint32_t i = 0; i < container->base.child_count; i++) {
        UIElement *child = container->base.children[i];
        if (!child->visible || child->flex_item.shrink <= 0.0f)
          continue;
        ...
        float new_size =
            child_main_size +
            (remaining_space * child->flex_item.shrink / visible_children);
  ...
  for (uint32_t i = 0; i < container->base.child_count; i++) {
    ...
    if (is_main_axis_horizontal(container->config.direction)) {
      child->layout.position.y =
          container->config.padding.top + child->margin.top;
      child->layout.size.height = fmaxf(
          0.0f, cross_size - get_cross_margin(&child->margin,
                                              container->config.direction));
    } else {
      child->layout.position.x =
          container->config.padding.left + child->margin.left;
      child->layout.size.width = fmaxf(
          0.0f, cross_size - get_cross_margin(&child->margin,
                                              container->config.direction));
    }
  ...
  if (is_main_axis_horizontal(container->config.direction)) {
    container->base.layout.size.width = total_main_size +
                                        container->config.padding.left +
                                        container->config.padding.right;
    container->base.layout.size.height = cross_size +
                                         container->config.padding.top +
                                         container->config.padding.bottom;
  } else {
    container->base.layout.size.width = cross_size +
                                        container->config.padding.left +
                                        container->config.padding.right;
    container->base.layout.size.height = total_main_size +
                                         container->config.padding.top +
                                         container->config.padding.bottom;
  }
  ...
  if (g_performance_profiling_enabled) {
    ...
    layout_profiling_update(&container->stats, total_ns, measure_ns,
                            arrange_ns);

    container->layout_time_ms = (float)total_ns / 1000000.0f;
    container->layout_iterations =
        (uint32_t)container->stats.total_layout_count;
  }

  LOGD("Layout completed for container %s: %.2fx%.2f", container->base.name,
       container->base.layout.size.width, container->base.layout.size.height);
}
...
void flexbox_get_performance_stats(const FlexboxContainer *container,
  ...
  if (iterations)
    *iterations = container->layout_iterations;
  if (time_ms)
    *time_ms = container->layout_time_ms;
}
