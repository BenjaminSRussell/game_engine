...
static float apply_easing(float t, UIEasingType type) {
  ...
  case UI_EASE_IN_QUAD:
    return ease_in_quad(t);
  case UI_EASE_OUT_QUAD:
    return ease_out_quad(t);
  case UI_EASE_IN_OUT_QUAD:
    return ease_in_out_quad(t);
  case UI_EASE_IN_CUBIC:
    return ease_in_cubic(t);
  case UI_EASE_OUT_CUBIC:
    return ease_out_cubic(t);
  case UI_EASE_IN_OUT_CUBIC:
    return ease_in_out_cubic(t);
  case UI_EASE_IN_ELASTIC:
    return ease_in_elastic(t);
  case UI_EASE_OUT_ELASTIC:
    return ease_out_elastic(t);
  case UI_EASE_IN_OUT_ELASTIC:
    return ease_in_out_elastic(t);
  case UI_EASE_IN_BOUNCE:
    return ease_in_bounce(t);
  case UI_EASE_OUT_BOUNCE:
    return ease_out_bounce(t);
  case UI_EASE_IN_OUT_BOUNCE:
...
UIAnimation *ui_animation_create(Widget *target, UIAnimationProperty property,
                                 float duration) {
  UIAnimation *anim = MALLOC(sizeof(UIAnimation));
  if (!anim) {
    LOG_ERROR("Failed to allocate UIAnimation");
    return NULL;
  }
...
void ui_animation_set_callback(UIAnimation *anim, UIAnimationCallback callback,
                               void *user_data) {
  if (anim) {
      ...
      if (curr->delay > 0) {
        ...
        if (curr->target) {
          ...
          case UI_ANIM_OPACITY: {
            float val =
                ui_lerp(curr->values.f.start, curr->values.f.end, eased_t);
          ...
          case UI_ANIM_POSITION: {
            Vec2 val = ui_vec2_lerp_custom(curr->values.v2.start,
                                           curr->values.v2.end, eased_t);
          ...
          case UI_ANIM_BACKGROUND_COLOR: {
            Vec4 val = ui_vec4_lerp_custom(curr->values.v4.start,
                                           curr->values.v4.end, eased_t);
...
// Helpers
UIAnimation *ui_animate_fade(Widget *widget, float target_opacity,
...
UIAnimation *ui_animate_move(Widget *widget, Vec2 target_pos, float duration,
                             UIEasingType easing) {
...
UIAnimation *ui_animate_resize(Widget *widget, Vec2 target_size, float duration,
                               UIEasingType easing) {
...
void ui_transition_screen(Widget *current, Widget *next, UITransitionType type,
                          float duration) {
  if (!next)
    return;
  ...
  case UI_TRANSITION_FADE: {
    if (current) {
      // Fade out current
      UIAnimation *anim =
          ui_animate_fade(current, 0.0f, duration, UI_EASE_IN_QUAD);
      if (anim) {
    ...
    UIAnimation *anim2 =
        ui_animate_fade(next, 1.0f, duration, UI_EASE_OUT_QUAD);
    if (anim2)
      anim2->auto_destroy = true;
    break;
  }
  ...
  case UI_TRANSITION_SLIDE_LEFT: {
    ...
    if (current) {
      ...
      target.x -= width;
      UIAnimation *anim =
          ui_animate_move(current, target, duration, UI_EASE_IN_QUAD);
      if (anim) {
        anim->auto_destroy = true;
  ...
  case UI_TRANSITION_SLIDE_UP: {
    ...
    // Next moves from Down (+height to 0) ? No, "Slide Up" usually means
    // appearing from bottom moving up. Or moving whole view up? Let's assume
    // Next comes from Bottom (+height) moving to 0. Current moves to Top
    // (-height).
  ...
  case UI_TRANSITION_SLIDE_DOWN: {
    // Next comes from Top (-height) moving to 0. Current moves to Bottom
    // (+height).
    float height = next->size.y;
    if (current)
      height = current->size.y;
  ...
  default:
    if (current)
      widget_set_visible(current, false);
    widget_set_visible(next, true);
    FREE(data);
    break;
  }
