...
static void orthographic_camera_update_projection_matrix(OrthographicCamera* camera);
static void orthographic_camera_update_view_matrix(OrthographicCamera* camera);
static void orthographic_camera_update_view_projection_matrix(OrthographicCamera* camera);
...
int orthographic_camera_create(OrthographicCamera* camera, float left, float right, float bottom, float top, float near_plane, float far_plane) {
    ...
    LOG_INFO("Orthographic camera created: L=%.1f, R=%.1f, B=%.1f, T=%.1f, N=%.1f, F=%.1f", left, right, bottom, top, near_plane, far_plane);
    return 0;
}
...
int orthographic_camera_set_projection(OrthographicCamera* camera, float left, float right, float bottom, float top, float near_plane, float far_plane) {
    ...
    LOG_DEBUG("Orthographic projection updated: L=%.1f, R=%.1f, B=%.1f, T=%.1f, N=%.1f, F=%.1f", left, right, bottom, top, near_plane, far_plane);
    return 0;
}

int orthographic_camera_set_view(Orthographic* camera, Vec3 position, Vec3 target, Vec3 up) {
    if (!camera) return -1;
    ...
    LOG_DEBUG("Orthographic view updated: pos=(%.1f,%.1f,%.1f), target=(%.1f,%.1f,%.1f)", 
...
int orthographic_camera_set_viewport(OrthographicCamera* camera, Vec2 origin, Vec2 size) {
    ...
    LOG_DEBUG("Orthographic viewport updated: origin=(%.1f,%.1f), size=(%.1f,%.1f)", origin.x, origin.y, size.x, size.y);
    return 0;
}
...
int orthographic_camera_pan(OrthographicCamera* camera, Vec2 delta) {
    ...
    camera->pan_offset = camera->pan_offset + delta;
    orthographic_camera_update_view_matrix(camera);
    orthographic_camera_update_view_projection_matrix(camera);
...
int orthographic_camera_center(OrthographicCamera* camera) {
    ...
    // Center the viewport
    Vec2 center = {
        camera->viewport_origin.x + camera->viewport_size.x * 0.5f,
        camera->viewport_origin.y + camera->viewport_size.y * 0.5f
    };
    
    Vec2 delta = {
        center.x - camera->viewport_origin.x,
        center.y - camera->viewport_origin.y
    };
...
int orthographic_camera_get_view_projection_matrix(const OrthographicCamera* camera, Mat4* out_matrix) {
    if (!camera || !out_matrix) return -1;
...
int orthographic_camera_world_to_screen(const OrthographicCamera* camera, Vec3 world_pos, Vec2* out_screen_pos) {
    if (!camera || !out_screen_pos) return -1;
    ...
    Vec4 clip_pos = mat4_mul_vec4(view_proj, (Vec4){world_pos.x, world_pos.y, world_pos.z, 1.0f});
    ...
    Vec2 screen_pos_ndc = {
        (clip_pos.x + 1.0f) * 0.5f,
        (1.0f - clip_pos.y) * 0.5f
    };
    
    // Apply viewport transformation
    *out_screen_pos = screen_pos_ndc * camera->viewport_size + camera->viewport_origin;
    
    return 0;
}

int orthographic_camera_screen_to_world(const OrthographicCamera* camera, Vec2 screen_pos, Vec3* out_world_pos) {
    if (!camera || !out_world_pos) return -1;
    ...
    Mat4 inv_view_proj;
    Mat4 inv_view;
    if (!mat4_inverse(&camera->view_projection_matrix, &inv_view_proj)) {
        LOG_ERROR("Failed to invert view-projection matrix");
        return -1;
    }
    ...
    if (world_pos4.w > 0.001f) {
        world_pos4 = world_pos4 / world_pos4.w;
    }
...
int orthographic_camera_ray_from_screen(const OrthographicCamera* camera, Vec2 screen_pos, Vec3* out_origin, Vec3* out_direction) {
    if (!camera || !out_origin || !out_direction) return -1;
    
    // Convert screen position to world space
    Vec3 world_pos;
    if (orthographic_camera_screen_to_world(camera, screen_pos, &world_pos) != 0) {
        return -1;
    }
...
int orthographic_camera_get_bounds(const OrthographicCamera* camera, Vec2* out_min, Vec2* out_max) {
    if (!camera || !out_min || !out_max) return -1;
...
int orthographic_camera_get_center(const OrthographicCamera* camera, Vec2* out_center) {
    if (!camera || !out_center) return -1;
    
    *out_center = (Vec2){
        (camera->left + camera->right) * 0.5f,
...
float orthographic_get_aspect_ratio(const Orthographic* camera) {
...
int orthographic_camera_set_maintain_aspect_ratio(OrthographicCamera* camera, bool enabled, float aspect_ratio) {
    ...
    if (camera->auto_resize) {
        orthographic_camera_set_viewport(camera, camera->viewport_origin, camera->viewport_size);
    }
    
    LOG_INFO("Orthographic aspect ratio maintenance %s, ratio: %.2f", enabled ? "enabled" : "disabled", aspect_ratio);
    return 0;
}
...
int orthographic_camera_set_background_color(OrthographicCamera* camera, Vec4 color) {
    ...
    LOG_INFO("Orthographic background color set to RGBA(%.1f, %.1f, %.1f, %.1f)", color.x, color.y, color.z, color.w);
    return 0;
}
...
int orthographic_camera_animate_to(OrthographicCamera* camera, Vec3 target_position, Vec3 target_target, float duration) {
    ...
    while (current_time < end_time) {
        ...
        orthographic_camera_set_view(camera, current_pos, current_target, camera->up);
        
        current_time += 0.016f; // ~60 FPS update rate
    }
    
    LOG_INFO("Orthographic camera animation completed to position (%.2f, %.2f, %.2f)", 
...
int orthographic_camera_animate_pan(OrthographicCamera* camera, Vec2 target_pan, float duration) {
    ...
    LOG_INFO("Orthographic pan animation completed to (%.2f, %.2f)", target_pan.x, target_pan.y);
    return 0;
}

// Validation and debugging
bool orthographic_camera_is_valid(const Orthographic* camera) {
    if (!camera) return false;
    
    // Check for valid projection parameters
    if (camera->left >= camera->right || camera->bottom >= camera->top || 
        camera->near_plane >= camera->far_plane || camera->near_plane < 0.0f || camera->far_plane <= 0.0f) {
        return false;
    }
...
int orthographic_camera_print_info(const Orthographic* camera) {
    if (!camera) return -1;
...
static void orthographic_camera_update_projection_matrix(OrthographicCamera* camera) {
    ...
    camera->projection_matrix.columns[0][0] = th;
    camera->projection_matrix.columns[1][1] = 0.0f;
    camera->projection_matrix.columns[2][2] = tz;
    camera->projection_matrix.columns[3][3] = 0.0f;
    
    camera->projection_matrix.columns[0][3] = tx;
    camera->projection_matrix.columns[1][3] = ty;
    camera->projection_matrix.columns[2][3] = -1.0f;
    camera->projection_matrix.columns[3][3] = 0.0f;
}

static void orthographic_camera_update_view_matrix(Orthographic* camera) {
    if (!camera) return;
    ...
    // Create view matrix
    camera->view_matrix.columns[0][0] = right.x;
    camera->view_matrix.columns[0][1] = up.x;
    camera->view_matrix.columns[0][2] = -forward.x;
    camera->view_matrix.columns[0][3] = 0.0f;
    
    camera->view_matrix.columns[1][0] = right.y;
    camera->view_matrix.columns[1][1] = up.y;
    camera->view_matrix.columns[1][2] = -forward.y;
    camera->view_matrix.columns[1][3] = 0.0f;
    
    camera->view_matrix.columns[2][0] = right.z;
    camera->view_matrix.columns[2][1] = up.z;
    camera->view_matrix.columns[2][2] = -forward.z;
    camera->view_matrix.columns[2][3] = 0.0f;
    
    camera->view_matrix.columns[3][0] = -vec3_dot(camera->position, right) - vec3_dot(camera->position, up) - vec3_dot(camera->position, forward);
    camera->view_matrix.columns[3][1] = -vec3_dot(camera->position, up) - vec3_dot(camera->position, forward);
    camera->view_matrix.columns[3][2] = -vec3_dot(camera->position, forward);
    camera->view_matrix.columns[3][3] = 1.0f;
}
...
static void orthographic_camera_update_view_projection_matrix(OrthographicCamera* camera) {
    ...
    camera->view_projection_matrix = mat4_mul(camera->projection_matrix, camera->view_matrix);
}
