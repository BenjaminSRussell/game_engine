...
static float ray_distance_to_line(const float *ray_o, const float *ray_d, 
    ...
    return sqrtf(closest[0]*closest[0] + closest[1]*closest[1] + closest[2]*closest[2]);
}
...
GizmoAxis gizmo_raycast(TransformGizmo *gizmo, const float *ray_origin, 
    ...
    if (gizmo->mode == GIZMO_MODE_TRANSLATE) {
        ...
        float dist_x = ray_distance_to_line(ray_origin, ray_dir, gizmo->position, x_axis);
        ...
        float dist_y = ray_distance_to_line(ray_origin, ray_dir, gizmo->position, y_axis);
        ...
        float dist_z = ray_distance_to_line(ray_origin, ray_dir, gizmo->position, z_axis);
...
void gizmo_begin_drag(TransformGizmo *gizmo, GizmoAxis axis, 
    ...
    if (axis == GIZMO_AXIS_X) {
        // Use YZ plane
        gizmo->drag_plane_normal[0] = 1; gizmo->drag_plane_normal[1] = 0; gizmo->drag_plane_normal[2] = 0;
    } else if (axis == GIZMO_AXIS_Y) {
        gizmo->drag_plane_normal[0] = 0; gizmo->drag_plane_normal[1] = 1; gizmo->drag_plane_normal[2] = 0;
    } else if (axis == GIZMO_AXIS_Z) {
        gizmo->drag_plane_normal[0] = 0; gizmo->drag_plane_normal[1] = 0; gizmo->drag_plane_normal[2] = 1;
    }
}
...
void gizmo_update_drag(TransformGizmo *gizmo, const float *ray_origin, 
    ...
    float to_plane[3] = {
        gizmo->drag_start_pos[0] - ray_origin[0],
    ...
    // Project onto axis
    float delta[3] = {
        hit[0] - gizmo->drag_start_pos[0],
        hit[1] - gizmo->drag_start_pos[1],
        hit[2] - gizmo->drag_start_pos[2]
    };
    
    if (gizmo->active_axis == GIZMO_AXIS_X) {
        out_delta[0] = delta[0]; out_delta[1] = 0; out_delta[2] = 0;
    } else if (gizmo->active_axis == GIZMO_AXIS_Y) {
        out_delta[0] = 0; out_delta[1] = delta[1]; out_delta[2] = 0;
    } else if (gizmo->active_axis == GIZMO_AXIS_Z) {
        out_delta[0] = 0; out_delta[1] = 0; out_delta[2] = delta[2];
    }
}
