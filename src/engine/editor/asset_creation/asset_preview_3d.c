...
void asset_preview_center_camera(const float *bbox_min, const float *bbox_max) {
    ...
    // Ensure min distance
    if (g_preview_ctx.camera_distance < 0.1f) g_preview_ctx.camera_distance = 2.0f;
}
...
void asset_preview_update(float delta_time) {
    if (g_preview_ctx.auto_rotate_speed != 0.0f) {
        ...
        // Keep in 0-2PI range
        if (g_preview_ctx.camera_yaw > 6.28318f) g_preview_ctx.camera_yaw -= 6.28318f;
    }
}
...
void asset_preview_render_scene(void *asset_ptr) {
    ...
    // Orbit camera logic
    float cx = sinf(g_preview_ctx.camera_yaw) * cosf(g_preview_ctx.camera_pitch) * g_preview_ctx.camera_distance;
    float cy = sinf(g_preview_ctx.camera_pitch) * g_preview_ctx.camera_distance; // This is a bit simplified for Y-up
    float cz = cosf(g_preview_ctx.camera_yaw) * cosf(g_preview_ctx.camera_pitch) * g_preview_ctx.camera_distance;
    
    // Camera Pos: Center + offset (simplified, assume object at origin)
    // float cam_pos[3] = { cx, cy - g_preview_ctx.camera_distance, cz }; // Adjusted
