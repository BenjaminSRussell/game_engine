...
static GridSettings g_grid_settings = {
    ...
    .adaptive_grid = true,
    .adaptive_scale = 1.0f
    // TODO(Jules): Make grid colors configurable in GridSettings.
};
...
GridSettings *grid_snapping_get_settings(void) { return &g_grid_settings; }
...
void grid_snap_position(float *pos_x, float *pos_y, float *pos_z) {
  if (!g_grid_settings.enabled)
    return;
  float step = g_grid_settings.grid_size;
  if (pos_x)
    *pos_x = grid_snap_value(*pos_x, step);
  if (pos_y)
    *pos_y = grid_snap_value(*pos_y, step);
  if (pos_z)
    *pos_z = grid_snap_value(*pos_z, step);
}
...
void grid_snap_rotation(float *rot_x, float *rot_y, float *rot_z) {
  ...
  if (rot_x)
    *rot_x = grid_snap_value(*rot_x, step);
  if (rot_y)
    *rot_y = grid_snap_value(*rot_y, step);
  if (rot_z)
    *rot_z = grid_snap_value(*rot_z, step);
  // TODO(Jules): Implement radial snapping for rotation in grid_snap_rotation.
}
...
void grid_snap_scale(float *scale_x, float *scale_y, float *scale_z) {
  ...
  if (scale_x)
    *scale_x = grid_snap_value(*scale_x, step);
  if (scale_y)
    *scale_y = grid_snap_value(*scale_y, step);
  if (scale_z)
    *scale_z = grid_snap_value(*scale_z, step);
  // TODO(Jules): Add support for isometric grid snapping.
}

void grid_snapping_render(const float *camera_pos, const float *camera_view,
                          const float *camera_proj) {
  if (!g_grid_settings.show_grid)
    return;
  ...
  // TODO(Jules): Optimization: cache log10f calculation in
  // grid_snapping_render.
