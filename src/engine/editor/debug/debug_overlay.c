...
DebugTextElement *debug_overlay_add_text(DebugOverlay *overlay,
  ...
  if (!overlay || !text ||
      overlay->text_count >= DEBUG_OVERLAY_MAX_TEXT_LINES) {
    return NULL;
  }
...
void debug_overlay_update_text(DebugOverlay *overlay, u32 index,
                               const char *text) {
  if (!overlay || !text || index >= overlay->text_count)
    return;
...
void debug_overlay_set_text_color(DebugOverlay *overlay, u32 index,
                                  Vec4 color) {
  if (!overlay || index >= overlay->text_count)
    return;
...
void debug_overlay_graph_add_value(DebugOverlay *overlay, u32 graph_index,
                                   f32 value) {
  if (!overlay || graph_index >= overlay->graph_count)
    return;
  ...
  if (graph->auto_scale) {
    if (value < graph->min_value)
      graph->min_value = value * 0.9f;
    if (value > graph->max_value)
      graph->max_value = value * 1.1f;
  }
}
...
DebugMarkerElement *debug_overlay_add_marker(DebugOverlay *overlay,
  ...
  DebugMarkerElement *element =
      &overlay->marker_elements[overlay->marker_count++];
  memset(element, 0, sizeof(DebugMarkerElement));

  element->base.position =
      (Vec2){0.0f, 0.0f}; // Will be calculated during render
...
void debug_overlay_marker_set_label(DebugOverlay *overlay, u32 marker_index,
                                    const char *label) {
  if (!overlay || !label || marker_index >= overlay->marker_count)
    return;
...
void debug_overlay_marker_update_position(DebugOverlay *overlay,
  ...
  if (!overlay || marker_index >= overlay->marker_count)
    return;
...
void debug_overlay_log(DebugOverlay *overlay, DebugLogLevel level,
                       const char *message) {
  if (!overlay || !message || level < overlay->min_log_level)
    return;

  if (overlay->log_count >= DEBUG_OVERLAY_MAX_LOG_ENTRIES) {
    // Remove oldest entry
...
void debug_overlay_render(DebugOverlay *overlay, void *render_context) {
  if (!overlay || !overlay->enabled || !render_context)
    return;
...
void debug_overlay_render_graphs(DebugOverlay *overlay, void *render_context) {
  for (u32 i = 0; i < overlay->graph_count; i++) {
    ...
    if (graph->base.visible) {
      // LOG_DEBUG("Rendering graph: %s with %u values", graph->label,
      // graph->value_count);
    }
  }
}
...
void debug_overlay_render_markers(DebugOverlay *overlay, void *render_context) {
  for (u32 i = 0; i < overlay->marker_count; i++) {
    ...
    if (marker->base.visible) {
      ...
      Vec2 screen_pos =
          debug_overlay_world_to_screen(overlay, marker->world_position);
      marker->base.position = screen_pos;

      /*
      LOG_DEBUG("Rendering marker at world (%.2f, %.2f, %.2f) -> screen (%.1f,
      %.1f)", marker->world_position.x, marker->world_position.y,
      marker->world_position.z, screen_pos.x, screen_pos.y);
      */
    }
  }
}

Vec2 debug_overlay_world_to_screen(const DebugOverlay *overlay,
                                   Vec3 world_pos) {
  // This would integrate with the camera system
  // For now, return a simple orthographic projection
  Vec2 screen_pos = {world_pos.x + overlay->screen_size.x * 0.5f,
                     overlay->screen_size.y -
                         (world_pos.y + overlay->screen_size.y * 0.5f)};
  return screen_pos;
}
...
void debug_overlay_sort_elements(DebugOverlay *overlay) {
  ...
  for (u32 i = 0; i < overlay->text_count - 1; i++) {
    for (u32 j = 0; j < overlay->text_count - i - 1; j++) {
      if (overlay->text_elements[j].base.priority <
          overlay->text_elements[j + 1].base.priority) {
  ...
  for (u32 i = 0; i < overlay->graph_count - 1; i++) {
    for (u32 j = 0; j < overlay->graph_count - i - 1; j++) {
      if (overlay->graph_elements[j].base.priority <
          overlay->graph_elements[j + 1].base.priority) {
  ...
  for (u32 i = 0; i < overlay->marker_count - 1; i++) {
    for (u32 j = 0; j < overlay->marker_count - i - 1; j++) {
      if (overlay->marker_elements[j].base.priority <
          overlay->marker_elements[j + 1].base.priority) {
...
void debug_overlay_setup_performance_view(DebugOverlay *overlay) {
  ...
  // Add FPS graph
  DebugGraphElement *fps_graph = debug_overlay_add_graph(
      overlay, "FPS", (Vec2){10.0f, 10.0f}, (Vec2){200.0f, 60.0f});
  if (fps_graph) {
  ...
  DebugGraphElement *frame_graph = debug_overlay_add_graph(
      overlay, "Frame Time", (Vec2){220.0f, 10.0f}, (Vec2){200.0f, 60.0f});
  if (frame_graph) {
...
void debug_overlay_setup_memory_view(DebugOverlay *overlay) {
  ...
  // Add memory graph
  DebugGraphElement *memory_graph = debug_overlay_add_graph(
      overlay, "Memory Usage", (Vec2){10.0f, 10.0f}, (Vec2){300.0f, 80.0f});
  if (memory_graph) {
...
void debug_overlay_setup_minimal_view(DebugOverlay *overlay) {
  ...
  // Only show FPS
  DebugTextElement *fps_text =
      debug_overlay_add_text(overlay, "", (Vec2){10.0f, 10.0f});
  if (fps_text) {
...
static void debug_overlay_render_memory(DebugOverlay *overlay,
  ...
  LOG_DEBUG(
      "Rendering memory overlay: Current=%zu MB, Peak=%zu MB, Allocations=%u",
