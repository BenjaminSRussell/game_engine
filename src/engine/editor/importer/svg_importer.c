...
static void svg_convert_rect_to_mesh(const SVGElement *element, Vec3 *vertices,
                                     u32 *vertex_count, u32 *indices,
                                     u32 *index_count);
static void svg_convert_circle_to_mesh(const SVGElement *element,
                                       Vec3 *vertices, u32 *vertex_count,
                                       u32 *indices, u32 *index_count);
static void svg_convert_path_to_mesh(const SVGElement *element, Vec3 *vertices,
                                     u32 *vertex_count, u32 *indices,
                                     u32 *index_count);
static void svg_write_element_to_file(FILE *file, const SVGElement *element);

// Helper for Mat4 * Vec4
static Vec4 mat4_mul_vec4_local(Mat4 m, Vec4 v) {
  Vec4 result;
...
SVGDocument *svg_importer_load_file(SVGImporter *importer,
                                    const char *filepath) {
...
SVGDocument *svg_importer_load_memory(SVGImporter *importer, const char *data,
                                      u32 size) {
  ...
  LOG_INFO("Successfully loaded SVG document with %u elements",
           doc->element_count);
  return doc;
}

bool svg_importer_save_file(SVGImporter *importer, const SVGDocument *doc,
                            const char *filepath) {
  ...
  fprintf(file,
          "<svg width=\"%.2f\" height=\"%.2f\" viewBox=\"%.2f %.2f %.2f %.2f\" "
          "xmlns=\"http://www.w3.org/2000/svg\">\n",
          doc->viewport_size.x, doc->viewport_size.y, doc->viewbox_min.x,
          doc->viewbox_min.y, doc->viewbox_max.x - doc->viewbox_min.x,
...
void svg_element_add_path_segment(SVGElement *element, SVGPathCommand command,
  ...
  if (!element || !points || point_count == 0 ||
      element->segment_count >= MAX_SVG_PATH_SEGMENTS) {
    return;
  }
...
bool svg_convert_to_mesh(const SVGDocument *doc, Vec3 *out_vertices,
                         u32 *out_vertex_count, u32 *out_indices,
                         u32 *out_index_count) {
  ...
  for (u32 elem_idx = 0; elem_idx < doc->element_count; elem_idx++) {
    ...
    case SVG_ELEMENT_RECT:
      svg_convert_rect_to_mesh(
          element, out_vertices + vertex_count, &vertex_count,
          out_indices ? out_indices + index_count : NULL, &index_count);
      break;

    case SVG_ELEMENT_CIRCLE:
      svg_convert_circle_to_mesh(
          element, out_vertices + vertex_count, &vertex_count,
          out_indices ? out_indices + index_count : NULL, &index_count);
      break;

    case SVG_ELEMENT_PATH:
      svg_convert_path_to_mesh(
          element, out_vertices + vertex_count, &vertex_count,
          out_indices ? out_indices + index_count : NULL, &index_count);
      break;

    default:
      LOG_WARN("Unsupported SVG element type for mesh conversion: %d",
               element->type);
      break;
    }
  }
  ...
  LOG_INFO("Converted SVG to mesh: %u vertices, %u indices", vertex_count,
           index_count);
  return true;
}

bool svg_convert_to_2d_paths(const SVGDocument *doc, Vec2 *out_points,
                             u32 *out_point_count) {
...
Vec2 svg_transform_point(const Mat4 *transform, Vec2 point) {
  ...
  Vec4 transformed = mat4_mul_vec4_local(*transform, point4);
...
Mat4 svg_create_transform_matrix(Vec2 translation, Vec2 scale, f32 rotation) {
  ...
  // Apply scale
  transform.data[0][0] = scale.x;
  transform.data[1][1] = scale.y;

  // Apply rotation (around Z axis)
  f32 cos_r = cosf(rotation);
  f32 sin_r = sinf(rotation);
  Mat4 rotation_matrix = {0};
  rotation_matrix.m00 = cos_r;
  rotation_matrix.m01 = -sin_r;
  rotation_matrix.m10 = sin_r;
  rotation_matrix.m11 = cos_r;
  rotation_matrix.m22 = 1.0f;
  rotation_matrix.m33 = 1.0f;

  transform = mat4_mul(transform, rotation_matrix);

  // Apply translation
  transform.data[3][0] = translation.x;
  transform.data[3][1] = translation.y;
...
void svg_document_print_info(const SVGDocument *doc) {
  ...
  LOG_INFO("  Viewport: %.2f x %.2f", doc->viewport_size.x,
           doc->viewport_size.y);
  LOG_INFO("  ViewBox: (%.2f, %.2f) to (%.2f, %.2f)", doc->viewbox_min.x,
           doc->viewbox_min.y, doc->viewbox_max.x, doc->viewbox_max.y);

  for (u32 i = 0; i < doc->element_count; i++) {
    SVGElement *element = &doc->elements[i];
    LOG_INFO("  Element %u: type=%d, pos=(%.2f,%.2f), size=(%.2f,%.2f)", i,
...
static void svg_convert_circle_to_mesh(const SVGElement *element,
  ...
  // Center vertex
  vertices[*vertex_count + segments] =
      (Vec3){element->position.x, element->position.y, 0.0f};
...
static void svg_convert_path_to_mesh(const SVGElement *element, Vec3 *vertices,
  ...
  for (u32 i = 0; i < element->segment_count; i++) {
    const SVGPathSegment *segment = &element->segments[i];
    ...
    case SVG_PATH_LINE_TO:
      if (segment->point_count > 0 &&
          *vertex_count + 1 < MAX_SVG_PATH_SEGMENTS * 3) {
        vertices[*vertex_count++] = (Vec3){current_pos.x, current_pos.y, 0.0f};
        vertices[*vertex_count++] =
            (Vec3){segment->points[0].x, segment->points[0].y, 0.0f};
        current_pos = segment->points[0];
      }
      break;
...
static void svg_write_element_to_file(FILE *file, const SVGElement *element) {
  ...
  if (element->has_fill) {
    fprintf(file, " fill=\"rgb(%d,%d,%d)\" fill-opacity=\"%.2f\"",
            (int)(element->fill_color.x * 255),
            (int)(element->fill_color.y * 255),
            (int)(element->fill_color.z * 255), element->fill_color.w);
  }

  if (element->has_stroke) {
    fprintf(file,
            " stroke=\"rgb(%d,%d,%d)\" stroke-opacity=\"%.2f\" "
            "stroke-width=\"%.2f\"",
            (int)(element->stroke_color.x * 255),
            (int)(element->stroke_color.y * 255),
            (int)(element->stroke_color.z * 255), element->stroke_color.w,
