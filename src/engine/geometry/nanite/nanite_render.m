...
ClusterHierarchyBuilder *cluster_hierarchy_builder_create(u32 max_nodes) {
  ClusterHierarchyBuilder *builder = (ClusterHierarchyBuilder *)calloc(1, sizeof(ClusterHierarchyBuilder));
  builder->max_nodes = max_nodes;
  builder->nodes = (ClusterHierarchyNode *)calloc(max_nodes, sizeof(ClusterHierarchyNode));
...
u32 cluster_hierarchy_add_node(ClusterHierarchyBuilder *builder, u32 parent_id, 
                              const Vec3 *center, f32 radius, u32 lod_level, f32 error) {
  if (!builder || builder->node_count >= builder->max_nodes) return UINT32_MAX;
  ...
  if (parent_id != UINT32_MAX && parent_id < builder->node_count) {
    ...
    parent->child_count++;
    parent->child_ids = (u32 *)realloc(parent->child_ids, parent->child_count * sizeof(u32));
  ...
  builder->average_error = (builder->average_error * (builder->total_clusters_processed - 1) + error) / builder->total_clusters_processed;
...
void cluster_hierarchy_build(ClusterHierarchyBuilder *builder, NaniteMesh *mesh) {
  ...
  for (u32 i = 0; i < mesh->cluster_count; i++) {
    ...
    if (cluster->vertex_count > 0) {
      // center = vec3_scale(center, 1.0f / cluster->vertex_count);
    }
    
    // Add leaf node
    cluster_hierarchy_add_node(builder, UINT32_MAX, &center, max_dist, 0, 0.01f);
  }
  ...
  while (current_level_count > 1 && builder->node_count < builder->max_nodes) {
    ...
    u32 groups_to_process = (current_level_count + 3) / 4; // Group 4 nodes per parent
    
    for (u32 group = 0; group < groups_to_process && builder->node_count < builder->max_nodes; group++) {
      ...
      for (u32 i = 0; i < 4 && (group * 4 + i + current_level_start) < builder->node_count; i++) {
      ...
      if (nodes_in_group > 0) {
        group_center = vec3_scale(group_center, 1.0f / nodes_in_group);
        group_radius *= 1.5f; // Expand radius to cover children
        group_error *= 1.2f; // Increase error for higher LODs
        
        // Add internal node
        u32 parent_id = cluster_hierarchy_add_node(builder, UINT32_MAX, &group_center, group_radius, level, group_error);
...
NaniteLightLinkingSystem *nanite_light_linking_create(id<MTLDevice> device, id<MTLLibrary> shader_lib, u32 max_lights) {
  NaniteLightLinkingSystem *system = (NaniteLightLinkingSystem *)calloc(1, sizeof(NaniteLightLinkingSystem));
  system->max_lights = max_lights;
  system->lights = (NaniteLightLink *)calloc(max_lights, sizeof(NaniteLightLink));
  
  @autoreleasepool {
    // Create light linking compute pipeline
    id<MTLFunction> light_link_function = [shader_lib newFunctionWithName:@"nanite_light_linking_compute"];
    if (light_link_function) {
      NSError *error = nil;
      system->light_linking_pipeline = [device newComputePipelineStateWithFunction:light_link_function error:&error];
      if (!system->light_linking_pipeline) {
        LOG_ERROR("Failed to create light linking pipeline: %s", [[error localizedDescription] UTF8String]);
      }
    }
    
    // Create shadow masking compute pipeline
    id<MTLFunction> shadow_mask_function = [shader_lib newFunctionWithName:@"nanite_shadow_mask_compute"];
    if (shadow_mask_function) {
      NSError *error = nil;
      system->shadow_mask_pipeline = [device newComputePipelineStateWithFunction:shadow_mask_function error:&error];
      if (!system->shadow_mask_pipeline) {
        LOG_ERROR("Failed to create shadow mask pipeline: %s", [[error localizedDescription] UTF8String]);
      }
    }
    
    // Allocate GPU buffers
    system->light_links_buffer = [device newBufferWithLength:max_lights * sizeof(NaniteLightLink) 
...
void nanite_light_linking_update(NaniteLightLinkingSystem *system, id<MTLCommandBuffer> cmd, 
                                 NaniteMesh *mesh, const Mat4 *view_proj) {
  if (!system || !cmd || !mesh) return;
  ...
  @autoreleasepool {
    ...
    if (system->light_linking_pipeline) {
      ...
      [compute setBuffer:(__bridge id<MTLBuffer>)mesh->gpu_cluster_buffer offset:0 atIndex:0];
...
u32 nanite_light_linking_add_light(NaniteLightLinkingSystem *system, const Vec3 *position, 
                                   f32 radius, const Vec3 *color, f32 intensity) {
  if (!system || system->light_count >= system->max_lights) return UINT32_MAX;
...
NaniteMeshShaderRenderer *nanite_mesh_shader_create(id<MTLDevice> device, id<MTLLibrary> shader_lib) {
  NaniteMeshShaderRenderer *renderer = (NaniteMeshShaderRenderer *)calloc(1, sizeof(NaniteMeshShaderRenderer));
  ...
  @autoreleasepool {
    ...
    if (@available(iOS 16.0, macOS 13.0, *)) {
      ...
      id<MTLFunction> mesh_function = [shader_lib newFunctionWithName:@"nanite_mesh_shader"];
      if (mesh_function) {
        NSError *error = nil;
        renderer->mesh_shader_pipeline = [device newComputePipelineStateWithFunction:mesh_function error:&error];
        if (!renderer->mesh_shader_pipeline) {
          LOG_ERROR("Failed to create mesh shader pipeline: %s", [[error localizedDescription] UTF8String]);
        }
      }
      
      // Create amplification shader pipeline
      id<MTLFunction> amplification_function = [shader_lib newFunctionWithName:@"nanite_amplification_shader"];
      if (amplification_function) {
        NSError *error = nil;
        renderer->amplification_shader_pipeline = [device newComputePipelineStateWithFunction:amplification_function error:&error];
        if (!renderer->amplification_shader_pipeline) {
          LOG_ERROR("Failed to create amplification shader pipeline: %s", [[error localizedDescription] UTF8String]);
        }
      }
    } else {
    ...
    renderer->meshlet_buffer = [device newBufferWithLength:renderer->max_meshlets * sizeof(Meshlet) 
                                                 options:MTLResourceStorageModeShared];
    renderer->visible_meshlets_buffer = [device newBufferWithLength:renderer->max_meshlets * sizeof(u32) 
                                                        options:MTLResourceStorageModeShared];
    renderer->indirect_args_buffer = [device newBufferWithLength:sizeof(MTLDrawPrimitivesIndirectArguments) * 1000 
                                                        options:MTLResourceStorageModeShared];
  }
  
  LOG_INFO("Nanite mesh shader renderer created (mesh shaders: %s)", renderer->supports_mesh_shaders ? "enabled" : "disabled");
  return renderer;
}

void nanite_mesh_shader_render(NaniteMeshShaderRenderer *renderer, id<MTLCommandBuffer> cmd, 
                              NaniteMesh *mesh, const Mat4 *view_proj, const Vec3 *camera_pos) {
  ...
  @autoreleasepool {
    ...
    if (renderer->mesh_shader_pipeline) {
      ...
      u32 visible_count = 0; // This would be read back from amplification shader
...
NaniteRenderer *nanite_renderer_create(id<MTLDevice> device,
  ...
  @autoreleasepool {
    ...
    renderer->mesh_shader_renderer = nanite_mesh_shader_create(device, shader_lib);
...
void nanite_render_mesh(NaniteRenderer *renderer,
  ...
  @autoreleasepool {
    ...
    if (renderer->mesh_shader_renderer && renderer->mesh_shader_renderer->supports_mesh_shaders) {
      // Use GPU-driven mesh pipeline
      id<MTLCommandBuffer> cmd = [encoder commandBuffer];
      nanite_mesh_shader_render(renderer->mesh_shader_renderer, cmd, mesh, view_proj, camera_pos);
      return;
    }
    ...
    for (u32 lod = 0; lod < mesh->lod_count; lod++) {
      ...
      for (u32 i = 0; i < cluster_count; i++) {
        ...
        nanite_gpu_cull(renderer, [encoder commandBuffer], mesh, view_proj, camera_pos);
...
void nanite_gpu_cull(NaniteRenderer *renderer, id<MTLCommandBuffer> cmd, 
  ...
  @autoreleasepool {
    ...
    LOG_DEBUG("GPU culling: %u clusters processed, %u total", cluster_count, total_culled);
  }
}
...
VisibilityBuffer* visibility_buffer_create(id<MTLDevice> device, uint32_t width, uint32_t height, uint32_t max_surfaces) {
    VisibilityBuffer *buffer = (VisibilityBuffer*)malloc(sizeof(VisibilityBuffer));
    
    // Create surface ID texture (R32_UINT)
    MTLTextureDescriptor *surfaceDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatR32Uint
                                                                                             width:width
                                                                                            height:height
                                                                                        mipmapped:NO];
    ...
    buffer->material_buffer = [device newBufferWithLength:max_surfaces * sizeof(uint32_t)
                                                   options:MTLResourceStorageModeShared];
    
    // Create surface data buffer
    buffer->surface_data_buffer = [device newBufferWithLength:max_surfaces * sizeof(SurfaceData)
                                                        options:MTLResourceStorageModeShared];
...
void nanite_render_visibility_pass(NaniteRenderer *renderer, 
    ...
    if (!renderer || !encoder || !mesh || !vis_buffer) return;
...
void nanite_render_material_pass(NaniteRenderer *renderer,
    ...
    if (!renderer || !encoder || !vis_buffer) return;
    ...
    [encoder setFragmentBuffer:vis_buffer->surface_data_buffer offset:0 atIndex:1];
...
void nanite_update_visibility_buffer(NaniteRenderer *renderer,
    ...
    if (!renderer || !cmd || !vis_buffer || !visible_surfaces) return;
    
    // Update surface data for visible surfaces
    SurfaceData *surface_data = (SurfaceData *)[vis_buffer->surface_data_buffer contents];
    ...
    for (uint32_t i = 0; i < visible_count && i < vis_buffer->max_surfaces; i++) {
        ...
        surface_data[surface_id].material_id = surface_id % 8; // Example material assignment
...
ProgrammableRaster* programmable_raster_create(id<MTLDevice> device, id<MTLLibrary> shader_lib) {
    ProgrammableRaster *raster = (ProgrammableRaster*)malloc(sizeof(ProgrammableRaster));
    
    // Create programmable rasterization compute pipeline
    NSError *error = nil;
    id<MTLFunction> raster_function = [shader_lib newFunctionWithName:@"nanite_programmable_raster"];
    if (raster_function) {
        raster->programmable_raster_pipeline = [device newComputePipelineStateWithFunction:raster_function error:&error];
        if (!raster->programmable_raster_pipeline) {
            NSLog(@"Failed to create programmable raster pipeline: %@", error.localizedDescription);
        }
    }
    
    // Create raster parameters buffer
    raster->raster_params_buffer = [device newBufferWithLength:256 * sizeof(float)
                                                    options:MTLResourceStorageModeShared];
...
void nanite_render_programmable_raster(NaniteRenderer *renderer,
    ...
    if (!renderer || !cmd || !raster || !mesh) return;
    
    id<MTLComputeCommandEncoder> compute = [cmd computeCommandEncoder];
    [compute setComputePipelineState:raster->programmable_raster_pipeline];
    
    // Bind mesh data
    [compute setBuffer:(__bridge id<MTLBuffer>)mesh->gpu_vertex_buffer offset:0 atIndex:0];
    [compute setBuffer:(__bridge id<MTLBuffer>)mesh->gpu_cluster_buffer offset:0 atIndex:1];
...
LumenIntegration* lumen_integration_create(id<MTLDevice> device, uint32_t width, uint32_t height) {
    LumenIntegration *lumen = (LumenIntegration*)malloc(sizeof(LumenIntegration));
    
    // Create GI texture
    MTLTextureDescriptor *giDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA16Float
                                                                                         width:width
                                                                                        height:height
                                                                                    mipmapped:YES];
    ...
    MTLTextureDescriptor *reflectionDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA16Float
                                                                                               width:width
                                                                                              height:height
                                                                                         mipmapped:YES];
    reflectionDesc.usage = MTLTextureUsageRenderTarget | MTLTextureUsageShaderRead;
    ...
    lumen->lumen_params_buffer = [device newBufferWithLength:64 * sizeof(float)
                                                    options:MTLResourceStorageModeShared];
...
void nanite_render_lumen_gi(NaniteRenderer *renderer,
    ...
    if (!renderer || !encoder || !lumen || !lumen->lumen_enabled) return;
    ...
    float lumen_params[4] = {
        camera_pos->x, camera_pos->y, camera_pos->z, 1.0f
    };
    [encoder setFragmentBytes:lumen_params length:sizeof(lumen_params) atIndex:11];
    
    float light_params[4] = {
        light_direction->x, light_direction->y, light_direction->z, 0.0f
    };
    [encoder setFragmentBytes:light_params length:sizeof(light_params) atIndex:12];
}
...
MeshPainting* mesh_painting_create(id<MTLDevice> device, uint32_t texture_size) {
    MeshPainting *painting = (MeshPainting*)malloc(sizeof(MeshPainting));
    
    // Create paint data buffer
    painting->paint_data_buffer = [device newBufferWithLength:1024 * sizeof(uint32_t)
                                                    options:MTLResourceStorageModeShared];
    
    // Create paint texture
    MTLTextureDescriptor *paintDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA8Unorm
                                                                                          width:texture_size
                                                                                         height:texture_size
                                                                                    mipmapped:NO];
...
void nanite_paint_cluster(MeshPainting *painting, uint32_t cluster_id, uint32_t paint_color) {
    if (!painting || cluster_id >= 1024) return;
...
void nanite_apply_paint_modifications(NaniteRenderer *renderer,
                                  ...
                                  NaniteMesh *mesh) {
    if (!renderer || !painting || !mesh || !painting->painting_enabled) return;
