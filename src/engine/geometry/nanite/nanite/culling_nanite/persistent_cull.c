...
typedef struct nanite_persistent_cull_internal {
    uint32_t id;
    uint32_t flags;
    ResourceHandle visibility_history_buffer;
    bool initialized;
    bool dirty;
...
static void nanite_persistent_cull_cleanup_internal(nanite_persistent_cull_internal_t* item) {
    if (!item) return;
    item->visibility_history_buffer = INVALID_HANDLE;
...
int nanite_persistent_cull_init(void) {
    ...
    g_persistent_cull_ctx.capacity = 256;
...
int nanite_persistent_cull_create(nanite_persistent_cull_handle_t* out_handle, const nanite_persistent_cull_desc_t* desc) {
    ...
    if (g_persistent_cull_ctx.count >= g_persistent_cull_ctx.capacity) {
        uint32_t new_capacity = g_persistent_cull_ctx.capacity * 2;
        nanite_persistent_cull_internal_t* new_items = realloc(g_persistent_cull_ctx.items, new_capacity * sizeof(nanite_persistent_cull_internal_t));
        if (!new_items) return -3;
        g_persistent_cull_ctx.items = new_items;
        g_persistent_cull_ctx.capacity = new_capacity;
    }
...
int nanite_persistent_cull_process_pending(void) {
    if (!g_persistent_cull_ctx.initialized) return 0;

    int processed = 0;
    for (uint32_t i = 0; i < g_persistent_cull_ctx.count; i++) {
        nanite_persistent_cull_internal_t* item = &g_persistent_cull_ctx.items[i];
        if (item->initialized && item->dirty) {
            // Swap visibility buffers (current frame becomes previous frame)
            // Use previous frame visibility for Phase 1 of Two-Pass Culling
            item->dirty = false;
            processed++;
        }
    }
...
size_t nanite_persistent_cull_get_memory_usage(void) {
    size_t total = sizeof(nanite_persistent_cull_context_t);
...
void nanite_persistent_cull_debug_print(void) {
    if (!g_persistent_cull_ctx.initialized) return;
    
    printf("Nanite Persistent Cull Context:\n");
    printf("  Count: %u/%u\n", g_persistent_cull_ctx.count, g_persistent_cull_ctx.capacity);
}
