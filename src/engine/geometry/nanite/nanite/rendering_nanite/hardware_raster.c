...
#include "geometry/nanite/nanite/rendering_nanite/hardware_raster.h"
...
typedef struct nanite_hardware_raster_internal {
    ...
    ResourceHandle depth_stencil_state;
    ResourceHandle rasterizer_state;
    bool initialized;
    bool dirty;
...
static void nanite_hardware_raster_cleanup_internal(nanite_hardware_raster_internal_t* item) {
    if (!item) return;
    item->pipeline = INVALID_HANDLE;
    item->depth_stencil_state = INVALID_HANDLE;
    item->rasterizer_state = INVALID_HANDLE;
...
int nanite_hardware_raster_init(void) {
    ...
    g_hardware_raster_ctx.capacity = 256;
...
int nanite_hardware_raster_create(nanite_hardware_raster_handle_t* out_handle, const nanite_hardware_raster_desc_t* desc) {
    ...
    if (g_hardware_raster_ctx.count >= g_hardware_raster_ctx.capacity) {
        uint32_t new_capacity = g_hardware_raster_ctx.capacity * 2;
        nanite_hardware_raster_internal_t* new_items = realloc(g_hardware_raster_ctx.items, new_capacity * sizeof(nanite_hardware_raster_internal_t));
        if (!new_items) return -3;
        g_hardware_raster_ctx.items = new_items;
        g_hardware_raster_ctx.capacity = new_capacity;
    }
...
int nanite_hardware_raster_process_pending(void) {
    if (!g_hardware_raster_ctx.initialized) return 0;
    ...
    for (uint32_t i = 0; i < g_hardware_raster_ctx.count; i++) {
        ...
        if (item->initialized && item->dirty) {
            // Hardware Rasterization Path:
            // 1. Used for clusters with triangles > 1 pixel in size
            // 2. Uses Vertex Shader + Pixel Shader
            // 3. Pixel shader outputs (cluster_index << 7) | triangle_index to Visibility Buffer
            // 4. Requires "No-Cull" rasterizer state and depth testing enabled
...
size_t nanite_hardware_raster_get_memory_usage(void) {
    size_t total = sizeof(nanite_hardware_raster_context_t);
...
void nanite_hardware_raster_debug_print(void) {
    if (!g_hardware_raster_ctx.initialized) return;
    
    printf("Nanite Hardware Raster Status:\n");
    printf("  Count: %u / %u\n", g_hardware_raster_ctx.count, g_hardware_raster_ctx.capacity);
}
