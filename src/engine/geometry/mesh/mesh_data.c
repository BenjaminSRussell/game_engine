...
// Helper functions replaced by mesh_bounds.h implementation
...
int geometry_mesh_data_create(geometry_mesh_data_handle_t* out_handle, const geometry_mesh_data_desc_t* desc)
{
    ...
    // Calculate bounds
    int res = geometry_bounds_compute_aabb(
        item->vertex_buffer,
        desc->vertex_count,
        fmt_info.vertex_stride,
        0, // Position offset assume 0 for now as per original code
        &item->data.bounds
    );
    if (res == 0) {
        geometry_bounds_sphere_from_aabb(&item->data.bounds, &item->data.bounding_sphere);
    }
