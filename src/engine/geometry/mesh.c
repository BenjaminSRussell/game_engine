...
void mesh_set_vertices(mesh_t* mesh, const vertex_t* vertices, u32 count, u32 offset) {
    ...
    memcpy(&mesh->vertices[offset], vertices, count * sizeof(vertex_t));
...
void mesh_add_submesh(mesh_t* mesh, submesh_t submesh) {
    ...
    submesh_t* new_submeshes = (submesh_t*)realloc(mesh->submeshes, 
                                                   (mesh->submesh_count + 1) * sizeof(submesh_t));
    if (!new_submeshes) {
...
void mesh_calculate_bounds(mesh_t* mesh) {
    ...
    for (u32 i = 1; i < mesh->vertex_count; i++) {
        ...
        if (pos.x < mesh->bounds.min.x) mesh->bounds.min.x = pos.x;
        if (pos.y < mesh->bounds.min.y) mesh->bounds.min.y = pos.y;
        if (pos.z < mesh->bounds.min.z) mesh->bounds.min.z = pos.z;
        
        if (pos.x > mesh->bounds.max.x) mesh->bounds.max.x = pos.x;
        if (pos.y > mesh->bounds.max.y) mesh->bounds.max.y = pos.y;
        if (pos.z > mesh->bounds.max.z) mesh->bounds.max.z = pos.z;
    }
...
void mesh_calculate_tangents(mesh_t* mesh) {
    ...
    for (u32 i = 0; i < mesh->index_count; i += 3) {
        ...
        Vec2 uv0 = mesh->vertices[i0].uv;
        Vec2 uv1 = mesh->vertices[i1].uv;
        Vec2 uv2 = mesh->vertices[i2].uv;
    ...
    for (u32 i = 0; i < mesh->vertex_count; i++) {
        ...
        f32 handedness = (expected_bitangent.x * bitangent.x + 
                         ...
                         expected_bitangent.z * bitangent.z) < 0.0f ? -1.0f : 1.0f;
