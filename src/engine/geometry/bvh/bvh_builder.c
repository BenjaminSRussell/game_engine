...
#include <float.h>
#include <stdatomic.h>
#include <stdlib.h> // For malloc/free of job data
...
#define BVH_PARALLEL_THRESHOLD 4096

// External thread pool from engine core
...
static inline AABB compute_centroid_bounds(const bvh_primitive_t* prims, const uint32_t* indices, uint32_t count) {
    AABB bounds;
    bounds.min = vec3(FLT_MAX, FLT_MAX, FLT_MAX);
    bounds.max = vec3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
    
    for (uint32_t i = 0; i < count; i++) {
        Vec3 c = prims[indices[i]].centroid;
...
static inline uint32_t find_best_split_axis(const AABB* centroid_bounds) {
    Vec3 extent = vec3_sub(centroid_bounds->max, centroid_bounds->min);
...
static float evaluate_sah_split(
    ...
    AABB centroid_bounds = compute_centroid_bounds(prims, indices + start, end - start);
    Vec3 extent = vec3_sub(centroid_bounds.max, centroid_bounds.min);
    ...
    AABB bounds_left[BVH_SAH_BINS - 1];
    AABB bounds_right[BVH_SAH_BINS - 1];
    uint32_t count_left[BVH_SAH_BINS - 1];
    uint32_t count_right[BVH_SAH_BINS - 1];
    
    // Left sweep
    AABB accum_bounds = bins[0].bounds;
...
static void build_bvh_job(void* data) {
    bvh_job_data_t* job_data = (bvh_job_data_t*)data;
    ...
    if (job_data->ctx->jobs_running) {
        atomic_fetch_sub(&job_data->ctx->jobs_running, 1);
    }
    
    // We allocated the job_data, so we free it here
    free(job_data);
}

/* Recursive BVH construction (Parallel aware) */
static void build_bvh_recursive(
    bvh_build_context_t* ctx,
    uint32_t start,
    uint32_t end,
    ...
    for (uint32_t i = start; i < end; i++) {
        const AABB* prim_bounds = &ctx->primitives[ctx->prim_indices[i]].bounds;
    ...
    AABB centroid_bounds = compute_centroid_bounds(ctx->primitives, ctx->prim_indices + start, prim_count);
    ...
    // Allocate children atomically
    uint32_t left_idx = atomic_fetch_add(&ctx->node_count, 1);
    uint32_t right_idx = atomic_fetch_add(&ctx->node_count, 1);
    
    node->left_child = left_idx;
    node->right_child = right_idx;
    
    // Check if we should parallelize
    // Parallelize if we have enough primitives and thread pool is available
    if (prim_count > BVH_PARALLEL_THRESHOLD && g_thread_pool) {
        // Spawn job for left child
        bvh_job_data_t* job_data = malloc(sizeof(bvh_job_data_t));
        if (job_data) {
            job_data->ctx = ctx;
            job_data->start = start;
            job_data->end = mid;
            job_data->depth = depth + 1;
            job_data->node_idx = left_idx;
            
            atomic_fetch_add(&ctx->jobs_running, 1);
            thread_pool_submit(g_thread_pool, build_bvh_job, job_data, 1); // Priority 1 (Normal?)
        } else {
            // Fallback to serial
            build_bvh_recursive(ctx, start, mid, depth + 1, left_idx);
        }
        
        // Process right child in current thread
        build_bvh_recursive(ctx, mid, end, depth + 1, right_idx);
    } else {
        // Serial build
        build_bvh_recursive(ctx, start, mid, depth + 1, left_idx);
        build_bvh_recursive(ctx, mid, end, depth + 1, right_idx);
...
int geometry_bvh_builder_update(geometry_bvh_builder_handle_t handle, const void* data, size_t size) {
    ...
    // Atomically initialize counters
    atomic_init(&ctx->node_count, 0);
    atomic_init(&ctx->jobs_running, 0);
    
    ctx->primitives = MALLOC_GEOMETRY(prim_count * sizeof(bvh_primitive_t));
    ctx->prim_indices = MALLOC_GEOMETRY(prim_count * sizeof(uint32_t));
    ctx->nodes = MALLOC_GEOMETRY(ctx->node_capacity * sizeof(bvh_node_t));
    ...
    // Allocate root node
    uint32_t root_idx = atomic_fetch_add(&ctx->node_count, 1);
    
    // Build BVH tree (start root job or run)
    // We run root on main thread but treat as parallel entry point
    atomic_fetch_add(&ctx->jobs_running, 1); // Account for main thread "job"
    
    build_bvh_recursive(ctx, 0, prim_count, 0, root_idx);
    
    atomic_fetch_sub(&ctx->jobs_running, 1); // Main thread finished
    
    // Wait for all child jobs to complete
    while (atomic_load(&ctx->jobs_running) > 0) {
        // Spin/yield
        // TODO: Use better wait mechanism if available
    }
