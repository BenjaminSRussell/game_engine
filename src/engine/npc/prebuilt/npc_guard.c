...
#include <npc/prebuilt/npc_guard.h>
#include <stdbool.h>
#include <stdint.h>
#include <math.h>
...
bool guard_can_see_player(GuardData *data, float guard_pos[3], float guard_forward[3], 
    ...
    if (angle < data->fov_angle / 2.0f) {
        ...
        // Raycast from guard eye position to player position
        // Layer mask checks world geometry only (not other guards/dynamic objects if possible)
        if (physics_raycast(guard_pos, to_player, distance, PHYS_LAYER_WORLD, &result)) {
            // If we hit something closer than the player distance, checking if it obscures view
            if (result.distance < distance - 0.5f) { // 0.5f buffer
                 // We hit something before the player -> Occluded
                 return false; 
            }
        }
        return true;
