...
static inline uint32_t pool_get_block_index(PoolAllocator *allocator, void *ptr) {
    return (uint32_t)((char*)ptr - (char*)allocator->buffer) / allocator->actual_block_size;
}
...
static inline void pool_set_allocated(PoolAllocator *allocator, uint32_t index, bool allocated) {
    if (allocated) {
...
PoolAllocator *pool_allocator_create(size_t block_size, size_t block_count, size_t alignment) {
...
void *pool_allocator_alloc(PoolAllocator *allocator) {
    ...
    if (allocator->debug_mode) {
        ...
        uint32_t *back_guard = (uint32_t*)((char*)block + 16 + allocator->block_size);
...
void pool_allocator_free(PoolAllocator *allocator, void *ptr) {
    ...
    if (block < allocator->buffer || 
        block >= (char*)allocator->buffer + allocator->actual_block_size * allocator->block_count) {
        pool_unlock(allocator);
        if (allocator->debug_mode) {
    ...
    if (((uintptr_t)block - (uintptr_t)allocator->buffer) % allocator->actual_block_size != 0) {
        pool_unlock(allocator);
        if (allocator->debug_mode) {
    ...
    if (allocator->debug_mode) {
        ...
        if (*front_guard != GUARD_MAGIC) {
            printf("Pool allocator: Front guard corrupted at %p (found 0x%08X, expected 0x%08X)\n",
                   ptr, *front_guard, GUARD_MAGIC);
            assert(false && "Pool allocator: front guard corrupted");
        }
        
        if (*back_guard != GUARD_MAGIC) {
            printf("Pool allocator: Back guard corrupted at %p (found 0x%08X, expected 0x%08X)\n",
...
void pool_allocator_destroy(PoolAllocator *allocator) {
    ...
    if (leaked_blocks > 0) {
        if (allocator->debug_mode) {
            printf("Pool allocator: Memory leak detected - %u blocks still allocated\n", 
                   leaked_blocks);
...
void pool_allocator_set_thread_safe(PoolAllocator *allocator, bool thread_safe) {
    if (!allocator || allocator->signature != POOL_SIGNATURE) {
        return;
    }
...
void pool_allocator_get_stats(PoolAllocator *allocator, uint32_t *free_count, 
    ...
    if (free_count) *free_count = free_cnt;
    if (peak_usage) *peak_usage = allocator->peak_usage;
    if (allocated_count) *allocated_count = allocator->block_count - free_cnt;
}
...
bool pool_allocator_validate(PoolAllocator *allocator) {
    ...
    while (current) {
        ...
        if (current < allocator->buffer || 
            current >= (char*)allocator->buffer + allocator->actual_block_size * allocator->block_count) {
            return false;
        }
        
        // Validate alignment
        if (((uintptr_t)current - (uintptr_t)allocator->buffer) % allocator->actual_block_size != 0) {
            return false;
        }
...
void pool_allocator_debug_dump(PoolAllocator *allocator) {
    ...
    printf("Peak Usage: %u blocks (%.1f%%)\n", 
           allocator->peak_usage, (double)allocator->peak_usage / allocator->block_count * 100.0);
