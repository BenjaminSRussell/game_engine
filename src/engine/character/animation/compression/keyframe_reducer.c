...
static Vec3 lerp_scale(Vec3 s0, Vec3 s1, f32 t) { return vec3_lerp(s0, s1, t); }

static bool can_remove_keyframe(AnimationKeyframe *prev,
                                AnimationKeyframe *current,
                                AnimationKeyframe *next, f32 pos_tol,
                                f32 rot_tol, f32 scale_tol) {
...
AnimationClip *keyframe_reducer_reduce(KeyframeReducer *reducer) {
  if (!reducer || !reducer->original_clip)
    return NULL;
  ...
  reduced->channels =
      malloc(sizeof(AnimationChannel) * original->channel_count);

  for (i32 channel_idx = 0; channel_idx < original->channel_count;
       channel_idx++) {
    ...
    if (orig_channel->keyframe_count <= 2) {
      ...
      red_channel->keyframes =
          malloc(sizeof(AnimationKeyframe) * orig_channel->keyframe_count);
    ...
    for (i32 i = 1; i < orig_channel->keyframe_count - 1; i++) {
      ...
      if (!can_remove_keyframe(prev, current, next, reducer->position_tolerance,
...
static Vec3 evaluate_bezier_position(BezierCurve *curve, f32 t) {
  ...
  Vec3 result;
  result.x = mt3 * curve->p0.x + 3.0f * mt2 * t * curve->p1.x +
             3.0f * mt * t2 * curve->p2.x + t3 * curve->p3.x;
  result.y = mt3 * curve->p0.y + 3.0f * mt2 * t * curve->p1.y +
             3.0f * mt * t2 * curve->p2.y + t3 * curve->p3.y;
  result.z = mt3 * curve->p0.z + 3.0f * mt2 * t * curve->p1.z +
             3.0f * mt * t2 * curve->p2.z + t3 * curve->p3.z;
...
static void fit_bezier_curve(AnimationKeyframe *keyframes, i32 count,
                             BezierCurve *curve) {
  if (count < 2)
    return;
...
AnimationClip *keyframe_reducer_fit_curves(KeyframeReducer *reducer) {
  if (!reducer || !reducer->reduced_clip)
    return NULL;
  ...
  curve_clip->channels =
      malloc(sizeof(AnimationChannel) * reduced->channel_count);

  for (i32 channel_idx = 0; channel_idx < reduced->channel_count;
       channel_idx++) {
    AnimationChannel *red_channel = &reduced->channels[channel_idx];
    ...
    curve_channel->keyframes =
        malloc(sizeof(AnimationKeyframe) * red_channel->keyframe_count);
    ...
    if (red_channel->keyframe_count > 2) {
      BezierCurve curve;
      fit_bezier_curve(red_channel->keyframes, red_channel->keyframe_count,
                       &curve);
...
void keyframe_reducer_set_tolerance(KeyframeReducer *reducer, f32 pos_tol,
                                    f32 rot_tol, f32 scale_tol) {
  if (reducer) {
...
i32 keyframe_reducer_get_reduction_stats(KeyframeReducer *reducer,
                                         i32 *original_count,
                                         i32 *reduced_count) {
  if (!reducer || !reducer->original_clip || !reducer->reduced_clip)
    return 0;
  ...
  if (original_count)
    *original_count = orig_total;
  if (reduced_count)
    *reduced_count = red_total;

  return orig_total > 0
             ? (i32)(((f32)(orig_total - red_total) / (f32)orig_total) * 100.0f)
             : 0;
}
