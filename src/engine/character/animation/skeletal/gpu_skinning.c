...
#include "character/animation/skeletal/gpu_skinning.h"
#include <math.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
typedef struct bone_weight {
  ...
  float weights[GPU_SKINNING_MAX_WEIGHTS_PER_VERTEX];   // Blend weights (sum
                                                        // to 1.0)
} bone_weight_t;
...
static bool
animation_gpu_skinning_validate(const animation_gpu_skinning_internal_t *item) {
  if (!item)
    return false;
  if (!item->initialized)
    return false;
...
static void animation_gpu_skinning_cleanup_internal(
    animation_gpu_skinning_internal_t *item) {
  if (!item)
    return;
...
int animation_gpu_skinning_init(void) {
  ...
  // g_gpu_skinning_ctx.lbs_compute_pipeline =
  // create_compute_pipeline("shaders/lbs_skinning.comp"); Dual quaternion
  // skinning pipeline g_gpu_skinning_ctx.dq_compute_pipeline =
  // create_compute_pipeline("shaders/dq_skinning.comp");
...
int animation_gpu_skinning_create(animation_gpu_skinning_handle_t *out_handle,
  ...
  if (g_gpu_skinning_ctx.count >= g_gpu_skinning_ctx.capacity) {
    ...
    animation_gpu_skinning_internal_t *new_items =
        realloc(g_gpu_skinning_ctx.items,
    ...
    memset(&new_items[g_gpu_skinning_ctx.capacity], 0,
           (new_capacity - g_gpu_skinning_ctx.capacity) *
               sizeof(animation_gpu_skinning_internal_t));
...
int animation_gpu_skinning_setup(animation_gpu_skinning_handle_t handle,
                                 uint32_t bone_count, uint32_t vertex_count,
                                 skinning_method_t method) {
  if (handle.id >= g_gpu_skinning_ctx.count) {
    return -1;
  }
  ...
  animation_gpu_skinning_internal_t *item =
      &g_gpu_skinning_ctx.items[handle.id];
  if (!item->initialized) {
    return -2;
  }

  int result =
      skinning_buffers_allocate(&item->buffers, bone_count, vertex_count);
  if (result != 0) {
    return result;
  }
  ...
  item->dispatch_x = (vertex_count + GPU_SKINNING_WORKGROUP_SIZE - 1) /
                     GPU_SKINNING_WORKGROUP_SIZE;
  item->dispatch_y = 1;
  item->dispatch_z = 1;
...
int animation_gpu_skinning_upload_matrices(
    animation_gpu_skinning_handle_t handle, const void *matrices,
    uint32_t count) {
...
int animation_gpu_skinning_update(animation_gpu_skinning_handle_t handle,
  ...
  if (data && size >= item->buffers.bone_count * sizeof(mat4_t)) {
    return animation_gpu_skinning_upload_matrices(handle, data,
                                                  item->buffers.bone_count);
  }
...
int animation_gpu_skinning_get_info(animation_gpu_skinning_handle_t handle,
  ...
  const animation_gpu_skinning_internal_t *item =
      &g_gpu_skinning_ctx.items[handle.id];
  out_info->id = item->id;
...
size_t animation_gpu_skinning_get_memory_usage(void) {
  ...
  total +=
      g_gpu_skinning_ctx.capacity * sizeof(animation_gpu_skinning_internal_t);
