...
#include "character/animation/skeletal/animation_player.h"
#include <math.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
static void sample_channel(animation_channel_t *channel, float time,
  ...
  if (channel->interp == INTERP_LINEAR) {
    ...
    out_vec3->x =
        h00 * frame0->value.vec3.x + h10 * dt * frame0->out_tangent.vec3.x +
        h01 * frame1->value.vec3.x + h11 * dt * frame1->in_tangent.vec3.x;
    out_vec3->y =
        h00 * frame0->value.vec3.y + h10 * dt * frame0->out_tangent.vec3.y +
        h01 * frame1->value.vec3.y + h11 * dt * frame1->in_tangent.vec3.y;
    out_vec3->z =
        h00 * frame0->value.vec3.z + h10 * dt * frame0->out_tangent.vec3.z +
...
static void animation_animation_player_cleanup_internal(
    animation_animation_player_internal_t *item) {
  if (!item)
    return;
...
int animation_animation_player_init(void) {
  ...
  g_animation_player_ctx.items =
      calloc(g_animation_player_ctx.capacity,
...
void animation_animation_player_shutdown(void) {
  ...
  for (uint32_t i = 0; i < g_animation_player_ctx.count; i++) {
    animation_animation_player_cleanup_internal(
        &g_animation_player_ctx.items[i]);
  }
...
int animation_animation_player_create(
    animation_animation_player_handle_t *out_handle,
  ...
  animation_animation_player_internal_t *item =
      &g_animation_player_ctx.items[index];
...
void animation_animation_player_destroy(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return;
  }

  animation_animation_player_cleanup_internal(
      &g_animation_player_ctx.items[handle.id]);
}
...
int animation_animation_player_setup(animation_animation_player_handle_t handle,
  ...
  if (!item->output_translations || !item->output_rotations ||
      !item->output_scales) {
    return -3;
  }
...
int animation_animation_player_play(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_pause(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_stop(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_set_time(
    animation_animation_player_handle_t handle, float time) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_set_speed(
    animation_animation_player_handle_t handle, float speed) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_set_loop_mode(
    animation_animation_player_handle_t handle, loop_mode_t mode) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_advance(
    animation_animation_player_handle_t handle, float delta_time) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
int animation_animation_player_sample(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
  ...
  for (uint32_t c = 0; c < clip->channel_count; c++) {
    ...
    if (bone >= item->output_bone_count)
      continue;
...
int animation_animation_player_update(
    animation_animation_player_handle_t handle, const void *data, size_t size) {
  if (handle.id >= g_animation_player_ctx.count) {
    return -1;
  }
...
bool animation_animation_player_is_valid(
    animation_animation_player_handle_t handle) {
  if (handle.id >= g_animation_player_ctx.count) {
    return false;
  }
...
int animation_animation_player_get_info(
    animation_animation_player_handle_t handle,
  ...
  const animation_animation_player_internal_t *item =
      &g_animation_player_ctx.items[handle.id];
  out_info->id = item->id;
...
void animation_animation_player_mark_dirty(
    animation_animation_player_handle_t handle) {
  if (handle.id < g_animation_player_ctx.count) {
...
size_t animation_animation_player_get_memory_usage(void) {
  ...
  total += g_animation_player_ctx.capacity *
           sizeof(animation_animation_player_internal_t);
