...
static Vec3 look_at_world_to_local(Skeleton *skeleton, i32 bone_index,
                                   Vec3 world_pos) {
  if (bone_index < 0 || bone_index >= (i32)skeleton->bone_count) {
    return world_pos;
  }

  // Check global_transforms is Mat4*.
  Mat4 bone_matrix = skeleton->global_transforms[bone_index];
  Mat4 inverse_bone = mat4_inverse(bone_matrix);

  // Mat4 has function mat4_transform_point which handles w=1
  // or if manual vec4 mul is needed check mat4.h API.
  // Assuming mat4_transform_point exists and works for point.
  // Wait, logs didn't complain about mat4_mul_vec4, meaning it might exist or
  // wasn't compiled yet. mat4.h contents showed mat4_transform_point. Not
  // mat4_mul_vec4. I'll use mat4_transform_point.

  return mat4_transform_point(inverse_bone, world_pos);
...
static Quat look_at_clamp_angles(Quat rotation, Vec2 yaw_clamp,
                                 Vec2 pitch_clamp) {
  f32 yaw, pitch;
...
static void look_at_apply_spine_distribution(Skeleton *skeleton, LookAtIK *ik,
  ...
  f32 total =
      ik->head_distribution + ik->neck_distribution + ik->chest_distribution;
  if (total <= 0.0f) {
    return;
  }
  ...
  if (ik->chest_bone_index >= 0 &&
      ik->chest_bone_index < (i32)skeleton->bone_count) {
    Quat chest_rotation =
        quat_slerp(quat_identity(), target_rotation, chest_weight * ik->weight);
    Mat4 chest_matrix = quat_to_mat4(chest_rotation);
    skeleton->global_transforms[ik->chest_bone_index] = mat4_mul(
        skeleton->global_transforms[ik->chest_bone_index], chest_matrix);
  }

  if (ik->neck_bone_index >= 0 &&
      ik->neck_bone_index < (i32)skeleton->bone_count) {
    Quat neck_rotation =
        quat_slerp(quat_identity(), target_rotation, neck_weight * ik->weight);
  ...
  if (ik->head_bone_index >= 0 &&
      ik->head_bone_index < (i32)skeleton->bone_count) {
    Quat head_rotation =
        quat_slerp(quat_identity(), target_rotation, head_weight * ik->weight);
...
static void look_at_update_eye_bones(Skeleton *skeleton, LookAtIK *ik,
                                     Vec3 target_pos) {
  ...
  Vec3 head_pos =
      (ik->head_bone_index >= 0)
          ? vec3(skeleton->global_transforms[ik->head_bone_index].data[3][0],
                 skeleton->global_transforms[ik->head_bone_index].data[3][1],
                 skeleton->global_transforms[ik->head_bone_index].data[3][2])
          : vec3_zero();
  ...
  if (ik->left_eye_bone_index >= 0 &&
      ik->left_eye_bone_index < (i32)skeleton->bone_count) {
    Vec3 left_offset = vec3(-0.05f, 0.0f, 0.0f); // Approx eye separation
    Vec3 left_target =
        vec3_add(target_pos, vec3_mul(left_offset, convergence_factor));
    Quat left_rotation =
        look_at_calculate_rotation(vec3(0.0f, 0.0f, 1.0f), left_target);

    Mat4 left_eye_matrix = quat_to_mat4(left_rotation);
    skeleton->global_transforms[ik->left_eye_bone_index] = mat4_mul(
        skeleton->global_transforms[ik->left_eye_bone_index], left_eye_matrix);
  }

  if (ik->right_eye_bone_index >= 0 &&
      ik->right_eye_bone_index < (i32)skeleton->bone_count) {
    Vec3 right_offset = vec3(0.05f, 0.0f, 0.0f); // Approx eye separation
    Vec3 right_target =
        vec3_add(target_pos, vec3_mul(right_offset, convergence_factor));
    Quat right_rotation =
        look_at_calculate_rotation(vec3(0.0f, 0.0f, 1.0f), right_target);

    Mat4 right_eye_matrix = quat_to_mat4(right_rotation);
    skeleton->global_transforms[ik->right_eye_bone_index] =
        mat4_mul(skeleton->global_transforms[ik->right_eye_bone_index],
                 right_eye_matrix);
  }
}
...
LookAtIK *look_at_create(i32 head_bone, i32 neck_bone, i32 chest_bone) {
  LookAtIK *ik = (LookAtIK *)malloc(sizeof(LookAtIK));
  if (!ik)
    return NULL;
...
void look_at_update(LookAtIK *ik, Skeleton *skeleton, f32 dt) {
  ...
  Vec3 local_target =
      look_at_world_to_local(skeleton, ik->head_bone_index, smoothed_target);

  // Calculate rotation
  Quat target_rotation =
      look_at_calculate_rotation(vec3(0.0f, 0.0f, 1.0f), local_target);

  // Clamp angles
  target_rotation =
      look_at_clamp_angles(target_rotation, ik->clamp_yaw, ik->clamp_pitch);
  ...
  if (ik->enable_spine_distribution) {
    ...
    if (ik->head_bone_index >= 0 &&
        ik->head_bone_index < (i32)skeleton->bone_count) {
      Quat head_rotation =
          quat_slerp(quat_identity(), target_rotation, ik->weight);
      Mat4 head_matrix = quat_to_mat4(head_rotation);
      skeleton->global_transforms[ik->head_bone_index] = mat4_mul(
          skeleton->global_transforms[ik->head_bone_index], head_matrix);
    }
  }
...
void look_at_set_distribution_weights(LookAtIK *ik, f32 head, f32 neck,
                                      f32 chest) {
  if (ik) {
