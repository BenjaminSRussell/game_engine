...
static void hand_ik_solve_two_bone(HandIK *ik, Skeleton *skeleton,
                                   Vec3 target_pos) {
  if (ik->shoulder_bone_index < 0 || ik->elbow_bone_index < 0 ||
      ik->wrist_bone_index < 0) {
    return;
  }

  // Get bone positions
  // Check global_transforms type. Using .data accesses if .m is 1D.
  Vec3 shoulder_pos =
      vec3(skeleton->global_transforms[ik->shoulder_bone_index].data[3][0],
           skeleton->global_transforms[ik->shoulder_bone_index].data[3][1],
           skeleton->global_transforms[ik->shoulder_bone_index].data[3][2]);

  Vec3 elbow_pos =
      vec3(skeleton->global_transforms[ik->elbow_bone_index].data[3][0],
           skeleton->global_transforms[ik->elbow_bone_index].data[3][1],
           skeleton->global_transforms[ik->elbow_bone_index].data[3][2]);
  ...
  if (ik->enable_spherical_constraints) {
    target_distance =
        clampf(target_distance, ik->min_reach_distance, ik->max_reach_distance);
  ...
  Vec3 elbow_offset = vec3_mul(
      bend_direction, sinf(elbow_angle) * elbow_distance_from_shoulder);
  Vec3 forward_offset =
      vec3_mul(shoulder_to_target_norm,
               cosf(elbow_angle) * elbow_distance_from_shoulder);
  Vec3 new_elbow_pos =
      vec3_add(shoulder_pos, vec3_add(forward_offset, elbow_offset));

  // Apply rotations to bones
  // Shoulder rotation
  Vec3 shoulder_to_elbow =
      vec3_normalize(vec3_sub(new_elbow_pos, shoulder_pos));
  Quat shoulder_rotation =
      quat_from_vectors(vec3(0, -1, 0), shoulder_to_elbow); // Down
  Mat4 shoulder_matrix = quat_to_mat4(shoulder_rotation);
  skeleton->global_transforms[ik->shoulder_bone_index] = mat4_mul(
      skeleton->global_transforms[ik->shoulder_bone_index], shoulder_matrix);

  // Elbow rotation
  Vec3 elbow_to_wrist = vec3_normalize(vec3_sub(target_pos, new_elbow_pos));
  Vec3 elbow_to_shoulder =
      vec3_normalize(vec3_sub(shoulder_pos, new_elbow_pos));
...
static void hand_ik_update_reach(HandIK *ik, Skeleton *skeleton) {
  ...
  // In a full implementation, this would be determined by interactable object
  // anchors
...
static void hand_ik_update_finger_posing(HandIK *ik, Skeleton *skeleton) {
  ...
  if (ik->use_fist_pose) {
    ...
    finger_rotation =
        quat_from_axis_angle(vec3_right(), ik->finger_curl_amount);
  } else if (ik->use_point_pose) {
    // Pointing gesture - index finger extended, others curled
    finger_rotation =
        quat_from_axis_angle(vec3_right(), ik->finger_curl_amount * 0.5f);
  } else if (ik->use_open_pose) {
    // Open hand - minimal curl
    finger_rotation =
        quat_from_axis_angle(vec3_right(), ik->finger_curl_amount * 0.1f);
  } else {
    // Procedural curl based on object size (simplified)
    f32 object_size =
        vec3_length(ik->target_position); // Simplified object size estimation
    f32 curl_factor = clampf(object_size * 0.5f, 0.0f, 1.0f);
    finger_rotation = quat_from_axis_angle(
        vec3_right(), curl_factor * ik->finger_curl_amount);
  }

  // Apply thumb spread
  // Assuming vec3_forward() is (0,0,1)
  Quat thumb_rotation = quat_from_axis_angle(vec3(0, 0, 1), ik->thumb_spread);
  ...
  if (ik->wrist_bone_index >= 0 &&
      ik->wrist_bone_index < (i32)skeleton->bone_count) {
    Mat4 finger_matrix = quat_to_mat4(finger_rotation);
    skeleton->global_transforms[ik->wrist_bone_index] = mat4_mul(
        skeleton->global_transforms[ik->wrist_bone_index], finger_matrix);
  }
}
...
static void hand_ik_update_two_handed_grip(HandIK *ik, Skeleton *skeleton,
                                           HandIK *other_hand) {
  ...
  if (ik->is_master_hand) {
    ...
    if (other_hand && other_hand->wrist_bone_index >= 0) {
      Vec3 foregrip_pos =
          vec3_sub(weapon_pos, vec3_mul(weapon_forward, ik->grip_distance));
...
HandIK *hand_ik_create(i32 shoulder, i32 elbow, i32 wrist) {
  HandIK *ik = (HandIK *)malloc(sizeof(HandIK));
  if (!ik)
    return NULL;
  ...
  ik->elbow_pole_target = vec3(0, 0, 1); // Forward
  ...
  ik->weapon_direction = vec3(0, 0, 1); // Forward
