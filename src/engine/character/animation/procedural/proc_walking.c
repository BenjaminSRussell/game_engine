...
ProceduralWalk* procedural_walk_create(i32 num_legs, Skeleton* target_skeleton) {
    if (num_legs <= 0 || num_legs > MAX_LEGS || !target_skeleton) return NULL;
    
    ProceduralWalk* walk = malloc(sizeof(ProceduralWalk));
    if (!walk) return NULL;
...
void procedural_walk_update(ProceduralWalk* walk, f32 dt, Vec3 velocity) {
    if (!walk || !walk->target_skeleton) return;
    ...
    for (i32 i = 0; i < walk->num_legs; i++) {
        ...
        if (leg->is_stepping) {
            ...
            if (leg->step_progress >= 1.0f) {
                ...
                leg->current_foot_pos = parabola_step(
                    leg->old_foot_pos, leg->new_foot_pos, leg->step_progress, walk->step_height);
            }
        }
    }
}
...
void procedural_walk_update_terrain_adaptation(ProceduralWalk* walk) {
    if (!walk || !walk->terrain_adaptation_enabled) return;
