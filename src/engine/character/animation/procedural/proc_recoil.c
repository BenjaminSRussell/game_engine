...
ProceduralRecoil* procedural_recoil_create(i32 bone_index, Skeleton* target_skeleton) {
    ...
    recoil->data.kick_translation = vec3_create(0.0f, -0.05f, 0.1f);
    recoil->data.kick_rotation = vec3_create(-0.1f, 0.05f, 0.0f);
    ...
    recoil->spring.current_pos = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->spring.target_pos = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->spring.velocity = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->spring.acceleration = vec3_create(0.0f, 0.0f, 0.0f);
    
    recoil->spring.current_rot = quat_identity();
    recoil->spring.target_rot = quat_identity();
    recoil->spring.angular_velocity = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->spring.angular_acceleration = vec3_create(0.0f, 0.0f, 0.0f);
    ...
    recoil->rest_position = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->rest_rotation = quat_identity();
    recoil->accumulated_offset = vec3_create(0.0f, 0.0f, 0.0f);
    recoil->climb_direction = vec3_create(0.0f, 1.0f, 0.0f);
...
static void update_spring_mass(SpringMassSystem* spring, f32 dt) {
    ...
    Vec3 pos_diff = vec3_subtract(spring->target_pos, spring->current_pos);
    spring->acceleration = vec3_scale(pos_diff, spring->spring_constant / spring->mass);
    spring->acceleration = vec3_subtract(spring->acceleration, vec3_scale(spring->velocity, spring->damping_constant / spring->mass));
    
    // Semi-implicit Euler integration
    spring->velocity = vec3_add(spring->velocity, vec3_scale(spring->acceleration, dt));
    spring->current_pos = vec3_add(spring->current_pos, vec3_scale(spring->velocity, dt));
    
    // Angular spring-mass simulation
    Quat rot_diff = quat_multiply(quat_inverse(spring->current_rot), spring->target_rot);
    Vec3 rot_axis;
    f32 rot_angle;
    ...
    spring->angular_acceleration = vec3_scale(rot_axis, (rot_angle * spring->spring_constant) / spring->mass);
    spring->angular_acceleration = vec3_subtract(spring->angular_acceleration, 
                                               vec3_scale(spring->angular_velocity, spring->damping_constant / spring->mass));
    
    spring->angular_velocity = vec3_add(spring->angular_velocity, vec3_scale(spring->angular_acceleration, dt));
    ...
    if (angular_speed > 0.001f) {
        ...
        spring->current_rot = quat_multiply(delta_rot, spring->current_rot);
    }
    ...
    if (vec3_length_squared(pos_diff) < pos_threshold * pos_threshold && 
        ...
        spring->velocity = vec3_create(0.0f, 0.0f, 0.0f);
        spring->current_rot = spring->target_rot;
        spring->angular_velocity = vec3_create(0.0f, 0.0f, 0.0f);
    }
}
...
void procedural_recoil_fire(ProceduralRecoil* recoil) {
    ...
    Vec3 kick_trans = vec3_scale(recoil->data.kick_translation, recoil->data.intensity * random_factor);
    Vec3 kick_rot = vec3_scale(recoil->data.kick_rotation, recoil->data.intensity * random_factor);
    
    // Apply impulsive force to velocity
    recoil->spring.velocity = vec3_add(recoil->spring.velocity, vec3_scale(kick_trans, 1.0f / recoil->data.snap_duration));
    recoil->spring.angular_velocity = vec3_add(recoil->spring.angular_velocity, vec3_scale(kick_rot, 1.0f / recoil->data.snap_duration));
...
static void update_procedural_patterns(ProceduralRecoil* recoil, f32 dt) {
    ...
    Vec3 climb_offset_vec = vec3_scale(recoil->climb_direction, climb_offset);
    ...
    Vec3 noise_offset = vec3_create(noise_x, 0.0f, noise_z);
...
void procedural_recoil_update(ProceduralRecoil* recoil, f32 dt) {
    ...
    if (recoil->bone_index < recoil->target_skeleton->bone_count) {
        ...
        Quat new_rot = quat_multiply(recoil->spring.current_rot, bone_rot);
        
        // Update bone transform
        Mat4 new_transform = mat4_from_transform(new_pos, new_rot, vec3_create(1.0f, 1.0f, 1.0f));
        recoil->target_skeleton->global_transforms[recoil->bone_index] = new_transform;
    }
}
