...
#include "../../include/render/gpu_texture.h"
...
bool gpu_texture_create(GPUTextureManager* manager, u32 texture_id, const char* name,
    ...
    // In production, would use vkCreateImage, allocate memory, bind, and upload data
...
bool gpu_texture_upload_async(GPUTextureManager* manager, u32 texture_id,
    ...
    // In production, would create staging buffer, copy data, submit transfer queue command
...
GPUTexture* gpu_texture_get(GPUTextureManager* manager, u32 texture_id) {
    ...
    return manager->textures[texture_id].initialized ? &manager->textures[texture_id] : NULL;
}
...
bool gpu_texture_generate_mipmaps(GPUTextureManager* manager, u32 texture_id) {
    if (!manager || !manager->initialized || texture_id >= manager->texture_count) {
        return false;
    }
...
bool gpu_texture_create_with_mipmaps(GPUTextureManager* manager, u32 texture_id,
    ...
    // Create base texture
    if (!gpu_texture_create(manager, texture_id, name, mipmap_data[0],
                           base_width, base_height, TEXFMT_RGBA8)) {
        return false;
    }

    // Upload mipmap levels
    for (u32 i = 1; i < mipmap_count; i++) {
        gpu_texture_upload_async(manager, texture_id, mipmap_data[i], mipmap_sizes[i]);
    }
...
void gpu_texture_bind_descriptor_set(GPUTextureManager* manager,
    ...
    if (gpu_tex->descriptor_set != VK_NULL_HANDLE) {
        vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                               layout, set_index, 1, &gpu_tex->descriptor_set, 0, NULL);
    }
}
...
void gpu_texture_transition_all_to_read(GPUTextureManager* manager,
    ...
    for (u32 i = 0; i < manager->texture_count; i++) {
        if (manager->textures[i].initialized && manager->textures[i].layout != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
...
void gpu_texture_get_dimensions(GPUTextureManager* manager, u32 texture_id,
    ...
    GPUTexture* gpu_tex = &manager->textures[texture_id];
    if (width) *width = gpu_tex->width;
    if (height) *height = gpu_tex->height;
}
...
void gpu_texture_bind_textures(GPUTextureManager* manager,
    ...
    for (u32 i = 0; i < count; i++) {
        gpu_texture_bind_descriptor_set(manager, cmd_buffer, texture_ids[i], layout, i);
    }
}
