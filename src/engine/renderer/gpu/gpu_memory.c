...
bool gpu_memory_init(GPUMemoryAllocator *allocator, VkDevice device,
  ...
  if (!gpu_memory_create_pool(allocator, MEMORY_POOL_STAGING,
                              allocator->staging_buffer_size)) {
    fprintf(stderr, "[GPU_MEMORY] Failed to create staging pool\n");
    return false;
  }

  if (!gpu_memory_create_pool(allocator, MEMORY_POOL_DEVICE_LOCAL,
                              device_local_budget)) {
    fprintf(stderr, "[GPU_MEMORY] Failed to create device-local pool\n");
    return false;
  }

  if (!gpu_memory_create_pool(allocator, MEMORY_POOL_DYNAMIC,
                              allocator->dynamic_buffer_size)) {
    fprintf(stderr, "[GPU_MEMORY] Failed to create dynamic pool\n");
    return false;
  }

  if (!gpu_memory_create_pool(allocator, MEMORY_POOL_READBACK,
                              128 * 1024 * 1024)) {
...
bool gpu_memory_allocate_buffer(GPUMemoryAllocator *allocator, u64 size,
  ...
  for (u32 i = 0; i < pool->free_block_count; i++) {
    if (pool->free_blocks[i].size >= size) {
      if (best_block == -1 ||
          pool->free_blocks[i].size < pool->free_blocks[best_block].size) {
        best_block = i;
      }
    }
  }
  ...
  if (best_block == -1) {
    fprintf(
        stderr,
        "[GPU_MEMORY] Allocation failed: insufficient free space in pool %d\n",
        pool_type);
    return false;
  }
  ...
  AllocationInfo alloc = {
      ...
      .size = size,
      .alignment = 256,
      .mapped_ptr = pool->mapped_ptr
                        ? (void *)((u8 *)pool->mapped_ptr +
                                   pool->free_blocks[best_block].offset)
                        : NULL,
      .is_dedicated = false,
...
u32 gpu_memory_defragment_all(GPUMemoryAllocator *allocator) {
  ...
  for (u32 i = 0; i < MEMORY_POOL_COUNT; i++) {
    total_relocations +=
        gpu_memory_defragment_pool(allocator, (MemoryPoolType)i);
  }
  ...
  for (u32 i = 0; i < MEMORY_POOL_COUNT; i++) {
    ...
    if (pool->initialized) {
      fprintf(stderr, "[GPU_MEMORY] Pool %d: %u allocs, %.0f/%.0f MB used\n", i,
              pool->allocation_count, pool->allocated_size / (1024.0 * 1024.0),
...
void gpu_memory_log_fragmentation(GPUMemoryAllocator *allocator) {
  ...
  for (u32 i = 0; i < MEMORY_POOL_COUNT; i++) {
    ...
    if (pool->initialized) {
      f32 ratio =
          gpu_memory_get_fragmentation_ratio(allocator, (MemoryPoolType)i);
      u64 largest =
          gpu_memory_get_largest_free_block(allocator, (MemoryPoolType)i);
      fprintf(stderr,
              "[GPU_MEMORY] Pool %d: fragmentation %.2f, largest free block "
              "%.0f MB\n",
