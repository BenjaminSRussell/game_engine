...
#include "network/network_state_sync.h"
#include <stdlib.h>
#include <string.h>
...
void state_sync_shutdown(void) {
  ...
  for (uint32_t i = 0; i < g_state_sync.history_count; i++) {
    uint32_t index =
        (g_state_sync.history_head - i + SYNC_HISTORY_SIZE) % SYNC_HISTORY_SIZE;
...
bool state_sync_register_entity(uint32_t entity_id, size_t state_size,
                                uint8_t priority) {
  if (!g_state_sync.initialized ||
      g_state_sync.entity_count >= MAX_SYNCED_ENTITIES) {
    return false;
  }
  ...
  if (!entity->current_state || !entity->previous_state) {
    ...
    if (entity->current_state)
      memory_free(entity->current_state);
    if (entity->previous_state)
      memory_free(entity->previous_state);
    return false;
  }
  ...
  switch (priority) {
  case 2: // High
    g_state_sync.high_priority_entities[g_state_sync.high_priority_count++] =
        entity_id;
    break;
  case 1: // Medium
    g_state_sync
        .medium_priority_entities[g_state_sync.medium_priority_count++] =
        entity_id;
    break;
  case 0: // Low
    g_state_sync.low_priority_entities[g_state_sync.low_priority_count++] =
        entity_id;
    break;
  }

  g_state_sync.entity_count++;
  log_debug("Registered entity %u for state sync with size %zu", entity_id,
            state_size);
  return true;
}
      ...
      for (uint32_t j = 0; j < g_state_sync.high_priority_count; j++) {
        if (g_state_sync.high_priority_entities[j] == entity_id) {
          memmove(&g_state_sync.high_priority_entities[j],
                  ...
                  (g_state_sync.high_priority_count - j - 1) *
                      sizeof(uint32_t));
...
bool state_sync_update_entity(uint32_t entity_id, const void *state_data,
                              size_t state_size) {
  if (!g_state_sync.initialized) {
    return false;
  }
...
static size_t apply_delta(const uint8_t *base_data, const uint8_t *delta_data,
                          size_t delta_size, uint8_t *output_data,
                          size_t output_size) {
...
static state_snapshot_t *create_snapshot(bool use_delta_compression) {
  ...
  state_snapshot_t *snapshot =
      &g_state_sync.snapshot_history[g_state_sync.history_head];
  ...
  for (uint32_t i = 0; i < g_state_sync.entity_count; i++) {
    total_size += sizeof(uint32_t) + sizeof(uint32_t) +
                  g_state_sync.entities[i].state_size;
  }
  ...
  for (uint32_t i = 0; i < g_state_sync.entity_count; i++) {
    ...
    *(uint32_t *)ptr = entity->state_size;
    ...
    if (use_delta_compression && snapshot->is_delta) {
      ...
      for (uint32_t j = 1;
           j < g_state_sync.history_count && j <= SYNC_HISTORY_SIZE; j++) {
        uint32_t base_index =
            (g_state_sync.history_head - j + SYNC_HISTORY_SIZE) %
            SYNC_HISTORY_SIZE;
        if (g_state_sync.snapshot_history[base_index].frame_number ==
            snapshot->base_frame) {
      ...
      if (base_snapshot) {
        // Apply delta compression (simplified - would need proper
        // entity-to-data mapping)
...
bool state_sync_create_snapshot(uint8_t **snapshot_data,
  ...
  // Update history
  g_state_sync.history_head =
      (g_state_sync.history_head + 1) % SYNC_HISTORY_SIZE;
...
bool state_sync_apply_snapshot(const uint8_t *snapshot_data,
                               size_t snapshot_size) {
...
void state_sync_get_statistics(uint32_t *synced_entities,
                               uint32_t *bandwidth_used,
  ...
  if (synced_entities)
    *synced_entities = g_state_sync.entity_count;
  if (bandwidth_used)
    *bandwidth_used = g_state_sync.bandwidth_used;
  if (snapshot_count)
    *snapshot_count = g_state_sync.history_count;
}
