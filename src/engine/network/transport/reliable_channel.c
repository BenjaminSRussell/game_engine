// reliable_channel.c - Reliable Channel Implementation
...
static u64 get_time_ms(void) {
  return (u64)(time(NULL) * 1000);
}

static u32 sequence_diff(u32 a, u32 b) {
  return (u32)((i32)b - (i32)a);
}
...
ReliableChannel* reliable_channel_create(UDPSocket* socket, const NetworkAddress* remote_address, ReliableMode mode) {
  if (!socket || !remote_address) {
    LOG_ERROR("Invalid parameters for reliable channel creation");
    return NULL;
  }
  
  ReliableChannel* channel = (ReliableChannel*)calloc(1, sizeof(ReliableChannel));
  if (!channel) {
  ...
  channel->last_ack_time = get_time_ms();
...
bool reliable_channel_send(ReliableChannel* channel, const void* data, u32 size) {
  ...
  if (size > RELIABLE_CHANNEL_MAX_PACKET_SIZE) {
    LOG_ERROR("Packet too large: %u bytes (max: %u)", size, RELIABLE_CHANNEL_MAX_PACKET_SIZE);
    return false;
  }
...
bool reliable_channel_receive(ReliableChannel* channel, void* data, u32* size) {
  if (!channel || !data || !size) return false;
...
void reliable_channel_update(ReliableChannel* channel, f32 delta_time) {
  ...
  if (current_time - channel->last_ack_time > 100) { // 100ms interval
    reliable_channel_send_ack(channel);
    channel->last_ack_time = current_time;
  }
...
void reliable_channel_send_packet(ReliableChannel* channel, ReliablePacket* packet) {
  if (!channel || !packet || !packet->data) return;
  ...
  ReliableHeader header = {
    ...
    .ack_bits = reliable_channel_calculate_ack_bits(channel)
  };
...
void reliable_channel_send_ack(ReliableChannel* channel) {
  ...
  // Create ack packet
  ReliableHeader header = {
    .sequence = 0, // Not used for ack packets
    .ack = channel->recv_sequence,
    .ack_bits = reliable_channel_calculate_ack_bits(channel)
  };
...
u32 reliable_channel_calculate_ack_bits(ReliableChannel* channel) {
  if (!channel) return 0;
  ...
  for (u32 i = 0; i < 32; i++) {
    ...
    for (u32 j = 0; j < RELIABLE_CHANNEL_BUFFER_SIZE; j++) {
      ReceivedPacket* packet = &channel->recv_buffer[j];
...
void reliable_channel_process_packet(ReliableChannel* channel, const u8* data, u32 size) {
  if (!channel || !data || size < sizeof(ReliableHeader)) return;
  ...
  if (size == sizeof(ReliableHeader)) {
    LOG_DEBUG("Received ack-only packet (ack: %u, bits: 0x%08X)", header.ack, header.ack_bits);
    return;
  }
  ...
  reliable_channel_process_data_packet(channel, header.sequence, packet_data, packet_size);
}

void reliable_channel_process_ack(ReliableChannel* channel, u32 ack, u32 ack_bits) {
  if (!channel) return;
...
void reliable_channel_process_data_packet(ReliableChannel* channel, u32 sequence, const u8* data, u32 size) {
  if (!channel || !data || size == 0) return;
  
  // Check if packet is too old
  if (!sequence_greater_than(sequence, channel->recv_sequence)) {
    LOG_DEBUG("Discarding old packet %u (current: %u)", sequence, channel->recv_sequence);
    return;
  }
...
void reliable_channel_retransmit_lost_packets(ReliableChannel* channel, u64 current_time) {
  if (!channel) return;
    ...
    if (packet->in_flight && !packet->acked) {
      ...
      if (time_since_send > RELIABLE_CHANNEL_RESEND_INTERVAL_MS) {
        if (packet->retry_count < RELIABLE_CHANNEL_MAX_RETRIES) {
          ...
          LOG_DEBUG("Retransmitting packet %u (attempt %u)", packet->sequence, packet->retry_count);
        } else {
          // Max retries exceeded, drop packet
          LOG_WARN("Packet %u max retries exceeded, dropping", packet->sequence);
          packet->in_flight = false;
        }
      }
    }
...
void reliable_channel_cleanup_old_packets(ReliableChannel* channel, u64 current_time) {
  if (!channel) return;
...
void reliable_channel_get_statistics(const ReliableChannel* channel, u32* out_packets_sent, 
                                    u32* out_packets_received, u32* out_packets_lost,
                                    f32* out_rtt_ms) {
  if (!channel) return;
  ...
  if (out_packets_sent) *out_packets_sent = packets_sent;
  if (out_packets_received) *out_packets_received = packets_received;
  if (out_packets_lost) *out_packets_lost = packets_lost;
  if (out_rtt_ms) *out_rtt_ms = rtt_ms;
}
