...
// Simple XOR-based encryption for demonstration (in production, use proper
// crypto libraries)
...
static void generate_random_bytes(uint8_t *buffer, size_t size) {
  // Simple pseudo-random generator (in production, use cryptographically secure
  // RNG)
...
static void xor_encrypt(const uint8_t *input, uint8_t *output, size_t size,
                        const uint8_t *key) {
  for (size_t i = 0; i < size; i++) {
...
static void xor_decrypt(const uint8_t *input, uint8_t *output, size_t size,
                        const uint8_t *key) {
...
static EncryptionKey *find_or_create_player_key(uint64_t player_id) {
  ...
  if (g_secure_storage.key_count >= g_secure_storage.key_capacity) {
    uint32_t new_capacity = g_secure_storage.key_capacity == 0
                                ? 64
                                : g_secure_storage.key_capacity * 2;
    EncryptionKey *new_keys = realloc(g_secure_storage.player_keys,
                                      new_capacity * sizeof(EncryptionKey));
    if (!new_keys) {
      return NULL;
    }
  ...
  EncryptionKey *key =
      &g_secure_storage.player_keys[g_secure_storage.key_count++];
  ...
  key->expires_time =
      key->created_time + g_secure_storage.key_rotation_interval_ms;
  key->is_valid = true;
...
bool encryption_init(uint64_t key_rotation_interval_ms) {
  ...
  // Generate master key
  generate_random_bytes(g_secure_storage.master_key.data,
                        sizeof(g_secure_storage.master_key.data));
  generate_random_bytes(g_secure_storage.master_key.iv,
                        sizeof(g_secure_storage.master_key.iv));
  g_secure_storage.master_key.created_time = get_current_time_ms();
  g_secure_storage.master_key.expires_time =
      g_secure_storage.master_key.created_time + key_rotation_interval_ms;
  ...
  LOG_INFO("Encryption system initialized with key rotation interval %llu ms",
...
void encryption_shutdown(void) {
  ...
  if (g_secure_storage.player_keys) {
    memset(g_secure_storage.player_keys, 0,
           g_secure_storage.key_capacity * sizeof(EncryptionKey));
...
bool encryption_encrypt_player_data(uint64_t player_id, const char *data,
...
bool encryption_decrypt_player_data(uint64_t player_id,
  ...
  if (encrypted_len >=
      MAX_ENCRYPTED_DATA_SIZE * 2) { // hex encoding doubles size
  ...
  for (size_t i = 0;
       i < encrypted_len - 1 && decoded_len < MAX_ENCRYPTED_DATA_SIZE; i += 2) {
    char hex_byte[3] = {encrypted_data[i], encrypted_data[i + 1], '\0'};
    encrypted_raw[decoded_len++] = (uint8_t)strtol(hex_byte, NULL, 16);
  }

  // Decrypt
  xor_decrypt(encrypted_raw, (uint8_t *)decrypted_output, decoded_len,
              key->data);
...
bool encryption_hash_password(const char *password, const char *salt,
  ...
  if (!derive_key_from_password(password, salt_bytes, key_bytes,
                                sizeof(key_bytes))) {
    return false;
  }
...
bool encryption_verify_password(const char *password, const char *stored_hash,
                                const char *salt) {
  ...
  if (!encryption_hash_password(password, salt, computed_hash,
...
bool encryption_rotate_master_key(void) {
  ...
  g_secure_storage.master_key.created_time = get_current_time_ms();
  g_secure_storage.master_key.expires_time =
      g_secure_storage.master_key.created_time +
      g_secure_storage.key_rotation_interval_ms;
  g_secure_storage.master_key.is_valid = true;
  ...
  // In a real implementation, we'd need to re-encrypt all player data with the
  // new master key
  return true;
}

void encryption_get_statistics(uint32_t *encryptions, uint32_t *decryptions,
                               uint32_t *key_rotations) {
  if (encryptions)
    *encryptions = g_secure_storage.encryptions_performed;
  if (decryptions)
    *decryptions = g_secure_storage.decryptions_performed;
  if (key_rotations)
    *key_rotations = g_secure_storage.key_rotations;
}

bool encryption_should_rotate_keys(void) {
  uint64_t current_time = get_current_time_ms();
  return (current_time - g_secure_storage.last_rotation_time) >=
         g_secure_storage.key_rotation_interval_ms;
}
