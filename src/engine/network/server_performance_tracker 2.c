...
bool platform_get_cpu_usage(float* usage_percent, u32* cores_available, u32* cores_active) {
    ...
    *cores_active = 0;
    ...
    if (sysctlbyname("hw.ncpu", &num_cpus, sizeof(num_cpus), NULL, 0) == 0) {
    ...
    mach_msg_type_t count = 0;
    if (host_processor_info(mach_host_self(), &count, &cpu_info) == KERN_SUCCESS) {
        float total_cpu = 0.0f;
        for (natural_t i = 0; i < count; i++) {
            total_cpu += cpu_info[i].cpu_ticks[CPU_STATE_USER] + 
        ...
        if (total_cpu > 0) {
            float used_cpu = 0.0f;
            for (natural_t i = 0; i < count; i++) {
                used_cpu += cpu_info[i].cpu_ticks[CPU_STATE_USER] + 
                           cpu_info[i].cpu_ticks[CPU_STATE_SYSTEM];
            }
            *usage_percent = (used_cpu / total_cpu) * 100.0f;
        }
        
        vm_deallocate(mach_task_self(), count * sizeof(processor_info_array_t), (vm_address_t)cpu_info);
    }
    ...
    *cores_active = sys_info.dwNumberOfProcessors;
    ...
    // Get CPU usage via performance counters
    static ULARGE_INTEGER last_cpu, last_sys_cpu, last_usr_cpu;
    ULARGE_INTEGER sys_cpu, usr_cpu;
    
    if (GetSystemTimeAsFileTime((FILETIME*)&sys_cpu) &&
        GetProcessTimes(GetCurrentProcess(), (FILETIME*)&usr_cpu, (FILETIME*)&sys_cpu, NULL, NULL)) {
        
        ULARGE_INTEGER cpu_diff = sys_cpu.QuadPart - last_sys_cpu.QuadPart;
        ULARGE_INTEGER usr_diff = usr_cpu.QuadPart - last_usr_cpu.QuadPart;
        ULARGE_INTEGER total_diff = cpu_diff + usr_diff;
        
        if (total_diff.QuadPart > 0) {
            *usage_percent = (float)(usr_diff.QuadPart * 100.0 / total_diff.QuadPart);
        }
        
        last_cpu = sys_cpu;
        last_usr_cpu = usr_cpu;
    }
    
#elif defined(__linux__)
    // Get CPU usage on Linux
    *cores_available = sysconf(_SC_NPROCESSORS_ONLN);
    *cores_active = *cores_available;
    
    // Read from /proc/stat
    FILE* fp = fopen("/proc/stat", "r");
    if (fp) {
        unsigned long user, nice, system, idle, iowait, irq, softirq;
        static unsigned long last_user = 0, last_system = 0, last_idle = 0;
        ...
        if (fscanf(fp, "cpu %lu %lu %lu %lu %lu %lu %lu", &user, &nice, &system, &idle, &iowait, &irq, &softirq) == 7) {
            ...
            if (last_total > 0) {
                ...
                unsigned long active_diff = active - last_active;
                
                if (total_diff > 0) {
                    *usage_percent = (float)(active_diff * 100.0 / total_diff);
                }
            }
...
bool platform_get_memory_usage(u64* total_mb, u64* used_mb, u64* available_mb) {
    ...
    *available_mb = 0;
    ...
    mach_msg_type_t count = HOST_VM_INFO64_COUNT;
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vm_stats, &count) == KERN_SUCCESS) {
        *total_mb = (u64)(vm_stats.total_count * vm_stats.page_size / (1024 * 1024));
        *used_mb = (u64)((vm_stats.active_count + vm_stats.inactive_count + vm_stats.wire_count) * vm_stats.page_size / (1024 * 1024));
    ...
    if (GlobalMemoryStatusEx(&mem_info)) {
        *total_mb = mem_info.ullTotalPhys / (1024 * 1024);
        *used_mb = mem_info.ullAvailPhys / (1024 * 1024);
        *available_mb = *total_mb - *used_mb;
    }
...
bool platform_get_network_stats(u32* packets_sent, u32* packets_received, 
    ...
    if (!packets_sent || !packets_received || !bytes_sent || !bytes_received || !latency_ms) {
        return false;
    }
    
    // For now, return zeros - actual implementation would require network stack integration
    *packets_sent = 0;
    *packets_received = 0;
    *bytes_sent = 0;
    *bytes_received = 0;
    *latency_ms = 0.0f;
    ...
    LOG_DEBUG("Network stats monitoring not fully implemented");
    return true;
}
...
bool platform_get_process_time(float* user_time_ms, float* system_time_ms) {
    ...
    *system_time_ms = 0.0f;
    ...
    if (getrusage(RUSAGE_SELF, &usage) == 0) {
        *user_time_ms = usage.ru_utime.tv_sec * 1000.0f + usage.ru_utime.tv_usec / 1000.0f;
        *system_time_ms = usage.ru_stime.tv_sec * 1000.0f + usage.ru_stime.tv_usec / 1000.0f;
    }
    
#elif defined(_WIN32)
    FILETIME creation, exit, kernel, user;
    if (GetProcessTimes(GetCurrentProcess(), &kernel, &user, &exit, &creation)) {
        ULARGE_INTEGER user_time, kernel_time;
        ...
        *user_time_ms = (float)(user_time.QuadPart / 10000.0f);
        *system_time_ms = (float)(kernel_time.QuadPart / 10000.0f);
    }
...
#else
    LOG_WARN("Process time monitoring not implemented for this platform");
    return false;
#endif
...
static void create_alert(ServerPerformanceTracker* tracker, AlertLevel level, const char* message, 
                      const char* metric_name, float current_value, float threshold_value) {
...
static void check_threshold(ServerPerformanceTracker* tracker, const char* metric_name, 
                           float current_value, float warning_threshold, float critical_threshold) {
    ...
    if (level != ALERT_NONE) {
        create_alert(tracker, level, message, metric_name, current_value, 
                   level == ALERT_CRITICAL ? critical_threshold : warning_threshold);
    }
}
...
bool server_perf_tracker_init(ServerPerformanceTracker* tracker, u32 history_capacity, u32 alert_capacity) {
    ...
    tracker->history.metrics = (ServerPerformanceMetrics*)calloc(history_capacity, sizeof(ServerPerformanceMetrics));
    ...
    tracker->alerts = (PerformanceAlert*)calloc(alert_capacity, sizeof(PerformanceAlert));
    if (!tracker->alerts) {
...
void server_perf_tracker_update(ServerPerformanceTracker* tracker) {
    ...
    if (current_time - tracker->last_update_time_ms < tracker->update_interval_ms) {
    ...
    tracker->current_metrics.server_load_percentage = 
        (tracker->current_metrics.cpu_usage_percent * 0.4f + 
         ...
         (tracker->current_metrics.tick_time_ms / 16.67f) * 0.3f); // 16.67ms = 60 FPS
    ...
    check_threshold(tracker, "CPU Usage", tracker->current_metrics.cpu_usage_percent,
                   tracker->thresholds.cpu_warning_threshold, tracker->thresholds.cpu_critical_threshold);
    check_threshold(tracker, "Memory Usage", tracker->current_metrics.memory_usage_percent,
                   tracker->thresholds.memory_warning_threshold, tracker->thresholds.memory_critical_threshold);
    check_threshold(tracker, "Network Latency", tracker->current_metrics.network_latency_ms,
                   tracker->thresholds.latency_warning_threshold, tracker->thresholds.latency_critical_threshold);
    check_threshold(tracker, "Tick Time", tracker->current_metrics.tick_time_ms,
                   tracker->thresholds.tick_time_warning_threshold, tracker->thresholds.tick_time_critical_threshold);
...
void server_perf_collect_memory_metrics(ServerPerformanceTracker* tracker) {
    ...
    if (platform_get_memory_usage(&total_mb, &used_mb, &available_mb)) {
        ...
        if (total_mb > 0) {
            tracker->current_metrics.memory_usage_percent = (float)used_mb / (float)total_mb * 100.0f;
        }
    }
}
...
void server_perf_collect_network_metrics(ServerPerformanceTracker* tracker) {
    ...
    u32 packets_sent = 0, packets_received = 0, bytes_sent = 0, bytes_received = 0;
    float latency = 0.0f;
    
    if (platform_get_network_stats(&packets_sent, &packets_received, &bytes_sent, &bytes_received, &latency)) {
...
void server_perf_collect_server_metrics(ServerPerformanceTracker* tracker) {
    ...
    tracker->current_metrics.network_events_processed_per_tick = 50; // Example value
...
const ServerPerformanceMetrics* server_perf_get_metrics_at_time(const ServerPerformanceTracker* tracker, u32 index) {
    ...
    u32 actual_index = (tracker->history.current_index - index - 1 + tracker->history.count) % tracker->history.count;
...
const ServerPerformanceMetrics* server_perf_get_average_metrics(const ServerPerformanceTracker* tracker, u32 sample_count) {
    if (!tracker || !tracker->history.metrics || tracker->history.count == 0 || sample_count == 0) {
        return NULL;
    }
    
    u32 samples_to_average = sample_count > tracker->history.count ? tracker->history.count : sample_count;
    
    ServerPerformanceMetrics* avg = (ServerPerformanceMetrics*)calloc(1, sizeof(ServerPerformanceMetrics));
    if (!avg) {
        return NULL;
    }
    
    for (u32 i = 0; i < samples_to_average; i++) {
        const ServerPerformanceMetrics* metrics = server_perf_get_metrics_at_time(tracker, i);
        if (metrics) {
...
float server_perf_get_average_cpu_usage(const ServerPerformanceTracker* tracker, u32 sample_count) {
    const ServerPerformanceMetrics* avg = server_perf_get_average_metrics(tracker, sample_count);
    if (!avg) {
        return 0.0f;
    }
...
void server_perf_print_metrics(const ServerPerformanceMetrics* metrics) {
    ...
    printf("CPU Usage: %.2f%% (%u/%u cores)\n", 
           metrics->cpu_usage_percent, metrics->cpu_cores_active, metrics->cpu_cores_available);
    printf("Memory: %llu MB used / %llu MB total (%.2f%%)\n", 
           metrics->used_memory_mb, metrics->total_memory_mb, metrics->memory_usage_percent);
    ...
    printf("Server: %.1f FPS (%.2f ms tick), %u entities, %u events\n", 
           ...
           metrics->entities_processed_per_tick, metrics->network_events_processed_per_tick);
...
void server_perf_print_summary(const ServerPerformanceTracker* tracker) {
    ...
    printf("History: %u/%u samples\n", tracker->history.count, tracker->history.capacity);
    printf("Alerts: %u/%u active\n", tracker->alert_count, tracker->alert_capacity);
    printf("Worst Alert: %s\n", 
           server_perf_get_worst_alert_level(tracker) == ALERT_NONE ? "NONE" :
           server_perf_get_worst_alert_level(tracker) == ALERT_WARNING ? "WARNING" : "CRITICAL");
    ...
    if (tracker->history.count > 0) {
        ...
        printf("Average Memory: %.2f%% (last %u samples)\n", 
...
void server_perf_export_to_csv(const ServerPerformanceTracker* tracker, const char* filename) {
    ...
    // Write CSV header
    fprintf(fp, "timestamp_ms,cpu_usage_percent,memory_usage_percent,network_latency_ms,tick_time_ms,server_load_percentage,active_connections,packets_sent_per_second,packets_received_per_second,bytes_sent_per_second,bytes_received_per_second,is_healthy\n");
    ...
    for (u32 i = 0; i < tracker->history.count; i++) {
        ...
        if (metrics) {
            fprintf(fp, "%llu,%.2f,%.2f,%.2f,%.2f,%.2f,%u,%u,%u,%u,%u,%s\n",
                   ...
                   metrics->tick_time_ms,
                   metrics->server_load_percentage,
                   metrics->active_connections,
                   metrics->packets_sent_per_second,
