...
// Local vec3 functions (using engine's vec3 type)
...
typedef struct AdvancedPathCache {
  AdvancedCachedPath
      paths[ADVANCED_CACHE_MAX_PATHS_PER_REGION * ADVANCED_CACHE_MAX_REGIONS];
...
static u32 find_or_create_region(vec3 position) {
  ...
  for (u32 i = 0; i < g_advanced_cache->region_count; i++) {
    if (vec3_distance(position, g_advanced_cache->regions[i].center) <=
        g_advanced_cache->regions[i].radius) {
      g_advanced_cache->regions[i].last_access_time =
          g_advanced_cache->current_time;
      return i;
    }
  }
...
static u8 calculate_path_quality(const vec3 *waypoints, u32 waypoint_count) {
  ...
  f32 direct_distance =
      vec3_distance(waypoints[0], waypoints[waypoint_count - 1]);
...
bool pathfinding_cache_advanced_lookup(vec3 start, vec3 goal,
                                       vec3 **out_waypoints, u32 *out_count,
                                       u8 *out_quality) {
  if (!g_advanced_cache || !out_waypoints || !out_count)
    return false;
  ...
  for (u32 i = 0; i < g_advanced_cache->path_count; i++) {
    ...
    if (start_dist <= ADVANCED_CACHE_TOLERANCE &&
        goal_dist <= ADVANCED_CACHE_TOLERANCE) {
  ...
  if (best_match_index != UINT32_MAX) {
    ...
    *out_count = path->waypoint_count;
    if (out_quality)
      *out_quality = path->quality_score;
...
void pathfinding_cache_advanced_store(vec3 start, vec3 goal,
                                      const vec3 *waypoints, u32 waypoint_count,
                                      f32 computation_time) {
  if (!g_advanced_cache || !waypoints || waypoint_count == 0 ||
      waypoint_count > ADVANCED_CACHE_MAX_WAYPOINTS) {
    return;
  }
  ...
  if (path_index == UINT32_MAX) {
    if (g_advanced_cache->path_count <
        ADVANCED_CACHE_MAX_PATHS_PER_REGION * ADVANCED_CACHE_MAX_REGIONS) {
...
void pathfinding_cache_predict_path(vec3 current_position, vec3 likely_goal,
                                    f32 confidence) {
  if (!g_advanced_cache || g_advanced_cache->prediction_count >=
                               ADVANCED_CACHE_MAX_PATHS_PER_REGION) {
    return;
  }
  ...
  PathPrediction *pred =
      &g_advanced_cache->predictions[g_advanced_cache->prediction_count++];
  pred->predicted_goal = likely_goal;
...
bool pathfinding_cache_get_predicted_path(vec3 current_position,
  ...
  for (u32 i = 0; i < g_advanced_cache->prediction_count; i++) {
    ...
    if (vec3_within_tolerance(current_position, pred->current_position, 2.0f) &&
      ...
      if (pathfinding_cache_advanced_lookup(current_position,
                                            pred->predicted_goal, out_waypoints,
                                            out_count, NULL)) {
...
void pathfinding_cache_advanced_update(f32 delta_time) {
  ...
  for (u32 i = 0; i < g_advanced_cache->prediction_count; i++) {
    ...
    if (pred->confidence < 0.1f ||
        pred->frames_since_prediction > ADVANCED_CACHE_PREDICTION_FRAMES) {
...
void pathfinding_cache_advanced_get_statistics(AdvancedCacheStats *out_stats) {
  ...
  out_stats->hit_rate =
      g_advanced_cache->total_requests > 0
          ? (f32)g_advanced_cache->cache_hits / g_advanced_cache->total_requests
          : 0.0f;
  out_stats->average_path_length =
      out_stats->valid_paths > 0 ? total_length / out_stats->valid_paths : 0.0f;
  out_stats->average_quality = out_stats->valid_paths > 0
                                   ? total_quality / out_stats->valid_paths
                                   : 0.0f;
...
void pathfinding_cache_advanced_print_statistics(void) {
  ...
  LOG_INFO(LOG_CAT_AI, "Paths: %u/%u valid", stats.valid_paths,
           stats.total_paths);
