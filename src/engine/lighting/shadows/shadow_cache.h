...
#ifndef LIGHTING_SHADOW_CACHE_H
#define LIGHTING_SHADOW_CACHE_H
...
typedef struct lighting_shadow_cache_handle {
    uint32_t id;
} lighting_shadow_cache_handle_t;

typedef struct lighting_shadow_cache_desc {
    uint32_t flags;
    void* user_data;
} lighting_shadow_cache_desc_t;

typedef struct lighting_shadow_cache_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} lighting_shadow_cache_info_t;
