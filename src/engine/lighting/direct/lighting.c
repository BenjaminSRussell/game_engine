...
// 1. Shadow cascades: IMPLEMENTED (better shadow quality at different distances)
...
u32 lighting_add_point_light(LightingSystem* system, Vec3 position, Vec4 color, f32 radius) {
    ...
    fprintf(stderr, "[LIGHTING] Added point light #%u at (%.1f, %.1f, %.1f), radius=%.1f\n",
...
void lighting_remove_point_light(LightingSystem* system, u32 light_id) {
    ...
    if (light_id < system->point_light_count - 1) {
        system->point_lights[light_id] = system->point_lights[system->point_light_count - 1];
    }
...
void lighting_remove_spot_light(LightingSystem* system, u32 light_id) {
    ...
    if (light_id < system->spot_light_count - 1) {
        system->spot_lights[light_id] = system->spot_lights[system->spot_light_count - 1];
    }
...
void lighting_remove_area_light(LightingSystem* system, u32 light_id) {
    ...
    if (light_id < system->area_light_count - 1) {
        system->area_lights[light_id] = system->area_lights[system->area_light_count - 1];
    }
...
void lighting_update_cycle(LightingSystem* system, f32 delta_time) {
    ...
    system->time.time_of_day += delta_time * time_per_second * system->time.day_cycle_speed;
    ...
    // Determine day phase
    if (system->time.time_of_day >= 4500.0f && system->time.time_of_day < 6000.0f) {
        system->time.current_phase = PHASE_DAWN;
    } else if (system->time.time_of_day >= 6000.0f && system->time.time_of_day < 18000.0f) {
        system->time.current_phase = PHASE_DAY;
    } else if (system->time.time_of_day >= 18000.0f && system->time.time_of_day < 19500.0f) {
...
Vec4 lighting_get_total_light(LightingSystem* system, Vec3 position) {
    ...
    total_light.x += system->directional.color.x * system->directional.intensity;
    total_light.y += system->directional.color.y * system->directional.intensity;
    total_light.z += system->directional.color.z * system->directional.intensity;

    // Add ambient
    total_light.x += system->environment.ambient_color.x * system->environment.ambient_intensity;
    total_light.y += system->environment.ambient_color.y * system->environment.ambient_intensity;
    total_light.z += system->environment.ambient_color.z * system->environment.ambient_intensity;

    // Add point lights
    for (u32 i = 0; i < system->point_light_count; i++) {
        if (!system->point_lights[i].enabled) continue;

        f32 influence = lighting_get_point_light_influence(&system->point_lights[i], position);
        total_light.x += system->point_lights[i].color.x * influence;
        total_light.y += system->point_lights[i].color.y * influence;
        total_light.z += system->point_lights[i].color.z * influence;
    }
...
f32 lighting_get_point_light_influence(PointLight* light, Vec3 position) {
    ...
    // Linear falloff
    f32 attenuation = 1.0f - (distance / light->radius);
...
bool lighting_is_position_shadowed(LightingSystem* system, Vec3 position, Vec3 normal) {
    ...
    // Placeholder: would check shadow map
    return false;
}
