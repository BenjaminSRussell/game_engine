...
#include "../../include/render/gpu_shader_reload.h"
...
bool gpu_shader_reload_init(GPUShaderReloadManager* manager, VkDevice device,
    ...
    fprintf(stderr, "[GPU_SHADER_RELOAD] GPU shader hot-reload manager initialized\n");
...
void gpu_shader_reload_shutdown(GPUShaderReloadManager* manager) {
    ...
    fprintf(stderr, "[GPU_SHADER_RELOAD] GPU shader hot-reload manager shut down\n");
}

void gpu_shader_reload_set_enabled(GPUShaderReloadManager* manager, bool enabled) {
    if (!manager || !manager->initialized) {
        return;
    }
...
bool gpu_shader_reload_register_pipeline(GPUShaderReloadManager* manager,
    ...
    strncpy(pipeline->vertex_path, vertex_path, sizeof(pipeline->vertex_path) - 1);
    strncpy(pipeline->fragment_path, fragment_path, sizeof(pipeline->fragment_path) - 1);

    if (geometry_path) {
        strncpy(pipeline->geometry_path, geometry_path, sizeof(pipeline->geometry_path) - 1);
    }

    if (compute_path) {
        strncpy(pipeline->compute_path, compute_path, sizeof(pipeline->compute_path) - 1);
    }
...
void gpu_shader_reload_unregister_pipeline(GPUShaderReloadManager* manager, u32 pipeline_id) {
    if (!manager || !manager->initialized) {
        return;
    }

    ShaderPipelineInfo* pipeline = gpu_shader_reload_get_pipeline(manager, pipeline_id);
    if (!pipeline) {
        return;
    }
...
ShaderPipelineInfo* gpu_shader_reload_get_pipeline(GPUShaderReloadManager* manager,
                                                   u32 pipeline_id) {
    if (!manager || !manager->initialized) {
        return NULL;
    }
    ...
    for (u32 i = 0; i < 64; i++) {
        if (manager->pipelines[i].initialized && manager->pipelines[i].pipeline_id == pipeline_id) {
...
u32 gpu_shader_reload_watch_file(GPUShaderReloadManager* manager, const char* file_path) {
...
void gpu_shader_reload_unwatch_file(GPUShaderReloadManager* manager, u32 watcher_id) {
...
u32 gpu_shader_reload_check_for_changes(GPUShaderReloadManager* manager) {
    ...
    for (u32 i = 0; i < 128; i++) {
        ...
        if (new_modified > watcher->last_modified || new_hash != watcher->file_hash) {
...
bool gpu_shader_reload_compile_shader(GPUShaderReloadManager* manager,
                                      ...
                                      u8** output_binary,
                                      u32* output_size) {
    if (!manager || !manager->initialized || !source_path || !output_binary || !output_size) {
        fprintf(stderr, "[GPU_SHADER_RELOAD] Invalid parameters\n");
        return false;
    }
...
bool gpu_shader_reload_reload_pipeline(GPUShaderReloadManager* manager, u32 pipeline_id) {
    if (!manager || !manager->initialized) {
        return false;
    }
...
u32 gpu_shader_reload_reload_all(GPUShaderReloadManager* manager) {
    ...
    for (u32 i = 0; i < 64; i++) {
        if (manager->pipelines[i].initialized) {
            if (gpu_shader_reload_reload_pipeline(manager, manager->pipelines[i].pipeline_id)) {
                reloaded++;
            }
        }
    }
...
u32 gpu_shader_reload_process_pending(GPUShaderReloadManager* manager) {
    ...
    for (u32 i = 0; i < 64; i++) {
        ...
        if (pipeline->geometry_path[0] && strlen(pipeline->geometry_path) > 0) {
            time_t geometry_time = get_file_modification_time(pipeline->geometry_path);
...
void gpu_shader_reload_get_statistics(GPUShaderReloadManager* manager,
    ...
    if (!manager || !manager->initialized) {
        ...
        if (out_successful) *out_successful = 0;
        if (out_failed) *out_failed = 0;
        return;
    }

    if (out_total) *out_total = manager->total_reloads;
    if (out_successful) *out_successful = manager->successful_reloads;
    if (out_failed) *out_failed = manager->failed_reloads;
}
...
u32 gpu_shader_reload_get_reload_count(GPUShaderReloadManager* manager, u32 pipeline_id) {
    if (!manager || !manager->initialized) {
        return 0;
    }
...
void gpu_shader_reload_log_statistics(GPUShaderReloadManager* manager) {
    ...
    fprintf(stderr, "[GPU_SHADER_RELOAD] Success rate: %.1f%%\n",
            manager->total_reloads > 0 ? (100.0f * manager->successful_reloads / manager->total_reloads) : 0.0f);
}
