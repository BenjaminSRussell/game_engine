...
bool scene_init(Scene *scene, struct EngineCore *engine, const char *name) {
  ...
  scene->game_objects =
      (GameObject *)calloc(scene->game_object_capacity, sizeof(GameObject));
...
bool scene_is_active(Scene *scene) { return scene && scene->active; }
...
GameObject *scene_create_game_object(Scene *scene, const char *name) {
  ...
  for (u32 i = 0; i < scene->game_object_capacity; i++) {
    if (!scene->game_objects[i].active) {
      ...
      obj->entity = ecs_create_entity(&scene->engine->ecs_world);
  ...
  GameObject *new_objects = (GameObject *)realloc(
      scene->game_objects, new_capacity * sizeof(GameObject));
  if (new_objects) {
...
void scene_destroy_game_object(Scene *scene, GameObject *obj) {
  ...
  if (obj->entity.id != 0) {
    ecs_destroy_entity(&scene->engine->ecs_world, obj->entity);
  }
...
GameObject *scene_find_game_object(Scene *scene, const char *name) {
  ...
  for (u32 i = 0; i < scene->game_object_capacity; i++) {
    if (scene->game_objects[i].active && scene->game_objects[i].name &&
...
GameObject *scene_get_game_object(Scene *scene, Entity entity) {
  if (!scene || entity.id == 0) {
    return NULL;
  }

  for (u32 i = 0; i < scene->game_object_capacity; i++) {
    if (scene->game_objects[i].active &&
        scene->game_objects[i].entity.id == entity.id) {
      return &scene->game_objects[i];
    }
  }
...
void scene_set_layer_count(Scene *scene, u32 layer_count) {
  ...
  scene->layer_depths =
      (f32 *)realloc(scene->layer_depths, layer_count * sizeof(f32));
  if (scene->layer_depths) {
