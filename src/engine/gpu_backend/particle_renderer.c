...
#include "../../include/rendering/particle_renderer.h"
#include "../../include/world/plant_vfx.h"
#include "../../include/weather/weather_particles.h"
...
static const char *PARTICLE_TEXTURE_FILES[PARTICLE_TEXTURE_COUNT] = {
    "textures/particles/pollen.png",
    "textures/particles/spore.png",
    "textures/particles/seed.png",
    "textures/particles/petal.png",
    "textures/particles/rain.png",
    "textures/particles/snow.png",
    "textures/particles/dust.png",
    "textures/particles/bloom.png",
...
static const Vec3 PARTICLE_COLORS[PARTICLE_TEXTURE_COUNT] = {
    vec3(1.0f, 0.9f, 0.3f),  // Pollen - yellow
    vec3(0.6f, 0.4f, 0.8f),  // Spore - purple
    vec3(0.8f, 0.6f, 0.4f),  // Seed - brown
    vec3(1.0f, 0.6f, 0.8f),  // Petal - pink
    vec3(0.7f, 0.8f, 1.0f),  // Rain - blue
    vec3(1.0f, 1.0f, 1.0f),  // Snow - white
    vec3(0.8f, 0.7f, 0.6f),  // Dust - tan
    vec3(1.0f, 0.8f, 0.2f),  // Bloom - orange
    vec3(0.8f, 0.8f, 0.8f)   // Generic - gray
};
...
void particle_renderer_init(ParticleRenderer *renderer, VulkanRenderer *vulkan,
    ...
    renderer->vertex_buffer = (ParticleVertex *)malloc(
    ...
    LOG_INFO("Particle renderer initialized with %u max particles", max_particles);
}
...
bool particle_renderer_load_texture(ParticleRenderer *renderer,
                                   ...
                                   const char *filepath) {
    if (!renderer || !filepath || type >= PARTICLE_TEXTURE_COUNT) return false;
    ...
    LOG_TRACE("Loaded particle texture: %s (handle: %u)", filepath,
...
void particle_renderer_add_particle(ParticleRenderer *renderer,
                                   Vec3 position, f32 size, Vec3 color,
                                   f32 brightness, f32 rotation,
                                   ParticleTextureType texture, f32 lifetime_ratio) {
    if (!renderer || renderer->vertex_count >= renderer->max_vertices) return;
...
void particle_renderer_end_batch(ParticleRenderer *renderer) {
    ...
    LOG_TRACE("Particle batch: %u vertices, %u instances",
             renderer->vertex_count, renderer->instance_count);
}

void particle_renderer_render_plant_particles(ParticleRenderer *renderer,
                                             const PlantParticle *particles,
                                             u32 particle_count) {
    if (!renderer || !particles || particle_count == 0) return;
    ...
    for (u32 i = 0; i < particle_count; i++) {
        ...
        ParticleTextureType texture = particle_renderer_get_texture_for_type(p->type);
...
void particle_renderer_render_weather_particles(ParticleRenderer *renderer,
                                               const WeatherParticle *particles,
                                               u32 particle_count) {
    if (!renderer || !particles || particle_count == 0) return;
    ...
    for (u32 i = 0; i < particle_count; i++) {
        ...
        if (lifetime_ratio > 1.0f) lifetime_ratio = 1.0f;
...
void particle_renderer_sort_particles(ParticleRenderer *renderer,
    ...
    if (!renderer || renderer->vertex_count == 0) return;
...
Vec3 particle_renderer_get_color_for_type(u32 particle_type) {
    ParticleTextureType texture = particle_renderer_get_texture_for_type(particle_type);
...
ParticleRenderStats particle_renderer_get_stats(const ParticleRenderer *renderer) {
    ...
    if (stats.batches > 0) {
        stats.average_batch_size = (f32)stats.particles_rendered / (f32)stats.batches;
    }
