...
//
#include "../../include/render/sprite_renderer_2_5d.h"
...
bool sprite_renderer_2_5d_init(SpriteRenderer2_5D *renderer, 
    ...
    renderer->sprites = (Sprite2_5D *)calloc(renderer->sprite_capacity, sizeof(Sprite2_5D));
    
    renderer->batch_capacity = 32;
    renderer->batches = (SpriteBatch2_5D *)calloc(renderer->batch_capacity, sizeof(SpriteBatch2_5D));
    
    renderer->sort_indices = (u32 *)calloc(renderer->sprite_capacity, sizeof(u32));
...
u32 sprite_renderer_2_5d_add_sprite(SpriteRenderer2_5D *renderer, 
    ...
    if (renderer->sprite_count >= renderer->sprite_capacity) {
        ...
        Sprite2_5D *new_sprites = (Sprite2_5D *)realloc(renderer->sprites, 
                                                         new_capacity * sizeof(Sprite2_5D));
        if (new_sprites) {
...
void sprite_renderer_2_5d_remove_sprite(SpriteRenderer2_5D *renderer, u32 sprite_id) {
    ...
    if (sprite_id < renderer->sprite_count - 1) {
        renderer->sprites[sprite_id] = renderer->sprites[renderer->sprite_count - 1];
    }
...
void sprite_renderer_2_5d_update_sprite(SpriteRenderer2_5D *renderer, 
                                        u32 sprite_id, 
                                        const Sprite2_5D *sprite) {
    if (renderer && renderer->initialized && sprite_id < renderer->sprite_count && sprite) {
...
void sprite_renderer_2_5d_render(SpriteRenderer2_5D *renderer) {
    ...
    for (u32 i = 0; i < renderer->sprite_count; i++) {
        ...
        if (renderer->renderer->render_sprite) {
            renderer->renderer->render_sprite(renderer->renderer,
                                               sprite->position,
                                               sprite->size,
                                               sprite->texture_id,
                                               sprite->rotation);
...
void sprite_renderer_2_5d_render_batch(SpriteRenderer2_5D *renderer, u32 batch_id) {
    if (!renderer || !renderer->initialized || batch_id >= renderer->batch_count) {
        return;
    }
...
void sprite_renderer_2_5d_sort_by_depth(SpriteRenderer2_5D *renderer) {
    ...
    qsort(renderer->sort_indices, renderer->sprite_count, sizeof(u32), compare_sprite_depth);
}
...
void sprite_renderer_2_5d_update_projection(SpriteRenderer2_5D *renderer, 
    ...
    if (renderer->mode == SPRITE_2_5D_MODE_ISOMETRIC) {
        ...
        f32 size = 10.0f;
        renderer->projection_matrix = mat4_ortho(-size * aspect, size * aspect,
        ...
        // Perspective
        renderer->projection_matrix = camera_get_projection_matrix(renderer->camera, aspect);
    }
