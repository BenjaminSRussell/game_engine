...
static int io_scene_renderer_03_create_asset_bundle(const char *name,
                                                    const void *data,
                                                    size_t size);
static int io_scene_renderer_03_compress_bundle_data(asset_bundle_t *bundle);
static int
io_scene_renderer_03_decompress_bundle_data(const asset_bundle_t *bundle,
                                            void **output);
...
static int io_scene_renderer_03_register_format_converter(
    const char *from_ext, const char *to_ext,
    int (*convert_func)(const void *, void **, size_t *));
static int io_scene_renderer_03_convert_gltf_to_obj(const void *input,
                                                    void **output,
                                                    size_t *output_size);
static int io_scene_renderer_03_convert_fbx_to_gltf(const void *input,
                                                    void **output,
                                                    size_t *output_size);
...
// TAA functions
static int io_scene_renderer_03_accumulate_taa_samples(void *current_frame,
                                                       void *history_buffer);
static int io_scene_renderer_03_apply_temporal_stability(void *frame_data);

// Multi-draw indirect functions
static int io_scene_renderer_03_setup_indirect_draw(void *draw_commands,
...
static int io_scene_renderer_03_validate_internal(io_scene_renderer_03_t *ctx) {
  ...
  if (ctx->visibility_buffer) {
    if (!ctx->visibility_buffer->surface_id_buffer ||
        !ctx->visibility_buffer->depth_buffer) {
...
int io_scene_renderer_03_render(io_scene_renderer_03_t *ctx, void *params) {
  ...
  if (ctx->ray_tracing &&
      (ctx->flags & IO_SCENE_RENDERER_03_FLAG_RAY_TRACING)) {
    ...
    s_renderer_03_stats.ray_tracing_rays_cast +=
        ctx->ray_tracing->samples_per_pixel;
  }
...
int io_scene_renderer_03_prepare(io_scene_renderer_03_t *ctx, void *params) {
  ...
  if (ctx->visibility_buffer &&
      (ctx->flags & IO_SCENE_RENDERER_03_FLAG_VISIBILITY)) {
...
int io_scene_renderer_03_bind(io_scene_renderer_03_t *ctx, void *params) {
  ...
  if (ctx->flags & IO_SCENE_RENDERER_03_FLAG_ASSET_BUNDLES) {
    ...
    int result = io_scene_renderer_03_create_asset_bundle(
        bundle_name, asset_data, data_size);
    if (result == 0) {
  ...
  if (ctx->taa && (ctx->flags & IO_SCENE_RENDERER_03_FLAG_TAA)) {
    ...
    io_scene_renderer_03_accumulate_taa_samples(params,
                                                ctx->taa->history_buffer);
...
int io_scene_renderer_03_draw(io_scene_renderer_03_t *ctx, void *params) {
  ...
  if (ctx->vrs && (ctx->flags & IO_SCENE_RENDERER_03_FLAG_VRS)) {
    ...
    s_renderer_03_stats.vrs_tiles_shaded +=
        ctx->vrs->map_width * ctx->vrs->map_height;
  }
...
int io_scene_renderer_03_dispatch(io_scene_renderer_03_t *ctx, void *params) {
  ...
  if (ctx->async_compute &&
      (ctx->flags & IO_SCENE_RENDERER_03_FLAG_ASYNC_COMPUTE)) {
 ...
 * Complexity: O(n) where n is the number of elements
 */
int io_scene_renderer_03_submit_commands(io_scene_renderer_03_t *ctx,
                                         void *params) {
  if (!ctx) {
 ...
 * Complexity: O(n) where n is the number of elements
 */
int io_scene_renderer_03_build_commands(io_scene_renderer_03_t *ctx,
                                        void *params) {
  if (!ctx) {
...
int io_scene_renderer_03_get_stats(io_scene_renderer_03_t *ctx) {
  ...
  if (ctx->mesh_shaders &&
      (ctx->flags & IO_SCENE_RENDERER_03_FLAG_MESH_SHADERS)) {
...
int io_scene_renderer_03_get_memory_usage(io_scene_renderer_03_t *ctx) {
  ...
  if (ctx->asset_bundles && ctx->bundle_count > 0) {
    ...
    for (uint32_t i = 0; i < ctx->bundle_count; i++) {
      asset_bundle_t *bundle = &ctx->asset_bundles[i];
      s_renderer_03_stats.memory_used +=
          bundle->data_size + bundle->compressed_size;
    }
  }
  ...
  if (ctx->async_compute &&
    ...
    if (ctx->async_compute->work_queue) {
      s_renderer_03_stats.memory_used +=
          ctx->async_compute->queue_capacity * sizeof(void *);
    }
  }
...
int io_scene_renderer_03_debug_print(io_scene_renderer_03_t *ctx) {
  ...
  if (ctx->format_converters && ctx->converter_count > 0) {
    ...
    for (uint32_t i = 0; i < ctx->converter_count; i++) {
      format_converter_t *converter = &ctx->format_converters[i];
      // LOG_DEBUG("  %s -> %s", converter->from_extension,
      // converter->to_extension);
    }
  }
