...
int io_export_processor_04_process_batch(io_export_processor_04_t *ctx,
  ...
  if (s_format_converter_count > 0) {
    ...
    if (result == 0 && converted_data) {
      /* Note: In a full implementation, converted_data would be stored in
       * ctx->internal_data or passed to next stage. Currently we just verify
       * the conversion succeeds and then free it to avoid leaks. */
      free(converted_data);
    }
  }
...
int io_export_processor_04_process_single(io_export_processor_04_t *ctx,
  ...
  if (serialize_result == 0 && serialized_data) {
    ...
    s_processor_04_stats.memory_peak =
        (s_processor_04_stats.memory_used > s_processor_04_stats.memory_peak)
            ? s_processor_04_stats.memory_used
            : s_processor_04_stats.memory_peak;

    /* In a real implementation, we would write this to disk or send it over
     * network */
...
int io_export_processor_04_module_shutdown(void) {
  ...
  if (s_scene.nodes || s_scene.meshes) {
    ...
    /* Note: In a real flow, this would likely happen during init or a specific
     * load call */
  ...
  for (uint32_t i = 0; i < s_worker_thread_count; i++) {
    ...
    *worker_id = i;
    ...
    if (pthread_create(&s_worker_threads[i], NULL,
                       io_export_processor_04_worker_thread, worker_id) != 0) {
      free(worker_id);
      return -1;
    }
  }
...
static int io_export_processor_04_init_simd(void) {
  ...
  s_simd_ctx.simd_workspace =
      aligned_alloc(s_simd_ctx.alignment, s_simd_ctx.simd_workspace_size);
...
static int io_export_processor_04_serialize_data(const void *data,
  ...
  if (s_binary_serializer.buffer_size + data_size >
      s_binary_serializer.buffer_capacity) {
    s_binary_serializer.buffer_capacity =
        s_binary_serializer.buffer_size + data_size * 2;
    s_binary_serializer.buffer = realloc(s_binary_serializer.buffer,
                                         s_binary_serializer.buffer_capacity);
    if (!s_binary_serializer.buffer)
      return -2;
  }
...
static int io_export_processor_04_compress_zstd(const void *input,
                                                ...
                                                void **output,
                                                size_t *output_size) {
  /* Placeholder: passthrough since ZSTD lib is not available in this
   * environment */
  *output = malloc(input_size);
  if (!*output)
    return -1;
