...
void gpu_text_shutdown(GPUTextManager *manager) {
  ...
  if (manager->descriptor_layout != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(manager->device, manager->descriptor_layout,
                                 NULL);
...
bool gpu_text_upload_font_atlas(GPUTextManager *manager, Font *font) {
  ...
  // In production, would:
  // 1. Create VkImage with VK_IMAGE_USAGE_TRANSFER_DST_BIT |
  // VK_IMAGE_USAGE_SAMPLED_BIT
  // 2. Allocate VkDeviceMemory
  // 3. Record vkCmdCopyBufferToImage on transfer queue
  // 4. Create VkImageView
  // 5. Perform layout transition from TRANSFER_DST_OPTIMAL to
...
bool gpu_text_create_font_atlas_descriptor(GPUTextManager *manager) {
  ...
  // vkUpdateDescriptorSets to bind the font atlas image and sampler
...
void gpu_text_bind_font_atlas(GPUTextManager *manager,
  ...
  vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout,
                          0, 1, &manager->font_atlas_descriptor, 0, NULL);
}
...
bool gpu_text_create_mesh(GPUTextManager *manager, u32 mesh_id,
  ...
  fprintf(stderr,
          "[GPU_TEXT] Created text mesh %u (%s): %u vertices, %u indices\n",
...
GPUTextMesh *gpu_text_get_mesh(GPUTextManager *manager, u32 mesh_id) {
  ...
  for (u32 i = 0; i < 256; i++) {
    if (manager->text_meshes[i].initialized &&
        manager->text_meshes[i].mesh_id == mesh_id) {
...
void gpu_text_set_position(GPUTextManager *manager, u32 mesh_id,
                           Vec2 position) {
  if (!manager || !manager->initialized) {
    return;
  }
...
void gpu_text_bind_mesh(GPUTextManager *manager, u32 mesh_id,
  ...
  vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          manager->pipeline_layout, 1, 1, &mesh->descriptor_set,
                          0, NULL);
}
