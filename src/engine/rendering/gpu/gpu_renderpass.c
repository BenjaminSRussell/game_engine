...
#include <rendering/gpu_renderpass.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
bool gpu_renderpass_init(GPURenderPassManager *manager, VkDevice device,
  ...
  if (!gpu_texture_init(manager->texture_manager, device, physical_device,
                        transfer_pool)) {
    fprintf(stderr, "[GPU_RENDERPASS] Failed to initialize texture manager\n");
    return false;
  }

  if (!gpu_text_init(manager->text_manager, device, physical_device,
                     transfer_pool, transfer_queue)) {
    fprintf(stderr, "[GPU_RENDERPASS] Failed to initialize text manager\n");
    return false;
  }

  if (!gpu_postprocess_init(manager->postprocess_manager, device,
                            physical_device, graphics_queue, transfer_pool,
                            width, height)) {
    fprintf(stderr,
            "[GPU_RENDERPASS] Failed to initialize post-processing manager\n");
    return false;
  }

  if (!gpu_shader_reload_init(manager->shader_reload_manager, device,
                              physical_device, transfer_pool, true)) {
    fprintf(stderr,
            "[GPU_RENDERPASS] Failed to initialize shader reload manager\n");
    return false;
  }

  if (!gpu_memory_init(manager->memory_allocator, device, physical_device,
                       4ULL * 1024 * 1024 * 1024)) {
...
bool gpu_renderpass_resize(GPURenderPassManager *manager, u32 new_width,
  ...
  if (manager->postprocess_manager) {
    gpu_postprocess_resize_framebuffer(manager->postprocess_manager, 0,
...
bool gpu_renderpass_render_scene(GPURenderPassManager *manager,
  ...
  // Update lighting
  if (manager->lighting_manager) {
    // gpu_lighting_update_from_system would be called here with LightingSystem
    // data
  }
...
void gpu_renderpass_draw_geometry(GPURenderPassManager *manager,
  ...
  // Bind material
  if (manager->material_manager) {
    gpu_material_bind(manager->material_manager, cmd_buffer, material_id);
  }
...
void gpu_renderpass_set_view_matrix(GPURenderPassManager *manager,
  ...
  manager->view_projection =
      mat4_mul(manager->projection_matrix, manager->view_matrix);
}
...
void gpu_renderpass_get_statistics(GPURenderPassManager *manager,
  ...
  if (out_draw_calls)
    *out_draw_calls = manager->draw_call_count;
  if (out_vertices)
    *out_vertices = manager->vertex_count;
  if (out_triangles)
    *out_triangles = manager->triangle_count;
}
...
void gpu_renderpass_log_info(GPURenderPassManager *manager) {
  ...
  fprintf(stderr, "[GPU_RENDERPASS]   Subsystems: 7 (materials, lighting, "
                  "textures, text, post-process, shaders, memory)\n");
}
...
void gpu_renderpass_log_subsystems(GPURenderPassManager *manager) {
  ...
  if (manager->material_manager) {
    gpu_material_log_statistics(manager->material_manager);
  }
  ...
  if (manager->texture_manager) {
    gpu_texture_log_statistics(manager->texture_manager);
  }
...
bool gpu_renderpass_validate(GPURenderPassManager *manager) {
  ...
  // Validate subsystems
  if (manager->material_manager &&
      /* !gpu_material_validate(manager->material_manager) */ false) {
    return false;
  }

  if (manager->lighting_manager &&
      !gpu_lighting_validate(manager->lighting_manager)) {
    return false;
  }

  if (manager->memory_allocator &&
      !gpu_memory_validate(manager->memory_allocator)) {
    return false;
  }
