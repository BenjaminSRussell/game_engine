...
static void generate_halton_sequence(f32 sequence[8][2]) {
    ...
    const f32 halton2_8[8] = {0.0f, 0.5f, 0.25f, 0.75f, 0.125f, 0.625f, 0.375f, 0.875f};
    const f32 halton3_8[8] = {0.0f, 0.333f, 0.667f, 0.111f, 0.444f, 0.778f, 0.222f, 0.556f};
...
TAAComputeContext* taa_compute_create(u32 width, u32 height) {
    LOG_INFO(LOG_CAT_RENDERER, "Creating TAA compute context (%ux%u)", width, height);
    ...
    // Load compute shader
    ctx->compute_shader = shader_load_compute("shaders/post_processing/taa_compute.comp");
...
void taa_compute_process(TAAComputeContext* ctx, 
    ...
    if (ctx->settings.enable_jitter) {
        ...
        ctx->uniforms.jitter_offset.x = ctx->halton_sequence[pattern_index][0] * ctx->settings.jitter_scale;
        ctx->uniforms.jitter_offset.y = ctx->halton_sequence[pattern_index][1] * ctx->settings.jitter_scale;
    } else {
    ...
    // Set uniforms
    shader_set_uniform_compute(ctx->compute_shader, "params", &ctx->uniforms, sizeof(ctx->uniforms));
...
void taa_compute_get_jitter_offset(TAAComputeContext* ctx, f32* out_x, f32* out_y) {
    ...
    if (ctx->settings.enable_jitter) {
        u32 pattern_index = ctx->frame_index % 8;
        *out_x = ctx->halton_sequence[pattern_index][0] * ctx->settings.jitter_scale;
        *out_y = ctx->halton_sequence[pattern_index][1] * ctx->settings.jitter_scale;
    } else {
        *out_x = 0.0f;
        *out_y = 0.0f;
    }
