...
static void bloom_threshold_execute(RGPassContext *ctx, void *user_data) {
    ...
    // Dispatch bloom threshold extraction shader
    // Bind bloom shader resources
    texture_manager_bind(input_tex, 0);
    texture_manager_bind(output_tex, 1);
    ...
    struct BloomThresholdUniforms {
        ...
        float color_shift_g;
        float color_shift_b;
        uint enable_color_shift;
        float padding;
    } uniforms = {
        .threshold = data->ctx->threshold,
        .soft_knee = data->ctx->soft_knee,
        .intensity = data->ctx->intensity,
        .color_shift_r = 0.1f,
        .color_shift_g = 0.05f,
        .color_shift_b = -0.05f,
        .enable_color_shift = data->ctx->enable_color_shift ? 1 : 0,
        .padding = 0.0f
    };
...
static void bloom_downsample_execute(RGPassContext *ctx, void *user_data) {
    ...
    for (u32 i = 0; i < data->mip_count; i++) {
        mips[i] = rg_ctx_get_texture(ctx, data->output_mips[i]);
    }
    ...
    // Dispatch downsampling compute shader for mip pyramid
    for (u32 mip = 0; mip < data->mip_count; mip++) {
        // Bind input and output for this mip level
        if (mip == 0) {
            texture_manager_bind(input_tex, 0);
        } else {
            texture_manager_bind(mips[mip - 1], 0);
        }
        texture_manager_bind(mips[mip], 1);
        
        // Set downsample uniforms
        struct BloomDownsampleUniforms {
            vec2 texel_size;
            uint mip_level;
            uint filter_type; // 0 = box, 1 = tent
            float padding;
        } uniforms = {
            .texel_size = {1.0f / 1920.0f, 1.0f / 1080.0f}, // TODO: Get actual resolution
            .mip_level = mip,
            .filter_type = 1, // Use tent filter
            .padding = 0.0f
        };
        ...
        u32 tex_width, tex_height;
        texture_manager_get_dimensions(mips[mip], &tex_width, &tex_height);
...
static void bloom_upsample_execute(RGPassContext *ctx, void *user_data) {
    ...
    // Dispatch upsampling compute shader
    // Start from smallest mip level and work up
    for (i32 mip = data->mip_count - 1; mip >= 0; mip--) {
        // Bind current mip as input
        texture_manager_bind(mips[mip], 0);
        
        // Bind output texture
        if (mip == 0) {
            texture_manager_bind(output_tex, 1);
        } else {
            texture_manager_bind(mips[mip - 1], 1);
        }
        
        // Set upsample uniforms
        ...
        // Get texture dimensions for target
        u32 tex_width, tex_height;
        if (mip == 0) {
            texture_manager_get_dimensions(output_tex, &tex_width, &tex_height);
        } else {
            texture_manager_get_dimensions(mips[mip - 1], &tex_width, &tex_height);
        }
