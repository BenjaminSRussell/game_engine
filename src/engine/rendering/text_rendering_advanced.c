...
FontLibrary *font_library_create(void) {
  ...
  library->font_cache = (struct FontCacheEntry *)calloc(
      library->cache_capacity, sizeof(struct FontCacheEntry));
...
uint32_t font_library_load_font(FontLibrary *library, const char *file_path,
  ...
  font->kerning_pairs =
      (KerningPair *)calloc(font->kerning_capacity, sizeof(KerningPair));
  ...
  if (render_mode == FONT_RENDER_SDF &&
      library->default_sdf_font_id == 0xFFFFFFFF) {
...
bool text_layout_calculate(TextLayout *layout) {
  ...
  layout->lines =
      (struct TextLine *)malloc(estimated_lines * sizeof(struct TextLine));
  if (!layout->lines) {
...
bool text_renderer_load_font(TextRenderer *renderer, const char *file_path,
  ...
  uint32_t font_id =
      font_library_load_font(renderer->font_library, file_path, size, mode);
