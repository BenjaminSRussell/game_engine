...
#ifndef POSTPROCESSING_BLOOM_FLARES_H
#define POSTPROCESSING_BLOOM_FLARES_H
...
typedef struct postprocessing_bloom_flares_handle {
    uint32_t id;
} postprocessing_bloom_flares_handle_t;

typedef struct postprocessing_bloom_flares_desc {
    uint32_t flags;
    void* user_data;
} postprocessing_bloom_flares_desc_t;

typedef struct postprocessing_bloom_flares_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} postprocessing_bloom_flares_info_t;
...
#endif /* POSTPROCESSING_BLOOM_FLARES_H */
