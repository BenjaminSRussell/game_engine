...
#if defined(__OBJC__)
    if (encoder) {
        [TO_RENDER_ENC(encoder) setDepthBias:depth_bias slopeScale:slope_scale clamp:clamp];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_RENDER_ENC(encoder) setBlendColorRed:red green:green blue:blue alpha:alpha];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_RENDER_ENC(encoder) setStencilReferenceValue:reference_value];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_RENDER_ENC(encoder) setStencilFrontReferenceValue:front_reference backReferenceValue:back_reference];
    }
#endif
}

/* Render Encoder Debugging */

void metal_render_encoder_push_debug_group(mtl_render_command_encoder_t encoder, const char* label) {
#if defined(__OBJC__)
    if (encoder && label) {
        [TO_RENDER_ENC(encoder) pushDebugGroup:[NSString stringWithUTF8String:label]];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_RENDER_ENC(encoder) popDebugGroup];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder && label) {
        [TO_RENDER_ENC(encoder) insertDebugSignpost:[NSString stringWithUTF8String:label]];
    }
#endif
}

/* Render Encoder Resource Usage (Argument Buffers) */

void metal_render_encoder_use_resource(mtl_render_command_encoder_t encoder, void* resource, int usage) {
#if defined(__OBJC__)
    if (encoder && resource) {
        [TO_RENDER_ENC(encoder) useResource:(__bridge id<MTLResource>)resource usage:(MTLResourceUsage)usage];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder && heap) {
        [TO_RENDER_ENC(encoder) useHeap:(__bridge id<MTLHeap>)heap];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder && buffer) {
        // Tile shaders are only available on Apple Silicon
        #if TARGET_OS_IOS || (TARGET_OS_OSX && defined(__aarch64__))
            if (@available(macOS 11.0, iOS 11.0, *)) {
                [TO_RENDER_ENC(encoder) setTileBuffer:TO_BUF(buffer) offset:offset atIndex:index];
            }
        #endif
    }
#endif
...
void metal_render_encoder_set_tile_texture(mtl_render_command_encoder_t encoder, void* texture, unsigned long index) {
    ...
    if (encoder && texture) {
        #if TARGET_OS_IOS || (TARGET_OS_OSX && defined(__aarch64__))
            if (@available(macOS 11.0, iOS 11.0, *)) {
                [TO_RENDER_ENC(encoder) setTileTexture:TO_TEX(texture) atIndex:index];
            }
        #endif
    }
#endif
...
#if defined(__OBJC__)
    if (encoder && threadsPerTile) {
        #if TARGET_OS_IOS || (TARGET_OS_OSX && defined(__aarch64__))
            if (@available(macOS 11.0, iOS 11.0, *)) {
                MTLSize size = MTLSizeMake(threadsPerTile[0], threadsPerTile[1], threadsPerTile[2]);
                [TO_RENDER_ENC(encoder) dispatchThreadsPerTile:size];
            }
        #endif
    }
#endif
...
#if defined(__OBJC__)
    if (encoder && buffer) {
        // Conditional rendering uses visibility result mode
        // Metal doesn't have direct predication, but we can use visibility results
        // This is a simplified implementation - full predication requires more infrastructure
        
        // For now, set visibility result mode which is available
        // Real conditional rendering would require checking the buffer value on CPU
        // or using compute shaders to conditionally encode commands
        
        // Note: True GPU predication is not available in Metal like it is in other APIs
...
void metal_render_encoder_end_conditional_rendering(mtl_render_command_encoder_t encoder) {
    ...
    if (encoder) {
        ...
        // Placeholder - Metal doesn't have native predication
    }
#endif
}
...
/* Compute Encoder Debugging */

void metal_compute_encoder_push_debug_group(mtl_compute_command_encoder_t encoder, const char* label) {
#if defined(__OBJC__)
    if (encoder && label) {
        [TO_COMPUTE_ENC(encoder) pushDebugGroup:[NSString stringWithUTF8String:label]];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_COMPUTE_ENC(encoder) popDebugGroup];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder && label) {
        [TO_COMPUTE_ENC(encoder) insertDebugSignpost:[NSString stringWithUTF8String:label]];
    }
#endif
}

/* Compute Encoder Resource Usage (Argument Buffers) */

void metal_compute_encoder_use_resource(mtl_compute_command_encoder_t encoder, void* resource, int usage) {
#if defined(__OBJC__)
    if (encoder && resource) {
        [TO_COMPUTE_ENC(encoder) useResource:(__bridge id<MTLResource>)resource usage:(MTLResourceUsage)usage];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder && heap) {
        [TO_COMPUTE_ENC(encoder) useHeap:(__bridge id<MTLHeap>)heap];
    }
#endif
}
...
#if defined(__OBJC__)
    if (encoder) {
        [TO_COMPUTE_ENC(encoder) setBufferOffset:offset atIndex:index];
    }
#endif
}
