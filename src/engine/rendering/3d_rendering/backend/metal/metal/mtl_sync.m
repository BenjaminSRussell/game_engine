...
void metal_insert_texture_barrier(metal_command_buffer_t *cmd_buffer,
  ...
  // Metal uses fences for memory barriers
  // Create a temporary fence for this barrier
  id<MTLDevice> device = cmd_buffer->buffer.device;
  ...
  // Convert stage flags to MTLRenderStages
  MTLRenderStages mtl_stages = 0;
  if (stages & METAL_STAGE_VERTEX) {
    mtl_stages |= MTLRenderStageVertex;
  }
  if (stages & METAL_STAGE_FRAGMENT) {
    mtl_stages |= MTLRenderStageFragment;
  }

  // Encode fence update to ensure texture writes are visible
  // This would typically be done within a render encoder context
  // For now, encode at command buffer level
  [cmd_buffer->buffer encodeSignalEvent:(id<MTLEvent>)fence value:1];
  [cmd_buffer->buffer encodeWaitForEvent:(id<MTLEvent>)fence value:1];
...
void metal_insert_buffer_barrier(metal_command_buffer_t *cmd_buffer,
  ...
  if (stages & METAL_STAGE_COMPUTE) {
    // Compute barriers are handled differently
  }

  // Encode barrier using event signaling
  id<MTLBuffer> mtl_buffer = (__bridge id<MTLBuffer>)buffer;

  // Ensure buffer writes complete before subsequent reads
  [cmd_buffer->buffer encodeSignalEvent:(id<MTLEvent>)fence value:1];
  [cmd_buffer->buffer encodeWaitForEvent:(id<MTLEvent>)fence value:1];
