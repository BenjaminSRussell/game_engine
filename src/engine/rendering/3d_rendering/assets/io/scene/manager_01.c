...
#include "rendering/3d_rendering/assets/io/scene/manager_01.h"
 ...
 * Complexity: O(n) where n is the number of elements
 */
int io_scene_manager_01_create(io_scene_manager_01_t* ctx, void* params) {
    if (!ctx) {
 ...
 * Complexity: O(n) where n is the number of elements
 */
int io_scene_manager_01_destroy(io_scene_manager_01_t* ctx, void* params) {
    if (!ctx) {
