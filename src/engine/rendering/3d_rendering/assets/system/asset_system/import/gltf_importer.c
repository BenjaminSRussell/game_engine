...
typedef struct asset_gltf_importer_internal {
    uint32_t id;
    uint32_t flags;
    gltf_data_t data;
    char file_path[512];
...
} asset_gltf_importer_internal_t;

typedef struct asset_gltf_importer_context {
    asset_gltf_importer_internal_t* items;
    uint32_t count;
    uint32_t capacity;
    bool initialized;
} asset_gltf_importer_context_t;

static asset_gltf_importer_context_t g_gltf_ctx = {0};
...
int asset_gltf_importer_init(void) {
    if (g_gltf_ctx.initialized) {
        return 0;
    }

    g_gltf_ctx.capacity = GLTF_IMPORTER_MAX_COUNT;
    g_gltf_ctx.items = calloc(g_gltf_ctx.capacity, sizeof(asset_gltf_importer_internal_t));
    if (!g_gltf_ctx.items) {
        return -1;
    }
...
void asset_gltf_importer_shutdown(void) {
    if (!g_gltf_ctx.initialized) {
        return;
    }
...
int asset_gltf_importer_create(asset_gltf_importer_handle_t* out_handle, 
                                 const asset_gltf_importer_desc_t* desc) {
    if (!out_handle || !desc) {
        return -1;
    }
    ...
    asset_gltf_importer_internal_t* item = &g_gltf_ctx.items[index];
...
void asset_gltf_importer_destroy(asset_gltf_importer_handle_t handle) {
...
int asset_gltf_importer_load(asset_gltf_importer_handle_t handle, const char* path) {
    if (handle.id >= g_gltf_ctx.count || !path) {
        return -1;
    }
    
    asset_gltf_importer_internal_t* item = &g_gltf_ctx.items[handle.id];
    if (!item->initialized) {
        return -2;
    }
...
uint32_t asset_gltf_importer_get_mesh_count(asset_gltf_importer_handle_t handle) {
    if (handle.id >= g_gltf_ctx.count) {
        return 0;
    }
...
uint32_t asset_gltf_importer_get_material_count(asset_gltf_importer_handle_t handle) {
    if (handle.id >= g_gltf_ctx.count) {
        return 0;
    }
...
uint32_t asset_gltf_importer_get_animation_count(asset_gltf_importer_handle_t handle) {
    if (handle.id >= g_gltf_ctx.count) {
        return 0;
    }
...
const char* asset_gltf_importer_get_version(asset_gltf_importer_handle_t handle) {
    if (handle.id >= g_gltf_ctx.count) {
        return NULL;
    }
...
int asset_gltf_importer_update(asset_gltf_importer_handle_t handle, 
...
bool asset_gltf_importer_is_valid(asset_gltf_importer_handle_t handle) {
    if (handle.id >= g_gltf_ctx.count) {
        return false;
    }
...
int asset_gltf_importer_get_info(asset_gltf_importer_handle_t handle, 
                                   asset_gltf_importer_info_t* out_info) {
    if (!out_info) {
        return -1;
    }
    ...
    const asset_gltf_importer_internal_t* item = &g_gltf_ctx.items[handle.id];
    out_info->id = item->id;
...
void asset_gltf_importer_mark_dirty(asset_gltf_importer_handle_t handle) {
    if (handle.id < g_gltf_ctx.count) {
        g_gltf_ctx.items[handle.id].dirty = true;
    }
}

int asset_gltf_importer_process_pending(void) {
    int processed = 0;
    for (uint32_t i = 0; i < g_gltf_ctx.count; i++) {
        asset_gltf_importer_internal_t* item = &g_gltf_ctx.items[i];
...
uint32_t asset_gltf_importer_get_count(void) {
    return g_gltf_ctx.count;
}

size_t asset_gltf_importer_get_memory_usage(void) {
    size_t total = sizeof(g_gltf_ctx);
    total += g_gltf_ctx.capacity * sizeof(asset_gltf_importer_internal_t);
    return total;
}

void asset_gltf_importer_debug_print(void) {
    // Debug output
}
