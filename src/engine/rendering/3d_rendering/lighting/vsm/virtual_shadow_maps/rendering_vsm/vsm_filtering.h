...
#ifndef VIRTUAL_SHADOW_MAPS_VSM_FILTERING_H
#define VIRTUAL_SHADOW_MAPS_VSM_FILTERING_H
...
typedef struct virtual_shadow_maps_vsm_filtering_handle {
    uint32_t id;
} virtual_shadow_maps_vsm_filtering_handle_t;

typedef struct virtual_shadow_maps_vsm_filtering_desc {
    uint32_t flags;
    void* user_data;
} virtual_shadow_maps_vsm_filtering_desc_t;

typedef struct virtual_shadow_maps_vsm_filtering_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} virtual_shadow_maps_vsm_filtering_info_t;
...
#endif /* VIRTUAL_SHADOW_MAPS_VSM_FILTERING_H */
