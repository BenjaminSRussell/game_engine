...
#ifndef PLATFORM_DX_HEAP_H
#define PLATFORM_DX_HEAP_H
...
typedef struct platform_dx_heap_handle {
    uint32_t id;
} platform_dx_heap_handle_t;

typedef struct platform_dx_heap_desc {
    uint32_t flags;
    void* user_data;
} platform_dx_heap_desc_t;

typedef struct platform_dx_heap_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} platform_dx_heap_info_t;
