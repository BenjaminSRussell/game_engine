...
#ifndef GEOMETRY_BVH_BUILDER_H
#define GEOMETRY_BVH_BUILDER_H
...
typedef struct geometry_bvh_builder_handle {
    uint32_t id;
} geometry_bvh_builder_handle_t;

typedef struct geometry_bvh_builder_desc {
    uint32_t flags;
    void* user_data;
} geometry_bvh_builder_desc_t;

typedef struct geometry_bvh_builder_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} geometry_bvh_builder_info_t;
