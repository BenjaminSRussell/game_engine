...
void gpu_skinning_destroy_skeleton(GPUSkeleton *skeleton) {
  if (!skeleton || !g_gpu_skinning_context)
    return;
  ...
  for (u32 i = 0; i < g_gpu_skinning_context->skeleton_count; i++) {
    if (&g_gpu_skinning_context->skeletons[i] == skeleton) {
      ...
      for (u32 j = i; j < g_gpu_skinning_context->skeleton_count - 1; j++) {
        g_gpu_skinning_context->skeletons[j] =
            g_gpu_skinning_context->skeletons[j + 1];
      }
      g_gpu_skinning_context->skeleton_count--;
...
void gpu_skinning_update_skeleton(GPUSkeleton *skeleton,
  ...
  if (!skeleton || !bone_transforms)
    return;

  memcpy(skeleton->bones, bone_transforms,
...
void gpu_skinning_set_bone_parents(GPUSkeleton *skeleton,
  ...
  memcpy(skeleton->parent_indices, parent_indices,
...
GPUSkinningMesh *gpu_skinning_create_mesh(const GPUSkinningVertex *vertices,
  ...
  for (u32 i = 0; i < vertex_count; i++) {
    normalize_bone_weights(mesh->vertices[i].bone_weights,
                           mesh->vertices[i].bone_indices,
...
void gpu_skinning_destroy_mesh(GPUSkinningMesh *mesh) {
  if (!mesh || !g_gpu_skinning_context)
    return;
  ...
  for (u32 i = 0; i < g_gpu_skinning_context->mesh_count; i++) {
    if (&g_gpu_skinning_context->meshes[i] == mesh) {
      ...
      for (u32 j = i; j < g_gpu_skinning_context->mesh_count - 1; j++) {
        g_gpu_skinning_context->meshes[j] =
            g_gpu_skinning_context->meshes[j + 1];
      }
      g_gpu_skinning_context->mesh_count--;
...
void gpu_skinning_destroy_instance(GPUAnimationInstance *instance) {
  if (!instance || !g_gpu_skinning_context)
    return;
  ...
  for (u32 i = 0; i < g_gpu_skinning_context->instance_count; i++) {
    if (&g_gpu_skinning_context->instances[i] == instance) {
      ...
      for (u32 j = i; j < g_gpu_skinning_context->instance_count - 1; j++) {
        g_gpu_skinning_context->instances[j] =
            g_gpu_skinning_context->instances[j + 1];
      }
      g_gpu_skinning_context->instance_count--;
...
void gpu_skinning_update_instance(GPUAnimationInstance *instance,
                                  f32 delta_time) {
  if (!instance || !instance->is_playing)
    return;
  ...
  if (instance->current_time >= GPU_SKINNING_MAX_TIME) {
    if (instance->is_looping) {
      instance->current_time =
          fmod(instance->current_time, GPU_SKINNING_MAX_TIME);
    } else {
...
void gpu_skinning_process_instance(GPUAnimationInstance *instance) {
  if (!instance || !g_gpu_skinning_context ||
      !g_gpu_skinning_context->initialized)
    return;
  ...
  struct {
    ...
    u32 bone_offset;
    f32 animation_time;
  } uniforms = {.vertex_count = instance->mesh->vertex_count,
  ...
  g_gpu_skinning_context->total_bones_processed +=
...
void gpu_skinning_get_statistics(GPUSkinningStats *out_stats) {
  if (!g_gpu_skinning_context || !out_stats)
    return;
  ...
  out_stats->average_skinning_time_ms =
      g_gpu_skinning_context->average_skinning_time_ms;
...
void gpu_skinning_calculate_bone_bounds(GPUSkeleton *skeleton, vec3 *out_min,
  ...
  *out_max = (vec3){-FLT_MAX, -FLT_MAX, -FLT_MAX};
  ...
  for (u32 i = 0; i < skeleton->bone_count; i++) {
    vec3 bone_pos = (vec3){skeleton->bones[i].transform.m[3][0],
                           skeleton->bones[i].transform.m[3][1],
                           skeleton->bones[i].transform.m[3][2]};

    if (bone_pos.x < out_min->x)
      out_min->x = bone_pos.x;
    if (bone_pos.y < out_min->y)
      out_min->y = bone_pos.y;
    if (bone_pos.z < out_min->z)
      out_min->z = bone_pos.z;

    if (bone_pos.x > out_max->x)
      out_max->x = bone_pos.x;
    if (bone_pos.y > out_max->y)
      out_max->y = bone_pos.y;
    if (bone_pos.z > out_max->z)
      out_max->z = bone_pos.z;
  }
}
