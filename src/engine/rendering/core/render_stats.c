...
static float nanos_to_ms(uint64_t nanos) {
    return (float)nanos / 1000000.0f;
}

static void timer_start(Timer *timer) {
    timer->start_time = get_time_nanos();
}
...
bool render_stats_init(uint32_t frame_history_size) {
    ...
    g_stats_system.frame_capacity = frame_history_size > 0 ? frame_history_size : 300;
    g_stats_system.frames = calloc(g_stats_system.frame_capacity, sizeof(FrameStats));
    ...
    LOG_INFO("Render statistics system initialized (history: %u frames)", g_stats_system.frame_capacity);
    return true;
}
...
void render_stats_end_frame(void) {
    ...
    current->frame_time_ms = nanos_to_ms(current_time - g_stats_system.last_frame_time);
    current->fps = current->frame_time_ms > 0.0f ? 1000.0f / current->frame_time_ms : 0.0f;
    
    // Update history
    g_stats_system.fps_history[g_stats_system.history_index] = current->fps;
    g_stats_system.frame_time_history[g_stats_system.history_index] = current->frame_time_ms;
    ...
    if (g_stats_system.total.frame_count > 0) {
        g_stats_system.average.frame_time_ms = g_stats_system.total.frame_time_ms / g_stats_system.total.frame_count;
        g_stats_system.average.fps = 1000.0f / g_stats_system.average.frame_time_ms;
    }
    
    // Move to next frame
    g_stats_system.current_frame = (g_stats_system.current_frame + 1) % g_stats_system.frame_capacity;
    g_stats_system.frame_count = (g_stats_system.frame_count < g_stats_system.frame_capacity) ? 
                                 g_stats_system.frame_count + 1 : g_stats_system.frame_capacity;
}
...
void render_stats_add_culling_stats(uint32_t total, uint32_t visible, uint32_t frustum, 
                                  uint32_t distance, uint32_t occlusion) {
    if (!g_stats_system.enabled || !g_stats_system.detailed_tracking)
        return;
...
void render_stats_get_session_time(float *seconds) {
    ...
    uint64_t current_time = get_time_nanos();
    *seconds = nanos_to_ms(current_time - g_stats_system.session_start_time) / 1000.0f;
}
...
void render_stats_print_summary(void) {
    ...
    LOG_INFO("Min/Max Frame Time: %.2f/%.2f ms", g_stats_system.min.frame_time_ms, g_stats_system.max.frame_time_ms);
    LOG_INFO("Min/Max FPS: %.2f/%.2f", g_stats_system.min.fps, g_stats_system.max.fps);
    ...
    if (g_stats_system.detailed_tracking) {
        ...
        LOG_INFO("Average Draw Calls/Frame: %.2f", 
                (float)g_stats_system.total.draw_calls / g_stats_system.total.frame_count);
        LOG_INFO("Average Triangles/Frame: %.2f", 
                (float)g_stats_system.total.triangles_drawn / g_stats_system.total.frame_count);
    }
...
void render_stats_print_current_frame(void) {
    ...
    LOG_DEBUG("Frame: %u, FPS: %.2f, Time: %.2f ms, Draws: %u, Triangles: %u",
             g_stats_system.total.frame_count, current.fps, current.frame_time_ms,
             current.draw_calls, current.triangles_drawn);
...
bool render_stats_is_enabled(void) {
    return g_stats_system.enabled;
}
