...
// Pass batch (group of passes on same queue)
typedef struct RGBatch {
    RGQueueType queue_type;
    u32 start_pass_index; // Index into execution_order array
    u32 pass_count;
    // Future: Synchronization primitives (semaphores)
} RGBatch;
...
struct RenderGraph {
    ...
    // Phase 2: Multi-Queue Scheduling
    RGBatch batches[RG_MAX_PASSES];
    u32 batch_count;
};
