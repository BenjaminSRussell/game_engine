...
static void update_light_bounding_volume(Light *light) {
  ...
  case LIGHT_TYPE_POINT:
    light->bounding_radius =
        light->radius > 0.0f ? light->radius : calculate_light_radius(light);
    break;
  ...
  case LIGHT_TYPE_AREA:
    ...
    light->bounding_radius = sqrtf(light->size[0] * light->size[0] +
                                   light->size[1] * light->size[1]) *
                                 0.5f +
...
static void calculate_shadow_matrix(Light *light, const float *view_matrix,
  ...
  case LIGHT_TYPE_DIRECTIONAL: {
    ...
    float light_pos[3] = {-light->direction[0] * 50.0f,
...
static void initialize_clusters(uint32_t cluster_x, uint32_t cluster_y,
                                uint32_t cluster_z) {
  ...
  g_lighting_system.cluster_capacity =
      total_clusters * 16; // 16 lights per cluster average

  g_lighting_system.light_clusters = calloc(total_clusters, sizeof(uint32_t));
  g_lighting_system.light_indices =
...
static void assign_lights_to_clusters(const float *view_matrix,
                                      const float *proj_matrix) {
  if (!g_lighting_system.clustered_shading_enabled ||
      !g_lighting_system.light_clusters) {
    return;
  }
  ...
  memset(g_lighting_system.light_clusters, 0,
         total_clusters * sizeof(uint32_t));
  ...
  for (uint32_t i = 0; i < g_lighting_system.light_count; i++) {
    ...
    uint32_t cluster_x =
        (uint32_t)(view_pos[0] / g_lighting_system.max_distance *
                   g_lighting_system.cluster_count_x);
    uint32_t cluster_y =
        (uint32_t)(view_pos[1] / g_lighting_system.max_distance *
                   g_lighting_system.cluster_count_y);
    uint32_t cluster_z =
        (uint32_t)(view_pos[2] / g_lighting_system.max_distance *
                   g_lighting_system.cluster_count_z);

    // Clamp to valid range
    cluster_x = (cluster_x < g_lighting_system.cluster_count_x)
                    ? cluster_x
                    : g_lighting_system.cluster_count_x - 1;
    cluster_y = (cluster_y < g_lighting_system.cluster_count_y)
                    ? cluster_y
                    : g_lighting_system.cluster_count_y - 1;
    cluster_z = (cluster_z < g_lighting_system.cluster_count_z)
                    ? cluster_z
                    : g_lighting_system.cluster_count_z - 1;

    uint32_t cluster_index = cluster_z * g_lighting_system.cluster_count_x *
                                 g_lighting_system.cluster_count_y +
                             cluster_y * g_lighting_system.cluster_count_x +
                             cluster_x;
...
bool lighting_system_init(uint32_t max_lights, bool enable_clustered_shading) {
  ...
  /* g_lighting_system.lights is a fixed array, no need to allocate */
...
void lighting_destroy_light(Light *light) {
  ...
  for (uint32_t i = 0; i < g_lighting_system.light_count; i++) {
    if (g_lighting_system.lights[i] == light) {
      g_lighting_system.lights[i] =
          g_lighting_system.lights[g_lighting_system.light_count - 1];
...
void lighting_update_lights(const float *view_matrix,
                            const float *proj_matrix) {
  if (!g_lighting_system.initialized)
    return;
...
void lighting_get_stats(uint32_t *total_lights, uint32_t *visible_lights,
  ...
  if (total_lights)
    *total_lights = g_lighting_system.light_count;
  if (visible_lights)
    *visible_lights = g_lighting_system.visible_lights;
  if (shadow_casters)
    *shadow_casters = g_lighting_system.shadow_casting_lights;
}
