...
bool shadow_mapping_init(uint32_t cascade_count, uint32_t cascade_size,
  ...
  for (uint32_t i = 0; i < cascade_count; i++) {
    ...
    cascade->depth_texture =
        gpu_memory_allocate(MEMORY_TYPE_TEXTURE, MEMORY_USAGE_STATIC,
  ...
  if (atlas_size > 0) {
    ...
    g_shadow_system.shadow_atlas = gpu_memory_allocate(
        MEMORY_TYPE_TEXTURE, MEMORY_USAGE_DYNAMIC, atlas_size * atlas_size * 4,
