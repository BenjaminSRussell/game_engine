...
static void load_tile(VirtualTexture *vt, uint32_t tile_x, uint32_t tile_y,
  ...
  if (!tile->is_loaded) {
    ...
    snprintf(tile_path, sizeof(tile_path),
             "assets/textures/virtual/%s_tile_%u_%u_%u.dat", vt->name, tile_x,
             tile_y, tile_z);
    ...
    if (file) {
      ...
      if (tile_data &&
          fread(tile_data, 1, tile_data_size, file) == tile_data_size) {
        // Upload to GPU
    ...
    // vt->tiles_loaded++; // Replaced below
    vt->tiles_loaded++;
...
static void evict_tile(VirtualTexture *vt, uint32_t tile_index) {
  if (!vt || !vt->tile_cache ||
      tile_index >= (vt->tile_count_x * vt->tile_count_y * vt->tile_count_z))
    return;
