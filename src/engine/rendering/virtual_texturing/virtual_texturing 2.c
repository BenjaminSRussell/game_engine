...
static uint32_t calculate_virtual_size(uint32_t virtual_width,
                                       uint32_t virtual_height,
                                       uint32_t tile_size) {
...
static uint32_t hash_tile_position(uint32_t tile_x, uint32_t tile_y,
                                   uint32_t tile_z, uint32_t table_size) {
  // Simple hash function for tile positions
  return ((tile_x * 73856093) ^ (tile_y * 193496309) ^ (tile_z * 834988911)) %
         table_size;
}

static void update_page_table(VirtualTexture *vt) {
  if (!vt || !vt->page_table || !vt->tile_cache)
    return;
  ...
  for (uint32_t i = 0; i < vt->tile_count; i++) {
    ...
    if (tile->is_loaded) {
      uint32_t hash = hash_tile_position(tile->tile_x, tile->tile_y,
                                         tile->tile_z, vt->page_table_size);
...
static void initialize_page_table(VirtualTexture *vt) {
  ...
  uint32_t table_size =
      vt->tile_count * 2; // 2x hash table for reduced collision
  ...
  LOG_DEBUG("Initialized page table for virtual texture '%s': %u entries",
            vt->name, table_size);
}
...
static void load_tile(VirtualTexture *vt, uint32_t tile_x, uint32_t tile_y,
  ...
  if (tile_index >= vt->tile_count) {
    LOG_ERROR("Tile index out of range: %u (max: %u)", tile_index,
              vt->tile_count);
    return;
  }
  ...
  if (!tile->is_loaded) {
    ...
    // For now, we simulate loading by allocating dummy data
    void *data = malloc(vt->tile_size * vt->tile_size * 4); // RGBA8
    if (data) {
      // Upload to GPU (placeholder)
      // tile->physical_index = gpu_upload_texture(data, ...);
      free(data);
    }
    ...
    LOG_DEBUG("Loaded tile (%u, %u, %u): %.2f ms", tile_x, tile_y, tile_z,
...
static void evict_tile(VirtualTexture *vt, uint32_t tile_index) {
  if (!vt || !vt->tile_cache || tile_index >= vt->tile_count)
    return;
  ...
  // if (tile->physical_index) {
  //     gpu_destroy_texture(tile->physical_index);
  //     tile->physical_index = 0;
  // }
  ...
  LOG_DEBUG("Evicted tile (%u, %u, %u)", tile->tile_x, tile->tile_y,
            tile->tile_z);
}
...
bool virtual_texture_system_init(uint32_t max_textures,
                                 uint32_t default_tile_size,
  ...
  LOG_INFO("Virtual texture system initialized (max textures: %u, tile size: "
...
VirtualTexture *virtual_texture_create(const char *name, uint32_t virtual_width,
  ...
  LOG_INFO("Created virtual texture '%s': %ux%ux%u virtual, %ux%ux%u physical "
           "(%.2f MB)",
           name, virtual_width, virtual_height, virtual_depth, tile_size,
           vt->total_memory / (1024.0f * 1024.0f));
  return vt;
}
...
void virtual_texture_destroy(VirtualTexture *vt) {
  ...
  for (uint32_t i = 0; i < g_vt_system.texture_count; i++) {
    if (g_vt_system.textures[i] == vt) {
      g_vt_system.textures[i] =
          g_vt_system.textures[g_vt_system.texture_count - 1];
...
void virtual_texture_get_tile(VirtualTexture *vt, uint32_t tile_x,
                              uint32_t tile_y, uint32_t tile_z,
                              void **texture_handle) {
...
void virtual_texture_get_streaming_stats(VirtualTexture *vt, uint32_t *loaded,
  ...
  if (loaded)
    *loaded = vt->tiles_loaded;
  if (evicted)
    *evicted = vt->tiles_evicted;
  if (cache_hits)
    *cache_hits = vt->cache_hits;
  if (cache_misses)
    *cache_misses = vt->cache_misses;
  if (streaming_time)
...
void virtual_texture_mark_tile_dirty(VirtualTexture *vt, uint32_t tile_x,
                                     uint32_t tile_y, uint32_t tile_z) {
  if (!vt)
    return;
...
void virtual_texture_get_dimensions(VirtualTexture *vt, uint32_t *width,
  ...
  if (width)
    *width = vt->virtual_width;
  if (height)
    *height = vt->virtual_height;
  if (depth)
...
void virtual_texture_set_default_parameters(uint32_t tile_size,
                                            uint32_t cache_size,
                                            float distance_threshold,
                                            float priority) {
  if (!g_vt_system.initialized)
...
void virtual_texture_get_system_stats(
    uint32_t *total_textures, uint64_t *total_memory, uint64_t *used_memory,
  ...
  if (cached_tiles)
    *cached_tiles = g_vt_system.cache_hits;
  if (streamed_tiles)
