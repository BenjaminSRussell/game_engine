...
#include "rendering/visibility/visibility_resolve.h"
...
typedef struct rendering_visibility_resolve_internal {
    uint32_t id;
    uint32_t flags;
    ResourceHandle resolve_pipeline;
    ResourceHandle output_target;
    bool initialized;
    bool dirty;
    uint32_t width;
    uint32_t height;
...
static void rendering_visibility_resolve_cleanup_internal(rendering_visibility_resolve_internal_t* item) {
    if (!item) return;
    item->resolve_pipeline = INVALID_HANDLE;
    item->output_target = INVALID_HANDLE;
...
int rendering_visibility_resolve_init(void) {
    ...
    g_visibility_resolve_ctx.capacity = 256;
...
int rendering_visibility_resolve_create(rendering_visibility_resolve_handle_t* out_handle, const rendering_visibility_resolve_desc_t* desc) {
    ...
    if (g_visibility_resolve_ctx.count >= g_visibility_resolve_ctx.capacity) {
        uint32_t new_capacity = g_visibility_resolve_ctx.capacity * 2;
        rendering_visibility_resolve_internal_t* new_items = realloc(g_visibility_resolve_ctx.items, new_capacity * sizeof(rendering_visibility_resolve_internal_t));
        if (!new_items) return -3;
        g_visibility_resolve_ctx.items = new_items;
        g_visibility_resolve_ctx.capacity = new_capacity;
    }
...
int rendering_visibility_resolve_process_pending(void) {
    if (!g_visibility_resolve_ctx.initialized) return 0;
    ...
    for (uint32_t i = 0; i < g_visibility_resolve_ctx.count; i++) {
        ...
        if (item->initialized && item->dirty) {
            ...
            // Read Visibility Buffer -> Output G-Buffer or Final Color
            item->dirty = false;
            processed++;
        }
    }
...
size_t rendering_visibility_resolve_get_memory_usage(void) {
    size_t total = sizeof(rendering_visibility_resolve_context_t);
...
void rendering_visibility_resolve_debug_print(void) {
    if (!g_visibility_resolve_ctx.initialized) return;
    
    printf("Visibility Resolve Context:\n");
    printf("  Count: %u/%u\n", g_visibility_resolve_ctx.count, g_visibility_resolve_ctx.capacity);
}
