...
#include <rendering/hdr_tonemap.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

// PI already defined in common.h
...
static Vec4 aces_rrt(Vec4 color) {
  ...
  Vec4 result;
  result.x = fminf(
      (color.x * (a * color.x + b)) / (color.x * (c * color.x + d) + e), 1.0f);
  result.y = fminf(
      (color.y * (a * color.y + b)) / (color.y * (c * color.y + d) + e), 1.0f);
  result.z = fminf(
      (color.z * (a * color.z + b)) / (color.z * (c * color.z + d) + e), 1.0f);
  result.w = color.w;

  return result;
}

Vec4 tonemap_aces(Vec4 hdr_color) { return aces_rrt(hdr_color); }

static f32 filmic_curve(f32 x) {
...
Vec4 tonemap_filmic(Vec4 hdr_color, f32 exposure) {
  ...
  Vec4 result;
  result.x = filmic_curve(x);
  result.y = filmic_curve(y);
  result.z = filmic_curve(z);
  result.w = hdr_color.w;

  return result;
}

static f32 unreal_curve(f32 x) {
  f32 a = 0.22f;
  f32 b = 0.30f;
  f32 c = 0.10f;
  f32 d = 0.20f;
...
Vec4 tonemap_unreal(Vec4 hdr_color) {
  Vec4 result;
  result.x = unreal_curve(hdr_color.x);
  result.y = unreal_curve(hdr_color.y);
  result.z = unreal_curve(hdr_color.z);
  result.w = hdr_color.w;

  return result;
}

static f32 gran_turismo_curve(f32 x) {
  f32 p = 1.0f;
  f32 a = 1.0f;
  f32 m = 0.22f;
  f32 l = 0.4f;
...
Vec4 tonemap_gran_turismo(Vec4 hdr_color) {
  Vec4 result;
  result.x = gran_turismo_curve(hdr_color.x);
  result.y = gran_turismo_curve(hdr_color.y);
  result.z = gran_turismo_curve(hdr_color.z);
  result.w = hdr_color.w;
...
static f32 srgb_component(f32 x) {
  if (x <= 0.0031308f) {
    return 12.92f * x;
  }
...
static f32 linear_component(f32 x) {
  if (x <= 0.04045f) {
    return x / 12.92f;
  }
...
Vec4 grading_gamma(Vec4 color, f32 gamma) {
  ...
  return (Vec4){powf(color.x, inv_gamma), powf(color.y, inv_gamma),
                powf(color.z, inv_gamma), color.w};
}
...
Vec4 grading_temperature(Vec4 color, f32 temperature) {
  ...
  if (temp_factor > 0.0f) {
    ...
    return (Vec4){color.x * (1.0f + temp_factor * 0.2f),
                  color.y * (1.0f + temp_factor * 0.1f),
                  color.z * (1.0f - temp_factor * 0.3f), color.w};
  } else {
...
Vec4 grading_tint(Vec4 color, f32 tint) {
  ...
  if (tint_factor > 0.0f) {
    // Magenta tint
    return (Vec4){color.x * (1.0f + tint_factor * 0.2f),
                  color.y * (1.0f - tint_factor * 0.2f),
                  color.z * (1.0f + tint_factor * 0.1f), color.w};
  } else {
    // Green tint
    return (Vec4){color.x * (1.0f - tint_factor * 0.2f),
                  color.y * (1.0f + tint_factor * 0.1f),
                  color.z * (1.0f - tint_factor * 0.2f), color.w};
  }
}
...
Vec4 grading_lift(Vec4 color, f32 lift) {
  ...
  return (Vec4){color.x + lift_amount, color.y + lift_amount,
                color.z + lift_amount, color.w};
}
...
Vec4 extract_bloom(Vec4 hdr_color, f32 threshold) {
  ...
  return (Vec4){hdr_color.x * amount, hdr_color.y * amount,
                hdr_color.z * amount, hdr_color.w};
}

Vec4 apply_bloom(Vec4 base_color, Vec4 bloom_color, f32 strength) {
  return (Vec4){base_color.x + bloom_color.x * strength,
                base_color.y + bloom_color.y * strength,
                base_color.z + bloom_color.z * strength, base_color.w};
}
...
Vec4 apply_lens_flare(Vec4 color, Vec4 light_position, f32 intensity) {
  ...
  return (Vec4){color.x + flare_amount, color.y + flare_amount,
                color.z + flare_amount, color.w};
}
...
Vec4 apply_vignette(Vec4 color, f32 strength, f32 radius) {
  ...
  return (Vec4){color.x * vignette, color.y * vignette, color.z * vignette,
                color.w};
}
