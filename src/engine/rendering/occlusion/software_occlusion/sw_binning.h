...
#ifndef OCCLUSION_SW_BINNING_H
#define OCCLUSION_SW_BINNING_H
...
typedef struct occlusion_sw_binning_handle {
    uint32_t id;
} occlusion_sw_binning_handle_t;

typedef struct occlusion_sw_binning_desc {
    uint32_t flags;
    void* user_data;
} occlusion_sw_binning_desc_t;

typedef struct occlusion_sw_binning_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} occlusion_sw_binning_info_t;
