...
#include "rendering/gpu_driven/material_data_gpu.h"
...
typedef struct rendering_material_data_gpu_internal {
    uint32_t id;
    uint32_t flags;
    ResourceHandle parameter_buffer;
    ResourceHandle texture_table;
    bool initialized;
    bool dirty;
...
static void rendering_material_data_gpu_cleanup_internal(rendering_material_data_gpu_internal_t* item) {
    if (!item) return;
    item->parameter_buffer = INVALID_HANDLE;
    item->texture_table = INVALID_HANDLE;
...
int rendering_material_data_gpu_init(void) {
    ...
    g_material_data_gpu_ctx.capacity = 256;
...
int rendering_material_data_gpu_create(rendering_material_data_gpu_handle_t* out_handle, const rendering_material_data_gpu_desc_t* desc) {
    ...
    if (g_material_data_gpu_ctx.count >= g_material_data_gpu_ctx.capacity) {
        uint32_t new_capacity = g_material_data_gpu_ctx.capacity * 2;
        rendering_material_data_gpu_internal_t* new_items = realloc(g_material_data_gpu_ctx.items, new_capacity * sizeof(rendering_material_data_gpu_internal_t));
        if (!new_items) return -3;
        g_material_data_gpu_ctx.items = new_items;
        g_material_data_gpu_ctx.capacity = new_capacity;
    }
...
int rendering_material_data_gpu_process_pending(void) {
    if (!g_material_data_gpu_ctx.initialized) return 0;

    int processed = 0;
    for (uint32_t i = 0; i < g_material_data_gpu_ctx.count; i++) {
        rendering_material_data_gpu_internal_t* item = &g_material_data_gpu_ctx.items[i];
        if (item->initialized && item->dirty) {
            // Update material parameter buffers
            // Manage texture binding table for bindless access
            item->dirty = false;
            processed++;
        }
    }
...
size_t rendering_material_data_gpu_get_memory_usage(void) {
    size_t total = sizeof(rendering_material_data_gpu_context_t);
...
void rendering_material_data_gpu_debug_print(void) {
    if (!g_material_data_gpu_ctx.initialized) return;
    
    printf("Material Data GPU Status:\n");
    printf("  Count: %u / %u\n", g_material_data_gpu_ctx.count, g_material_data_gpu_ctx.capacity);
}
