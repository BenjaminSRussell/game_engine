...
static bool foot_ik_ground_trace(FootIK* ik, Skeleton* skeleton) {
    if (ik->ankle_bone_index < 0 || ik->ankle_bone_index >= skeleton->bone_count) {
        return false;
    }
...
static void foot_ik_solve_two_bone(FootIK* ik, Skeleton* skeleton, Vec3 target_pos) {
    if (ik->hip_bone_index < 0 || ik->knee_bone_index < 0 || ik->ankle_bone_index < 0) {
        return;
    }
    ...
    f32 cos_knee_angle = (upper_leg_length * upper_leg_length + lower_leg_length * lower_leg_length - target_distance * target_distance) / 
...
static void foot_ik_update_locking(FootIK* ik, Skeleton* skeleton, f32 velocity) {
...
FootIK* foot_ik_create(i32 hip_bone, i32 knee_bone, i32 ankle_bone) {
    FootIK* ik = malloc(sizeof(FootIK));
    if (!ik) return NULL;
