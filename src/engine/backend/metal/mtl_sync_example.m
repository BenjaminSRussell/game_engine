...
void example_complete_frame_rendering(void) {
  ...
  metal_sync_manager_t *sync_mgr =
      metal_sync_manager_create(mtl_device, &sync_config);
  if (!sync_mgr) {
  ...
  metal_timestamp_pool_t *timestamps =
      metal_sync_manager_get_timestamp_pool(sync_mgr);
  ...
  while (running && frame_count < 600) { // Run for 10 seconds at 60fps
    ...
    metal_command_buffer_t cmd_buffer = metal_command_buffer_create(device);
    if (!cmd_buffer)
      continue;

    // --- GPU Timestamp Example ---
    uint32_t frame_query =
        metal_timestamp_begin(timestamps, cmd_buffer, "Full Frame");
    uint32_t render_query =
        metal_timestamp_begin(timestamps, cmd_buffer, "Render Pass");
    ...
    uint32_t compute_query =
        metal_timestamp_begin(timestamps, cmd_buffer, "Post-Process");
    ...
    if (frame_count % 60 == 0) {
      ...
      metal_sync_manager_get_stats(sync_mgr, &frame_stats, &hazard_report,
                                   &vsync_stats);

      printf("FPS: %.2f, Avg Frame Time: %.2f ms\n",
             1000.0 / frame_stats.avg_frame_time_ms,
             frame_stats.avg_frame_time_ms);
      printf("Frame Drop Rate: %.2f%%\n", (double)frame_stats.dropped_frames /
                                              frame_stats.total_frames * 100.0);
  ...
  metal_frame_sync_destroy(NULL); // Placeholder cleanup
}
...
void example_manual_synchronization(void) {
  ...
  metal_frame_pacing_config_t pacer_config = {.target_fps = 144,
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    metal_command_buffer_t cmd = metal_command_buffer_create(device);
  ...
  metal_frame_sync_get_stats(frame_sync, &stats); // Not implemented in pacer
...
void example_hazard_tracking(void) {
  ...
  metal_barrier_manager_t *barriers = metal_barrier_manager_create(
      mtl_device, METAL_BARRIER_POLICY_CONSERVATIVE);
  ...
  uint32_t barriers_inserted =
      metal_barrier_manager_access_resource(barriers, &write_access, cmd);
  printf("Barriers inserted after write: %u\n", barriers_inserted);

  // Read from texture (should detect RAW hazard)
  metal_resource_access_desc_t read_access = {.resource = texture,
                                              .access_flags = METAL_ACCESS_READ,
                                              .pipeline_stages =
                                                  METAL_STAGE_FRAGMENT,
                                              .frame_index = 1,
                                              .pass_index = 1};

  barriers_inserted =
      metal_barrier_manager_access_resource(barriers, &read_access, cmd);
