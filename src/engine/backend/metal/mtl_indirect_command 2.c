...
mtl_indirect_command_buffer_t metal_indirect_command_buffer_create(
    ...
    icb_desc.maxFragmentBufferBindCount = descriptor->max_fragment_buffer_bind_count;
    
    if (@available(macOS 11.0, *)) {
        icb_desc.maxKernelBufferBindCount = descriptor->max_kernel_buffer_bind_count;
    }
    
    // Create the ICB
    id<MTLIndirectCommandBuffer> icb = [mtl_device newIndirectCommandBufferWithDescriptor:icb_desc 
                                                                             maxCommandCount:max_command_count 
                                                                                     options:MTLResourceStorageModePrivate];
    
    return (__bridge_retained void*)icb;
#else
    return NULL;
#endif
}
