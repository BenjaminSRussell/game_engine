...
@property(nonatomic, strong)
    NSMutableDictionary<NSString *, id<MTLSamplerState>> *samplerCache;
@property(nonatomic, strong)
...
- (void)setupCommandQueues {
  ...
  self.commandQueue =
      [self.device newCommandQueueWithMaxCommandBufferCount:maxCommandBuffers];
  ...
  for (NSUInteger i = 0; i < maxCommandBuffers; i++) {
    [self.commandBuffers addObject:(id)[NSNull null]];
  }

  NSLog(@"Metal command queue created with %lu command buffers",
        (unsigned long)maxCommandBuffers);
}
...
- (void)setupFeatureDetection {
  ...
  self.supportsArgumentBuffers =
      [self.device supportsFamily:MTLGPUFamilyApple2];

  NSLog(@"Metal features - Ray Tracing: %@, Mesh Shaders: %@, Argument "
        @"Buffers: %@",
...
- (void)setupMemoryTracking {
  ...
  if (@available(macOS 10.15, *)) {
    self.maxWorkingSet = self.device.recommendedMaxWorkingSetSize;
    NSLog(@"Metal device max working set: %llu MB",
          (unsigned long long)(self.maxWorkingSet / (1024 * 1024)));
  }

  // Query hasUnifiedMemory (true for Apple Silicon)
  self.hasUnifiedMemory = self.device.hasUnifiedMemory;
  NSLog(@"Metal device unified memory: %@",
        self.hasUnifiedMemory ? @"YES" : @"NO");
  ...
  if (@available(macOS 11.0, *)) {
    ...
    // Memory pressure handling for macOS can be added later via dispatch_source
    // For now, we skip the iOS-specific notification observer to fix the crash
  }
}
...
- (void)setupHeaps {
  ...
  textureHeapDesc.type = MTLHeapTypeAutomatic;
  ...
  bufferHeapDesc.type = MTLHeapTypeAutomatic;
  ...
  uploadHeapDesc.type = MTLHeapTypeAutomatic;
...
- (void)releaseAllResources {
  ...
  for (NSUInteger i = 0; i < self.commandBuffers.count; i++) {
    id<MTLCommandBuffer> buffer = self.commandBuffers[i];
    if (buffer && ![buffer isEqual:[NSNull null]]) {
      [buffer release];
      self.commandBuffers[i] = (id)[NSNull null];
    }
  }
...
// Enable Metal API Validation in debug builds
  ...
  setenv("MTL_VALIDATE_COMMAND_BUFFERS", "1", 1);
...
- (void)loadPrecompiledLibraries {
  ...
  NSString *libraryPath = [[NSBundle mainBundle] pathForResource:@"Default"
                                                          ofType:@"metallib"];

  if (libraryPath &&
      [[NSFileManager defaultManager] fileExistsAtPath:libraryPath]) {
    NSError *error = nil;
    id<MTLLibrary> library =
        [self.device newLibraryWithURL:[NSURL fileURLWithPath:libraryPath]
                                 error:&error];
...
- (void)createSamplerStates {
  ...
  pointDesc.sAddressMode = MTLSamplerAddressModeClampToEdge;
  pointDesc.tAddressMode = MTLSamplerAddressModeClampToEdge;
  pointDesc.rAddressMode = MTLSamplerAddressModeClampToEdge;

  id<MTLSamplerState> pointSampler =
      [self.device newSamplerStateWithDescriptor:pointDesc];
  ...
  linearDesc.rAddressMode = MTLSamplerAddressModeRepeat;

  id<MTLSamplerState> linearSampler =
      [self.device newSamplerStateWithDescriptor:linearDesc];
  ...
  for (NSNumber *level in anisotropyLevels) {
    ...
    anisotropicDesc.rAddressMode = MTLSamplerAddressModeRepeat;

    id<MTLSamplerState> anisotropicSampler =
        [self.device newSamplerStateWithDescriptor:anisotropicDesc];
    NSString *samplerName =
        [NSString stringWithFormat:@"anisotropic_%@", level];
    self.samplerCache[samplerName] = anisotropicSampler;
  }

  NSLog(@"Created %lu Metal sampler states",
        (unsigned long)self.samplerCache.count);
}
...
- (void)setupBinaryArchive {
  ...
  if (@available(macOS 10.15, *)) {
    ...
    NSString *archivePath =
        [[NSBundle mainBundle] pathForResource:@"PipelineCache"
                                        ofType:@"metalar"];
    if (archivePath &&
...
static MetalDevice *g_metalDeviceInstance = nil;

bool metal_device_init(void) {
  if (g_metalDeviceInstance)
    return true;
  g_metalDeviceInstance = [MetalDevice sharedInstance];
  return g_metalDeviceInstance != nil;
}

void metal_device_shutdown(void) {
  // MetalDevice is singleton/managed by ARC, but we can nil out global ref if
  // needed Typically lifecycle is app-bound.
  g_metalDeviceInstance = nil;
}

id<MTLDevice> metal_device_get(void) {
  if (!g_metalDeviceInstance)
    metal_device_init();
  return g_metalDeviceInstance.device;
}

id<MTLCommandQueue> metal_device_get_command_queue(void) {
  if (!g_metalDeviceInstance)
    metal_device_init();
  return g_metalDeviceInstance.commandQueue;
}

void metal_device_set_layer(void *layer) {
  if (!g_metalDeviceInstance)
    metal_device_init();
  [g_metalDeviceInstance setupCAMetalLayer:(__bridge CAMetalLayer *)layer];
}
