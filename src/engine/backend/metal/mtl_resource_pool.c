...
metal_resource_pool_t *metal_resource_pool_create(metal_device_t *device) {
  ...
  metal_resource_pool_t *pool =
      (metal_resource_pool_t *)calloc(1, sizeof(metal_resource_pool_t));
  if (!pool)
    return NULL;
...
metal_buffer_t *metal_pool_acquire_buffer(metal_resource_pool_t *pool,
                                          const metal_buffer_desc_t *desc) {
  if (!pool || !desc)
    return NULL;
...
void metal_pool_return_buffer(metal_resource_pool_t *pool,
                              metal_buffer_t *buffer) {
  if (!pool || !buffer)
    return;
...
metal_texture_t *metal_pool_acquire_texture(metal_resource_pool_t *pool,
                                            const metal_texture_desc_t *desc) {
  if (!pool || !desc)
    return NULL;
...
void metal_pool_return_texture(metal_resource_pool_t *pool,
                               metal_texture_t *texture) {
  if (!pool || !texture)
    return;
...
void metal_pool_cleanup(metal_resource_pool_t *pool, uint32_t max_age_frames) {
  ...
  for (uint32_t i = 0; i < pool->buffer_count; i++) {
    if (pool->buffers[i].buffer && !pool->buffers[i].in_use) {
      uint32_t age = pool->current_frame - pool->buffers[i].last_used_frame;
  ...
  for (uint32_t i = 0; i < pool->texture_count; i++) {
    if (pool->textures[i].texture && !pool->textures[i].in_use) {
      uint32_t age = pool->current_frame - pool->textures[i].last_used_frame;
      if (age > max_age_frames) {
        metal_texture_destroy(pool->textures[i].texture);
