...
metal_dependency_graph_create(uint32_t initial_capacity) {
  if (initial_capacity == 0)
    initial_capacity = 128;

  metal_dependency_graph_t *graph =
      (metal_dependency_graph_t *)calloc(1, sizeof(metal_dependency_graph_t));
  if (!graph)
    return NULL;

  graph->resources = (metal_resource_dependency_t **)calloc(
      initial_capacity, sizeof(metal_resource_dependency_t *));
  if (!graph->resources) {
    free(graph);
    return NULL;
  }
...
bool metal_dependency_graph_record_access(
    metal_dependency_graph_t *graph,
    const metal_resource_access_desc_t *access) {
  if (!graph || !access || !access->resource)
    return false;

  uint32_t index = hash_pointer(access->resource, graph->capacity);

  // Create new dependency node
  metal_resource_dependency_t *dep = (metal_resource_dependency_t *)calloc(
      1, sizeof(metal_resource_dependency_t));
  if (!dep)
    return false;
...
bool metal_dependency_graph_check_hazards(
    metal_dependency_graph_t *graph, const metal_resource_access_desc_t *access,
    metal_resource_dependency_t **out_hazards, uint32_t max_hazards,
    uint32_t *out_count) {
  if (!graph || !access || !access->resource)
    return false;
  ...
  while (dep) {
    ...
    if (dep->access.resource == access->resource) {
      ...
      bool was_write_access =
          (dep->access.access_flags & METAL_ACCESS_WRITE) != 0;
  ...
  if (out_count)
    *out_count = hazard_count;
  return has_hazards;
}

void metal_dependency_graph_advance_frame(metal_dependency_graph_t *graph,
...
metal_barrier_manager_create(id device, metal_barrier_policy_t policy) {
  if (!device)
    return NULL;

  metal_barrier_manager_t *manager =
      (metal_barrier_manager_t *)calloc(1, sizeof(metal_barrier_manager_t));
  if (!manager)
    return NULL;
...
uint32_t metal_barrier_manager_access_resource(
    metal_barrier_manager_t *manager,
    const metal_resource_access_desc_t *access,
    mtl_command_buffer_t *cmd_buffer) {
  if (!manager || !access || !cmd_buffer)
    return 0;
  ...
  if (has_hazards) {
    ...
    if (manager->policy == METAL_BARRIER_POLICY_CONSERVATIVE ||
      ...
      // Insert barrier
      metal_insert_texture_barrier(cmd_buffer, access->resource,
                                   access->pipeline_stages);
      barriers_count++;
...
void metal_barrier_manager_insert_barrier(metal_barrier_manager_t *manager,
                                          ...
                                          mtl_command_buffer_t *cmd_buffer,
                                          uint32_t stages) {
  if (!manager || !resource || !cmd_buffer)
    return;
...
void metal_barrier_manager_get_stats(metal_barrier_manager_t *manager,
  ...
  if (out_inserted)
    *out_inserted = manager->barriers_inserted;
  if (out_hazards)
    *out_hazards = manager->hazards_detected;
}
...
metal_pass_resource_tracker_t *metal_pass_tracker_create(uint32_t max_passes) {
  ...
  metal_pass_resource_tracker_t *tracker =
      (metal_pass_resource_tracker_t *)calloc(
          1, sizeof(metal_pass_resource_tracker_t));
...
void metal_pass_tracker_record_access(
    metal_pass_resource_tracker_t *tracker,
    const metal_resource_access_desc_t *access) {
  if (!tracker || !access)
    return;
...
bool metal_coherence_update(metal_coherence_tracker_t *tracker,
                            uint32_t access_type, uint32_t stage) {
  if (!tracker)
    return false;
...
void metal_generate_hazard_report(metal_barrier_manager_t *manager,
