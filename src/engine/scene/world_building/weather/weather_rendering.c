...
bool weather_rendering_init(void) {
    ...
    const char* sky_fragment_shader =
        ...
        "    float rayleigh_phase = 3.0 / (16.0 * 3.14159) * (1.0 + cos_theta * cos_theta);\n"
        "    float mie_phase = 3.0 / (8.0 * 3.14159) * ((1.0 - 0.76) * (1.0 + cos_theta * cos_theta)) / pow(1.0 + 0.76 * 0.76 - 2.0 * 0.76 * cos_theta, 1.5);\n"
        "    \n"
        ...
        "    \n"
        "    vec3 rayleigh_scattering = rayleigh_coefficients * rayleigh_phase * rayleigh_density;\n"
        ...
        "    vec3 scattered_light = atmosphere_scattering(ray_dir, sun_direction);\n"
        "    \n"
        "    // Add sun disk\n"
        "    float sun_angular_distance = acos(dot(ray_dir, sun_direction));\n"
        "    float sun_disk = exp(-sun_angular_distance * sun_angular_distance * 10000.0);\n"
        "    scattered_light += vec3(10.0, 10.0, 8.0) * sun_disk * sun_intensity;\n"
        "    \n"
        "    frag_color = vec4(scattered_light, 1.0);\n"
        "}\n";
    
    g_weather_rendering.sky_shader = shader_compile(sky_vertex_shader, sky_fragment_shader);
    ...
    const char* cloud_fragment_shader =
        ...
        "    \n"
        "    return clamp((combined_density + coverage - 0.5) * density * weather_influence, 0.0, 1.0);\n"
        "}\n"
        "\n"
        ...
        "            float light_transmittance = exp(-cloud_density * step_size * 2.0);\n"
        "            vec3 cloud_color = mix(vec3(0.9, 0.9, 0.95), vec3(0.7, 0.7, 0.8), cloud_type);\n"
        "            \n"
        "            accumulated_color += cloud_color * cloud_density * step_size * light_transmittance * (1.0 - accumulated_alpha);\n"
        "            accumulated_alpha += (1.0 - light_transmittance) * (1.0 - accumulated_alpha);\n"
        "            \n"
        ...
        "    \n"
        "    frag_color = vec4(cloud_color, clamp(cloud_color.r + cloud_color.g + cloud_color.b, 0.0, 1.0));\n"
        "}\n";
    
    g_weather_rendering.cloud_shader = shader_compile(cloud_vertex_shader, cloud_fragment_shader);
    ...
    // Initialize textures
    g_weather_rendering.weather_map_texture = gpu_texture_create_2d(512, 512, GPU_FORMAT_R32F);
    g_weather_rendering.noise_texture_3d = gpu_texture_create_3d(128, 128, 128, GPU_FORMAT_R32F);
    g_weather_rendering.blue_noise_texture = gpu_texture_create_2d(512, 512, GPU_FORMAT_RGBA8);
        ...
        for (int x = 0; x < 128; x++) {
            for (int y = 0; y < 128; y++) {
                for (int z = 0; z < 128; z++) {
                    ...
                    noise_data[(x * 128 + y) * 128 + z] = n1 * 0.6f + n2 * 0.3f + n3 * 0.1f;
                }
            }
        }
...
void weather_manager_apply_to_rendering(WeatherManager* manager, const float* view_matrix, const float* projection_matrix, const float* camera_position) {
    ...
    cloud_weather_map(&manager->clouds, get_time_seconds(), weather_map, 512, 512);
    ...
    weather_rendering_apply_clouds(&manager->clouds, &current_params, view_matrix, projection_matrix, camera_position);
    
    // Apply fog effects
    weather_rendering_apply_fog(&current_params, camera_position);
    
    // Render precipitation
    weather_rendering_apply_precipitation(&manager->precipitation, view_matrix, projection_matrix, camera_position);
    
    // Render lightning effects
    weather_rendering_apply_lightning(&manager->lightning, view_matrix, projection_matrix, camera_position);
    
    // Update temporal state
    g_weather_rendering.frame_count++;
    
    uint64_t end_time = get_performance_counter();
    g_weather_rendering.last_frame_time_ms = (end_time - start_time) * 1000.0 / get_performance_frequency();
}

// Render atmospheric sky with scattering
static void weather_rendering_apply_sky(const WeatherParameters* params, const float* camera_position) {
    if (!params) return;
    ...
    float rayleigh_coeffs[3] = {
        0.0058f * params->sky_color[0],
    ...
    float mie_coeffs[3] = {
        0.0035f * params->horizon_color[0],
        0.0035f * params->horizon_color[1],
        0.0035f * params->horizon_color[2]
    };
    material_set_float3("mie_coefficients", mie_coeffs);
    
    material_set_float("rayleigh_scale_height", ATMOSPHERE_DEFAULT.rayleigh_scale_height);
...
// Render volumetric clouds
static void weather_rendering_apply_clouds(const VolumetricCloudSettings* cloud_settings, 
                                         const WeatherParameters* weather_params,
                                         const float* view_matrix, const float* projection_matrix,
                                         const float* camera_position) {
    if (!cloud_settings || !cloud_settings->enabled) return;
...
static void weather_rendering_apply_fog(const WeatherParameters* params, const float* camera_position) {
    if (!params || params->fog_density <= 0.0f) return;
    
    // Set global fog parameters for the rendering pipeline
    set_global_fog_parameters(
        params->fog_density,
...
static void weather_rendering_apply_precipitation(const PrecipitationSystem* precipitation,
                                                const float* view_matrix, const float* projection_matrix,
                                                const float* camera_position) {
    if (!precipitation || !precipitation->active || precipitation->active_particles == 0) return;
    ...
    material_set_float3("wind_direction", (const float[]){1.0f, 0.0f, 0.0f}); // Simplified
    material_set_texture("particle_texture", precipitation->precipitation_texture);
...
static void weather_rendering_apply_lightning(const LightningSystem* lightning,
                                            const float* view_matrix, const float* projection_matrix,
                                            const float* camera_position) {
    if (!lightning || lightning->bolt_count == 0) return;
    ...
    for (uint32_t i = 0; i < lightning->bolt_count; i++) {
        ...
        render_lightning_bolt(bolt->segments, bolt->segment_count, view_matrix, projection_matrix);
...
static float perlin_noise_3d(float x, float y, float z) {
    ...
    float a = grad(xi, yi, zi, xf, yf, zf);
    float b = grad(xi+1, yi, zi, xf-1, yf, zf);
    float c = grad(xi, yi+1, zi, xf, yf-1, zf);
    float d = grad(xi+1, yi+1, zi, xf-1, yf-1, zf);
    float e = grad(xi, yi, zi+1, xf, yf, zf-1);
    float f = grad(xi+1, yi, zi+1, xf-1, yf, zf-1);
    float g = grad(xi, yi+1, zi+1, xf, yf-1, zf-1);
    float h = grad(xi+1, yi+1, zi+1, xf-1, yf-1, zf-1);
...
void weather_rendering_get_metrics(float* frame_time_ms, uint32_t* triangles, uint32_t* draw_calls) {
    if (frame_time_ms) *frame_time_ms = g_weather_rendering.last_frame_time_ms;
    if (triangles) *triangles = g_weather_rendering.triangles_rendered;
    if (draw_calls) *draw_calls = g_weather_rendering.draw_calls;
}
