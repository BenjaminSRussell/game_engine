...
static uint64_t get_performance_counter(void) { return 0; }
static uint64_t get_performance_frequency(void) { return 1000; }
static void terrain_system_process_streaming_advanced(TerrainSystem *system) {}
void terrain_system_init_biomes(TerrainSystem *system) {}
void terrain_chunk_unload_advanced(TerrainSystem *system, TerrainChunk *chunk) {
  if (chunk->is_loaded) {
    if (chunk->heightmap)
...
terrain_config_validate(const TerrainChunkConfig *config) {
  ...
  if (config->chunk_pool_size < 64 ||
    ...
    snprintf(result.error_message, sizeof(result.error_message),
             "Invalid chunk pool size: %zu (must be 64-%u)",
             config->chunk_pool_size, config->max_chunks);
    result.is_valid = false;
    return result;
  }
  ...
  if (config->chunk_pool_size < config->max_chunks / 4) {
    snprintf(result.warnings[result.warning_count++], 128,
             "Chunk pool size (%zu) is less than 25%% of max chunks",
 ...
 */
bool internal_terrain_config_serialize(const TerrainChunkConfig *config,
...
static bool chunk_pool_initialize(ChunkPool *pool, uint32_t pool_size) {
  ...
  pool->used_count = 0;
  // Calculate total memory used by chunks
  size_t memory_used = pool_size * sizeof(TerrainChunk);
  g_terrain_state.chunk_pool.total_memory_used = memory_used;
  g_terrain_state.chunk_pool.peak_memory_used = memory_used;

  printf("Initialized chunk pool: %u chunks, %.1f MB\n", pool_size,
         (float)memory_used / (1024.0f * 1024.0f));
...
static bool terrain_config_save_to_file(const char *config_path) {
  ...
  if (!internal_terrain_config_serialize(&g_terrain_state.config, config_json,
                                         sizeof(config_json))) {
    return false;
  }
...
// Duplicate terrain_config_serialize removed
