...
static inline AABB aabb_create_empty(void) {
  return aabb_create(vec3_create(FLT_MAX, FLT_MAX, FLT_MAX),
...
static inline AABB aabb_expand(AABB box, Vec3 point) {
  return aabb_create(vec3_min(box.min, point), vec3_max(box.max, point));
}
...
static inline Vec3 aabb_extent(AABB box) { return vec3_sub(box.max, box.min); }
