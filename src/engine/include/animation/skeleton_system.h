...
typedef enum { BONE_SPACE_LOCAL, BONE_SPACE_MODEL, BONE_SPACE_WORLD } BoneSpace;
...
bool skeleton_system_init(SkeletonSystem *system, u32 max_skeletons,
                          u32 max_animations, u32 max_vertices);
void skeleton_system_shutdown(SkeletonSystem *system);

// MARK: - Skeleton Management

Skeleton *skeleton_create(SkeletonSystem *system, const char *name,
                          u32 max_bones);
...
Bone *skeleton_add_bone(Skeleton *skeleton, const char *name, BoneType type,
                        u32 parent_id);
...
bool skeleton_set_bone_transform(Skeleton *skeleton, u32 bone_id,
                                 const Vec3 *position, const Quat *rotation,
                                 const Vec3 *scale, BoneSpace space);
bool skeleton_get_bone_transform(Skeleton *skeleton, u32 bone_id,
...
AnimationClip *animation_create(SkeletonSystem *system, const char *name,
...
AnimationState *skeleton_play_animation(Skeleton *skeleton,
                                        AnimationClip *animation, float weight,
                                        AnimBlendMode blend_mode);
...
bool skeleton_set_animation_time(Skeleton *skeleton, AnimationState *state,
                                 float time);
bool skeleton_set_animation_speed(Skeleton *skeleton, AnimationState *state,
                                  float speed);
bool skeleton_set_animation_weight(Skeleton *skeleton, AnimationState *state,
                                   float weight);
...
bool skeleton_crossfade_animations(Skeleton *skeleton,
                                   AnimationState *from_state,
                                   AnimationState *to_state, float duration);

// MARK: - Vertex Skinning

bool skeleton_set_vertex_bone_data(SkeletonSystem *system, u32 vertex_index,
                                   const BoneInfluence *influences,
                                   u32 influence_count);
bool skeleton_update_skinning_matrices(SkeletonSystem *system,
                                       Skeleton *skeleton);
Mat4 *skeleton_get_skinning_matrices(SkeletonSystem *system, Skeleton *skeleton,
                                     u32 *matrix_count);

// MARK: - IK Solving

bool skeleton_solve_ik_chain(Skeleton *skeleton, u32 *bone_chain,
                             u32 chain_length, const Vec3 *target_position,
                             const Quat *target_rotation);
bool skeleton_solve_ccd_ik(Skeleton *skeleton, u32 *bone_chain,
                           u32 chain_length, const Vec3 *target_position,
                           u32 max_iterations, float tolerance);
bool skeleton_solve_fabrik_ik(Skeleton *skeleton, u32 *bone_chain,
                              u32 chain_length, const Vec3 *target_position,
                              u32 max_iterations, float tolerance);
...
bool skeleton_set_ragdoll_bone_properties(Skeleton *skeleton, u32 bone_id,
                                          ...
                                          float stiffness);
bool skeleton_apply_ragdoll_force(Skeleton *skeleton, u32 bone_id,
                                  const Vec3 *force, const Vec3 *point);
...
bool animation_import_from_json(const char *filename,
...
void skeleton_calculate_bounds(Skeleton *skeleton, Vec3 *min_bounds,
                               Vec3 *max_bounds);
bool skeleton_is_bone_descendant(Skeleton *skeleton, u32 bone_id,
                                 u32 ancestor_id);
...
void skeleton_system_set_settings(SkeletonSystem *system, bool enable_blending,
                                  bool enable_ik, bool enable_ragdoll,
                                  float max_influences);

void skeleton_system_set_callbacks(
    SkeletonSystem *system, void (*on_skeleton_created)(Skeleton *),
...
#define SKELETON_CREATE(system, name, bones)                                   \
  skeleton_create(system, name, bones)
#define SKELETON_ADD_BONE(skel, name, type, parent)                            \
  skeleton_add_bone(skel, name, type, parent)
#define SKELETON_PLAY_ANIM(skel, anim, weight, blend)                          \
  skeleton_play_animation(skel, anim, weight, blend)
#define SKELETON_UPDATE(system, delta_time)                                    \
  skeleton_system_update(system, delta_time)
#else
