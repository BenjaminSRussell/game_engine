...
typedef struct {
    ...
    void *input_state;
    u32 input_size;
    u32 component_mask; // Which components are valid
} PredictionFrame;
...
Quat prediction_predict_rotation(ClientPrediction *pred, u32 entity_id,
                               Quat current_rot, Vec3 angular_vel, f32 delta_time);
void prediction_predict_full_state(ClientPrediction *pred, u32 entity_id,
                                const PredictionFrame *current_state, f32 delta_time,
                                PredictionFrame *predicted_state);

// Enhanced reconciliation
void prediction_reconcile(ClientPrediction *pred, u32 entity_id, 
                          u32 confirmed_frame, Vec3 server_pos, Vec3 server_vel);
void prediction_reconcile_full_state(ClientPrediction *pred, u32 entity_id,
                                   u32 confirmed_frame, const PredictionFrame *server_state);

// Component management
void prediction_enable_components(ClientPrediction *pred, u32 component_mask);
void prediction_set_component_weight(ClientPrediction *pred, u32 component_type, f32 weight);
bool prediction_is_component_enabled(ClientPrediction *pred, u32 component_type);

// Custom data support
void prediction_add_custom_data(ClientPrediction *pred, u32 entity_id, 
                              u32 data_type, void *data, u32 data_size);
void *prediction_get_custom_data(ClientPrediction *pred, u32 entity_id, u32 data_type);

// Enhanced state access
void prediction_get_predicted_state(ClientPrediction *pred, u32 entity_id, PredictionFrame *state);
void prediction_get_smoothed_state(ClientPrediction *pred, u32 entity_id, PredictionFrame *state);
void prediction_get_server_state(ClientPrediction *pred, u32 entity_id, PredictionFrame *state);

// Error and quality metrics
void prediction_get_entity_errors(ClientPrediction *pred, u32 entity_id,
                                f32 *pos_error, f32 *rot_error, f32 *vel_error, f32 *health_error);
