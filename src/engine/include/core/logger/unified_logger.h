...
// Convenience functions for different log levels with explicit category
...
// Implicit category macros (defaults to LOG_CAT_GENERAL)
...
#define LOG_TRACE_STRUCT(cat, msg, ctx)                                        \
  unified_logger_log_structured(LOG_LEVEL_TRACE, cat, __FILE__, __LINE__,      \
                                __func__, msg, ctx)
#define LOG_DEBUG_STRUCT(cat, msg, ctx)                                        \
  unified_logger_log_structured(LOG_LEVEL_DEBUG, cat, __FILE__, __LINE__,      \
                                __func__, msg, ctx)
#define LOG_INFO_STRUCT(cat, msg, ctx)                                         \
  unified_logger_log_structured(LOG_LEVEL_INFO, cat, __FILE__, __LINE__,       \
                                __func__, msg, ctx)
#define LOG_WARN_STRUCT(cat, msg, ctx)                                         \
  unified_logger_log_structured(LOG_LEVEL_WARN, cat, __FILE__, __LINE__,       \
                                __func__, msg, ctx)
#define LOG_ERROR_STRUCT(cat, msg, ctx)                                        \
  unified_logger_log_structured(LOG_LEVEL_ERROR, cat, __FILE__, __LINE__,      \
                                __func__, msg, ctx)
#define LOG_FATAL_STRUCT(cat, msg, ctx)                                        \
  unified_logger_log_structured(LOG_LEVEL_FATAL, cat, __FILE__, __LINE__,      \
                                __func__, msg, ctx)
...
void unified_logger_encrypt_entry(const LogEntry *entry, char *encrypted,
                                  size_t *size);
...
#define LOGD(format, ...)                                                      \
  unified_logger_log(LOG_LEVEL_DEBUG, LOG_CAT_GENERAL, __FILE__, __LINE__,     \
                     __func__, format, ##__VA_ARGS__)
#define LOGI(format, ...)                                                      \
  unified_logger_log(LOG_LEVEL_INFO, LOG_CAT_GENERAL, __FILE__, __LINE__,      \
                     __func__, format, ##__VA_ARGS__)
#define LOGW(format, ...)                                                      \
  unified_logger_log(LOG_LEVEL_WARN, LOG_CAT_GENERAL, __FILE__, __LINE__,      \
                     __func__, format, ##__VA_ARGS__)
#define LOGE(format, ...)                                                      \
  unified_logger_log(LOG_LEVEL_ERROR, LOG_CAT_GENERAL, __FILE__, __LINE__,     \
                     __func__, format, ##__VA_ARGS__)
// The file ends here.
// But wait, the file I read earlier HAD these at the end:
// #define LOG(level, format, ...) LOG_DEBUG(LOG_CAT_GENERAL, format,
// ##__VA_ARGS__)
// ...
