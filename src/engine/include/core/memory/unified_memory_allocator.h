...
#define memory_calloc(count, size)                                             \
  unified_memory_alloc((count) * (size), MEMORY_STRATEGY_TRACKED,              \
                       MEMORY_FLAG_ZERO | MEMORY_FLAG_TRACK, __FILE__,         \
                       __LINE__, __func__)
#define memory_realloc(ptr, size)                                              \
  unified_memory_realloc(ptr, size, MEMORY_FLAG_TRACK, __FILE__, __LINE__,     \
                         __func__)
...
#define object_pool_create(block_size, count)                                  \
  unified_memory_pool_create(                                                  \
      &(MemoryPoolConfig){.block_size = block_size, .block_count = count})
#define object_pool_destroy(pool) unified_memory_pool_destroy(pool)
#define object_pool_alloc(pool, size)                                          \
  unified_memory_pool_alloc(pool, size, MEMORY_FLAG_NONE)
...
#define stack_allocator_alloc(stack, size)                                     \
  unified_memory_stack_alloc(stack, size, MEMORY_FLAG_NONE)
