...
#include "effects/gpu_particles/dead_list.h"
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
#include <string.h>
...
static void effects_dead_list_cleanup_internal(effects_dead_list_internal_t* item) {
    if (!item) return;
    // Destroy buffer if we own it
    // render_buffer_destroy(item->buffer);
...
int effects_dead_list_create(effects_dead_list_handle_t* out_handle, const effects_dead_list_desc_t* desc) {
    ...
    item->max_particles = desc->max_particles > 0 ? desc->max_particles : 10000;
    item->initialized = true;
    item->dirty = true;
    item->frame_updated = 0;
    
    // Create buffer via render resource manager (placeholder)
    // item->buffer = render_buffer_create(...);
