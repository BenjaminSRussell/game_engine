...
#ifndef WATER_RIVER_FOAM_H
#define WATER_RIVER_FOAM_H
...
typedef struct water_river_foam_handle {
    uint32_t id;
} water_river_foam_handle_t;

typedef struct water_river_foam_desc {
    uint32_t flags;
    void* user_data;
} water_river_foam_desc_t;

typedef struct water_river_foam_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} water_river_foam_info_t;
