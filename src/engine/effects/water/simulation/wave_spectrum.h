...
#ifndef WATER_WAVE_SPECTRUM_H
#define WATER_WAVE_SPECTRUM_H
...
typedef struct water_wave_spectrum_handle {
    uint32_t id;
} water_wave_spectrum_handle_t;

typedef struct water_wave_spectrum_desc {
    uint32_t flags;
    void* user_data;
} water_wave_spectrum_desc_t;

typedef struct water_wave_spectrum_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} water_wave_spectrum_info_t;
