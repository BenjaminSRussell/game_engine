...
#include "effects/particles/particle_rendering.h"
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
#include <string.h>
...
#define EFFECTS_PARTICLE_RENDERING_MAX_COUNT 4096
#define EFFECTS_PARTICLE_RENDERING_DEFAULT_CAPACITY 256
#define EFFECTS_PARTICLE_RENDERING_ALIGNMENT 16
...
} effects_particle_rendering_internal_t;

typedef struct effects_particle_rendering_context {
    effects_particle_rendering_internal_t* items;
    uint32_t count;
    uint32_t capacity;
    void* allocator;
    bool initialized;
} effects_particle_rendering_context_t;

static effects_particle_rendering_context_t g_particle_rendering_ctx = {0};
...
int effects_particle_rendering_init(void) {
    ...
    g_particle_rendering_ctx.capacity = EFFECTS_PARTICLE_RENDERING_DEFAULT_CAPACITY;
    g_particle_rendering_ctx.items = calloc(g_particle_rendering_ctx.capacity, sizeof(effects_particle_rendering_internal_t));
    if (!g_particle_rendering_ctx.items) {
        return -1;
    }

    g_particle_rendering_ctx.count = 0;
    g_particle_rendering_ctx.initialized = true;
...
void effects_particle_rendering_shutdown(void) {
    ...
    for (uint32_t i = 0; i < g_particle_rendering_ctx.count; i++) {
        effects_particle_rendering_cleanup_internal(&g_particle_rendering_ctx.items[i]);
    }

    free(g_particle_rendering_ctx.items);
    g_particle_rendering_ctx.items = NULL;
    g_particle_rendering_ctx.count = 0;
    g_particle_rendering_ctx.capacity = 0;
    g_particle_rendering_ctx.initialized = false;
}
...
int effects_particle_rendering_create(effects_particle_rendering_handle_t* out_handle, const effects_particle_rendering_desc_t* desc) {
    ...
    uint32_t index = g_particle_rendering_ctx.count++;
    effects_particle_rendering_internal_t* item = &g_particle_rendering_ctx.items[index];
...
void effects_particle_rendering_destroy(effects_particle_rendering_handle_t handle) {
    ...
    effects_particle_rendering_cleanup_internal(&g_particle_rendering_ctx.items[handle.id]);
}
...
int effects_particle_rendering_update(effects_particle_rendering_handle_t handle, const void* data, size_t size) {
    ...
    effects_particle_rendering_internal_t* item = &g_particle_rendering_ctx.items[handle.id];
    if (!item->initialized) {
        return -2;
    }
...
bool effects_particle_rendering_is_valid(effects_particle_rendering_handle_t handle) {
    ...
    return g_particle_rendering_ctx.items[handle.id].initialized;
}
...
uint32_t effects_particle_rendering_get_count(void) {
    return g_particle_rendering_ctx.count;
}
