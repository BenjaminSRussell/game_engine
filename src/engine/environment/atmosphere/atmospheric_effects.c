...
static void update_weather_transition(AtmosphericEffectsSystem *system, f32 delta_time);
static void apply_weather_parameters(AtmosphericEffectsSystem *system, WeatherCondition weather, f32 blend_factor);
static void update_cloud_physics(AtmosphericEffectsSystem *system, f32 delta_time);
static void update_precipitation_physics(AtmosphericEffectsSystem *system, f32 delta_time);

AtmosphericEffectsSystem *atmospheric_effects_create(void *device) {
  AtmosphericEffectsSystem *system = calloc(1, sizeof(AtmosphericEffectsSystem));
  if (!system) {
  ...
  system->precipitation.particles = calloc(system->precipitation.max_count, sizeof(Vec3));
  system->precipitation.velocities = calloc(system->precipitation.max_count, sizeof(Vec3));
  system->precipitation.lifetimes = calloc(system->precipitation.max_count, sizeof(f32));
  system->precipitation.sizes = calloc(system->precipitation.max_count, sizeof(f32));
  ...
  system->params.sun_direction = calculate_sun_position(system->time_of_day_hours, system->current_season);
  system->params.moon_direction = calculate_moon_position(system->time_of_day_hours);
...
void atmospheric_effects_set_time_of_day(AtmosphericEffectsSystem *system, f32 hours) {
  ...
  } else if (system->time_of_day_hours >= 7.0f && system->time_of_day_hours < 10.0f) {
    system->current_time = TIME_MORNING;
  } else if (system->time_of_day_hours >= 10.0f && system->time_of_day_hours < 14.0f) {
    system->current_time = TIME_NOON;
  } else if (system->time_of_day_hours >= 14.0f && system->time_of_day_hours < 17.0f) {
    system->current_time = TIME_AFTERNOON;
  } else if (system->time_of_day_hours >= 17.0f && system->time_of_day_hours < 19.0f) {
    system->current_time = TIME_DUSK;
  } else if (system->time_of_day_hours >= 19.0f && system->time_of_day_hours < 22.0f) {
    system->current_time = TIME_EVENING;
  } else if (system->time_of_day_hours >= 22.0f || system->time_of_day_hours < 2.0f) {
  ...
  if (sun_elevation > 0.0f) {
    ...
    f32 t = sun_elevation;
    system->params.zenith_color = vec3_lerp((Vec3){0.1f, 0.1f, 0.2f}, (Vec3){0.5f, 0.7f, 1.0f}, t);
    system->params.horizon_color = vec3_lerp((Vec3){0.2f, 0.1f, 0.1f}, (Vec3){1.0f, 0.8f, 0.6f}, t);
  } else {
    // Nighttime
...
void atmospheric_effects_advance_time(AtmosphericEffectsSystem *system, f32 delta_hours) {
  if (!system) return;
  atmospheric_effects_set_time_of_day(system, system->time_of_day_hours + delta_hours);
}
...
void atmospheric_effects_set_weather(AtmosphericEffectsSystem *system, WeatherCondition weather, f32 transition_time) {
  ...
  system->weather_transition_duration = transition_time > 0.0f ? transition_time : DEFAULT_WEATHER_TRANSITION_TIME;
...
static void update_weather_transition(AtmosphericEffectsSystem *system, f32 delta_time) {
  if (system->current_weather == system->target_weather) return;
  
  system->weather_transition_time += delta_time;
  f32 blend_factor = system->weather_transition_time / system->weather_transition_duration;
...
static void apply_weather_parameters(AtmosphericEffectsSystem *system, WeatherCondition weather, f32 blend_factor) {
  ...
  // Blend parameters
  params->fog_density = vec_lerp(params->fog_density, target_fog_density, blend_factor);
  params->fog_color = vec3_lerp(params->fog_color, target_fog_color, blend_factor);
  params->cloud_coverage = vec_lerp(params->cloud_coverage, target_cloud_coverage, blend_factor);
  params->precipitation_intensity = vec_lerp(params->precipitation_intensity, target_precipitation, blend_factor);
  params->wind_strength = vec_lerp(params->wind_strength, target_wind_strength, blend_factor);
  params->wind_direction = vec3_lerp(params->wind_direction, target_wind_direction, blend_factor);
  
  // Update visibility based on fog density
  target_visibility = 1.0f / (params->fog_density + 0.001f);
  target_visibility = fminf(target_visibility, 1000.0f); // Max visibility of 1km
}
...
void atmospheric_effects_update_parameters(AtmosphericEffectsSystem *system) {
  ...
  if (system->volumetric_fog) {
    ...
    system->volumetric_fog->fog_color = (simd_float3){
      system->params.fog_color.x,
      system->params.fog_color.y,
  ...
  if (system->atmosphere_scattering) {
    ...
    system->atmosphere_scattering->sun_direction = (simd_float3){
      system->params.sun_direction.x,
      system->params.sun_direction.y,
...
static void update_cloud_physics(AtmosphericEffectsSystem *system, f32 delta_time) {
  ...
  for (u32 i = 0; i < system->clouds.count; i++) {
    Vec3 wind_velocity = vec3_scale(system->params.wind_direction, system->params.wind_strength);
    system->clouds.positions[i] = vec3_add(system->clouds.positions[i], 
                                          vec3_scale(wind_velocity, delta_time));
    
    // Wrap clouds around world boundaries
    if (system->clouds.positions[i].x > 1500.0f) system->clouds.positions[i].x = -1500.0f;
    if (system->clouds.positions[i].x < -1500.0f) system->clouds.positions[i].x = 1500.0f;
    if (system->clouds.positions[i].z > 1500.0f) system->clouds.positions[i].z = -1500.0f;
    if (system->clouds.positions[i].z < -1500.0f) system->clouds.positions[i].z = 1500.0f;
    ...
    system->clouds.densities[i] = fclampf(system->clouds.densities[i], 0.1f, 1.0f);
  }
}

void atmospheric_effects_update_precipitation(AtmosphericEffectsSystem *system, f32 delta_time) {
  if (!system || system->params.precipitation_intensity <= 0.0f) return;
  
  update_precipitation_physics(system, delta_time);
  
  // Generate new precipitation particles
  u32 particles_to_spawn = (u32)(system->params.precipitation_intensity * 100.0f * delta_time);
  particles_to_spawn = fminu(particles_to_spawn, system->precipitation.max_count - system->precipitation.count);
  
  for (u32 i = 0; i < particles_to_spawn; i++) {
    if (system->precipitation.count >= system->precipitation.max_count) break;
    
    u32 idx = system->precipitation.count++;
    
    // Random position above camera
    system->precipitation.particles[idx] = (Vec3){
    ...
    Vec3 wind_effect = vec3_scale(system->params.wind_direction, system->params.wind_strength * 0.3f);
    Vec3 fall_velocity = (Vec3){0.0f, -system->precipitation.fall_speed, 0.0f};
    system->precipitation.velocities[idx] = vec3_add(fall_velocity, wind_effect);
...
static void update_precipitation_physics(AtmosphericEffectsSystem *system, f32 delta_time) {
  for (u32 i = 0; i < system->precipitation.count; i++) {
    ...
    system->precipitation.particles[i] = vec3_add(system->precipitation.particles[i],
                                                 vec3_scale(system->precipitation.velocities[i], delta_time));
    ...
    if (system->precipitation.particles[i].y < 0.0f || 
      ...
      if (system->precipitation.particles[i].y < 0.0f) {
        atmospheric_effects_create_splash_effects(system, system->precipitation.particles[i]);
      }
      
      // Replace with last particle
      if (i < system->precipitation.count - 1) {
        system->precipitation.particles[i] = system->precipitation.particles[system->precipitation.count - 1];
        system->precipitation.velocities[i] = system->precipitation.velocities[system->precipitation.count - 1];
        system->precipitation.lifetimes[i] = system->precipitation.lifetimes[system->precipitation.count - 1];
        system->precipitation.sizes[i] = system->precipitation.sizes[system->precipitation.count - 1];
  ...
  for (u32 i = 0; i < system->lightning.active_strikes; i++) {
    ...
    if (system->lightning.strike_times[i] <= 0.0f) {
      ...
      if (i < system->lightning.active_strikes - 1) {
        system->lightning.strike_positions[i] = system->lightning.strike_positions[system->lightning.active_strikes - 1];
        system->lightning.strike_times[i] = system->lightning.strike_times[system->lightning.active_strikes - 1];
        i--;
      }
...
void atmospheric_effects_trigger_lightning_strike(AtmosphericEffectsSystem *system, Vec3 position) {
  if (!system || system->lightning.active_strikes >= 16) return;
  ...
  LOG_INFO("Lightning strike at: %.2f, %.2f, %.2f", position.x, position.y, position.z);
}

static Vec3 calculate_sun_position(f32 time_hours, Season season) {
  // Simplified sun position calculation
  f32 hour_angle = (time_hours - 12.0f) * 15.0f * (M_PI / 180.0f); // Convert to radians
  
  // Seasonal declination angle
  f32 declination = 0.0f;
  switch (season) {
    case SEASON_SPRING: declination = 0.0f; break;
    case SEASON_SUMMER: declination = 23.5f * (M_PI / 180.0f); break;
    case SEASON_AUTUMN: declination = 0.0f; break;
    case SEASON_WINTER: declination = -23.5f * (M_PI / 180.0f); break;
  }
  ...
  f32 azimuth = atan2f(sinf(hour_angle), 
                       cosf(hour_angle) * sinf(latitude) - tanf(declination) * cosf(latitude));
...
Vec3 atmospheric_effects_get_sun_color(const AtmosphericEffectsSystem *system) {
  if (!system) return (Vec3){1.0f, 1.0f, 1.0f};
  return vec3_scale(system->params.sun_color, system->params.sun_intensity);
}

Vec3 atmospheric_effects_get_ambient_color(const AtmosphericEffectsSystem *system) {
  if (!system) return (Vec3){0.2f, 0.2f, 0.3f};
  
  // Blend horizon and zenith colors
  Vec3 sky_color = vec3_lerp(system->params.horizon_color, system->params.zenith_color, 0.5f);
  return vec3_scale(sky_color, system->params.sky_brightness * 0.3f);
}
...
const char *atmospheric_weather_to_string(WeatherCondition weather) {
  switch (weather) {
    ...
    case WEATHER_LIGHT_SNOW: return "Light Snow";
    case WEATHER_HEAVY_SNOW: return "Heavy Snow";
    case WEATHER_BLIZZARD: return "Blizzard";
    case WEATHER_FOG: return "Fog";
    case WEATHER_MIST: return "Mist";
    case WEATHER_HAZE: return "Haze";
