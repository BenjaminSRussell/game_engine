...
u64 get_time_nanos(void) {
  ...
  LARGE_INTEGER counter;
  QueryPerformanceCounter(&counter);
  return (u64)((double)counter.QuadPart * 1e9 / (double)frequency.QuadPart);
#elif defined(__APPLE__)
  static mach_timebase_info_data_t timebase = {0};
  if (timebase.denom == 0) {
    mach_timebase_info(&timebase);
...
f32 nanos_to_ms(u64 nanos) { return (f32)nanos / 1000000.0f; }
...
void time_system_shutdown(TimeSystem *time) { (void)time; }
...
void time_system_set_time_scale(TimeSystem *time, f32 scale) {
  if (time) {
    ...
    if (time->time_scale > 10.0f)
      time->time_scale = 10.0f;
  }
}
...
const char *time_format_timestamp(const Timestamp *ts, char *buffer, u32 size) {
  ...
  snprintf(buffer, size, "%04d-%02d-%02d %02d:%02d:%02d.%03d", ts->year,
