...
bool macos_promotion_init(void (*frame_callback)(f64, void *),
                          void *user_data) {
  // Get main display
  ...
  // Create display link
  CVReturn status =
      CVDisplayLinkCreateWithCGDisplay(display_id, &g_promotion.display_link);
  ...
  CVTime time =
      CVDisplayLinkGetNominalOutputVideoRefreshPeriod(g_promotion.display_link);
  if (time.flags & kCVTimeIsIndefinite) {
    g_promotion.nominal_refresh_rate = 60.0; // Default fallback
  } else {
    g_promotion.nominal_refresh_rate =
        (f64)time.timeScale / (f64)time.timeValue;
  }
  ...
  CVDisplayLinkSetOutputCallback(g_promotion.display_link,
                                 display_link_callback, NULL);

  // Start the display link
  CVDisplayLinkStart(g_promotion.display_link);

  os_log_info(OS_LOG_DEFAULT,
              "ProMotion initialized: %.1fHz, Variable Refresh: %s",
              g_promotion.nominal_refresh_rate,
              g_promotion.variable_refresh_enabled ? "YES" : "NO");
...
f64 macos_promotion_get_refresh_rate(void) {
  return g_promotion.actual_refresh_rate > 0 ? g_promotion.actual_refresh_rate
                                             : g_promotion.nominal_refresh_rate;
}
...
void macos_promotion_set_target_refresh_rate(f64 target_rate) {
  if (!g_promotion.variable_refresh_enabled)
    return;
...
bool macos_hdr_init(void) {
  ...
  kern_return_t result = IORegistryEntryCreateCFProperties(
      display_service, &properties, kCFAllocatorDefault, kNilOptions);

  if (result == kIOReturnSuccess && properties) {
    // Check for HDR support
    CFBooleanRef hdr_supported =
        CFDictionaryGetValue(properties, CFSTR("HDRSupported"));
    g_hdr.hdr_supported = hdr_supported && CFBooleanGetValue(hdr_supported);

    if (g_hdr.hdr_supported) {
      // Get luminance range
      CFNumberRef max_luminance =
          CFDictionaryGetValue(properties, CFSTR("MaxLuminance"));
      CFNumberRef min_luminance =
          CFDictionaryGetValue(properties, CFSTR("MinLuminance"));

      if (max_luminance) {
        CFNumberGetValue(max_luminance, kCFNumberFloatType,
                         &g_hdr.max_luminance);
      }
      if (min_luminance) {
        CFNumberGetValue(min_luminance, kCFNumberFloatType,
                         &g_hdr.min_luminance);
      }

      // Default values if not found
      if (g_hdr.max_luminance == 0)
        g_hdr.max_luminance = 1000.0f; // ProDisplay XDR typical
      if (g_hdr.min_luminance == 0)
        g_hdr.min_luminance = 0.1f;
    }
  ...
  os_log_info(OS_LOG_DEFAULT,
              "HDR initialized: Supported=%s, Max Luminance=%.1f nits",
              g_hdr.hdr_supported ? "YES" : "NO", g_hdr.max_luminance);

  return g_hdr.hdr_supported;
}

void macos_hdr_shutdown(void) { memset(&g_hdr, 0, sizeof(g_hdr)); }

bool macos_hdr_is_supported(void) { return g_hdr.hdr_supported; }
...
bool macos_hdr_is_enabled(void) { return g_hdr.hdr_enabled; }

void macos_hdr_get_luminance_range(f32 *min_luminance, f32 *max_luminance) {
  if (min_luminance)
    *min_luminance = g_hdr.min_luminance;
  if (max_luminance)
    *max_luminance = g_hdr.max_luminance;
}
...
bool macos_unified_memory_init(void) {
  ...
  if (sysctlbyname("hw.memsize", &g_unified_memory.total_memory, &size, NULL,
                   0) != 0) {
    return false;
  }
  ...
  if (g_unified_memory.unified_memory) {
    g_unified_memory.pool_size =
        g_unified_memory.total_memory / 8; // Use 1/8 of total memory
    ...
    if (g_unified_memory.shared_buffer_pool) {
      ...
      vm_protect(mach_task_self(),
                 (vm_address_t)g_unified_memory.shared_buffer_pool,
                 g_unified_memory.pool_size, FALSE, prot);
    }
  }

  os_log_info(OS_LOG_DEFAULT,
              "Unified Memory: %s, Total: %llu MB, Pool: %zu MB",
...
void *macos_unified_memory_alloc_shared(size_t size) {
  if (!g_unified_memory.unified_memory ||
      !g_unified_memory.shared_buffer_pool) {
...
void macos_unified_memory_free_shared(void *ptr, size_t size) {
  ...
  // In a real implementation, this would use a proper allocator with free
  // functionality
  (void)ptr;
  (void)size;
}
...
void macos_activity_begin(const char *activity_name) {
  g_current_activity = os_activity_create(activity_name, OS_ACTIVITY_CURRENT,
...
u64 macos_get_total_memory(void) { return g_unified_memory.total_memory; }

// MARK: - Accelerate Framework Integration

void macos_vector_add_simd(const f32 *a, const f32 *b, f32 *result,
                           size_t count) {
  vDSP_vadd(a, 1, b, 1, result, 1, count);
}

void macos_vector_multiply_simd(const f32 *a, const f32 *b, f32 *result,
                                size_t count) {
