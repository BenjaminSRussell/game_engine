...
static f32 calculate_triangle_area(const Vec3* v1, const Vec3* v2, const Vec3* v3) {
...
static f32 calculate_edge_error(const vertex_info_t* v1, const vertex_info_t* v2) {
    // Simple distance-based error metric
    f32 distance = vec3_distance(v1->position, v2->position);
    
    // Add normal difference penalty
    f32 normal_diff = acosf(fmaxf(-1.0f, fminf(1.0f, vec3_dot(v1->normal, v2->normal))));
    
    // Add UV difference penalty
    f32 uv_diff = vec2_distance(v1->uv, v2->uv);
    
    return distance + normal_diff * 0.1f + uv_diff * 0.05f;
}

static bool is_boundary_edge(const vertex_info_t* vertices, u32 vertex_count, u32 v1, u32 v2) {
    // Count how many triangles share this edge
    u32 shared_triangles = 0;
    
    // This is a simplified check - in a full implementation we'd need to track triangle adjacency
    for (u32 i = 0; i < vertex_count; i++) {
        ...
        for (u32 j = 0; j < vertices[i].adjacent_count; j++) {
            if (vertices[i].adjacent_vertices[j] == v1) adjacent_to_v1 = true;
            if (vertices[i].adjacent_vertices[j] == v2) adjacent_to_v2 = true;
        }
...
static vertex_info_t* create_vertex_info(const mesh_t* mesh) {
    ...
    for (u32 i = 0; i < mesh->vertex_count; i++) {
        ...
        vertices[i].adjacent_vertices = malloc(vertices[i].adjacent_capacity * sizeof(u32));
    ...
    for (u32 i = 0; i < mesh->index_count; i += 3) {
        ...
        for (u32 j = 0; j < 3; j++) {
            ...
            if (!already_adjacent) {
                if (vertices[vertex_idx].adjacent_count >= vertices[vertex_idx].adjacent_capacity) {
                    vertices[vertex_idx].adjacent_capacity *= 2;
                    vertices[vertex_idx].adjacent_vertices = realloc(
                        vertices[vertex_idx].adjacent_vertices,
                        vertices[vertex_idx].adjacent_capacity * sizeof(u32)
                    );
                }
                vertices[vertex_idx].adjacent_vertices[vertices[vertex_idx].adjacent_count++] = next_idx;
            }
            ...
            if (!already_adjacent) {
                ...
                vertices[vertex_idx].adjacent_vertices[vertices[vertex_idx].adjacent_count++] = prev_idx;
            }
        }
    }
...
static edge_collapse_t* create_edge_collapse_list(const vertex_info_t* vertices, u32 vertex_count, u32* collapse_count) {
    ...
    *collapse_count = 0;
    ...
    for (u32 i = 0; i < vertex_count; i++) {
        ...
        for (u32 j = 0; j < vertices[i].adjacent_count; j++) {
            ...
            if (adjacent_idx <= i || vertices[adjacent_idx].deleted) continue;
            
            if (*collapse_count >= max_collapses) {
                // Resize array
                max_collapses *= 2;
                edge_collapse_t* new_collapses = realloc(collapses, max_collapses * sizeof(edge_collapse_t));
                if (!new_collapses) {
                    free(collapses);
                    return NULL;
                }
            ...
            collapse->error = calculate_edge_error(&vertices[i], &vertices[adjacent_idx]);
            
            // Target position is the midpoint
            collapse->target_position = vec3_lerp(vertices[i].position, vertices[adjacent_idx].position, 0.5f);
            collapse->valid = true;
...
static int compare_edge_collapse(const void* a, const void* b) {
    ...
    if (ca->error < cb->error) return -1;
    if (ca->error > cb->error) return 1;
    return 0;
}

static mesh_t* simplify_mesh_qem(const mesh_t* source, f32 target_ratio, const lod_generation_config_t* config) {
    if (!source || target_ratio >= 1.0f || target_ratio <= 0.0f) return NULL;
    ...
    u32 collapse_count;
    edge_collapse_t* collapses = create_edge_collapse_list(vertices, source->vertex_count, &collapse_count);
    if (!collapses) {
    ...
    qsort(collapses, collapse_count, sizeof(edge_collapse_t), compare_edge_collapse);
    
    // Perform edge collapses
    u32 collapsed_edges = 0;
    u32 target_collapses = current_triangles - target_triangles;
    
    for (u32 i = 0; i < collapse_count && collapsed_edges < target_collapses; i++) {
        edge_collapse_t* collapse = &collapses[i];
        
        if (!collapse->valid || vertices[collapse->vertex1].deleted || vertices[collapse->vertex2].deleted) {
            continue;
        }
        
        // Check if this is a boundary edge and we should preserve it
        if (config && config->preserve_boundaries) {
            if (is_boundary_edge(vertices, source->vertex_count, collapse->vertex1, collapse->vertex2)) {
                continue;
            }
        }
        ...
        for (u32 j = 0; j < v2->adjacent_count; j++) {
            ...
            if (adjacent_idx == collapse->vertex1) continue;
    ...
    for (u32 i = 0; i < source->vertex_count; i++) {
        if (!vertices[i].deleted) {
            ...
            vertex_t* new_vertex = &simplified->vertices[vertex_index];
            new_vertex->position = vertices[i].position;
            new_vertex->normal = vertices[i].normal;
            new_vertex->uv = vertices[i].uv;
    ...
    for (u32 i = 0; i < source->index_count; i += 3) {
        ...
        f32 area = calculate_triangle_area(
            &simplified->vertices[i0].position,
    ...
    if (source->submesh_count > 0) {
        ...
        memcpy(simplified->submeshes, source->submeshes, source->submesh_count * sizeof(submesh_t));
    }
    
    // Copy material information
    simplified->material_count = source->material_count;
    memcpy(simplified->material_ids, source->material_ids, source->material_count * sizeof(u32));
...
lod_chain_t* lod_generate_chain_advanced(const mesh_t* source, const lod_generation_config_t* config) {
    if (!source) return NULL;
    
    lod_chain_t* chain = calloc(1, sizeof(lod_chain_t));
    if (!chain) return NULL;
    
    const lod_generation_config_t* cfg = config ? config : &lod_get_default_config();
    chain->config = *cfg;
    ...
    // Generate LOD levels
    u32 max_lods = sizeof(cfg->reduction_ratios) / sizeof(cfg->reduction_ratios[0]);
    ...
    for (u32 i = 0; i < max_lods && chain->lod_count < MESH_MAX_LODS; i++) {
        ...
        printf("Generated LOD %u: %u vertices, %u triangles (%.1f%% of original)\n",
               ...
               lod_mesh->index_count / 3,
               reduction_ratio * 100.0f);
    }
    
    return chain;
}

lod_chain_t* lod_generate_chain(const mesh_t* source, const f32* ratios, u32 count) {
    if (!source || !ratios || count == 0) return NULL;
    
    lod_generation_config_t config = lod_get_default_config();
    
    // Copy provided ratios
    u32 copy_count = fminf(count, sizeof(config.reduction_ratios) / sizeof(config.reduction_ratios[0]));
...
mesh_t* qem_simplify_advanced(const mesh_t* source, f32 target_ratio, 
                              const lod_generation_config_t* config, f32* out_error) {
