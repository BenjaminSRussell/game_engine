...
uint32_t font_library_load_font_from_memory(FontLibrary *library, const uint8_t *data, 
                                           size_t data_size, float size, FontRenderMode render_mode);
...
bool font_get_glyph_bounds(AdvancedFont *font, uint32_t codepoint, float *out_x, float *out_y, 
                          float *out_width, float *out_height);
float font_get_text_width(AdvancedFont *font, const char *text, size_t text_length);
float font_get_text_height(AdvancedFont *font, const char *text, size_t text_length);
...
bool sdf_renderer_add_text_with_outline(SDFRenderer *renderer, AdvancedFont *font, const char *text,
                                       float x, float y, float scale, const float color[4],
                                       const float outline_color[4], float outline_width);
bool sdf_renderer_add_text_with_shadow(SDFRenderer *renderer, AdvancedFont *font, const char *text,
                                      float x, float y, float scale, const float color[4],
                                      const float shadow_color[4], float shadow_offset_x, float shadow_offset_y);
...
bool bidi_text_resolve_paragraph(BidiText *bidi_text, uint32_t start_offset, uint32_t end_offset, 
...
bool bidi_text_get_logical_from_visual(const BidiText *bidi_text, uint32_t visual_index, uint32_t *out_logical);
bool bidi_text_get_visual_from_logical(const BidiText *bidi_text, uint32_t logical_index, uint32_t *out_visual);
...
bool text_shaper_reshape_text(ShapedText *shaped_text, const ShapingOptions *new_options);
ScriptType text_shaper_detect_script(const char *text, size_t length);
bool text_shaper_get_glyph_at_position(const ShapedText *shaped_text, float x, float y, 
                                      uint32_t *out_glyph_index, float *out_offset);
bool text_shaper_get_cursor_position(const ShapedText *shaped_text, uint32_t text_index, 
                                    float *out_x, float *out_y, bool *out_trailing);
...
typedef struct RichTextSpan {
    ...
    union {
        ...
        struct { float size; } font_size;
        struct { char family[64]; } font_family;
        struct { char url[256]; } link;
        struct { uint32_t custom_id; } custom;
    } data;
...
bool rich_text_add_span(RichText *rich_text, uint32_t start, uint32_t end, RichTextTagType type, const void *data);
bool rich_text_clear_spans(RichText *rich_text);
RichTextSpan *rich_text_get_spans_at_offset(const RichText *rich_text, uint32_t offset, uint32_t *out_count);
...
bool text_renderer_load_font_from_memory(TextRenderer *renderer, const uint8_t *data, size_t size, float font_size, FontRenderMode mode);
...
bool text_renderer_render_text_with_layout(TextRenderer *renderer, const char *text, 
                                          const TextLayout *layout, const ShapingOptions *shaping);
bool text_renderer_render_rich_text(TextRenderer *renderer, const RichText *rich_text, 
                                   float x, float y, uint32_t font_id, float font_size);
...
bool text_renderer_get_cursor_position(TextRenderer *renderer, const char *text, uint32_t font_id, 
                                      float font_size, uint32_t character_index, float *out_x, float *out_y);
...
bool text_renderer_dump_font_info(TextRenderer *renderer, uint32_t font_id, const char *output_path);
bool text_renderer_validate_fonts(TextRenderer *renderer, char (*out_messages)[256], uint32_t max_messages);
