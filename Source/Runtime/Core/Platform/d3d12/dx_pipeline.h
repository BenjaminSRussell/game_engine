...
#ifndef PLATFORM_DX_PIPELINE_H
#define PLATFORM_DX_PIPELINE_H
...
typedef struct platform_dx_pipeline_handle {
    uint32_t id;
} platform_dx_pipeline_handle_t;

typedef struct platform_dx_pipeline_desc {
    uint32_t flags;
    void* user_data;
} platform_dx_pipeline_desc_t;

typedef struct platform_dx_pipeline_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} platform_dx_pipeline_info_t;
