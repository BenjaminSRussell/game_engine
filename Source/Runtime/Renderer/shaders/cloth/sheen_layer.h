...
#ifndef SHADING_SHEEN_LAYER_H
#define SHADING_SHEEN_LAYER_H
...
typedef struct shading_sheen_layer_handle {
    uint32_t id;
} shading_sheen_layer_handle_t;

typedef struct shading_sheen_layer_desc {
    uint32_t flags;
    void* user_data;
} shading_sheen_layer_desc_t;

typedef struct shading_sheen_layer_info {
    uint32_t id;
    uint32_t flags;
    bool initialized;
} shading_sheen_layer_info_t;
