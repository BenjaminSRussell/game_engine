...
extern RGResourceHandle bloom_add_to_graph(RenderGraph *rg,
                                           RGResourceHandle scene_color,
                                           f32 intensity, u32 iterations);
extern RGResourceHandle tonemap_add_to_graph(RenderGraph *rg,
                                             RGResourceHandle hdr_color,
                                             u32 operator, f32 exposure);
extern RGResourceHandle color_grade_add_to_graph(RenderGraph *rg,
...
RGResourceHandle post_processing_add_to_graph(RenderGraph *rg,
  ...
  if (pipeline->config.enable_bloom) {
    current = bloom_add_to_graph(rg, current, pipeline->config.bloom_intensity,
  ...
  if (pipeline->config.enable_tonemapping) {
    current =
        tonemap_add_to_graph(rg, current, pipeline->config.tonemap_operator,
  ...
  if (pipeline->config.enable_ssao && pipeline->ssao && normal_buffer.id != 0) {
    current =
        ssao_add_to_graph(rg, pipeline->ssao, depth_buffer, normal_buffer);
  ...
  if (pipeline->config.enable_ssr && pipeline->ssr && normal_buffer.id != 0) {
    ...
    current = ssr_add_to_graph(rg, pipeline->ssr, current, normal_roughness,
                               depth_buffer);
  ...
  if (pipeline->config.enable_color_grading &&
      pipeline->config.lut_texture != 0) {
    current =
        color_grade_add_to_graph(rg, current, pipeline->config.lut_texture);
    LOG_DEBUG("Color grading added to post-processing graph");
  }

  LOG_DEBUG("Post-processing stack complete: %ux%u", pipeline->width,
            pipeline->height);
  return current;
}
