 ...
 * SDF rendering, complex text layout, bidirectional text, and professional text
 * shaping.
 *
 * =================================================================================================
 */
  ...
  while (remaining > 0) {
    ...
    if (str->length >= str->capacity) {
      ...
      uint32_t *new_codepoints =
          (uint32_t *)realloc(str->codepoints, new_capacity * sizeof(uint32_t));
      if (!new_codepoints) {
...
bool unicode_string_normalize_nfc(UnicodeString *str) {
  ...
  // Simplified NFC normalization - in production, use proper Unicode
  // normalization This is a placeholder that just validates the string
...
AdvancedFontLibrary *advanced_font_library_create(void) {
  AdvancedFontLibrary *library =
      (AdvancedFontLibrary *)calloc(1, sizeof(AdvancedFontLibrary));
  if (!library)
    return NULL;

  library->font_capacity = 64;
  library->fonts =
      (AdvancedFont *)calloc(library->font_capacity, sizeof(AdvancedFont));
  if (!library->fonts) {
    free(library);
    return NULL;
  }
  ...
  library->font_cache =
      (FontCacheEntry *)calloc(library->cache_capacity, sizeof(FontCacheEntry));
...
void advanced_font_library_destroy(AdvancedFontLibrary *library) {
  if (!library)
    return;
    ...
    if (library->fonts[i].initialized) {
      ...
      if (library->fonts[i].atlases) {
        for (uint32_t j = 0; j < library->fonts[i].atlas_count; j++) {
          if (library->fonts[i].atlases[j].owns_pixels &&
              library->fonts[i].atlases[j].pixels) {
...
static bool load_font_data(const char *file_path, uint8_t **out_data,
                           size_t *out_size) {
...
uint32_t advanced_font_library_load_font(AdvancedFontLibrary *library,
                                         const char *file_path, float size,
                                         AdvancedFontRenderMode render_mode) {
  if (!library || !library->initialized || !file_path)
    return 0xFFFFFFFF;

  if (library->font_count >= library->font_capacity)
    return 0xFFFFFFFF;
  ...
  font->hinting = ADVANCED_FONT_HINTING_AUTO;
  ...
  font->glyphs =
      (AdvancedGlyph *)calloc(font->glyph_capacity, sizeof(AdvancedGlyph));
  if (!font->glyphs) {
    free(font_data);
    library->font_count--;
    return 0xFFFFFFFF;
  ...
  font->kerning_pairs = (AdvancedKerningPair *)calloc(
      font->kerning_capacity, sizeof(AdvancedKerningPair));
  ...
  font->max_atlases = 8;
  font->atlases =
      (AdvancedFontAtlas *)calloc(font->max_atlases, sizeof(AdvancedFontAtlas));
  if (!font->atlases) {
  ...
  font->supported_codepoints =
      (uint32_t *)malloc(font->supported_capacity * sizeof(uint32_t));
  ...
  font->fallback_font_ids =
      (uint32_t *)malloc(font->fallback_capacity * sizeof(uint32_t));
  ...
  for (uint32_t c = 32; c < 127; c++) {
    ...
    glyph->bearing_x = 0;
    glyph->bearing_y =
        font->metrics.ascender * (size / font->metrics.units_per_em);
  ...
  if (render_mode == ADVANCED_FONT_RENDER_SDF &&
      library->default_sdf_font_id == 0xFFFFFFFF) {
...
AdvancedFont *advanced_font_library_get_font(AdvancedFontLibrary *library,
                                             uint32_t font_id) {
...
AdvancedGlyph *advanced_font_get_glyph(AdvancedFont *font, uint32_t codepoint) {
  ...
  // Simple linear search for now - in production, use a hash map or binary
  // search
  ...
  for (uint32_t i = 0; i < font->fallback_count; i++) {
    // In a real implementation, we would look up the fallback font and search
    // its glyphs For now, return NULL
  }
...
float advanced_font_get_text_width(AdvancedFont *font, const char *text,
  ...
  while (remaining > 0) {
    ...
    AdvancedGlyph *glyph = advanced_font_get_glyph(font, codepoint);
    if (!glyph) {
    ...
    // Add kerning
    if (prev_codepoint != 0) {
      total_width += advanced_font_get_kerning(font, prev_codepoint, codepoint);
    }
...
void sdf_renderer_shutdown(SDFRenderer *renderer) {
  ...
  // Clean up resources
  if (renderer->batch.positions)
    free(renderer->batch.positions);
  if (renderer->batch.texcoords)
    free(renderer->batch.texcoords);
  if (renderer->batch.colors)
    free(renderer->batch.colors);
  if (renderer->batch.indices)
    free(renderer->batch.indices);
...
bool sdf_renderer_add_text(SDFRenderer *renderer, AdvancedFont *font,
                           ...
                           const float color[4]) {
  if (!renderer || !font || !text || !renderer->initialized)
    return false;
  ...
  while (*p) {
    ...
    if (prev_codepoint != 0) {
      current_x +=
          advanced_font_get_kerning(font, prev_codepoint, codepoint) * scale;
    }

    // Add glyph to batch (simplified - in production, you'd add to vertex
    // buffers)
...
bool text_layout_calculate(TextLayout *layout) {
  if (!layout || !layout->font || !layout->text)
    return false;
  ...
  layout->lines = (TextLine *)malloc(estimated_lines * sizeof(TextLine));
  if (!layout->lines) {
  ...
  for (size_t i = 0; i <= unicode_text->length; i++) {
    ...
    if (force_break ||
      ...
      if (layout->line_count < estimated_lines) {
        TextLine *line = &layout->lines[layout->line_count];
      ...
      while (line_start < unicode_text->length &&
             is_whitespace(unicode_text->codepoints[line_start])) {
        line_start++;
      }
    ...
    if (i < unicode_text->length) {
      ...
      if (glyph) {
        ...
        if (i > line_start) {
          current_line_width += advanced_font_get_kerning(
              font, unicode_text->codepoints[i - 1], codepoint);
        }
      }
    }
  }
...
TextRenderer *text_renderer_create(void) {
  ...
  renderer->font_library = advanced_font_library_create();
  ...
  if (!renderer->sdf_renderer) {
    advanced_font_library_destroy(renderer->font_library);
    free(renderer);
    return NULL;
  }
...
bool text_renderer_load_font(TextRenderer *renderer, const char *file_path,
                             float size, AdvancedFontRenderMode mode) {
  if (!renderer || !renderer->initialized || !file_path)
    return false;

  uint32_t font_id = advanced_font_library_load_font(renderer->font_library,
                                                     file_path, size, mode);
...
bool text_renderer_render_text(TextRenderer *renderer, const char *text,
  ...
  AdvancedFont *font =
      advanced_font_library_get_font(renderer->font_library, font_id);
  if (!font)
    return false;

  bool result = sdf_renderer_add_text(renderer->sdf_renderer, font, text, x, y,
                                      font_size, color);
...
const char *text_error_to_string(TextError error) {
  switch (error) {
  case TEXT_ERROR_NONE:
    return "No error";
  case TEXT_ERROR_INVALID_PARAMETER:
    return "Invalid parameter";
  case TEXT_ERROR_OUT_OF_MEMORY:
    return "Out of memory";
  case TEXT_ERROR_FONT_NOT_FOUND:
    return "Font not found";
  case TEXT_ERROR_FONT_LOAD_FAILED:
    return "Font load failed";
  case TEXT_ERROR_GLYPH_NOT_FOUND:
    return "Glyph not found";
  case TEXT_ERROR_UNSUPPORTED_SCRIPT:
    return "Unsupported script";
  case TEXT_ERROR_SHAPING_FAILED:
    return "Text shaping failed";
  case TEXT_ERROR_LAYOUT_FAILED:
    return "Text layout failed";
  case TEXT_ERROR_RENDERING_FAILED:
    return "Text rendering failed";
  case TEXT_ERROR_UNICODE_ERROR:
    return "Unicode error";
  case TEXT_ERROR_FILE_NOT_FOUND:
    return "File not found";
  case TEXT_ERROR_PARSE_ERROR:
    return "Parse error";
  default:
...
TextError text_get_last_error(void) { return g_last_error; }

const char *text_get_last_error_message(void) { return g_last_error_message; }