...
// Forward declarations using standard types
...
static bool renderer_validate_framebuffer(Framebuffer *framebuffer) {
  ...
  if (framebuffer_get_width(framebuffer) == 0 ||
      framebuffer_get_height(framebuffer) == 0) {
    LOG_ERROR("Framebuffer has invalid dimensions: %ux%u",
              framebuffer_get_width(framebuffer),
              framebuffer_get_height(framebuffer));
    return false;
  }
