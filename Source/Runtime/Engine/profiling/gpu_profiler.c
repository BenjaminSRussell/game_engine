...
static const char *category_names[GPU_PROFILE_CATEGORY_COUNT] = {
    "Rendering", "Compute",   "Transfer", "PostProcess", "Shadows",
    "Geometry",  "Animation", "Physics",  "Audio",       "Other"};

// ============================================================================
// CREATION AND DESTRUCTION
// ============================================================================

GPUProfilerContext *gpu_profiler_create(u32 max_queries_per_frame,
                                        u32 frame_delay) {
  LOG_INFO_CAT(LOG_CAT_GRAPHICS,
               "Creating GPU profiler (queries: %u, delay: %u)",
               max_queries_per_frame, frame_delay);

  GPUProfilerContext *ctx = MALLOC_PERSISTENT(sizeof(GPUProfilerContext));
  if (!ctx) {
    LOG_ERROR_CAT(LOG_CAT_GRAPHICS, "Failed to allocate GPU profiler context");
    return NULL;
  }
  ...
  ctx->entries =
      MALLOC_PERSISTENT(ctx->entry_capacity * sizeof(GPUProfileEntry));
  if (!ctx->entries) {
    LOG_ERROR_CAT(LOG_CAT_GRAPHICS, "Failed to allocate GPU profiler entries");
    FREE(ctx);
    return NULL;
  }
  ...
  if (!ctx->timestamp_buffer) {
    LOG_ERROR_CAT(LOG_CAT_GRAPHICS, "Failed to allocate timestamp buffer");
    FREE(ctx->entries);
    FREE(ctx);
    return NULL;
  }
  ...
  ctx->timestamp_query_pool =
      gpu_create_timestamp_query_pool(max_queries_per_frame * 2);
  if (!ctx->timestamp_query_pool) {
    LOG_ERROR_CAT(LOG_CAT_GRAPHICS, "Failed to create timestamp query pool");
    FREE(ctx->entries);
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "GPU profiler created successfully");
  return ctx;
}
...
void gpu_profiler_destroy(GPUProfilerContext *ctx) {
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "Destroying GPU profiler");
...
bool gpu_profiler_initialize(u32 max_queries_per_frame, u32 frame_delay) {
  if (g_gpu_profiler) {
    LOG_WARN_CAT(LOG_CAT_GRAPHICS, "GPU profiler already initialized");
    return true;
  }
...
GPUProfilerContext *gpu_profiler_get_global(void) { return g_gpu_profiler; }
...
void gpu_profiler_begin_capture(void) {
  if (!g_gpu_profiler || !g_gpu_profiler->initialized) {
    LOG_WARN_CAT(LOG_CAT_GRAPHICS, "GPU profiler not initialized");
    return;
  }
  ...
  memset(g_gpu_profiler->category_times_ms, 0,
         sizeof(g_gpu_profiler->category_times_ms));
  memset(g_gpu_profiler->category_counts, 0,
         sizeof(g_gpu_profiler->category_counts));

  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "GPU profiler capture started");
}
...
void gpu_profiler_end_capture(void) {
  ...
  if (g_gpu_profiler->current_query_index > 0) {
    gpu_get_timestamp_query_results(g_gpu_profiler->timestamp_query_pool, 0,
                                    g_gpu_profiler->current_query_index,
    ...
    for (u32 i = 0; i < g_gpu_profiler->current_query_index / 2; i++) {
      ...
      if (start < end) {
        ...
        for (u32 j = 0; j < g_gpu_profiler->entry_count; j++) {
          ...
          if (entry->frame_index ==
                  g_gpu_profiler->current_frame - g_gpu_profiler->frame_delay &&
              entry->timestamp_start == 0) {
            entry->timestamp_start = start;
            entry->timestamp_end = end;
            entry->duration_ms =
                (f32)(end - start) / 1000000.0f; // Convert to ms

            // Update statistics
            g_gpu_profiler->category_times_ms[entry->category] +=
                entry->duration_ms;
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "GPU profiler capture ended");
}
...
GPUProfileHandle gpu_profiler_begin_event(const char *name,
                                          GPUProfileCategory category) {
  if (!g_gpu_profiler || !g_gpu_profiler->initialized ||
      !g_gpu_profiler->capturing) {
    return 0;
  }

  if (g_gpu_profiler->current_query_index >=
      g_gpu_profiler->max_queries_per_frame * 2) {
    LOG_WARN_CAT(LOG_CAT_GRAPHICS, "GPU profiler query limit reached");
    return 0;
  }
  ...
  if (g_gpu_profiler->entry_count < g_gpu_profiler->entry_capacity) {
    GPUProfileEntry *entry =
        &g_gpu_profiler->entries[g_gpu_profiler->entry_count];
...
void gpu_profiler_end_event(GPUProfileHandle handle) {
  if (!g_gpu_profiler || !g_gpu_profiler->initialized ||
      !g_gpu_profiler->capturing || handle == 0) {
    return;
  }
...
void gpu_profiler_begin_frame(void) {
  ...
  u32 min_frame =
      g_gpu_profiler->current_frame - g_gpu_profiler->frame_delay - 1;
  u32 write_index = 0;
...
void gpu_profiler_print_summary(void) {
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "=== GPU Profiler Summary ===");
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "Frame: %u, Total Time: %.2f ms",
               g_gpu_profiler->current_frame,
               g_gpu_profiler->total_frame_time_ms);

  for (int i = 0; i < GPU_PROFILE_CATEGORY_COUNT; i++) {
    if (g_gpu_profiler->category_counts[i] > 0) {
      f32 avg_time = g_gpu_profiler->category_times_ms[i] /
                     g_gpu_profiler->category_counts[i];
      LOG_INFO_CAT(LOG_CAT_GRAPHICS,
                   "%s: %.2f ms total, %.2f ms avg (%u calls)",
                   category_names[i], g_gpu_profiler->category_times_ms[i],
                   avg_time, g_gpu_profiler->category_counts[i]);
    }
  }

  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "=== End Summary ===");
}
...
void gpu_profiler_print_timeline(void) {
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "=== GPU Timeline ===");

  for (u32 i = 0; i < g_gpu_profiler->entry_count; i++) {
    GPUProfileEntry *entry = &g_gpu_profiler->entries[i];
    if (entry->duration_ms > 0.0f) {
      LOG_INFO_CAT(LOG_CAT_GRAPHICS, "[%s] %s: %.2f ms (frame %u)",
                   category_names[entry->category], entry->name,
                   entry->duration_ms, entry->frame_index);
    }
  }

  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "=== End Timeline ===");
}
...
void gpu_profiler_export_csv(const char *filename) {
  ...
  LOG_INFO_CAT(LOG_CAT_GRAPHICS, "Exporting GPU profiler data to CSV: %s",
               filename);
  ...
  for (u32 i = 0; i < g_gpu_profiler->entry_count; i++) {
    GPUProfileEntry *entry = &g_gpu_profiler->entries[i];
    if (entry->duration_ms > 0.0f) {
      // LOG_INFO_CAT(LOG_CAT_GRAPHICS, "%u,%s,%s,%.2f",
