...
bool logger_init(const LogConfig* config) {
    ...
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Unified logging system initialized");
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Session ID: %s", g_logging_state.session_id);
...
void logger_shutdown(void) {
    ...
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Shutting down unified logging system");
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Total messages logged: %llu", g_logger->total_messages_logged);
...
void logger_set_level(LogLevel level) {
    ...
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Log level set to: %s", get_level_string(level));
}

void logger_set_channels(LogChannel channels) {
    if (!g_logger) return;
    g_logger->config.active_channels = channels;
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Log channels updated");
}

void logger_set_format(LogFormat format) {
    if (!g_logger) return;
    g_logger->config.format = format;
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Log format set to: %d", format);
}
...
void logger_set_category_filter(LogCategory category, LogLevel min_level, bool enabled) {
    ...
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Category filter updated: %s = %s (level: %s)",
...
void logger_apply_config(const LogConfig* config) {
    ...
    LOG_INFO_CAT(LOG_CAT_GENERAL, "Log configuration applied");
}
...
void logger_set_file(const char* filename) {
    ...
    if (g_logger->log_file) {
        fseek(g_logger->log_file, 0, SEEK_END);
        g_logger->current_file_size = ftell(g_logger->log_file);
        LOG_INFO_CAT(LOG_CAT_GENERAL, "Log file opened: %s", filename);
    } else {
...
void logger_rotate_file(void) {
    ...
    if (g_logger->log_file) {
        ...
        LOG_INFO_CAT(LOG_CAT_GENERAL, "Log file rotated: %s -> %s", 
