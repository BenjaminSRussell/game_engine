...
static inline u32 clamp_u32(u32 value, u32 min, u32 max, const char *name) {
  if (value < min) {
    LOG_WARN_CAT(LOG_CAT_GENERAL, "Config %s=%u below minimum %u, clamping", name,
             value, min);
    return min;
  }
  if (value > max) {
    LOG_WARN_CAT(LOG_CAT_GENERAL, "Config %s=%u above maximum %u, clamping", name,
             value, max);
    return max;
  }
  return value;
...
static inline f32 clamp_f32(f32 value, f32 min, f32 max, const char *name) {
  if (value < min) {
    LOG_WARN_CAT(LOG_CAT_GENERAL, "Config %s=%.2f below minimum %.2f, clamping",
             name, value, min);
    return min;
  }
  if (value > max) {
    LOG_WARN_CAT(LOG_CAT_GENERAL, "Config %s=%.2f above maximum %.2f, clamping",
             name, value, max);
    return max;
  }
  return value;
