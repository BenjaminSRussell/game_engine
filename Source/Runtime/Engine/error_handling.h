...
#define REPORT_ERROR_WITH_CAUSE(code, message, cause) \
    error_report_with_cause(code, message, __FILE__, __FUNCTION__, __LINE__, cause)

// Conditional error reporting
#define REPORT_ERROR_IF(condition, code, message) \
    do { if (condition) REPORT_ERROR(code, message); } while(0)
...
#define RETURN_ERROR_IF(condition, code, message) \
    do { if (condition) { REPORT_ERROR(code, message); return code; } } while(0)
...
#define RETURN_ON_ERROR(expr) \
    do { error_code_t _err = (expr); if (_err != ERROR_NONE) return _err; } while(0)
...
// Graphics errors
#define REPORT_GRAPHICS_ERROR(code, message) \
...
// Audio errors
#define REPORT_AUDIO_ERROR(code, message) \
    REPORT_ERROR(code, message)
...
// Physics errors
#define REPORT_PHYSICS_ERROR(code, message) \
    REPORT_ERROR(code, message)
...
// AI errors
#define REPORT_AI_ERROR(code, message) \
    REPORT_ERROR(code, message)
