...
static void spine_ik_solve_fabrik(SpineIK *ik, Skeleton *skeleton,
                                  Vec3 target_pos) {
  ...
  for (u32 i = 0; i < ik->bone_count; i++) {
    ...
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      positions[i] = vec3(skeleton->global_transforms[bone_idx].data[3][0],
                          skeleton->global_transforms[bone_idx].data[3][1],
                          skeleton->global_transforms[bone_idx].data[3][2]);
    }
  }
  ...
  for (u32 iter = 0; iter < ik->max_iterations; iter++) {
    ...
    for (i32 i = (i32)ik->bone_count - 2; i >= 0; i--) {
      Vec3 direction = vec3_normalize(vec3_sub(positions[i + 1], positions[i]));
      positions[i] =
          vec3_sub(positions[i + 1], vec3_mul(direction, bone_lengths[i]));
    }

    // Backward pass: restore root position
    positions[0] =
        vec3(skeleton->global_transforms[ik->bone_indices[0]].data[3][0],
             skeleton->global_transforms[ik->bone_indices[0]].data[3][1],
             skeleton->global_transforms[ik->bone_indices[0]].data[3][2]);

    for (u32 i = 0; i < ik->bone_count - 1; i++) {
      Vec3 direction = vec3_normalize(vec3_sub(positions[i + 1], positions[i]));
      positions[i + 1] =
          vec3_add(positions[i], vec3_mul(direction, bone_lengths[i]));
    }
  ...
  for (u32 i = 0; i < ik->bone_count; i++) {
    i32 bone_idx = ik->bone_indices[i];
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      Vec3 current_pos = vec3(skeleton->global_transforms[bone_idx].data[3][0],
                              skeleton->global_transforms[bone_idx].data[3][1],
                              skeleton->global_transforms[bone_idx].data[3][2]);

      Vec3 new_pos = vec3_lerp(current_pos, positions[i], ik->stiffness);
      skeleton->global_transforms[bone_idx].data[3][0] = new_pos.x;
      skeleton->global_transforms[bone_idx].data[3][1] = new_pos.y;
      skeleton->global_transforms[bone_idx].data[3][2] = new_pos.z;
    }
  }
}
...
static void spine_ik_update_leaning(SpineIK *ik, Skeleton *skeleton, f32 dt) {
  ...
  forward_lean = clampf(forward_lean, -ik->max_lean_angle * 0.5f,
                        ik->max_lean_angle * 0.5f);
  ...
  for (u32 i = 0; i < ik->bone_count; i++) {
    ...
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      ...
      // vec3_forward() is assumed (0,0,1)
      Quat lean_rotation = quat_mul(
          quat_from_axis_angle(vec3(0, 0, 1), lateral_lean * influence),
          quat_from_axis_angle(vec3(1, 0, 0),
                               forward_lean *
                                   influence) // Forward lean typically X axis
      );
...
static void spine_ik_update_breathing(SpineIK *ik, Skeleton *skeleton, f32 dt) {
  ...
  f32 breathing_offset = sinf(ik->time * ik->breathing_frequency * 2.0f * PI) *
                         ik->breathing_amplitude;
  ...
  for (u32 i = start_bone; i < end_bone && i < ik->bone_count; i++) {
    ...
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      ...
      Quat breath_rotation =
          quat_from_axis_angle(vec3_up(), breathing_offset * 0.1f);
      ...
      breath_matrix.data[3][1] += breath_offset.y; // Use .data[3][1]
      skeleton->global_transforms[bone_idx] =
...
SpineIK *spine_ik_create(const i32 *bone_indices, u32 bone_count) {
  ...
  SpineIK *ik = (SpineIK *)malloc(sizeof(SpineIK));
  if (!ik)
    return NULL;
  ...
  ik->target_forward = vec3(0, 0, 1); // Forward
  ik->stiffness = 0.5f;
...
void spine_ik_enable_fabrik(SpineIK *ik, bool enable, u32 max_iterations,
                            f32 tolerance) {
  if (ik) {
...
void spine_ik_enable_leaning(SpineIK *ik, bool enable, f32 lean_factor,
                             f32 max_angle) {
  if (ik) {
...
void spine_ik_enable_breathing(SpineIK *ik, bool enable, f32 frequency,
                               f32 amplitude) {
  if (ik) {
...
void spine_ik_update(SpineIK *ik, Skeleton *skeleton, Vec3 target_pos,
                     Vec3 velocity, Vec3 acceleration, f32 dt) {
  if (!ik || !skeleton) {
    return;
  }
  ...
  if (!originals_stored) {
    for (u32 i = 0; i < ik->bone_count; i++) {
      i32 bone_idx = ik->bone_indices[i];
      if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
        ik->original_positions[i] =
            vec3(skeleton->global_transforms[bone_idx].data[3][0],
                 skeleton->global_transforms[bone_idx].data[3][1],
                 skeleton->global_transforms[bone_idx].data[3][2]);
        // Extract rotation from matrix using helper
        ik->original_rotations[i] =
            quat_from_mat4(skeleton->global_transforms[bone_idx]);
      }
    }
