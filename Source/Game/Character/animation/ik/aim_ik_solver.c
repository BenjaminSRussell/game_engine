...
// Helper: Quaternion from Matrix
static Quat quat_from_mat4(Mat4 m) {
  // Assuming m is orthogonal and unscaled for rotation extraction
...
static void aim_ik_apply_additive_pose(AimIK *ik, Skeleton *skeleton) {
  ...
  for (u32 i = 0; i < ik->spine_bone_count && i < 5; i++) {
    ...
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      ...
      // vec3_up() function might imply Y-up
      Quat additive_rotation =
          quat_from_axis_angle(vec3(0, 1, 0), final_rotation);
...
static Vec3 aim_ik_get_muzzle_position(AimIK *ik, Skeleton *skeleton,
                                       i32 weapon_bone) {
  if (weapon_bone < 0 || weapon_bone >= (i32)skeleton->bone_count) {
    return vec3_zero();
  }

  // Get weapon bone world position
  Mat4 weapon_matrix = skeleton->global_transforms[weapon_bone];
  Vec3 weapon_pos = vec3(weapon_matrix.data[3][0], weapon_matrix.data[3][1],
                         weapon_matrix.data[3][2]);

  // Add muzzle offset in local space
  Vec3 muzzle_local = ik->weapon_muzzle_local_offset;
  Vec3 muzzle_world =
      vec3_add(weapon_pos,
               quat_rotate_vec3(quat_from_mat4(weapon_matrix), muzzle_local));

  return muzzle_world;
}

static void aim_ik_update_procedural_correction(AimIK *ik, Skeleton *skeleton,
                                                i32 weapon_bone) {
  ...
  ik->current_muzzle_vector =
      vec3_normalize(vec3_sub(ik->aim_target_world, muzzle_pos));
  ...
  correction_angle = clampf(correction_angle, -ik->max_correction_angle,
                            ik->max_correction_angle);

  // Calculate correction axis
  Vec3 correction_axis = vec3_normalize(
      vec3_cross(ik->current_muzzle_vector, ik->desired_muzzle_vector));

  // Apply correction with strength
  f32 final_correction =
      correction_angle * ik->correction_strength * ik->aim_weight;
  Quat correction_rotation =
      quat_from_axis_angle(correction_axis, final_correction);
  ...
  for (u32 i = 0; i < ik->spine_bone_count; i++) {
    ...
    if (bone_idx >= 0 && bone_idx < (i32)skeleton->bone_count) {
      ...
      Quat bone_correction =
          quat_slerp(quat_identity(), correction_rotation, influence);
  ...
  if (weapon_bone >= 0 && weapon_bone < (i32)skeleton->bone_count) {
    Vec3 weapon_forward =
        vec3_normalize(vec3_sub(ik->aim_target_world, muzzle_pos));
    // Use vec3_forward() which is usually (0,0,-1) or (0,0,1). Assuming (0,0,1)
    // here or checking macros.
    Quat weapon_rotation = quat_from_vectors(vec3(0, 0, 1), weapon_forward);
...
static void aim_ik_update_scope_alignment(AimIK *ik, Skeleton *skeleton,
                                          i32 weapon_bone, i32 camera_bone) {
  ...
  // Use .data[col][row]. Translation is col 3.
  Vec3 weapon_pos = vec3(skeleton->global_transforms[weapon_bone].data[3][0],
                         skeleton->global_transforms[weapon_bone].data[3][1],
                         skeleton->global_transforms[weapon_bone].data[3][2]);
  ...
  if (camera_bone >= 0 && camera_bone < (i32)skeleton->bone_count) {
    // Smooth transition to desired position
    Vec3 current_camera_pos =
        vec3(skeleton->global_transforms[camera_bone].data[3][0],
             skeleton->global_transforms[camera_bone].data[3][1],
             skeleton->global_transforms[camera_bone].data[3][2]);

    Vec3 new_camera_pos =
        vec3_lerp(current_camera_pos, desired_camera_pos, ik->alignment_speed);

    skeleton->global_transforms[camera_bone].data[3][0] = new_camera_pos.x;
    skeleton->global_transforms[camera_bone].data[3][1] = new_camera_pos.y;
    skeleton->global_transforms[camera_bone].data[3][2] = new_camera_pos.z;

    // Align camera forward with weapon forward
    Vec3 weapon_forward =
        vec3_normalize(vec3_sub(ik->aim_target_world, weapon_pos));
    Quat camera_rotation =
        quat_from_vectors(ik->camera_forward, weapon_forward);
...
// Public API
AimIK *aim_ik_create() {
  AimIK *ik = (AimIK *)malloc(sizeof(AimIK));
  if (!ik)
    return NULL;
  ...
  ik->current_muzzle_vector = vec3(0, 0, 1);
  ik->desired_muzzle_vector = vec3(0, 0, 1);
  ik->correction_strength = 0.8f;
  ik->max_correction_angle = PI * 0.25f; // 45 degrees

  ik->is_ads = false;
  ik->camera_position = vec3_zero();
  ik->camera_forward = vec3(0, 0, 1);
...
void aim_ik_update(AimIK *ik, Skeleton *skeleton, i32 weapon_bone,
                   i32 camera_bone, f32 dt) {
  if (!ik || !skeleton) {
    return;
  }
  ...
  Vec3 weapon_pos =
      (weapon_bone >= 0)
          ? vec3(skeleton->global_transforms[weapon_bone].data[3][0],
                 skeleton->global_transforms[weapon_bone].data[3][1],
                 skeleton->global_transforms[weapon_bone].data[3][2])
          : vec3_zero();
